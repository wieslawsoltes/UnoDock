    ...
    public static void Run(DockingManager manager, string directory)
    {
        ...
        )
        {
            ...
            var doc = new LayoutDocument
            {
                ContentId = "editor",
                Title = "Workspace.cs",
                Content = "Application-owned document"
            };
            ...
            if (flyout == null || !flyout.IsVisible)
                throw new InvalidOperationException("Reference auto-hide view was not revealed.");
            ...
            var width = (int)Math.Ceiling(last.X) - x;
            var height = (int)Math.Ceiling(last.Y) - y;
            if (width <= 0 || height <= 0)
                throw new InvalidOperationException("Empty public flyout screen bounds.");
            ...
            observations.Add(new XElement("Window", new XAttribute("name", name), new XAttribute("side", side), new XAttribute("xPixels", x), new XAttribute("yPixels", y), new XAttribute("widthPixels", width), new XAttribute("heightPixels", height), new XAttribute("width", flyout.ActualWidth.ToString("R", CultureInfo.InvariantCulture)), new XAttribute("height", flyout.ActualHeight.ToString("R", CultureInfo.InvariantCulture)), new XAttribute("active", tool.IsActive)));
            tool.ToggleAutoHide();
            manager.UpdateLayout();
        }
