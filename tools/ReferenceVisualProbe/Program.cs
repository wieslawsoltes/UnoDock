...
internal static class Program
{
    ...
    {
        if (args.Length != 1)
            throw new ArgumentException("Usage: ReferenceVisualProbe <output-directory>");
            ...
            window.Loaded += (_, ___) => window.Dispatcher.BeginInvoke(DispatcherPriority.ApplicationIdle, new Action(() =>
            {
                try
                {
                    ...
                    )
                    {
                        ...
                        if (scenario == "active-tool")
                            manager.Layout.Descendents().OfType<LayoutAnchorable>().First(a => a.ContentId == "explorer").IsActive = true;
                        if (scenario == "auto-hide")
                            manager.Layout.Descendents().OfType<LayoutAnchorable>().First(a => a.ContentId == "explorer").ToggleAutoHide();
                        ...
                        using (var file = File.Create(Path.Combine(args[0], scenario + ".png")))
                            encoder.Save(file);
                        ...
                        Walk(manager, manager, elements);
                        new XDocument(elements).Save(Path.Combine(args[0], scenario + ".xml"));
    ...
    private static void Populate(DockingManager manager)
    {
        ...
        var docs = new LayoutDocumentPane(code);
        docs.Children.Add(Doc("readme", "Readme.md", SceneContent.Readme));
        docs.Children.Add(Doc("notes", "Notes.txt", SceneContent.Notes));
        var left = new LayoutAnchorablePane(Tool("explorer", "Solution Explorer", Rows(SceneContent.Explorer)))
        {
            DockWidth = new GridLength(200),
            DockMinWidth = 120
        };
        left.Children.Add(Tool("toolbox", "Toolbox", Rows(SceneContent.Toolbox)));
        var right = new LayoutAnchorablePane(Tool("properties", "Properties", Rows(SceneContent.Properties)))
        {
            DockWidth = new GridLength(230),
            DockMinWidth = 120
        };
        var bottom = new LayoutAnchorablePane(Tool("output", "Output", Text(SceneContent.Output)))
        {
            DockHeight = new GridLength(155),
            DockMinHeight = 60
        };
        bottom.Children.Add(Tool("errors", "Error List", Rows(SceneContent.Errors)));
        var middle = new LayoutPanel(docs)
        {
    ...
    private static LayoutDocument Doc(string id, string title, string text) => new LayoutDocument
    {
        ContentId = id,
        Title = title,
        Content = Text(text)
    };
    private static LayoutAnchorable Tool(string id, string title, object content) => new LayoutAnchorable
    {
        ContentId = id,
        Title = title,
        Content = content,
    ...
    private static TextBox Text(string text) => new TextBox
    {
        AcceptsReturn = true,
        Text = text,
    ...
    private static ScrollViewer Rows(string[] rows)
    {
        ...
        {
            Margin = new Thickness(10, 9, 8, 9)
        };
        foreach (var row in rows)
            items.Children.Add(new TextBlock { Text = row, FontSize = 12, Margin = new Thickness(0, 0, 0, 8), TextTrimming = TextTrimming.CharacterEllipsis });
    ...
    private static void Walk(DependencyObject node, FrameworkElement relative, XElement output)
    {
        if (node is FrameworkElement element && element.IsVisible && (node is ILayoutControl || node is TextBox))
        {
            ...
            output.Add(new XElement("Element", new XAttribute("type", node.GetType().Name), new XAttribute("content", ids ?? ""), new XAttribute("x", rect.X.ToString("R", CultureInfo.InvariantCulture)), new XAttribute("y", rect.Y.ToString("R", CultureInfo.InvariantCulture)), new XAttribute("width", rect.Width.ToString("R", CultureInfo.InvariantCulture)), new XAttribute("height", rect.Height.ToString("R", CultureInfo.InvariantCulture))));
        }

        for (var i = 0; i < VisualTreeHelper.GetChildrenCount(node); i++)
            Walk(VisualTreeHelper.GetChild(node, i), relative, output);
    }
}
