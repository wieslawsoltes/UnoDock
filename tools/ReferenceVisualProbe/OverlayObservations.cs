...
internal static class OverlayObservations
{
    ...
    {
        if (Environment.GetEnvironmentVariable("UNODOCK_REFERENCE_INPUT") != "1")
            return;
        ...
        _ = owner.Dispatcher.BeginInvoke(new Action(async () =>
        {
            try
            {
                ...
                )
                {
                    ...
                    var tools = new LayoutAnchorablePane(anchor)
                    {
                    ...
                    content.FloatingLeft = 40;
                    content.FloatingTop = 60;
                    content.FloatingWidth = 280;
                    content.FloatingHeight = 180;
                    content.Float();
                    ...
                    try
                    {
                        ...
                        if (!File.Exists(Path.Combine(output, name + "-message.xml")))
                            throw new InvalidOperationException("Native-message probe did not realize the reference overlay.");
                    }
                    finally
                    {
                        content.Dock();
        ...
        if (failure != null)
            throw new InvalidOperationException("Public overlay observation failed.", failure);
    }
