...
internal static class OverlayMessageObservation
{
    public static void TryObserve(LayoutFloatingWindowControl floating, Point point, string output, string name)
    {
        ...
        try
        {
            ...
            foreach (var overlay in Application.Current.Windows.OfType<OverlayWindow>())
            {
                ...
                using (var file = File.Create(Path.Combine(output, name + "-message.png")))
                    encoder.Save(file);
                var xml = new XElement("Overlay", new XAttribute("name", name), new XAttribute("method", "public-native-message-sequence-not-pointer-acceptance"), new XAttribute("width", root.ActualWidth), new XAttribute("height", root.ActualHeight));
                Walk(root, root, xml);
                new XDocument(xml).Save(Path.Combine(output, name + "-message.xml"));
            }
        }
        finally
        {
    ...
    private static void Walk(DependencyObject node, FrameworkElement root, XElement xml)
    {
        if (node is FrameworkElement e && e.IsVisible && e.ActualWidth > 0 && e.ActualHeight > 0)
        {
            ...
            xml.Add(new XElement("Element", new XAttribute("type", e.GetType().Name), new XAttribute("name", e.Name ?? ""), new XAttribute("x", bounds.X), new XAttribute("y", bounds.Y), new XAttribute("width", bounds.Width), new XAttribute("height", bounds.Height)));
        }

        for (var i = 0; i < VisualTreeHelper.GetChildrenCount(node); i++)
            Walk(VisualTreeHelper.GetChild(node, i), root, xml);
    }
