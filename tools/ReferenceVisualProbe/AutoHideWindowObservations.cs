...
internal static class AutoHideWindowObservations
{
    public static void Run(DockingManager manager, string directory)
    {
        ...
        foreach (var side in new[]
        {
            ...
            foreach (var explicitSize in new[]
            {
                false,
                true
            }
            ...
            )
            {
                ...
                var doc = new LayoutDocument
                {
                    ContentId = "editor",
                    Title = "Editor",
                    Content = "Application-owned probe content"
                };
                manager.Layout = new LayoutRoot
                {
                    RootPanel = new LayoutPanel(new LayoutDocumentPane(doc))
                };
                var tool = new LayoutAnchorable
                {
                    ...
                    {
                        Text = "Application-owned flyout editor"
                    },
                    AutoHideWidth = explicitSize ? 320 : 0,
                    AutoHideHeight = explicitSize ? 220 : 0
                };
                ...
                var row = new XElement("Window", new XAttribute("side", side), new XAttribute("explicitSize", explicitSize), new XAttribute("autoHideWidth", tool.AutoHideWidth), new XAttribute("autoHideHeight", tool.AutoHideHeight), new XAttribute("minWidth", tool.AutoHideMinWidth), new XAttribute("minHeight", tool.AutoHideMinHeight), new XAttribute("active", tool.IsActive), new XAttribute("visible", flyout != null && flyout.IsVisible));
                if (flyout != null)
                {
