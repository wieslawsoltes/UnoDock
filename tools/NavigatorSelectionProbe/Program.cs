...
internal static class Program
{
    ...
    {
        if (args.Length != 1) throw new ArgumentException("Usage: NavigatorSelectionProbe <output-directory>");
        ...
        app.Startup += (_, __) =>
        {
            ...
            var owner = new Window { Content = manager, Width = 900, Height = 600, ShowInTaskbar = false };
            owner.Loaded += (_, ___) => owner.Dispatcher.BeginInvoke(DispatcherPriority.ApplicationIdle, new Action(() =>
            {
                try
                {
                    var result = new XElement("NavigatorSelectionObservations", new XAttribute("schema", 1),
                        new XAttribute("method", "public-and-protected-api-only"));
    ...
    private static XElement Observe(DockingManager manager, Window owner, bool tool, string scenario)
    {
        ...
        manager.Layout = new LayoutRoot { RootPanel = panel }; documents.Children[0].IsActive = true;
        var ordinal = 0;
        ...
        var nav = new ObservedNavigator(manager) { Owner = owner };
        var row = new XElement("Case", new XAttribute("name", (tool ? "tool/" : "document/") + scenario));
        var trace = new List<string>(); var recording = false; var closed = false; var once = false; var calls = 0;
        Action<string> log = text => { if (recording) trace.Add(text); };
        nav.Trace = log;
        ...
        {
            log("closing");
            if (scenario == "closing-veto") { e.Cancel = true; log("cancel-close"); }
            if (scenario == "closing-reselect" && !once)
            {
                once = true; log("closing-reselect");
                nav.SelectedDocument = (LayoutDocumentItem)manager.GetLayoutItemFromModel(documents.Children[1]);
            }
        };
        nav.Closing += closing;
        nav.Closed += (_, __) => { closed = true; log("closed"); };
        try
        {
            nav.Show(); nav.UpdateLayout(); Pump(nav);
            ...
            if (scenario == "disabled") targetModel.IsEnabled = false;
            if (scenario == "veto" || scenario == "noop" || scenario.StartsWith("query-", StringComparison.Ordinal) || scenario.StartsWith("execute-", StringComparison.Ordinal))
            {
                ...
                }, () =>
                {
                    ...
                    {
                        once = true; log("execute-reselect");
                        nav.SelectedDocument = (LayoutDocumentItem)manager.GetLayoutItemFromModel(documents.Children[1]);
                    }
            ...
            Pump(nav); calls = 0;
            row.Add(Snapshot("before", manager, nav, closed)); recording = true;
            Apply("first", () =>
            {
                if (scenario == "null")
                {
                    if (tool) nav.SelectedAnchorable = null; else nav.SelectedDocument = null;
                }
                else if (scenario == "same")
                {
                    if (tool) nav.SelectedAnchorable = nav.SelectedAnchorable; else nav.SelectedDocument = nav.SelectedDocument;
                }
                else if (scenario == "setvalue") nav.SetValue(tool ? NavigatorWindow.SelectedAnchorableProperty : NavigatorWindow.SelectedDocumentProperty, target);
                else Set(nav, target);
            ...
            recording = false;
            Pump(nav); row.Add(Snapshot("settled", manager, nav, closed));
            return row;
        }
        finally
        {
            recording = false; nav.Trace = null; nav.Closing -= closing;
            manager.ActiveContentChanged -= active; nav.Close();
        }
    }
    ...
    {
        trace.Clear(); string failure = null;
        try { action(); } catch (Exception error) { failure = error.GetType().FullName; }
        var step = new XElement("Step", new XAttribute("name", name), new XAttribute("exception", failure ?? "none"));
        foreach (var entry in trace) step.Add(new XElement("Event", entry));
        step.Add(Snapshot("immediate", manager, nav, closed())); row.Add(step);
    }
    ...
    private static void Set(NavigatorWindow nav, LayoutItem item)
    {
        ...
        else nav.SelectedDocument = (LayoutDocumentItem)item;
    }
    ...
    {
        private readonly Func<bool> _query; private readonly Action _execute;
        internal Command(Func<bool> query, Action execute) { _query = query; _execute = execute; }
