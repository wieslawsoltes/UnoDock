...
for (var i = 2; i < args.Length; i++)
{
    ...
    {
        case "--ref-dir":
            directories.Add(Path.GetFullPath(args[i]));
            break;
        case "--profile":
            profile = args[i];
            break;
...
foreach (var(name, path)in paths)
{
    ...
    { /* Native dependency; not part of a managed API. */
    }
}

var compilation = CSharpCompilation.Create("UnoDock.ApiMetadata", references: references, options: new CSharpCompilationOptions(OutputKind.DynamicallyLinkedLibrary, metadataImportOptions: MetadataImportOptions.All));
var targetReference = references.OfType<PortableExecutableReference>().Single(r => r.FilePath == assemblyPath);
var assembly = compilation.GetAssemblyOrModuleSymbol(targetReference) as IAssemblySymbol ?? throw new InvalidDataException("Input is not a managed assembly.");
...
foreach (var type in types)
{
    ...
    {
        CheckType(parent);
        baseClasses.Add(TypeName(parent));
    }
...
{
    var hidden = type.GetMembers().Where(m => m is not IMethodSymbol { AssociatedSymbol: not null }).Select(m => m.MetadataName).ToHashSet(StringComparer.Ordinal);
    for (var parent = type.BaseType; parent != null; parent = parent.BaseType)
    {
        foreach (var member in parent.GetMembers().Where(VisibleMember).OrderBy(MemberKey, StringComparer.Ordinal))
        {
            if (member is IMethodSymbol { MethodKind: MethodKind.Constructor or MethodKind.StaticConstructor or MethodKind.Destructor } || hidden.Contains(member.MetadataName))
                continue;
            yield return MemberRecord(member);
        }

        foreach (var member in parent.GetMembers().Where(m => m is not IMethodSymbol { AssociatedSymbol: not null }))
            hidden.Add(member.MetadataName);
    }
}
...
void CheckType(ITypeSymbol? type)
{
    switch (type)
    {
        ...
        case IErrorTypeSymbol error:
            unresolved.Add(error.ToDisplayString());
            return;
        case IArrayTypeSymbol array:
            CheckType(array.ElementType);
            break;
        case IPointerTypeSymbol pointer:
            CheckType(pointer.PointedAtType);
            break;
        case INamedTypeSymbol named:
            foreach (var argument in named.TypeArguments)
                CheckType(argument);
            if (named.ContainingType != null)
                CheckType(named.ContainingType);
            break;
        case IFunctionPointerTypeSymbol function:
            CheckType(function.Signature.ReturnType);
            foreach (var p in function.Signature.Parameters)
                CheckType(p.Type);
            break;
    }
}
...
string Parameters(ImmutableArray<IParameterSymbol> parameters) => "(" + string.Join(",", parameters.Select(p => (p.IsParams ? "params " : "") + Ref(p.RefKind) + TypeNameChecked(p.Type) + " " + p.Name + (p.HasExplicitDefaultValue ? "=" + Constant(p.ExplicitDefaultValue) : p.IsOptional ? "=optional" : ""))) + ")";
object[] ParameterRecords(ImmutableArray<IParameterSymbol> parameters) => parameters.Select(p => (object)new { p.Name, type = TypeNameChecked(p.Type), refKind = p.RefKind.ToString(), p.IsParams, p.IsOptional, p.HasExplicitDefaultValue, defaultValue = p.HasExplicitDefaultValue ? Constant(p.ExplicitDefaultValue) : null, attributes = Attributes(p.GetAttributes()) }).ToArray();
...
static string Ref(RefKind kind) => kind switch
{
    RefKind.None => "",
    RefKind.Ref => "ref ",
    RefKind.Out => "out ",
...
static bool VisibleMember(ISymbol symbol) => Visible(symbol.DeclaredAccessibility) && symbol is not INamedTypeSymbol && (symbol is not IMethodSymbol method || method.MethodKind is MethodKind.Ordinary or MethodKind.Constructor or MethodKind.UserDefinedOperator or MethodKind.Conversion or MethodKind.Destructor);
...
static IEnumerable<INamedTypeSymbol> EnumerateType(INamedTypeSymbol type)
{
    ...
    foreach (var child in type.GetTypeMembers())
        foreach (var nested in EnumerateType(child))
            yield return nested;
}

static string MetadataName(INamedTypeSymbol type) => type.ContainingType != null ? MetadataName(type.ContainingType) + "+" + type.MetadataName : (type.ContainingNamespace.IsGlobalNamespace ? "" : type.ContainingNamespace.ToDisplayString() + ".") + type.MetadataName;
static string TypeName(ITypeSymbol? type) => type?.ToDisplayString(new SymbolDisplayFormat(globalNamespaceStyle: SymbolDisplayGlobalNamespaceStyle.Omitted, typeQualificationStyle: SymbolDisplayTypeQualificationStyle.NameAndContainingTypesAndNamespaces, genericsOptions: SymbolDisplayGenericsOptions.IncludeTypeParameters, miscellaneousOptions: SymbolDisplayMiscellaneousOptions.EscapeKeywordIdentifiers)) ?? "";
