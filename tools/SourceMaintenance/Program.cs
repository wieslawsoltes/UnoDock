...
internal static class Program
{
    ...
    [
        ...
        new(LanguageVersion.Preview, preprocessorSymbols: ["WINDOWS"]),
        new(LanguageVersion.Preview, preprocessorSymbols: ["DEBUG"]),
        new(LanguageVersion.Preview, preprocessorSymbols: ["WINDOWS", "DEBUG"])
    ];
    ...
    private static int Main(string[] args)
    {
        try
        {
            if (args.Length != 2 || args[0] is not ("--apply" or "--check"))
            {
                throw new ArgumentException("Usage: SourceMaintenance --apply|--check <repository>");
            }
            ...
            foreach (var file in files)
            {
                ...
                var units = Configurations.SelectMany(configuration => Types(Parse(text, configuration)))
                    .GroupBy(Key).ToDictionary(group => group.Key, group => group.First(), StringComparer.Ordinal);
                if (units.Count <= 1)
                {
                    continue;
                }
            ...
            var report = moves.ToDictionary(pair => Path.GetRelativePath(root, pair.Key).Replace('\\', '/'),
                pair => pair.Value.Select(path => Path.GetRelativePath(root, path).Replace('\\', '/')).ToArray());
    ...
    private static IEnumerable<string> Files(string root) => new[] { "src", "samples", "tests", "tools" }
        ...
        .Where(path => !Path.GetRelativePath(root, path).Replace('\\', '/').StartsWith("tools/ReferenceProbe/", StringComparison.Ordinal))
        .Where(path => !File.ReadLines(path).Take(3).Any(line => line.Contains("<auto-generated", StringComparison.OrdinalIgnoreCase)))
        .Order(StringComparer.Ordinal);

    private static CompilationUnitSyntax Parse(string text, CSharpParseOptions configuration) =>
        CSharpSyntaxTree.ParseText(text, configuration).GetCompilationUnitRoot();

    private static IEnumerable<MemberDeclarationSyntax> Types(CompilationUnitSyntax root) => root.DescendantNodes()
        .OfType<MemberDeclarationSyntax>()
        .Where(node => node is BaseTypeDeclarationSyntax or DelegateDeclarationSyntax)
        .Where(node => !node.Ancestors().Any(ancestor => ancestor is BaseTypeDeclarationSyntax or DelegateDeclarationSyntax));
    ...
    private static string[] Fingerprints(string text, CSharpParseOptions configuration) => Types(Parse(text, configuration))
        .Select(type => Key(type) + ":" + Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(string.Join("\n",
            type.DescendantTokens().Select(token => token.RawKind + ":" + token.ValueText))))))
