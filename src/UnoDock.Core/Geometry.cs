...
public readonly record struct DockRect(double X, double Y, double Width, double Height)
{
    ...
    public DockRect ClampTo(DockRect workArea, double minimumVisible = 32)
    {
        ...
        return new(Math.Clamp(X, workArea.X - w + minimumVisible, Math.Max(workArea.X - w + minimumVisible, workArea.Right - minimumVisible)),
            Math.Clamp(Y, workArea.Y, Math.Max(workArea.Y, workArea.Bottom - minimumVisible)), w, h);
    }
}
public enum DockPosition { Inside, Left, Top, Right, Bottom }
public enum DockLengthUnit { Pixel, Star, Auto }
