...
public sealed class UpdateBatch(Action flush)
{
    ...
    {
        if (_depth <= 0)
            throw new InvalidOperationException("Unbalanced update scope.");
        _depth--;
        Drain();
    }
    ...
    private void Drain()
    {
        ...
        try
        {
            ...
            {
                if (++iterations > 1024)
                    throw new InvalidOperationException("Layout notification did not converge after 1024 passes.");
                _dirty = false;
                _flush();
            }
        }
