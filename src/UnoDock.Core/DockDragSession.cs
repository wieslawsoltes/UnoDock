...
public sealed class DockDragSession
{
    ...
    public void Arm(uint pointerId, DockPoint origin, bool isDocument)
    {
        ...
        _pointer = pointerId;
        _origin = origin;
        _document = isDocument;
        State = DockDragState.Armed;
        Target = null;
        Position = DockPosition.Inside;
    }

    public bool Move(uint pointerId, DockPoint point, ReadOnlySpan<DockDropTarget> targets)
    {
        if (pointerId != _pointer || State is not (DockDragState.Armed or DockDragState.Dragging))
            return false;
        Validate(point);
        if (State == DockDragState.Armed)
        {
            if (double.Hypot(point.X - _origin.X, point.Y - _origin.Y) < Threshold)
                return false;
        ...
        {
            if ((_document ? !target.AcceptsDocuments : !target.AcceptsAnchorables) || !target.Bounds.Contains(point))
                continue;
            if (Target is not { } best || target.Priority > best.Priority || target.Priority == best.Priority && (Area(target) < Area(best) || Area(target) == Area(best) && string.CompareOrdinal(target.Id, best.Id) < 0))
                Target = target;
        }
