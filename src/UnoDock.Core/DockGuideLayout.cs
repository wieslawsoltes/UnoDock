...
public static class DockGuideLayout
{
    ...
    private static IReadOnlyList<DockGuideSlot> CreateCore(DockRect host, DockRect pane, bool workspace, bool paneTargets, bool toolTargets, double size, double gap, double inset, double rootLong, double rootShort)
    {
        Validate(host);
        Validate(pane);
        if (!double.IsFinite(size) || size < 16 || size > 128)
            throw new ArgumentOutOfRangeException(nameof(size));
        if (!double.IsFinite(gap) || gap < 0 || gap > 64)
            throw new ArgumentOutOfRangeException(nameof(gap));
        if (!double.IsFinite(inset) || inset < 0 || inset > 256)
            throw new ArgumentOutOfRangeException(nameof(inset));
        var result = new List<DockGuideSlot>(13);
        if (host.Width < size || host.Height < size)
        ...
        if (!paneTargets || visible.Width < size || visible.Height < size)
            return result.AsReadOnly();
        ...
        if (toolTargets)
            Ring(DockGuideScope.ToolBesideDocument, 2);
        return result.AsReadOnly();
        DockRect Cell(int x, int y)
        {
            ...
            return new(left, top, gap == 0 ? cx + (x + 1) * step - left : size, gap == 0 ? cy + (y + 1) * step - top : size);
        }
        ...
        {
            if (!Inside(clip, rect) || result.Any(slot => Overlaps(slot.Bounds, rect)))
                return;
    ...
    public static bool HitTest(DockRect bounds, DockPoint point) => bounds.Width > 0 && bounds.Height > 0 && double.IsFinite(point.X) && double.IsFinite(point.Y) && point.X >= bounds.X && point.X < bounds.Right && point.Y >= bounds.Y && point.Y < bounds.Bottom;
    ...
    {
        if (!double.IsFinite(r.X) || !double.IsFinite(r.Y) || !double.IsFinite(r.Width) || !double.IsFinite(r.Height) || !double.IsFinite(r.Right) || !double.IsFinite(r.Bottom) || r.Width < 0 || r.Height < 0)
