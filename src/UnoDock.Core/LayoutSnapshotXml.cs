...
public static class LayoutSnapshotXml
{
    private static XmlReaderSettings Settings(LayoutReadLimits limits) => new()
    {
        ...
        XmlResolver = null,
        CloseInput = false,
        MaxCharactersInDocument = limits.MaxCharacters,
        MaxCharactersFromEntities = limits.MaxCharacters,
        IgnoreComments = true,
    ...
    public static LayoutSnapshotNode Read(XmlReader reader, LayoutReadLimits? limits = null)
    {
        ...
        if (reader.ReadState == ReadState.Initial && !reader.Read())
            throw new XmlException("Empty layout document.");
        ...
        LayoutSnapshotNode Element(int depth)
        {
            if (depth > limits.MaxDepth || ++count > limits.MaxNodes)
                throw new XmlException("Layout node/depth limit exceeded.");
            if (reader.NodeType != XmlNodeType.Element)
                throw new XmlException("Expected a layout element, not " + reader.NodeType);
            if (reader.NamespaceURI.Length != 0)
                throw new XmlException("Namespaced layout elements are not supported.");
            var node = new LayoutSnapshotNode(reader.LocalName);
            characters += node.Name.Length;
            if (reader.AttributeCount > limits.MaxAttributesPerNode)
                throw new XmlException("Layout attribute limit exceeded.");
            if (reader.MoveToFirstAttribute())
            {
                do
                {
                    ...
                    if (characters > limits.MaxCharacters)
                        throw new XmlException("Layout character limit exceeded.");
                    // Standard serializer namespace declarations do not encode layout state.
                    if (reader.Prefix == "xmlns" || reader.Name == "xmlns")
                        continue;
                    if (reader.NamespaceURI.Length != 0)
                        throw new XmlException("Namespaced layout attributes are not supported.");
                    if (!node.Attributes.TryAdd(reader.LocalName, reader.Value))
                        throw new XmlException("Duplicate layout attribute.");
                }
            ...
            {
                if (reader.NodeType == XmlNodeType.Element)
                    node.Children.Add(Element(depth + 1));
                else if (reader.NodeType is XmlNodeType.Whitespace or XmlNodeType.SignificantWhitespace or XmlNodeType.Comment or XmlNodeType.ProcessingInstruction)
                {
                    ...
                    throw new XmlException("Unexpected layout content: " + reader.NodeType);
            }
