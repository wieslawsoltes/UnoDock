...
public static class DockInteractionGeometry
{
    ...
    public static DockPoint TranslateClientPoint(DockPoint point, DockPoint clientOffsetPixels, double sourceScale, double destinationScale)
    {
        ...
        if (!double.IsFinite(clientOffsetPixels.X) || !double.IsFinite(clientOffsetPixels.Y)) throw new ArgumentOutOfRangeException(nameof(clientOffsetPixels));
        if (!double.IsFinite(sourceScale) || sourceScale <= 0) throw new ArgumentOutOfRangeException(nameof(sourceScale));
        if (!double.IsFinite(destinationScale) || destinationScale <= 0) throw new ArgumentOutOfRangeException(nameof(destinationScale));
        var result = new DockPoint((point.X * sourceScale + clientOffsetPixels.X) / destinationScale,
            (point.Y * sourceScale + clientOffsetPixels.Y) / destinationScale);
        if (!double.IsFinite(result.X) || !double.IsFinite(result.Y)) throw new ArgumentOutOfRangeException(nameof(point), "Coordinate transform overflow.");
        return result;
    }
    ...
    public static double AutoScrollDelta(double pointerX, double viewportWidth, double offset, double maximumOffset,
        double elapsedSeconds, bool rightToLeft = false, double edgeWidth = 32, double maximumSpeed = 900)
    {
        if (!double.IsFinite(pointerX)) throw new ArgumentOutOfRangeException(nameof(pointerX));
        if (!double.IsFinite(viewportWidth) || viewportWidth < 0) throw new ArgumentOutOfRangeException(nameof(viewportWidth));
        if (!double.IsFinite(offset) || offset < 0) throw new ArgumentOutOfRangeException(nameof(offset));
        if (!double.IsFinite(maximumOffset) || maximumOffset < 0) throw new ArgumentOutOfRangeException(nameof(maximumOffset));
        if (!double.IsFinite(elapsedSeconds) || elapsedSeconds < 0) throw new ArgumentOutOfRangeException(nameof(elapsedSeconds));
        if (!double.IsFinite(edgeWidth) || edgeWidth <= 0) throw new ArgumentOutOfRangeException(nameof(edgeWidth));
        if (!double.IsFinite(maximumSpeed) || maximumSpeed < 0) throw new ArgumentOutOfRangeException(nameof(maximumSpeed));
        if (viewportWidth == 0 || maximumOffset == 0 || pointerX < 0 || pointerX > viewportWidth) return 0;
