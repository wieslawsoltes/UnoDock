...
public static class DockSplitSolver
{
    public static double[] Allocate(double available, double separator, ReadOnlySpan<DockMeasure> items)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            if (active[i])
                weights += Math.Max(item.Value, 1e-9);
            else
                fixedTotal += result[i] = Math.Max(item.Minimum, item.Unit == DockLengthUnit.Auto ? item.Desired : item.Value);
        }

        if (!double.IsFinite(fixedTotal + minimumTotal + weights))
            throw new ArgumentOutOfRangeException(nameof(items), "Aggregate layout dimensions overflow.");
        // Under impossible constraints, scale all minima proportionally instead of producing negatives.
        if (minimumTotal >= space)
        {
            for (var i = 0; i < count; i++)
                result[i] = minimumTotal > 0 ? space * (items[i].Minimum / minimumTotal) : space / count;
            return result;
        }
        ...
        for (var pass = 0; pass < count && weights > 0; pass++)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (size + 1e-9 >= items[i].Minimum)
                    continue;
                result[i] = items[i].Minimum;
                remaining -= result[i];
                weights -= Math.Max(items[i].Value, 1e-9);
                active[i] = false;
                clamped = true;
            }
        ...
        for (var i = 0; i < count; i++)
            if (active[i])
                result[i] = Math.Max(0, remaining * (Math.Max(items[i].Value, 1e-9) / weights));
