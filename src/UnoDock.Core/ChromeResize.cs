...
{
    public static DockRect Apply(DockRect start, ChromeHit hit, double dx, double dy, double minWidth, double minHeight, double maxWidth = double.PositiveInfinity, double maxHeight = double.PositiveInfinity)
    {
        if (!Enum.IsDefined(hit))
            throw new ArgumentOutOfRangeException(nameof(hit));
        if (!double.IsFinite(start.X) || !double.IsFinite(start.Y) || !double.IsFinite(start.Width) || !double.IsFinite(start.Height) || !double.IsFinite(start.Right) || !double.IsFinite(start.Bottom) || start.Width < 0 || start.Height < 0)
            throw new ArgumentOutOfRangeException(nameof(start));
        if (!double.IsFinite(dx) || !double.IsFinite(dy))
            throw new ArgumentOutOfRangeException(nameof(dx));
        Validate(minWidth, maxWidth);
        Validate(minHeight, maxHeight);
        if (hit == ChromeHit.Client)
            return start;
        DockRect result;
        if (hit == ChromeHit.Caption)
            result = start with
            {
                X = start.X + dx,
                Y = start.Y + dy
