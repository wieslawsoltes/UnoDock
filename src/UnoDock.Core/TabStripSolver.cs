...
public static class TabStripSolver
{
    ...
    {
        if (double.IsNaN(available) || available < 0) throw new ArgumentOutOfRangeException(nameof(available));
        var result = desired.ToArray();
        double sum = 0;
        foreach (var width in result)
        {
            if (!double.IsFinite(width) || width < 0) throw new ArgumentOutOfRangeException(nameof(desired));
            sum += width;
        }
        if (!double.IsFinite(sum)) throw new ArgumentOutOfRangeException(nameof(desired));
        if (sum <= available || result.Length == 0) return result;
        var sorted = (double[])result.Clone(); Array.Sort(sorted);
        var remaining = available; var cap = 0d;
        for (var i = 0; i < sorted.Length; i++)
        {
            cap = remaining / (sorted.Length - i);
            if (sorted[i] >= cap) break;
            remaining -= sorted[i];
        }
        for (var i = 0; i < result.Length; i++) result[i] = Math.Min(result[i], cap);
        return result;
    }
    ...
    public static int InsertionIndex(double position, ReadOnlySpan<double> widths)
    {
        ...
        for (var i = 0; i < widths.Length; i++)
        {
            var width = widths[i];
            if (!double.IsFinite(width) || width < 0) throw new ArgumentOutOfRangeException(nameof(widths));
            if (position < x + width / 2) return i;
            x += width;
        }
