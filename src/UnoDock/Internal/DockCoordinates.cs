...
internal static class DockCoordinates
{
    ...
    internal static Rect Bounds(FrameworkElement source, Rect bounds, FrameworkElement destination, ICrossWindowCoordinates? coordinates)
    {
        ...
        {
            translate = coordinates is DesktopWindowCoordinates desktop ? desktop.CreateTranslator(source, destination)
                : point => Translate(source, point, destination, coordinates);
        }
        return Enclose(translate(new(bounds.Left, bounds.Top)), translate(new(bounds.Right, bounds.Top)),
            translate(new(bounds.Left, bounds.Bottom)), translate(new(bounds.Right, bounds.Bottom)));
    }
    internal static Rect Enclose(Point a, Point b, Point c, Point d)
    {
        ...
        if (ContainsExtents(result, x, y, right, bottom)) return result;
        ...
        var leftEdge = RoundDown(x); var topEdge = RoundDown(y);
        var rightEdge = RoundUp(right); var bottomEdge = RoundUp(bottom);
    ...
    private static bool ContainsExtents(Rect value, double left, double top, double right, double bottom) =>
        double.IsFinite(value.Left) && double.IsFinite(value.Top) && double.IsFinite(value.Right) && double.IsFinite(value.Bottom) &&
    ...
    private static float RoundDown(double value)
    {
        ...
        if (result > value) result = MathF.BitDecrement(result);
        if (!float.IsFinite(result)) throw new InvalidOperationException("Coordinate bounds exceed WinRT range.");
        return result;
    }
    private static float RoundUp(double value)
    {
        var result = (float)value;
        if (result < value) result = MathF.BitIncrement(result);
        if (!float.IsFinite(result)) throw new InvalidOperationException("Coordinate bounds exceed WinRT range.");
        return result;
    }
