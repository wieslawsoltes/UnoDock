...
{
    internal IReadOnlyList<DockGuideTarget> GetDockingGuides(LayoutContent content, Point point)
        => BuildGuides(content, FindDropArea(point));

    private IReadOnlyList<DockGuideTarget> BuildGuides(LayoutContent content, IModelDropArea? area)
    {
        if (_disposed || Manager.DockingGuideMode == DockingGuideMode.EdgesOnly ||
            !ReferenceEquals(content.Root, Manager.Layout) || !DockOperations.CanMove(content) ||
            area?.Model is not ILayoutGroup target) return [];
        ...
        if (window != null)
        {
            try { host = DockCoordinates.Bounds(window, new(0, 0, window.ActualWidth, window.ActualHeight), this, Manager.CrossWindowCoordinates); }
            catch (Exception e) when (DockCoordinates.IsUnavailable(e)) { return []; }
        }
        ...
        var slots = DockGuideLayout.CreateStock(R(host), R(bounds), window == null && content is LayoutAnchorable,
            area.Type != DropAreaType.DockingManager, Manager.ShowDocumentPaneToolGuides && content is LayoutAnchorable && target is LayoutDocumentPane, size);
        var result = new List<DockGuideTarget>(slots.Count);
        foreach (var slot in slots)
        {
            var workspace = slot.Scope == DockGuideScope.Workspace;
            var offset = slot.Position switch { DockPosition.Left => 0, DockPosition.Top => 1, DockPosition.Right => 2, DockPosition.Bottom => 3, _ => 4 };
            var type = workspace ? (DropTargetType)offset : slot.Scope == DockGuideScope.ToolBesideDocument ? (DropTargetType)(15 + offset) :
                area.Type == DropAreaType.DocumentPane ? (DropTargetType)(4 + offset) :
    ...
    {
        if (_dragContent == null) { ShowDragPreview(null); return null; }
        var plan = ResolveDrop(_dragContent, point, out var guides);
        var windows = Manager.FloatingWindows.ToArray();
        var local = guides.Where(g => g.Plan.Target.FindParent<LayoutFloatingWindow>() is not { } f ||
            windows.All(w => !ReferenceEquals(w.Model, f) || w.NativeWindow == null)).ToArray();
