...
internal struct DockCleanup
{
    ...
    internal void Attempt(Action action)
    {
        ...
        catch (CleanupFailures errors) { (_failures ??= []).AddRange(errors.InnerExceptions); }
    ...
    private sealed class CleanupFailures(IEnumerable<Exception> failures)
        : AggregateException("Docking cleanup callbacks failed.", failures) { }
}
