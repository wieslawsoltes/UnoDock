...
internal sealed class FloatingResizeBoundsTracker(DockRect initial, bool splitRequests)
{
    ...
    internal bool Observe(DockRect actual)
    {
        ...
        for (var i = _pending.Count - 1; i >= 0; i--)
            ...
            {
                _acknowledged = _pending[i];
                _pending.RemoveRange(0, i + 1);
                return true;
            }

        if (Same(actual, _acknowledged))
            return true;
        if (splitRequests)
            for (var i = 0; i < _pending.Count; i++)
            {
                var before = i == 0 ? _acknowledged : _pending[i - 1];
                var after = _pending[i];
                // AppWindow.Resize precedes Move on the X11 backend. Also allow
                // position-first WM application; both components must be ours.
                if (Same(actual, new(before.X, before.Y, after.Width, after.Height)) || Same(actual, new(after.X, after.Y, before.Width, before.Height)))
                    return true;
            }
    ...
    private static bool Same(DockRect a, DockRect b) => Math.Abs(a.X - b.X) < 0.000001 && Math.Abs(a.Y - b.Y) < 0.000001 && Math.Abs(a.Width - b.Width) < 0.000001 && Math.Abs(a.Height - b.Height) < 0.000001;
}
