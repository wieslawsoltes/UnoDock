...
internal sealed class MacFloatingChrome(Window window) : NativeFloatingChrome(window)
{
    ...
    protected override void EnableCore()
    {
        ...
        _movable = Boolean(_handle, "isMovable"); _backgroundMovable = Boolean(_handle, "isMovableByWindowBackground");
        for (var i = 0; i < 3; i++)
        {
            var button = ObjectWithInteger(_handle, Sel("standardWindowButton:"), i);
            if (button != 0) _buttons.Add((Retain(button), Boolean(button, "isHidden")));
        }
        _changed = true;
        ...
        foreach (var (button, _) in _buttons) SetBoolean(button, Sel("setHidden:"), true);
        SetFrame(_handle, Sel("setFrame:display:"), frame, true);
    }
    internal override DockRect ReadBounds()
    {
        Verify(); var r = ReadRect(_handle, "frame");
    ...
    {
        Verify();
        SetFrame(_handle, Sel("setFrame:display:"), new NativeRect
        {
            Origin = new() { X = bounds.X, Y = -bounds.Y - bounds.Height },
            Size = new() { Width = bounds.Width, Height = bounds.Height }
        }, true);
    }
    protected override void RestoreCore()
    {
        if (!_changed || _handle == 0) return;
        var frame = ReadRect(_handle, "frame");
        ...
        if (Integer(_handle, Sel("titleVisibility")) == 1) SetInteger(_handle, Sel("setTitleVisibility:"), _titleVisibility);
        if (Boolean(_handle, "titlebarAppearsTransparent")) SetBoolean(_handle, Sel("setTitlebarAppearsTransparent:"), _transparent);
        if (!Boolean(_handle, "isMovable")) SetBoolean(_handle, Sel("setMovable:"), _movable);
        if (!Boolean(_handle, "isMovableByWindowBackground")) SetBoolean(_handle, Sel("setMovableByWindowBackground:"), _backgroundMovable);
        foreach (var (button, hidden) in _buttons) if (Boolean(button, "isHidden")) SetBoolean(button, Sel("setHidden:"), hidden);
        SetFrame(_handle, Sel("setFrame:display:"), frame, true);
    }
    protected override void ReleaseCore()
    {
        foreach (var (button, _) in _buttons) ReleaseObject(button);
        _buttons.Clear(); var handle = _handle; _handle = 0;
    ...
    private static NativeRect ReadRect(nint value, string selector)
    {
        ...
        { RectStret(out var rect, value, Sel(selector)); return rect; }
        return Rect(value, Sel(selector));
    }
    [StructLayout(LayoutKind.Sequential)] private struct NativePoint { internal double X, Y; }
    [StructLayout(LayoutKind.Sequential)] private struct NativeSize { internal double Width, Height; }
    [StructLayout(LayoutKind.Sequential)] private struct NativeRect { internal NativePoint Origin; internal NativeSize Size; }
    [DllImport(ObjC, EntryPoint = "objc_retain")] private static extern nint Retain(nint value);
    [DllImport(ObjC, EntryPoint = "objc_release")] private static extern void ReleaseObject(nint value);
    ...
    [DllImport(ObjC, EntryPoint = "objc_msgSend")] private static extern nint ObjectWithInteger(nint value, nint selector, long argument);
    [DllImport(ObjC, EntryPoint = "objc_msgSend")] private static extern void SetInteger(nint value, nint selector, long argument);
    [DllImport(ObjC, EntryPoint = "objc_msgSend")] private static extern void SetBoolean(nint value, nint selector, [MarshalAs(UnmanagedType.I1)] bool argument);
    ...
    [DllImport(ObjC, EntryPoint = "objc_msgSend")] private static extern void SetFrame(nint value, nint selector, NativeRect frame, [MarshalAs(UnmanagedType.I1)] bool display);
}
#endif
