...
internal sealed partial class DockSurface : Grid, IDisposable
{
    ...
    private readonly Grid _flyouts = new()
    {
    ...
    internal DockSurface(DockingManager manager)
    {
        ...
        {
            PositionAutoHide();
            foreach (var window in Manager.FloatingWindows.Where(w => w.NativeWindow == null && w.IsMaximized).ToArray())
                window.UpdateView();
        };
        AddHandler(PointerPressedEvent, new PointerEventHandler((_, e) =>
        {
            ...
            for (var d = e.OriginalSource as DependencyObject; d != null; d = VisualTreeHelper.GetParent(d))
                if (ReferenceEquals(d, _autoHide) || d is LayoutAnchorControl)
                    return;
            CloseAutoHide();
        }), true);
    }
    ...
    internal FrameworkElement GetView(ILayoutElement model)
    {
        ...
        {
            LayoutPanel p => new LayoutPanelControl(p),
            LayoutDocumentPaneGroup p => new LayoutDocumentPaneGroupControl(p),
            LayoutAnchorablePaneGroup p => new LayoutAnchorablePaneGroupControl(p),
    ...
    internal void UpdateView(ILayoutElement model)
    {
        ...
        if (view is IRefreshableLayoutControl refreshable)
            refreshable.Update(this);
        if (view is LayoutAnchorSideControl side)
            side.Update(Manager);
    }
    ...
    internal void Update()
    {
        ...
        Manager.LayoutRootPanel = panel;
        var top = (LayoutAnchorSideControl)GetView(root.TopSide);
        var bottom = (LayoutAnchorSideControl)GetView(root.BottomSide);
        var left = (LayoutAnchorSideControl)GetView(root.LeftSide);
        var right = (LayoutAnchorSideControl)GetView(root.RightSide);
        top.Update(Manager);
        bottom.Update(Manager);
        left.Update(Manager);
        right.Update(Manager);
        Manager.TopSidePanel = top;
        Manager.BottomSidePanel = bottom;
        Manager.LeftSidePanel = left;
        Manager.RightSidePanel = right;
        Grid.SetRow(panel, 1);
        ...
        {
            if (_views[stale] is LayoutCachePaneControl pane)
                pane.ReleaseViews();
            VisualParenting.Detach(_views[stale]);
            _views.Remove(stale);
        }

        foreach (var floating in _floats.Children.OfType<LayoutFloatingWindowControl>().Where(f => !ReferenceEquals(f.Model.Root, root)).ToArray())
            _floats.Children.Remove(floating);
        if (_autoHide?.Model is LayoutAnchorable a && (!ReferenceEquals(a.Root, root) || !a.IsAutoHidden))
            CloseAutoHide();
        else
            PositionAutoHide();
    }
    ...
    {
        if (_disposed || !model.IsEnabled || !model.IsAutoHidden || model.Root?.Manager != Manager)
            return;
        var generation = ++_autoHideGeneration;
        StopAutoHideTimer();
        var view = _autoHide ?? Manager.CreateAutoHideView();
        if (generation != _autoHideGeneration || _disposed)
            return;
        _autoHide = view;
        if (!_flyouts.Children.Contains(view))
            _flyouts.Children.Add(view);
        if (generation != _autoHideGeneration)
            return;
        _flyouts.IsHitTestVisible = true;
        view.Open(model, activate);
        if (generation != _autoHideGeneration)
            return;
        PositionAutoHide();
        if (generation == _autoHideGeneration && view.Model is LayoutAnchorable)
            Manager.SetAutoHideHost(view);
    }
    ...
    {
        if (_autoHide?.Model is not LayoutAnchorable model)
            return;
        if (!model.IsEnabled || !model.IsAutoHidden || !ReferenceEquals(model.Root, Manager.Layout))
        {
            CloseAutoHide();
            return;
        }
    ...
    internal void CloseAutoHide()
    {
        ...
        finally
        {
            ...
            {
                _flyouts.Children.Remove(view);
                _flyouts.IsHitTestVisible = _navigator != null;
                if (ReferenceEquals(Manager.AutoHideWindow, view))
    ...
    {
        _autoHideTimer.Stop();
        if (_disposed || _autoHide?.Model is not LayoutAnchorable)
            return;
    ...
    {
        _autoHideTimer.Stop();
        if (_autoHide == null || _autoHide.RetainOpen)
            return;
        CloseAutoHide();
    }
    ...
    internal void BeginDrag(LayoutContent content, FrameworkElement source, PointerRoutedEventArgs args)
    {
        ...
        {
            input.DockDragMoved += OnDragMoved;
            input.DockDragReleased += OnDragReleased;
            input.DockInputCancelled += OnDockInputCancelled;
        }
        else
        {
        ...
        source.PointerCanceled += OnDragCancelled;
        source.PointerCaptureLost += OnCaptureLost;
        source.Unloaded += OnDragSourceUnloaded;
    ...
    private void OnDockInputCancelled(object? sender, uint id)
    {
        if (ReferenceEquals(sender, _dragInput) && _drag.OwnsPointer(id))
            CancelDrag();
    }
    ...
    {
        if (view.XamlRoot == null || view.ActualWidth <= 0 || view.ActualHeight <= 0)
            return false;
    ...
    private IModelDropArea? FindDropArea(Point point)
    {
        ...
        try
        {
            if (Manager.CrossWindowCoordinates is DesktopWindowCoordinates coordinates && coordinates.TryGetTopmostRootExcludingWindow(this, point, out var hitRoot, _floatingDrag?.Window.NativeWindow))
            {
                if (hitRoot == null)
                    return null;
                floating = ReferenceEquals(hitRoot, XamlRoot) ? FloatingAt(point, inSurfaceOnly: true) : Manager.FloatingWindows.FirstOrDefault(w => ReferenceEquals(w.XamlRoot, hitRoot));
                if (!ReferenceEquals(hitRoot, XamlRoot) && floating == null)
                    return null;
            }
            else
    ...
    {
        if (area.Model is not ILayoutGroup target)
            return null;
        ...
        if (headerOnly && pane?.IsOverHeader(point, this) != true)
            return null;
        // A tab strip is an insertion surface, not the pane's top split zone.
        var position = pane?.IsOverHeader(point, this) == true ? DockPosition.Inside : DockSplitSolver.HitTest(new(bounds.X, bounds.Y, bounds.Width, bounds.Height), new(point.X, point.Y));
        ...
        switch (area.Type)
        {
            case DropAreaType.DockingManager:
                if (offset == 4)
                    return null;
                type = (DropTargetType)offset;
                break;
            case DropAreaType.DocumentPane:
                type = (DropTargetType)(4 + offset);
                break;
            case DropAreaType.AnchorablePane:
                type = (DropTargetType)(10 + offset);
                break;
            case DropAreaType.DocumentPaneGroup:
                type = DropTargetType.DocumentPaneGroupDockInside;
                break;
            default:
                return null;
        }
    ...
    {
        if (_dragContent == null || !_drag.OwnsPointer(args.Pointer.PointerId))
            return;
        if (!TryGetPoint(args, out var point))
        {
            CancelDrag();
    ...
    private void OnDragReleased(object sender, PointerRoutedEventArgs args)
    {
        ...
        if (content == null || !_drag.OwnsPointer(args.Pointer.PointerId))
            return;
        if (!TryGetPoint(args, out var point))
        {
            CancelDrag();
    ...
    {
        if (_drag.OwnsPointer(args.Pointer.PointerId))
            CancelDrag();
    }

    private void OnCaptureLost(object sender, PointerRoutedEventArgs args)
    {
        if (_dragSource != null && ReferenceEquals(args.OriginalSource, _dragSource) && _drag.OwnsPointer(args.Pointer.PointerId))
            CancelDrag();
    }
    ...
    private LayoutFloatingWindowControl? FloatingAt(Point point, bool inSurfaceOnly = false)
    {
        ...
        {
            if (ReferenceEquals(window, _floatingDrag?.Window) || inSurfaceOnly && window.NativeWindow != null || !Visible(window) || window.NativeWindow is { } native && !native.AppWindow.IsVisible)
                continue;
            try
            {
    ...
    private void ShowDragPreview(DockDropPlan? plan)
    {
        ...
        {
            if (generation != _floatingDragGeneration)
                break;
            cleanup.Attempt(window.HideDropPreview);
        }

        cleanup.ThrowIfFailed();
        if (generation != _floatingDragGeneration || plan == null)
            return;
        ...
        if (host?.NativeWindow == null)
            _overlay.ShowPreview(plan, accent);
        else
            host.ShowDropPreview(plan, this, accent);
    }
    ...
    private void DetachDrag()
    {
        ...
        if (source != null)
        {
            ...
            {
                source.RemoveHandler(PointerMovedEvent, new PointerEventHandler(OnDragMoved));
                source.RemoveHandler(PointerReleasedEvent, new PointerEventHandler(OnDragReleased));
            }
        ...
        if (source != null && !ReferenceEquals(source, _dragSource))
            cleanup.Attempt(source.ReleasePointerCaptures);
        cleanup.ThrowIfFailed();
    }
    ...
    internal void Reset()
    {
        ...
        foreach (var view in _views.Values)
            if (view is LayoutCachePaneControl pane)
                pane.ReleaseViews();
        _views.Clear();
        _docked.Children.Clear();
        _floats.Children.Clear();
    }
