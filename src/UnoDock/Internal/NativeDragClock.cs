...
internal sealed class NativeDragClock : IDisposable
{
    ...
    [StructLayout(LayoutKind.Sequential)] private struct TimerContext
    ...
    internal NativeDragClock(Action tick, Action<Exception> failed)
    {
        ...
        {
            _dispatcherTimer = new DispatcherTimer { Interval = TimeSpan.FromMilliseconds(16) };
        ...
        try
        {
            var context = new TimerContext { Info = GCHandle.ToIntPtr(_self) };
    ...
    private static void OnNativeTick(nint timer, nint info)
    {
        ...
        if (GCHandle.FromIntPtr(info).Target is NativeDragClock clock) clock.Invoke();
    }
    private void Invoke()
    {
        if (_disposed) return;
    ...
    [DllImport(CoreFoundation, EntryPoint = "CFRunLoopTimerInvalidate")] private static extern void InvalidateTimer(nint timer);
    [DllImport(CoreFoundation, EntryPoint = "CFRelease")] private static extern void ReleaseObject(nint value);
}
