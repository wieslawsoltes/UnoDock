...
internal sealed class FloatingDockSession : IDisposable
{
    ...
    {
        _manager = manager; _root = manager.Layout; Window = window; _model = window.Model;
        Contents = window.Contents.ToArray();
        if (Contents.Length == 0) throw new ArgumentException("The floating window has no content.", nameof(window));
        Representative = Contents.FirstOrDefault(c => c.IsActive) ?? Contents.FirstOrDefault(c => c.IsSelected) ?? Contents[0];
        _nodes = new[] { _model }.Concat(_model.Descendents()).Select(node => (node, node.Parent)).ToArray();
        foreach (var (node, _) in _nodes) node.PropertyChanged += OnNodeChanged;
    ...
    private void OnNodeChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        if (string.IsNullOrEmpty(e.PropertyName) || e.PropertyName is "Parent" or "Root" or "ChildrenCount" or
            "RootPanel" or "RootDocument" or "CanMove" or "CanFloat" or "CanRepositionItems" or
            "CanDockAsTabbedDocument" or "IsEnabled" or "IsHidden" or "Orientation") _invalid = true;
    }
    internal bool IsCurrent => !_disposed && !_invalid && _manager.IsEnabled && Window.IsEnabled && !Window.IsContentImmutable &&
        ReferenceEquals(_manager.Layout, _root) && ReferenceEquals(_root.Manager, _manager) &&
        ReferenceEquals(Window.Model, _model) && ReferenceEquals(_model.Root, _root) &&
        _manager.FloatingWindows.Any(w => ReferenceEquals(w, Window)) &&
        _nodes.All(pair => ReferenceEquals(pair.Node.Parent, pair.Parent)) &&
        Contents.All(c => DockOperations.CanMove(c) && ReferenceEquals(c.FindParent<LayoutFloatingWindow>(), _model));

    internal bool CanAccept(DockDropPlan? plan)
    {
        if (plan == null || !IsCurrent || !ReferenceEquals(plan.Content, Representative) ||
            !ReferenceEquals(plan.Target.Root, _root) || ReferenceEquals(plan.Target.FindParent<LayoutFloatingWindow>(), _model)) return false;
        foreach (var content in Contents)
            if (DockDropPlan.Create(content, plan.Target, plan.Type, new Rect(0, 0, 1, 1), plan.InsertionIndex) == null) return false;
        if (plan.Position == DockPosition.Inside && plan.Target is ILayoutPositionableElement { AllowDuplicateContent: false } &&
            Contents.Select(c => (c.Title, c.ContentId)).Distinct().Count() != Contents.Length) return false;
        return IsCurrent;
    }
    internal bool Execute(DockDropPlan? plan)
    {
        if (_consumed || !CanAccept(plan)) return false;
        _consumed = true;
        ...
        {
            if (string.IsNullOrEmpty(e.PropertyName) || e.PropertyName is "Parent" or "Root" or "ChildrenCount" or
                "Orientation" or "AllowDuplicateContent" or "CanRepositionItems") targetChanged = true;
        }
        foreach (var node in targetNodes) node.PropertyChanged += Changed;
        ...
        finally
        {
            ...
            foreach (var lease in leases)
                try { lease.Dispose(); }
                catch (Exception e) { failure = failure == null ? e : new AggregateException("Docking and completion observers failed.", failure, e); }
        }
        ...
        bool Apply()
        {
            foreach (var content in Contents)
            {
                if (targetChanged || !CanAccept(plan)) return false;
            ...
            if (plan.Position == DockPosition.Inside)
            {
                ...
                {
                    if (group.ChildrenCount != 0) return false;
                    var pane = new LayoutDocumentPane(); group.Children.Add(pane); destination = pane;
                }
                ...
                foreach (var content in Contents)
                {
                    ...
                    if (!WorkspaceCurrent() || !ReferenceEquals(destination.Root, _root) ||
                        !ReferenceEquals(content.FindParent<LayoutFloatingWindow>(), _model) ||
                        !DockOperations.CanDock(content, destination, DockPosition.Inside)) return false;
                    destination.InsertChildAt(Math.Min(index++, destination.ChildrenCount), content);
                    if (!ReferenceEquals(content.Parent, destination)) return false;
            ...
            else
            {
                ...
                ILayoutPanelElement incoming = !asDocument && _model is LayoutAnchorableFloatingWindow { RootPanel: { } panel }
                    ? panel : new LayoutDocumentPane();
                if ((int)plan.Type <= 3) DockOperations.AddAtRoot(_root, incoming, plan.Position switch
                { DockPosition.Left => AnchorSide.Left, DockPosition.Top => AnchorSide.Top, DockPosition.Bottom => AnchorSide.Bottom, _ => AnchorSide.Right });
                else if (!InsertBeside(target, incoming, plan.Position)) return false;
                if (!WorkspaceCurrent() || !ReferenceEquals(incoming.Root, _root)) return false;
                if (incoming is LayoutDocumentPane documents)
                    foreach (var content in Contents)
                    {
                        if (!WorkspaceCurrent() || !ReferenceEquals(content.FindParent<LayoutFloatingWindow>(), _model) || !DockOperations.CanMove(content)) return false;
                        documents.Children.Add(content);
                        if (!ReferenceEquals(content.Parent, documents)) return false;
                    }
                foreach (var content in Contents)
                    if (ReferenceEquals(content.Root, _root)) content.SetPrevious(null, 0);
            }
            if (!WorkspaceCurrent()) return false;
            if (ReferenceEquals(Representative.Root, _root)) Representative.IsActive = true;
    ...
    { for (ILayoutElement? current = element; current != null; current = current.Parent) yield return current; }
    private static bool InsertBeside(ILayoutGroup target, ILayoutPanelElement incoming, DockPosition position)
    {
        if (target is not ILayoutPanelElement anchor || target.Parent is not ILayoutGroup parent) return false;
        ...
        {
            if (parent is not ILayoutPanelElement outer || parent.Parent is not ILayoutGroup outerParent) return false;
            anchor = outer; parent = outerParent;
        }
        ...
        else
        {
            ILayoutGroup? wrapper = parent switch
            {
                ...
                LayoutDocumentPaneGroup when incoming is ILayoutDocumentPane && anchor is ILayoutDocumentPane => new LayoutDocumentPaneGroup { Orientation = orientation },
                LayoutAnchorablePaneGroup when incoming is ILayoutAnchorablePane && anchor is ILayoutAnchorablePane => new LayoutAnchorablePaneGroup { Orientation = orientation },
                _ => null
            };
            if (wrapper == null) return false;
            parent.ReplaceChild(anchor, wrapper);
            if (!ReferenceEquals(wrapper.Parent, parent)) return false;
