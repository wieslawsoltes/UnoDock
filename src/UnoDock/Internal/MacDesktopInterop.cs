...
internal static class MacDesktopInterop
{
    ...
    {
        public double X, Y;
        internal NativePoint(double x, double y)
        {
            X = x;
            Y = y;
        }
    ...
    internal static nint Handle(Window window)
    {
        ...
        if (value is not nint handle || handle == 0)
            throw new InvalidOperationException("The AppKit window has closed or its native handle is unavailable.");
        return handle;
    }
    ...
    internal static Point ToScreen(Window window, Point point)
    {
        ...
        var view = ContentView(handle);
        var bounds = Rect(view, "bounds");
        var local = new NativePoint(bounds.Origin.X + point.X, bounds.Origin.Y + (SendLong(view, Sel("isFlipped")) != 0 ? point.Y : bounds.Size.Height - point.Y));
    ...
    internal static bool IsCaption(Window window, Point point)
    {
        ...
        return point.X >= frame.Origin.X && point.X < frame.Origin.X + frame.Size.Width && point.Y >= top.Y && point.Y < frame.Origin.Y + frame.Size.Height;
    }

    internal static DesktopPointerState Pointer()
    {
        var type = Class("NSEvent");
        var p = SendPointResult(type, Sel("mouseLocation"));
        return new(new(p.X, p.Y), (SendLong(type, Sel("pressedMouseButtons")) & 1) != 0, (SendLong(type, Sel("modifierFlags")) & (1L << 18)) != 0, KeyState(0, 53));
    }
    ...
    internal static XamlRoot? HitTest(Point point, Window? excluded)
    {
        ...
        {
            if (ReferenceEquals(candidate, excluded) || candidate.Content?.XamlRoot is not { } root)
                continue;
            if (SendLong(Handle(candidate), Sel("windowNumber")) != number)
                continue;
    ...
    {
        var child = Handle(window);
        var parent = Handle(owner);
        if (child == parent)
            throw new ArgumentException("A floating window cannot own itself.");
        Retain(parent);
        Retain(child);
    ...
    private sealed class OwnerLease(nint parent, nint child) : IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            try
            {
                if (Send(child, Sel("parentWindow")) == owner)
                    SendVoidObject(owner, Sel("removeChildWindow:"), child);
            }
            finally
            {
                Release(child);
    ...
    private static NativeRect Rect(nint receiver, string selector)
    {
        ...
        {
            SendRectStret(out var rect, receiver, Sel(selector));
            return rect;
        }
    ...
    private static extern nint Class(string name);
    ...
    [DllImport(ObjC, EntryPoint = "objc_msgSend")]
    private static extern NativePoint SendPoint(nint receiver, nint selector, NativePoint point);
    [DllImport(ObjC, EntryPoint = "objc_msgSend")]
    private static extern NativePoint SendPointView(nint receiver, nint selector, NativePoint point, nint view);
    [DllImport(ObjC, EntryPoint = "objc_msgSend")]
    private static extern void SendVoidPoint(nint receiver, nint selector, NativePoint point);
    [DllImport(ObjC, EntryPoint = "objc_msgSend")]
    private static extern void SendVoidObject(nint receiver, nint selector, nint value);
    [DllImport(ObjC, EntryPoint = "objc_msgSend")]
    ...
    private static extern void SendRectStret(out NativeRect result, nint receiver, nint selector);
    [DllImport(ObjC, EntryPoint = "objc_msgSend")]
    private static extern nint WindowAtPoint(nint receiver, nint selector, NativePoint point, nint below);
    [DllImport(ObjC, EntryPoint = "objc_msgSend")]
    private static extern void SendChild(nint receiver, nint selector, nint child, nint order);
    [DllImport(Quartz, EntryPoint = "CGEventSourceKeyState")]
    [return: MarshalAs(UnmanagedType.I1)]
    private static extern bool KeyState(int state, ushort key);
}
#endif
