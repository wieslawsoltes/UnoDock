...
internal sealed partial class DockSurface
{
    ...
    internal void ShowNavigator(NavigatorWindow navigator)
    {
        ...
        if (!DispatcherQueue.HasThreadAccess)
            throw new InvalidOperationException("Navigator presentation requires its owning UI thread.");
        if (!ReferenceEquals(navigator.OwnerManager, Manager))
            throw new ArgumentException("The navigator belongs to another docking manager.", nameof(navigator));
        if (navigator.IsSelectionWindowClosed)
            throw new InvalidOperationException("A closed navigator cannot be shown again.");
        if (_disposed || !Manager.IsEnabled)
            return;
        if (_navigator is { } existing)
        {
            if (existing.HasSelectionSession)
                existing.Advance(InputState.ShiftDown ? -1 : 1);
            return;
        }
        ...
        try
        {
            ...
            if (!Current() || !navigator.HasSelectionSession)
                return;
            if (!navigator.Focus(FocusState.Programmatic))
        ...
        catch (Exception failure)
        {
            if (OwnsNavigator(navigator) && generation == _navigatorGeneration)
            {
                ...
                {
                    throw new AggregateException("Navigator opening and cleanup both failed.", failure, cleanup);
                }
            }
        ...
        {
            if (OwnsNavigator(navigator) && generation == _navigatorGeneration && (!navigator.HasSelectionSession || !ReferenceEquals(Manager.Layout, root) || !ReferenceEquals(root.Manager, Manager)))
                CloseNavigator(false);
        }
    }
    ...
    internal void CloseNavigator(bool commit)
    {
        ...
        _navigator = null;
        var generation = ++_navigatorGeneration;
        bool Current() => root != null && !_disposed && _navigator == null && _navigatorGeneration == generation && ReferenceEquals(Manager.Layout, root) && ReferenceEquals(root.Manager, Manager);
        var detached = false;
        void Detach()
        {
            if (detached)
        ...
        {
            if (commit)
                navigator.CommitClosingSelection(Current, Detach);
        }
        finally
        {
            Detach();
    ...
    internal void CommitDirectNavigatorSelection(NavigatorWindow navigator, bool closeWindow, Func<bool> ownsSelection)
    {
        if (!DispatcherQueue.HasThreadAccess)
            throw new InvalidOperationException("Navigator presentation requires its owning UI thread.");
        if (!OwnsNavigator(navigator) || navigator.SelectionSessionRoot is not { } root)
            return;
        var generation = _navigatorGeneration;
        var detached = false;
        bool Current() => ownsSelection() && !_disposed && _navigatorGeneration == generation && ReferenceEquals(Manager.Layout, root) && ReferenceEquals(root.Manager, Manager) && (detached ? _navigator == null : OwnsNavigator(navigator) && navigator.HasSelectionSession);
        navigator.CommitDirectSelection(Current, () =>
        {
            ...
            if (closeWindow && !navigator.RequestSelectionClose())
                return;
            if (!Current())
                return;
            _navigator = null;
            ...
            DetachNavigator(navigator);
            if (closeWindow)
                navigator.CompleteSelectionClose();
        });
        if (detached)
            RestoreNavigatorFocus(root, Current);
    }
    ...
    private void DetachNavigator(NavigatorWindow navigator)
    {
        ...
        finally
        {
            try
            {
                _flyouts.Children.Remove(navigator);
            }
            finally
            {
                _flyouts.IsHitTestVisible = _navigator != null || _autoHide?.Visibility == Visibility.Visible;
            }
        }
    }
    ...
    {
        if (root == null || !ownsOperation() || root.ActiveContent is not { } active || !ReferenceEquals(active.Root, root))
            return;
        ...
        if (!CurrentFocus())
            return;
        if (active is LayoutAnchorable { IsAutoHidden: true } tool)
            OpenAutoHide(tool);
        if (!CurrentFocus())
            return;
