...
internal static class MenuContext
{
    ...
    {
        if (States.TryGetValue(menu, out var state)) Request(menu, state, false, null);
    }
    private static void Request(MenuFlyout menu, State state, bool apply, object? context)
    {
        ...
        try
        {
            ...
            while (state.Pending)
            {
                if (--budget == 0) throw new InvalidOperationException("Menu context callbacks did not converge.");
                var assign = state.ApplyPending; var value = state.Context;
                state.Pending = false; state.Context = null;
                ...
                if (state.Pending || !assign) continue;
        ...
        {
            state.Pending = false; state.Context = null;
            state.CleaningAfterFailure = false; state.Updating = false;
        }
    }
}
