...
public partial class DockingManager : Control, IDisposable, Xceed.Wpf.AvalonDock.Compatibility.IWeakEventListener
{
    ...
    public LayoutRoot Layout
    {
        ...
        set
        {
            ...
            if (value.Manager != null && !ReferenceEquals(value.Manager, this)) throw new InvalidOperationException("A layout cannot be owned by two docking managers.");
    ...
    public IDisposable BeginLayoutUpdate()
    {
        ...
        _suspendSources++;
        return new ActionDisposable(() => { if (--_suspendSources == 0) { ReconcileSources(); InvalidateView(); } });
    }
        ...
        switch (name)
        {
            case nameof(ActiveContent):
                if (!_syncActive)
                {
                    ...
                    if (selected != null && selected.Root == Layout) selected.IsActive = true;
                    else if (e.NewValue == null) Layout.ActiveContent = null;
                }
                ActiveContentChanged?.Invoke(this, EventArgs.Empty); break;
            case nameof(DocumentsSource): _documentObserver?.Dispose(); _documentObserver = DocumentsSource == null ? null : new(this, DocumentsSource, true); ReconcileSources(); break;
            case nameof(AnchorablesSource): _anchorableObserver?.Dispose(); _anchorableObserver = AnchorablesSource == null ? null : new(this, AnchorablesSource, false); ReconcileSources(); break;
            case nameof(Theme):
                if (_themeResources != null) Resources.MergedDictionaries.Remove(_themeResources);
                _themeResources = Theme?.GetResourceDictionary(); if (_themeResources != null) Resources.MergedDictionaries.Add(_themeResources);
                break;
            case nameof(LayoutItemContainerStyle): case nameof(LayoutItemContainerStyleSelector):
                foreach (var item in _items.Values) ApplyItemStyle(item); break;
        }
        InvalidateView();
    }
    private void OnLayoutModelUpdated(object? sender, EventArgs e) { SyncActive(); InvalidateView(); }
    private void SyncActive()
    {
        if (_syncActive) return;
        var content = Layout.ActiveContent?.Content;
        if (ReferenceEquals(content, ActiveContent)) return;
        _syncActive = true;
        try { SetValue(ActiveContentProperty, content); } finally { _syncActive = false; }
    }
    internal void InvalidateView() { if (!_disposed) _updates.Invalidate(); }
    private void ScheduleRender()
    {
        if (_renderPending || _host == null || _disposed || !_loaded) return;
        _renderPending = true;
        if (DispatcherQueue?.TryEnqueue(() => { _renderPending = false; if (!_disposed && _loaded) RenderNow(); }) != true) _renderPending = false;
    }
    public void Refresh() { ObjectDisposedException.ThrowIf(_disposed, this); RenderNow(); }
    internal void RenderNow()
    {
        if (_host == null) return;
        _surface ??= new(this);
        if (!ReferenceEquals(_host.Content, _surface)) _host.Content = _surface;
        try
        {
            _surface.Update(); SyncFloatingWindows();
            foreach (var key in _items.Keys.Where(c => !ReferenceEquals(c.Root, Layout)).ToArray()) { _items[key].Dispose(); _items.Remove(key); }
            LastRenderedLayout = Layout;
        }
        catch (Exception ex) { RenderingFailed?.Invoke(this, ex); throw; }
    }
    ...
    public LayoutItem GetLayoutItemFromModel(LayoutContent content)
    {
        ...
        if (_items.TryGetValue(content, out var existing)) return existing;
        LayoutItem item = content is LayoutAnchorable ? new LayoutAnchorableItem() : new LayoutDocumentItem();
        _items.Add(content, item); item.Attach(content, this); ApplyItemStyle(item); return item;
    }
    ...
    internal bool RaiseDocumentClosing(LayoutDocument document) { var args = new DocumentClosingEventArgs(document); DocumentClosing?.Invoke(this, args); return args.Cancel; }
    ...
    internal bool RaiseDockEvent(DockRoutedEvent id, LayoutContent content)
    {
        ...
        if (id == PreviewDockEvent) PreviewDock?.Invoke(this, args); else if (id == DockedEvent) Docked?.Invoke(this, args);
        else if (id == PreviewFloatEvent) PreviewFloat?.Invoke(this, args); else Floated?.Invoke(this, args);
        if (_handlers.TryGetValue(id, out var handlers)) foreach (var handler in handlers.ToArray()) handler(this, args);
        if ((id == PreviewDockEvent || id == PreviewFloatEvent) && _transitions.ContainsKey(content)) _transitions[content] &= !args.Cancel;
        return !args.Cancel;
    }
    ...
    { if (!_handlers.TryGetValue(routedEvent, out var list)) _handlers[routedEvent] = list = []; list.Add(handler); }
    public void RemoveHandler(DockRoutedEvent routedEvent, RoutedEventHandler handler) { if (_handlers.TryGetValue(routedEvent, out var list)) list.Remove(handler); }
    ...
    {
        var existing = _floating.FirstOrDefault(w => ReferenceEquals(w.Model, model)); if (existing != null) return existing;
        ...
        _floating.Add(control); return control;
    }
    private void SyncFloatingWindows()
    {
        foreach (var window in _floating.Where(w => !Layout.FloatingWindows.Contains((LayoutFloatingWindow)w.Model)).ToArray()) { window.CloseHost(); _floating.Remove(window); }
        foreach (var model in Layout.FloatingWindows.Where(f => f.IsValid))
        {
            var control = EnsureFloatingWindow(model); control.UpdateView();
            ...
            if (_loaded) { if (native) control.ShowNative(); else _surface?.ShowFloating(control); }
        }
    }
    ...
    public void Dispose()
    {
        ...
        Loaded -= OnLoaded; Unloaded -= OnUnloaded;
        _documentObserver?.Dispose(); _anchorableObserver?.Dispose();
        Layout.Updated -= OnLayoutModelUpdated; Layout.Manager = null;
