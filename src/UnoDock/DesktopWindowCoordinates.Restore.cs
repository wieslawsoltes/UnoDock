...
public sealed partial class DesktopWindowCoordinates
{
    ...
    internal void PrepareRestore(Window window)
    {
        ...
        if (states.Contains(AtomX11("_NET_WM_STATE_HIDDEN")))
            return;
        var horizontal = AtomX11("_NET_WM_STATE_MAXIMIZED_HORZ");
        var vertical = AtomX11("_NET_WM_STATE_MAXIMIZED_VERT");
        if (!states.Contains(horizontal) && !states.Contains(vertical))
            return;
        ...
        {
            if (error != 0)
                throw new InvalidOperationException("The native X11 restore request was rejected by the X server.");
        }
        finally
        {
            Xcb.Free(error);
