...
public sealed partial class DesktopWindowCoordinates
{
    ...
    private sealed class X11FloatingChrome(Window window) : NativeFloatingChrome(window)
    {
        ...
        protected override void EnableCore()
        {
            ...
            _window = Id(native.WindowId); _atom = _coordinates.AtomX11("_MOTIF_WM_HINTS");
            _before = ReadHints();
        ...
        private uint[]? ReadHints()
        {
            ...
            try
            {
                ...
                if (Marshal.ReadByte(reply, 1) == 0) return null;
                var count = Marshal.ReadInt32(reply, 16);
                if (Marshal.ReadByte(reply, 1) != 32 || unchecked((uint)Marshal.ReadInt32(reply, 8)) != _atom ||
                    count < 5 || count > 64 || Marshal.ReadInt32(reply, 12) != 0 || (uint)Marshal.ReadInt32(reply, 4) < count)
                    throw new InvalidOperationException("The existing Motif decoration property is malformed or unbounded.");
                var values = new uint[count];
                for (var i = 0; i < count; i++) values[i] = unchecked((uint)Marshal.ReadInt32(reply, 32 + i * 4));
                return values;
            }
        ...
        internal override void WriteBounds(DockRect bounds)
        {
            ...
            if (!IsDisposed) MoveNative(Window, new(bounds.X, bounds.Y));
        }
        protected override void RestoreCore()
        {
            if (!_changed || _applied == null || ReadHints() is not { } current || !current.SequenceEqual(_applied)) return;
            if (_before != null) { _coordinates.PropertyX11(_window, _atom, _atom, _before); return; }
            var error = Xcb.RequestCheck(_coordinates.Connection, Xcb.DeleteProperty(_coordinates.Connection, _window, _atom));
            try { if (error != 0) throw new InvalidOperationException("Unable to restore the original X11 decorations."); }
