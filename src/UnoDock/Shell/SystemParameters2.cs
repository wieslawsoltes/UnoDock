...
public class SystemParameters2 : INotifyPropertyChanged
{
    ...
    public void Refresh()
    {
        if (!_dispatcher.HasThreadAccess)
            throw new InvalidOperationException("Refresh metrics on the owning UI thread.");
        var next = Read();
        var old = _value;
        if (old == next)
            return;
        ...
        if (old.Color != next.Color)
            _brush = new SolidColorBrush(next.Color);
        ...
        void Notify(string name, bool changed)
        {
            if (changed)
    ...
    private Snapshot Read()
    {
        ...
        var x = (double)Metric(32) + Metric(92);
        var y = (double)Metric(33) + Metric(92);
        var caption = (double)Metric(4);
        var buttonWidth = (double)Metric(30);
        var buttonHeight = (double)Metric(31);
        var themeFile = new StringBuilder(1024);
        var scheme = new StringBuilder(256);
        var size = new StringBuilder(256);
        var hasTheme = GetCurrentThemeName(themeFile, themeFile.Capacity, scheme, scheme.Capacity, size, size.Capacity) >= 0;
        return next with
        {
            Glass = glass && !highContrast,
            Color = color,
            Caption = caption,
            Resize = new(x, y, x, y),
            Frame = new(x, y + caption, x, y),
            Buttons = new(-3 * buttonWidth, 0, 3 * buttonWidth, buttonHeight),
            Icon = new(Metric(49), Metric(50)),
            ThemeName = hasTheme ? Path.GetFileNameWithoutExtension(themeFile.ToString()) : next.ThemeName,
            ThemeColor = hasTheme ? scheme.ToString() : "",
            Corners = new(0)
        };
    }
    ...
    private static extern bool SystemParametersInfo(uint action, uint size, ref HighContrastInfo info, uint flags);
    ...
    private static extern int GetCurrentThemeName(StringBuilder file, int fileLength, StringBuilder color, int colorLength, StringBuilder size, int sizeLength);
}
