...
public static class SystemCommands
{
    ...
    private static RoutedCommand Create(string name, WindowAction action) => new(name, typeof(SystemCommands), (parameter, target) => CanExecute(target ?? parameter, action), (parameter, target) =>
    {
        ...
        if (action == WindowAction.Menu)
        {
            ...
            if (anchor == null || !anchor.IsLoaded)
                return;
            if (parameter is Point point && target != null)
                ShowMenu(owner!, point);
            else
                CreateSystemMenu(owner!).ShowAt(anchor, new FlyoutShowOptions { Position = new Point(0, 0) });
        }
        else if (owner != null)
    ...
    private static bool CanExecute(object? target, WindowAction action)
    {
        if (target == null)
            return false;
        if (target is FrameworkElement element && !element.DispatcherQueue.HasThreadAccess)
            return false;
        if (target is LayoutFloatingWindowControl floating)
            return floating.CanPerformSystemAction(action);
        var window = target as Window ?? (target is FrameworkElement visual ? WindowRegistry.Find(visual) : null);
        if (window == null || !window.DispatcherQueue.HasThreadAccess || WindowRegistry.Observe(window))
            return false;
        if (action == WindowAction.Close)
            return true;
        if (action == WindowAction.Menu)
            return window.Content is FrameworkElement { IsLoaded: true };
    ...
    {
        Validate(window);
        if (!CanExecute(window, action))
            return;
        if (window is LayoutFloatingWindowControl floating)
            floating.PerformSystemAction(action);
        else
        {
            ...
            else if (native.AppWindow.Presenter is OverlappedPresenter presenter)
            {
                if (action == WindowAction.Maximize)
                    ...
                    presenter.Minimize();
                else if (action == WindowAction.Restore)
                    presenter.Restore();
            }
        }
    ...
    private static void ShowMenu(object window, Point screenLocation)
    {
        ...
        if (!double.IsFinite(screenLocation.X) || !double.IsFinite(screenLocation.Y))
            throw new ArgumentOutOfRangeException(nameof(screenLocation));
        if (!CanExecute(window, WindowAction.Menu))
            return;
        ...
        if (native != null && NativeChrome.TryShowSystemMenu(native, screenLocation))
            return;
    ...
    private static void Validate(object window)
    {
        ...
        if (window is not Window && window is not FrameworkElement)
            throw new ArgumentException("A command target must be a Window or FrameworkElement.", nameof(window));
        var dispatcher = window is Window w ? w.DispatcherQueue : ((FrameworkElement)window).DispatcherQueue;
        if (!dispatcher.HasThreadAccess)
            throw new InvalidOperationException("Window commands require the owning UI thread.");
    }
}
#if !WINDOWS
#else
