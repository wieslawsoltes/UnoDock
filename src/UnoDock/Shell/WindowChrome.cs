...
public partial class WindowChrome : DependencyObject, INotifyPropertyChanged
{
    ...
    public static readonly DependencyProperty IsHitTestVisibleInChromeProperty = DependencyProperty.RegisterAttached("IsHitTestVisibleInChrome", typeof(bool), typeof(WindowChrome), new PropertyMetadata(false, (d, _) =>
    {
        if (d is FrameworkElement e)
            FindAttachment(e)?.Invalidate();
    }));
    public static readonly DependencyProperty WindowChromeProperty = DependencyProperty.RegisterAttached("WindowChrome", typeof(WindowChrome), typeof(WindowChrome), new PropertyMetadata(null, (d, e) =>
    {
        if (d is not ContentControl control)
            return;
        if (Managed.TryGetValue(control, out var previous))
        {
            previous.Dispose();
            Managed.Remove(control);
        }

        if (e.NewValue is WindowChrome chrome)
            Managed.Add(control, new Attachment(control, null, chrome));
    }));
    ...
    public double CaptionHeight
    {
        ...
        set
        {
            Validate(CaptionHeightProperty, value);
    ...
    public CornerRadius CornerRadius
    {
        ...
        set
        {
            Validate(CornerRadiusProperty, value);
    ...
    public Thickness GlassFrameThickness
    {
        ...
        set
        {
            Validate(GlassFrameThicknessProperty, value);
            SetValue(GlassFrameThicknessProperty, NormalizeGlass(value));
        }
    }
    ...
    public Thickness ResizeBorderThickness
    {
        ...
        set
        {
            Validate(ResizeBorderThicknessProperty, value);
    ...
    public static bool GetIsHitTestVisibleInChrome(UIElement inputElement)
    {
        ...
        return (bool)inputElement.GetValue(IsHitTestVisibleInChromeProperty);
    }

    public static void SetIsHitTestVisibleInChrome(UIElement inputElement, bool hitTestVisible)
    {
        ArgumentNullException.ThrowIfNull(inputElement);
        inputElement.SetValue(IsHitTestVisibleInChromeProperty, hitTestVisible);
    }
    ...
    public static WindowChrome? GetWindowChrome(Window window)
    {
        ArgumentNullException.ThrowIfNull(window);
        Verify(window);
        return Native.TryGetValue(window, out var attachment) ? attachment.Chrome : null;
    }
    ...
    public static void SetWindowChrome(Window window, WindowChrome? chrome)
    {
        ...
        {
            if (ReferenceEquals(previous.Chrome, chrome))
                return;
            previous.Dispose();
            Native.Remove(window);
        }

        if (chrome != null)
            Native.Add(window, new Attachment(null, window, chrome));
    }
    ...
    public static WindowChromeCapabilities GetAppliedCapabilities(Window window)
    {
        ArgumentNullException.ThrowIfNull(window);
        Verify(window);
        return Native.TryGetValue(window, out var a) ? a.Capabilities : WindowChromeCapabilities.None;
    }

    public static WindowChromeCapabilities GetAppliedCapabilities(ContentControl window)
    {
        ArgumentNullException.ThrowIfNull(window);
        return Managed.TryGetValue(window, out var a) ? a.Capabilities : WindowChromeCapabilities.None;
    }

    public WindowChrome Clone() => new()
    {
    ...
    private static void Verify(Window window)
    {
        if (!window.DispatcherQueue.HasThreadAccess)
            throw new InvalidOperationException("Window chrome requires the owning UI thread.");
    }

    private static DependencyProperty Property(string name, Type type, object value) => DependencyProperty.Register(name, type, typeof(WindowChrome), new PropertyMetadata(value, (d, e) => ((WindowChrome)d).Changed(e, name)));
    private void Changed(DependencyPropertyChangedEventArgs e, string name)
    {
        ...
        try
        {
            Validate(e.Property, e.NewValue);
        }
        catch
        {
            _reverting = true;
        ...
        {
            SetValue(e.Property, NormalizeGlass((Thickness)e.NewValue));
            return;
        }
    ...
    {
        if (value is double d && (!double.IsFinite(d) || d < 0))
            throw new ArgumentOutOfRangeException(nameof(value));
        if (value is CornerRadius c && new[]
        {
            c.TopLeft,
            c.TopRight,
            c.BottomLeft,
            ...
            throw new ArgumentOutOfRangeException(nameof(value));
        if (value is Thickness t && new[]
        {
            t.Left,
            t.Top,
            t.Right,
            t.Bottom
        }.Any(v => !double.IsFinite(v) || property != GlassFrameThicknessProperty && v < 0))
            throw new ArgumentOutOfRangeException(nameof(value));
    }
    ...
    private static Attachment? FindAttachment(FrameworkElement element)
    {
        for (DependencyObject? cursor = element; cursor != null; cursor = VisualTreeHelper.GetParent(cursor))
            if (cursor is ContentControl c && Managed.TryGetValue(c, out var managed))
                return managed;
    ...
    private sealed partial class Attachment : IDisposable
    {
        ...
        internal Attachment(ContentControl? control, Window? window, WindowChrome chrome)
        {
            ...
            {
                _savedBorder = floating.ResizeBorderThickness;
                _savedCorner = floating.CornerRadius;
                _savedCaption = floating.ChromeCaptionHeight;
            }
        ...
        private void Closed(object sender, WindowEventArgs e)
        {
            Dispose();
            if (_window != null)
                Native.Remove(_window);
        }
        ...
        private void BindRoot()
        {
            ...
            if (!ReferenceEquals(_root, root))
            {
                if (_root != null)
                {
                    ...
                    _root.RightTapped -= RightTapped;
                    _root.PointerPressed -= PointerPressed;
                    _root.LayoutUpdated -= LayoutUpdated;
                    DetachGestures(_root);
                }
        ...
        private void Apply()
        {
            ...
            {
                floating.ResizeBorderThickness = Chrome.ResizeBorderThickness;
                floating.CornerRadius = Chrome.CornerRadius;
                floating.ChromeCaptionHeight = Chrome.CaptionHeight;
                _appliedBorder = Chrome.ResizeBorderThickness;
                _appliedCorner = Chrome.CornerRadius;
                _appliedCaption = Chrome.CaptionHeight;
                _managedApplied = true;
                Capabilities = WindowChromeCapabilities.ManagedFrame | WindowChromeCapabilities.SystemMenu;
            }

            if (_window == null || _root?.IsLoaded != true || !AppWindowTitleBar.IsCustomizationSupported())
                return;
            var title = _window.AppWindow.TitleBar;
            if (!_changedNative)
            {
                _extendedBefore = title.ExtendsContentIntoTitleBar;
                _changedNative = true;
            }

            if (!title.ExtendsContentIntoTitleBar)
                title.ExtendsContentIntoTitleBar = true;
            ...
            {
                var x = checked((int)Math.Ceiling(r.X * scale));
                var y = checked((int)Math.Ceiling(r.Y * scale));
                var right = checked((int)Math.Floor(r.Right * scale));
                var bottom = checked((int)Math.Floor(r.Bottom * scale));
            ...
            {
                title.SetDragRectangles(pixels);
                _nativeRegions = pixels;
            }
            ...
            if (_nativeGlass != Chrome.GlassFrameThickness || _nativeScale != scale)
            {
                ...
                _nativeGlass = Chrome.GlassFrameThickness;
                _nativeScale = scale;
            }
            ...
            if (SystemParameters2.Current.IsGlassEnabled)
                Capabilities |= WindowChromeCapabilities.Glass;
        }
        ...
        private ChromeHit Hit(Point p)
        {
            ...
            return ChromeGeometry.HitTest(new(0, 0, _root!.ActualWidth, _root.ActualHeight), new(p.X, p.Y), Chrome.CaptionHeight, border.Left, border.Top, border.Right, border.Bottom, presenter?.IsResizable ?? true, presenter?.State == OverlappedPresenterState.Maximized || _control is LayoutFloatingWindowControl { IsMaximized: true }, Exclusions());
        }
        ...
        {
            if (e.Handled || _root?.IsLoaded != true)
                return;
            var point = e.GetCurrentPoint(_root);
            if (!point.Properties.IsLeftButtonPressed && e.Pointer.PointerDeviceType == Microsoft.UI.Input.PointerDeviceType.Mouse)
                return;
            var hit = Hit(point.Position);
            if (hit == ChromeHit.Client)
                return;
            if (_control is LayoutFloatingWindowControl { NativeWindow: null } managed)
            {
                BeginManagedDrag(managed, hit, e);
                return;
            }

            if (_window == null || !OperatingSystem.IsWindows())
                return;
            using var coordinates = new DesktopWindowCoordinates();
            if (NativeChrome.BeginOperation(_window, hit, coordinates.ToScreen(_root, point.Position)))
                e.Handled = true;
        }
        ...
        public void Dispose()
        {
            ...
            {
                if (floating.ResizeBorderThickness == _appliedBorder)
                    floating.ResizeBorderThickness = _savedBorder;
                if (floating.CornerRadius == _appliedCorner)
                    floating.CornerRadius = _savedCorner;
                if (floating.ChromeCaptionHeight == _appliedCaption)
                    floating.ChromeCaptionHeight = _savedCaption;
            }
            ...
            if (_window != null)
            {
                ...
                {
                    if (_window.AppWindow.TitleBar.ExtendsContentIntoTitleBar)
                        _window.AppWindow.TitleBar.ExtendsContentIntoTitleBar = _extendedBefore;
                    NativeChrome.SetGlass(_window, new(0), 1);
                }
            }

            _registration?.Dispose();
            Capabilities = WindowChromeCapabilities.None;
            _root = null;
            _xamlRoot = null;
        }
    }
