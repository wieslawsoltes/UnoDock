...
public sealed class LayoutPaneAutomationPeer(LayoutCachePaneControl owner) : FrameworkElementAutomationPeer(owner), ISelectionProvider
{
    ...
    {
        if (owner.SelectedItem is not LayoutContent model || !owner.AutomationItems.Any(c => ReferenceEquals(c, model)) ||
            owner.TabFor(model) is not { } tab || CreatePeerForElement(tab) is not { } peer) return [];
        return [ProviderFromPeer(peer)];
    }
    protected override IList<AutomationPeer> GetChildrenCore()
    {
        // A snapshot is required: provider creation can invoke application peers.
        var models = owner.AutomationItems.ToArray(); var peers = new List<AutomationPeer>();
        foreach (var model in models)
            if (owner.TabFor(model) is { } tab && CreatePeerForElement(tab) is { } peer) peers.Add(peer);
        if (owner.SelectedItem is LayoutContent selected && models.Any(m => ReferenceEquals(m, selected)) &&
            selected.Root?.Manager?.GetLayoutItemFromModel(selected).ExistingView is { } view && CreatePeerForElement(view) is { } contentPeer)
    ...
    internal void Synchronize()
    {
        ...
        if (selected != null && !owner.AutomationItems.Any(c => ReferenceEquals(c, selected))) selected = null;
        if (ReferenceEquals(_lastSelection, selected)) return;
