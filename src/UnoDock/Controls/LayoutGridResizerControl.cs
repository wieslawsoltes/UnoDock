...
public partial class LayoutGridResizerControl : ContentControl
{
    ...
    private readonly Border _feedback = new() { IsHitTestVisible = false, Visibility = Visibility.Collapsed };
    ...
    {
        get => _horizontal;
        set { if (_horizontal == value) return; CancelDrag(); _horizontal = value; UpdateCursor(); }
    }
    ...
    public LayoutGridResizerControl()
    {
        ...
        {
            if (_pendingCompletion is { } pending && pending.Pointer == e.Pointer.PointerId &&
                !e.GetCurrentPoint(this).Properties.IsLeftButtonPressed) pending.Released = true;
        }), true);
        AddHandler(PointerPressedEvent, new PointerEventHandler((_, e) =>
        {
            if (!_dragging || _pointer != null) return;
            _coordinateSpace = VisualTreeHelper.GetParent(this) as UIElement ?? this;
            _pointer = e.Pointer.PointerId; _origin = e.GetCurrentPoint(_coordinateSpace).Position;
            Focus(FocusState.Pointer);
        }), true);
        AddHandler(PointerMovedEvent, new PointerEventHandler((_, e) =>
        {
            if (!_dragging || _pointer != e.Pointer.PointerId) return;
        ...
        IsEnabledChanged += (_, _) => { if (!IsEnabled) CancelDrag(); };
    ...
    {
        if (!ReferenceEquals(_pendingCompletion, completion)) return;
        _pendingCompletion = null;
        var e = completion.Args;
        var canceled = e.Canceled || !completion.Released || completion.Generation != _generation ||
            !_dragging || !_thumb.IsEnabled || !IsEnabled || !IsLoaded;
        try { if (completion.Generation == _generation) EndResize(canceled); }
        finally { DragCompleted?.Invoke(this, canceled == e.Canceled ? e : new DragCompletedEventArgs(e.HorizontalChange, e.VerticalChange, canceled)); }
    }
    private void UpdateCursor() => ProtectedCursor = Microsoft.UI.Input.InputSystemCursor.Create(
        Horizontal ? Microsoft.UI.Input.InputSystemCursorShape.SizeWestEast : Microsoft.UI.Input.InputSystemCursorShape.SizeNorthSouth);
    ...
    {
        if (!_dragging) return;
        if (!double.IsFinite(displacement)) { CancelDrag(); throw new ArgumentOutOfRangeException(nameof(displacement)); }
        try { ResizePreview?.Invoke(this, displacement); }
        catch { CancelDrag(); throw; }
    }
    internal void EndResize(bool canceled)
    {
        if (!_dragging || _ending) return;
    ...
    {
        try { EndResize(true); }
        finally { if (_thumb.IsDragging) _thumb.CancelDrag(); }
    }
    internal bool ResizeFromKey(VirtualKey key)
    {
        ...
        if (Horizontal && FlowDirection == FlowDirection.RightToLeft) delta = -delta;
        ResizeBy?.Invoke(this, delta); return true;
    }
    protected override void OnKeyDown(KeyRoutedEventArgs e)
    { base.OnKeyDown(e); if (!e.Handled && ResizeFromKey(e.Key)) e.Handled = true; }
}
