...
public abstract class DockInputControl : ContentControl
{
    ...
    protected DockInputControl()
    {
        ...
        GotFocus += (_, e) =>
        {
            ...
            if (!IsLoaded || !IsEnabled || XamlRoot is not { } root)
                return;
            ...
            _incomingOldFocus = null;
            _incomingNewFocus = null;
    ...
    {
        if (!IsEnabled || !AcceptsPointerEvent(e))
            return;
    ...
    private void Pressed(object sender, PointerRoutedEventArgs e)
    {
        ...
        try
        {
            if (button == DockMouseButton.Left)
                OnPreviewMouseLeftButtonDown(args);
            else if (button == DockMouseButton.Right)
                OnPreviewMouseRightButtonDown(args);
            if (!args.Handled)
                OnMouseDown(args);
        }
        catch
        {
            CancelInput(e.Pointer.PointerId);
            throw;
        }
        finally
        {
    ...
    private void Released(object sender, PointerRoutedEventArgs e)
    {
        ...
        if (!_pressed.TryGetValue(id, out var initial))
            return;
        ...
        try
        {
            if (button == DockMouseButton.Right)
        ...
        finally
        {
            ...
            if (button == initial)
                DockInputCancelled?.Invoke(this, id);
        }
    }
    ...
    private void Cancelled(object sender, PointerRoutedEventArgs e)
    {
        ...
        if (ReferenceEquals(e.OriginalSource, DockCaptureElement))
            CancelInput(e.Pointer.PointerId);
    }

    private void CancelInput(uint id)
    {
        _pressed.Remove(id);
        DockInputCancelled?.Invoke(this, id);
    }
    ...
    {
        var pointers = _pressed.Keys.ToArray();
        _pressed.Clear();
    ...
    {
        if (e.ChangedButton == DockMouseButton.Left)
            OnMouseLeftButtonDown(e);
        else if (e.ChangedButton == DockMouseButton.Right)
    ...
    {
        if (!e.Handled)
            DockDragReleased?.Invoke(this, e.NativeEvent);
    }
    ...
    {
        if (!e.Handled)
            DockDragMoved?.Invoke(this, e.NativeEvent);
    }
    ...
    protected virtual IEnumerator LogicalChildren => (Content is DependencyObject child ? new[]
    {
        child
    }
