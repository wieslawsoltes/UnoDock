...
public abstract partial class LayoutGridControl<T>
{
    ...
    {
        if (!IsLoaded || !splitter.IsLoaded || index < 0 || index + 1 >= _displayed.Length || !Children.Contains(splitter) ||
            _group.Root is not LayoutRoot { Manager: { } manager } root || !ReferenceEquals(root, manager.Layout) ||
            !_group.Children.OfType<ILayoutPanelElement>().Where(c => c.IsVisible).SequenceEqual(_displayed, ReferenceEqualityComparer.Instance) ||
            Orientation != _lastOrientation || _displayed[index] is not ILayoutPositionableElement before ||
    ...
    private void ResizeToValue(LayoutGridResizerControl splitter, int index, double value)
    {
        ...
        if (range.IsReadOnly) throw new InvalidOperationException("The splitter's workspace or endpoints are no longer available.");
        if (!double.IsFinite(value) || value < range.Minimum || value > range.Maximum) throw new ArgumentOutOfRangeException(nameof(value));
    ...
    {
        foreach (var observer in _resizeObservers) observer.Dispose();
    ...
    private sealed class ResizeEndpointObserver : IDisposable
    {
        ...
        { _owner = new(owner); View = view; view.SizeChanged += Changed; }
        ...
        public void Dispose()
        {
            if (View is { } view) { View = null; view.SizeChanged -= Changed; }
        }
    }
    private void RefreshResizeAutomation()
    {
        foreach (var splitter in Children.OfType<LayoutGridResizerControl>()) splitter.RefreshAutomation();
    }
}
