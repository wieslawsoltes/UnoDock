...
public partial class LayoutCachePaneControl
{
    ...
    {
        if (ReferenceEquals(Pane, pane) && _paneObserver != null) return;
        _paneObserver?.Dispose(); Pane = pane; Selector = (ILayoutContentSelector)pane;
        _lastSelection = Selector.SelectedContent;
        _writingSelection = true;
        try { SetValue(SelectedIndexProperty, Selector.SelectedContentIndex); SetValue(SelectedItemProperty, _lastSelection); }
        finally { _writingSelection = false; }
        _paneObserver = new(this, (INotifyPropertyChanged)pane);
    }
    private void SelectionRequested(bool index)
    {
        if (_writingSelection || Selector == null || Pane == null) return;
        try
        {
            if (index) Selector.SelectedContentIndex = SelectedIndex;
            else if (SelectedItem == null) Selector.SelectedContentIndex = -1;
    ...
    private void SynchronizeSelection()
    {
        ...
        try
        {
            ...
            {
                if (iteration == 64) throw new InvalidOperationException("Pane selection bindings did not converge after 64 passes.");
                _selectionDirty = false;
                var selected = Selector.SelectedContent;
                var previous = _lastSelection;
                _lastSelection = selected;
                _writingSelection = true;
                try { SetValue(SelectedIndexProperty, Selector.SelectedContentIndex); SetValue(SelectedItemProperty, selected); }
                finally { _writingSelection = false; }
                ...
                if (!ReferenceEquals(selected, Selector.SelectedContent) || SelectedIndex != Selector.SelectedContentIndex) _selectionDirty = true;
            }
        }
    ...
    { if (Selector?.SelectedContent is { IsEnabled: true, Root: not null } model) model.IsActive = true; }
    ...
    private sealed class PaneObserver : IDisposable
    {
        ...
        { _owner = new(owner); _source = source; source.PropertyChanged += Changed; }
        private void Changed(object? sender, PropertyChangedEventArgs e)
        {
            if (!_owner.TryGetTarget(out var target)) { Dispose(); return; }
        ...
        public void Dispose() { if (_source is { } source) { _source = null; source.PropertyChanged -= Changed; } }
    }
}
