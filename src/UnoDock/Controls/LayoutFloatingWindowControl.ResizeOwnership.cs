...
public abstract partial class LayoutFloatingWindowControl
{
    private sealed partial class FrameResize
    {
        ...
        {
            _owner = owner;
            _minimumWidth = owner.MinWidth;
            _minimumHeight = owner.MinHeight;
            _maximumWidth = owner.MaxWidth;
            _maximumHeight = owner.MaxHeight;
            _flowDirection = owner.FlowDirection;
            _resizeBorder = owner.ResizeBorderThickness;
            ...
            {
                owner.Model
            }.Concat(owner.Model.Descendents()).ToArray())
            {
                _nodes.Add(node);
                node.PropertyChanged += OnNodeChanged;
            }

            Manager.LayoutChanged += OnLayoutChanged;
            _layoutSubscribed = true;
            Window.AppWindow.Changed += OnNativeChanged;
            _nativeSubscribed = true;
        }

        private void Observe(DependencyObject target, DependencyProperty property) => _tokens.Add((target, property, target.RegisterPropertyChangedCallback(property, OnPolicyChanged)));
        ...
        {
            if (!ReferenceEquals(sender.Presenter, Presenter) || !Presenter.IsResizable || Presenter.State != OverlappedPresenterState.Restored)
                Revoke();
        }
        ...
        private void OnNodeChanged(object? sender, PropertyChangedEventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(e.PropertyName) || e.PropertyName is "Parent" or "Root" or "ChildrenCount" or "RootPanel" or "RootDocument" or "Content" or "IsEnabled" or "IsHidden")
                Revoke();
        }

        private void Revoke()
        {
            if (IsDisposed || Revoked)
                return;
            Revoked = true;
        ...
        public void Dispose()
        {
            ...
            foreach (var node in _nodes)
                cleanup.Attempt(() => node.PropertyChanged -= OnNodeChanged);
            _nodes.Clear();
            if (_layoutSubscribed)
                cleanup.Attempt(() => Manager.LayoutChanged -= OnLayoutChanged);
            if (_nativeSubscribed)
                cleanup.Attempt(() => Window.AppWindow.Changed -= OnNativeChanged);
