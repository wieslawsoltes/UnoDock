...
public abstract partial class LayoutGridControl<T> : Grid, ILayoutControl, IRefreshableLayoutControl where T : class, ILayoutPanelElement
{
    ...
    internal void Update(DockSurface surface)
    {
        ...
        if (!_displayed.SequenceEqual(models, ReferenceEqualityComparer.Instance) || _lastOrientation != Orientation || thickness != _lastThickness)
        {
            CancelResize();
            DetachResizeObservers();
            foreach (var view in Children.ToArray())
                if (view is not LayoutGridResizerControl)
            ...
            {
                var view = surface.GetView(models[i]);
                VisualParenting.Detach(view);
                if (horizontal)
                ...
                if (i + 1 < models.Length)
                {
                    ...
                    var resize = new LayoutGridResizerControl
                    {
                        ...
                        Background = DockVisuals.Brush(surface.Manager, "UnoDock.HeaderBrush", "ControlFillColorSecondaryBrush")
                    };
                    ...
                    SetColumn(resize, horizontal ? i * 2 + 1 : 0);
                    SetRow(resize, horizontal ? 0 : i * 2 + 1);
                    Children.Add(resize);
                }
            }
        }
        ...
        for (var i = 0; i < models.Length; i++)
        {
            ...
            if (horizontal)
            {
                ColumnDefinitions[i * 2].Width = pos?.DockWidth ?? new(1, GridUnitType.Star);
                ColumnDefinitions[i * 2].MinWidth = pos?.DockMinWidth ?? 0;
            }
            else
            {
                RowDefinitions[i * 2].Height = pos?.DockHeight ?? new(1, GridUnitType.Star);
                RowDefinitions[i * 2].MinHeight = pos?.DockMinHeight ?? 0;
            }
    ...
    protected void NormalizeLengths()
    {
        foreach (var child in _group.Children.OfType<ILayoutPositionableElement>())
        {
            ...
            if (Orientation == Orientation.Horizontal)
                child.DockWidth = new(1, GridUnitType.Star);
            else
                child.DockHeight = new(1, GridUnitType.Star);
        }
    }
}
