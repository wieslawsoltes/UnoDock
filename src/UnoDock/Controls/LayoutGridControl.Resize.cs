...
public abstract partial class LayoutGridControl<T>
{
    ...
    {
        CancelResize();
        if (!IsLoaded || index < 0 || index + 1 >= _displayed.Length || _displayed[index] is not ILayoutPositionableElement a || _displayed[index + 1] is not ILayoutPositionableElement b || _group.Root is not LayoutRoot { Manager: { } manager } root || !ReferenceEquals(root, manager.Layout))
        {
            splitter.CancelDrag();
            return;
        }
        ...
        SetColumnSpan(adorner, Math.Max(1, ColumnDefinitions.Count));
        SetRowSpan(adorner, Math.Max(1, RowDefinitions.Count));
        Canvas.SetZIndex(adorner, 1000);
        var ghost = new Border
        {
            Name = "PART_SplitterPreview",
    ...
    private void CancelResize()
    {
        if (_resize is not { } s)
            return;
        DetachResize(s);
        s.Splitter.CancelDrag();
    }

    private void DetachResize(ResizeSession s)
    {
        if (!ReferenceEquals(_resize, s))
            return;
        _resize = null;
        s.Root.Updated -= ResizeInvalidated;
        s.Manager.LayoutChanged -= ResizeInvalidated;
        Children.Remove(s.Adorner);
        s.Adorner.Children.Clear();
    }

    private void PreviewResize(LayoutGridResizerControl splitter, double displacement)
    {
        if (_resize is not { } s || !ReferenceEquals(s.Splitter, splitter))
            return;
        if (!IsCurrent(s))
        {
            CancelResize();
        ...
        s.Displacement = pair.Before - s.BeforePixels;
        PlacePreview(s);
    }
    ...
    private void EndResize(LayoutGridResizerControl splitter, bool canceled)
    {
        ...
        try
        {
            ...
            attemptedAfter = true;
            Write(s.After, b);
            if (!IsStructureCurrent(s) || Read(s.Before) != a || Read(s.After) != b)
                Rollback();
        }
        catch (Exception original)
        {
            ...
            {
                throw new AggregateException("Resize and observer rollback both failed.", original, rollback);
            }
        ...
        {
            if (s.Horizontal)
                item.DockWidth = value;
            else
                item.DockHeight = value;
        }
        ...
        void Rollback()
        {
            ...
            try
            {
                if (attemptedAfter && Read(s.After) == b)
                    Write(s.After, s.AfterLength);
            }
            catch (Exception e)
            {
                (failures ??= []).Add(e);
            ...
            if (failures != null)
                throw new AggregateException("Resize rollback observers failed.", failures);
        }
    }
    ...
    private void ResizeOnce(LayoutGridResizerControl splitter, int index, double delta, bool absolutePixels = false)
    {
        ...
        if (_resize is { } session)
            session.AbsolutePixels = absolutePixels;
        try
        {
            PreviewResize(splitter, delta);
            EndResize(splitter, false);
        }
        finally
        {
            CancelResize();
