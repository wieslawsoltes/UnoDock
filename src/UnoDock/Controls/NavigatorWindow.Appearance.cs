...
public partial class NavigatorWindow
{
    ...
    private readonly TextBlock _documentHeading = new()
    {
        FontWeight = Microsoft.UI.Text.FontWeights.Bold
    };
    private readonly TextBlock _toolHeading = new()
    {
        FontWeight = Microsoft.UI.Text.FontWeights.Bold
    };
    ...
    private readonly TextBlock _selectionDescription = new()
    {
        TextTrimming = TextTrimming.CharacterEllipsis
    };
    private static ListBox CreateList(string name) => new NavigatorListBox
    {
        Name = name
    };
    private void BuildDefaultChrome()
    {
        ...
        _chrome.Children.Add(details);
        BindLabel(_toolHeading, nameof(LayoutAnchorablesLabel));
        BindLabel(_documentHeading, nameof(LayoutDocumentsLabel));
        _toolHeading.Margin = new(5, 8, 5, 4);
        _documentHeading.Margin = new(5, 8, 5, 4);
        ...
        Grid.SetRow(_toolHeading, 1);
        Grid.SetRow(_documentHeading, 1);
        Grid.SetColumn(_documentHeading, 1);
        _chrome.Children.Add(_toolHeading);
        _chrome.Children.Add(_documentHeading);
        Grid.SetRow(_defaultDocuments, 2);
        Grid.SetColumn(_defaultDocuments, 1);
        Grid.SetRow(_defaultAnchorables, 2);
        _chrome.Children.Add(_defaultDocuments);
        _chrome.Children.Add(_defaultAnchorables);
        Content = _chrome;
