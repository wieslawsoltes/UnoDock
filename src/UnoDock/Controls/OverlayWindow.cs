...
public class OverlayWindow : DockWindowControl
{
    ...
    protected override void OnApplyTemplate()
    {
        ...
        if (ReferenceEquals(canvas, _canvas))
            return;
        foreach (var child in _canvas.Children.ToArray())
        {
    ...
    internal void ShowPreview(DockDropPlan? plan, Rect rect, Brush accent)
    {
        ...
        HideCore(version);
        if (version != _presentationVersion || plan?.CanExecute != true || !Valid(rect))
            return;
        CurrentPlan = plan;
        PaintPreview(rect, accent);
        Visibility = Visibility.Visible;
    }

    public void ShowGuides(IReadOnlyList<DockGuideTarget> guides, DockDropPlan? selectedPlan, DockingManager manager) => ShowGuides(guides, selectedPlan, manager, null);
    internal void ShowGuides(IReadOnlyList<DockGuideTarget> guides, DockDropPlan? selectedPlan, DockingManager manager, Rect? projectedPreview)
    {
        ...
        foreach (var g in valid)
            if (!keys.Add((g.Plan.Target, g.Type)))
                throw new ArgumentException("Duplicate docking guide identity.", nameof(guides));
        ...
        foreach (var group in valid.Where(g => g.Type is >= DropTargetType.DocumentPaneDockLeft and <= DropTargetType.DocumentPaneDockInside or >= DropTargetType.AnchorablePaneDockLeft and <= DropTargetType.AnchorablePaneDockInside).GroupBy(g => g.Plan.Target))
        {
            if (group.Count() != 5)
                continue;
            ...
            if (Math.Abs(left.Right - center.Left) > .01 || Math.Abs(top.Bottom - center.Top) > .01 || Math.Abs(center.Right - right.Left) > .01 || Math.Abs(center.Bottom - bottom.Top) > .01)
                continue;
        ...
        {
            _canvas.Children.Remove(_plates[key]);
            _plates.Remove(key);
        }
        ...
        foreach (var pair in plateBounds)
        {
            ...
            {
                plate = new();
                _plates.Add(pair.Key, plate);
                _canvas.Children.Add(plate);
            }

            Canvas.SetLeft(plate, pair.Value.X);
            Canvas.SetTop(plate, pair.Value.Y);
            plate.Width = pair.Value.Width;
            plate.Height = pair.Value.Height;
            plate.Paint(p);
        }

        foreach (var key in _guideViews.Keys.Where(k => !keys.Contains(k)).ToArray())
        {
            _canvas.Children.Remove(_guideViews[key]);
            _guideViews.Remove(key);
        }
        ...
        foreach (var guide in valid)
        {
            ...
            {
                visual = new DockGuideVisual(guide.Type, guide.Plan.Position);
                _guideViews.Add(key, visual);
            ...
            visual.Paint(p, selected != null && ReferenceEquals(selected.Target, guide.Plan.Target) && selected.Type == guide.Type, _plates.ContainsKey(guide.Plan.Target) && (guide.Type is >= DropTargetType.DocumentPaneDockLeft and <= DropTargetType.DocumentPaneDockInside or >= DropTargetType.AnchorablePaneDockLeft and <= DropTargetType.AnchorablePaneDockInside));
        }

        _guides = Array.AsReadOnly(valid);
        CurrentPlan = selected;
        var preview = projectedPreview ?? selected?.PreviewRect ?? default;
        if (selected != null && Valid(preview))
            PaintPreview(preview, p.Ink);
        else
        {
            _preview.Visibility = _fill.Visibility = Visibility.Collapsed;
        }
    ...
    private void HideCore(long version)
    {
        ...
        CurrentPlan = null;
        _guides = Array.Empty<DockGuideTarget>();
        _guideViews.Clear();
        _plates.Clear();
        ...
        foreach (var view in staleGuides)
            cleanup.Attempt(() => _canvas.Children.Remove(view));
        foreach (var plate in stalePlates)
            cleanup.Attempt(() => _canvas.Children.Remove(plate));
        cleanup.Attempt(() =>
        {
            if (version == _presentationVersion)
