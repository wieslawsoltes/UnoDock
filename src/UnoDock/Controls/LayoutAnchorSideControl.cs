...
public class LayoutAnchorSideControl : ContentControl, ILayoutControl
{
    ...
    {
        SetIsLeftSide(_model.Side == AnchorSide.Left);
        SetIsRightSide(_model.Side == AnchorSide.Right);
        SetIsTopSide(_model.Side == AnchorSide.Top);
        SetIsBottomSide(_model.Side == AnchorSide.Bottom);
        var models = _model.Children.ToArray();
        foreach (var stale in Children.Where(c => !models.Contains(c.Model)).ToArray())
            Children.Remove(stale);
        foreach (var model in models)
        {
            var child = Children.FirstOrDefault(c => ReferenceEquals(c.Model, model));
            if (child == null)
                Children.Add(child = new(model));
        ...
        if (manager.AnchorSideTemplate != null)
            Template = manager.AnchorSideTemplate;
    }
}
