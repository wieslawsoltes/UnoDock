...
public partial class NavigatorWindow : DockWindowControl
{
    ...
    public NavigatorWindow(DockingManager manager)
    {
        ...
        BuildDefaultChrome(); Attach(_documentsList); Attach(_anchorablesList);
    ...
    protected override void OnApplyTemplate()
    {
        ...
        PublishLists(); PublishSelection(); UpdateAppearance(); QueueReveal();
    }
    ...
    {
        if (!_publishing && ((ListBox)sender).SelectedItem is LayoutItem item && !ReferenceEquals(item, _selected)) Select(item);
    }
    private void ListTapped(object sender, TappedRoutedEventArgs e)
    {
        if (_sessionRoot == null || e.Handled) return;
    ...
    private IEnumerable<LayoutItem> EnumerateItems() => _manager.Layout.Descendents().OfType<LayoutContent>()
        .Where(Eligible).Select(_manager.GetLayoutItemFromModel);
    private static IOrderedEnumerable<LayoutItem> SortItems(IEnumerable<LayoutItem> items) => items
        .OrderByDescending(i => i.LayoutElement.LastActivationTimeStamp).ThenBy(i => i.ContentId, StringComparer.Ordinal);
    ...
    {
        _indices.Clear(); _groupIndices.Clear();
        for (var i = 0; i < _ordered.Length; i++) _indices.Add(_ordered[i], i);
        ...
        var anchorables = modelOrder != null ? modelOrder.OfType<LayoutAnchorableItem>().ToArray() :
            _anchorableItems.Where(item => _indices.ContainsKey(item))
                .Concat(EnumerateItems().OfType<LayoutAnchorableItem>().Where(item => !existingTools.Contains(item))).ToArray();
        if (!SameItems(_documentItems, documents)) _documentItems = documents;
        if (!SameItems(_anchorableItems, anchorables)) _anchorableItems = anchorables;
        for (var i = 0; i < _documentItems.Length; i++) _groupIndices.Add(_documentItems[i], i);
        for (var i = 0; i < _anchorableItems.Length; i++) _groupIndices.Add(_anchorableItems[i], i);
    }
    private static bool SameItems<T>(T[] a, T[] b) where T : class
    {
    ...
    private void ModelUpdated(object? sender, EventArgs e)
    {
        ...
        if (!DispatcherQueue.TryEnqueue(() =>
        {
            ...
            if (_sessionRoot != null) { RefreshItems(); UpdateAppearance(); }
        })) _refreshQueued = false;
    }
    private void RefreshItems()
    {
        if (!_itemsDirty || _refreshing || _sessionRoot == null) return;
        _refreshing = true; _itemsDirty = false;
        var session = _sessionVersion; var selection = _selectionVersion;
        try
        {
            var items = EnumerateItems().ToArray(); var old = _selected;
            ...
            {
                _ordered = ordered; Reindex(); PublishLists();
                if (session != _sessionVersion) return;
            ...
            else if (!Eligible(_selected)) Select(null);
    ...
    {
        var previous = _publishing; _publishing = true; var session = _sessionVersion;
        try
        {
            if (!ReferenceEquals(Documents, _documentItems)) SetDocuments(_documentItems);
            if (session != _sessionVersion) return;
            if (!ReferenceEquals(Anchorables, _anchorableItems)) SetAnchorables(_anchorableItems);
            if (session != _sessionVersion) return;
            if (!ReferenceEquals(_documentsList.ItemsSource, _documentItems)) _documentsList.ItemsSource = _documentItems;
            if (session != _sessionVersion) return;
            if (!ReferenceEquals(_anchorablesList.ItemsSource, _anchorableItems)) _anchorablesList.ItemsSource = _anchorableItems;
        }
        finally { _publishing = previous; }
    }
    private void Select(LayoutItem? item)
    {
        _pendingSelection = item; _hasPendingSelection = true;
        ...
        try
        {
            ...
            {
                if (--remaining < 0) throw new InvalidOperationException("Navigator selection callbacks did not converge.");
                item = _pendingSelection; _hasPendingSelection = false;
                if (item != null && !Eligible(item)) item = Eligible(_selected) ? _selected : null;
                var changed = !ReferenceEquals(item, _selected);
                _selected = item;
                if (changed) _selectionVersion++;
                var session = _sessionVersion;
                PublishSelection();
                if (session != _sessionVersion) return;
    ...
    {
        RefreshItems(); var remembered = documents ? _lastDocument : _lastAnchorable;
    ...
    private void HandleNavigatorKeyDown(KeyRoutedEventArgs e)
    {
        switch (e.Key)
        {
            ...
            case Windows.System.VirtualKey.Home: SelectBoundary(false); break;
    ...
    { base.OnPreviewKeyUp(e); if (!e.Handled) HandleNavigatorKeyUp(e); }
