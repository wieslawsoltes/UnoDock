...
public abstract partial class LayoutFloatingWindowControl
{
    ...
    private sealed partial class FrameResize(NativeFloatingChrome chrome, Window window, LayoutRoot root, ChromeHit hit, Point down, DockRect bounds, DockRect modelBounds, double scale, uint? pointer, ResizeGrip? grip) : IDisposable
    {
    ...
    private bool CurrentResize(FrameResize resize) => ReferenceEquals(_frameResize, resize) && ReferenceEquals(_nativeChrome, resize.Chrome) && !resize.Revoked && !resize.IsDisposed && resize.HasCurrentPolicy(this) && !_hostDisposed && IsEnabled && resize.Manager.IsEnabled && !IsMaximized && !_minimized && ReferenceEquals(_window, resize.Window) && ReferenceEquals(Model.Root, resize.Root) && ReferenceEquals(resize.Root.Manager, resize.Manager) && ReferenceEquals(resize.Manager.Layout, resize.Root) && ReferenceEquals(resize.Window.AppWindow.Presenter, resize.Presenter) && resize.Window.AppWindow.Presenter is OverlappedPresenter { IsResizable: true, State: OverlappedPresenterState.Restored } && Math.Abs(NativeFloatingChrome.ScreenScale(this) - resize.Scale) < .000001;
    ...
    {
        if (_frameResize != null || _nativeChrome == null || _window == null || _hostDisposed || !IsEnabled || IsMaximized || _minimized || Model.Root is not LayoutRoot root || root.Manager is not { IsEnabled: true } manager || !ReferenceEquals(manager.Layout, root) || manager.Surface == null || _window.AppWindow.Presenter is not OverlappedPresenter { IsResizable: true, State: OverlappedPresenterState.Restored } presenter || hit is < ChromeHit.Left or > ChromeHit.BottomRight || !IsResizeEdgeEnabled(hit))
            return null;
        if (!double.IsFinite(screenPoint.X) || !double.IsFinite(screenPoint.Y))
            throw new ArgumentOutOfRangeException(nameof(screenPoint));
        var chrome = _nativeChrome;
        var window = _window;
        var generation = _resizeGeneration;
        CancelCaptionDrag();
        manager.Surface.CancelDrag();
        ...
        if (_resizeGeneration != generation || _frameResize != null || _hostDisposed || !IsEnabled || !manager.IsEnabled || IsMaximized || _minimized || !ReferenceEquals(_nativeChrome, chrome) || !ReferenceEquals(_window, window) || !ReferenceEquals(Model.Root, root) || !ReferenceEquals(root.Manager, manager) || !ReferenceEquals(manager.Layout, root) || !ReferenceEquals(window.AppWindow.Presenter, presenter) || !presenter.IsResizable || presenter.State != OverlappedPresenterState.Restored || !IsResizeEdgeEnabled(hit))
            return null;
        var resize = new FrameResize(chrome, window, root, hit, screenPoint, chrome.ReadBounds(), RestoredBounds, NativeFloatingChrome.ScreenScale(this), pointer, grip);
        _frameResize = resize;
        _resizeGeneration++;
        try
        {
        ...
        catch (Exception error)
        {
            ...
            cleanup.Attempt(() => EndFrameResize(resize, false));
    ...
    private void MoveFrameResize(FrameResize resize, Point point)
    {
        ...
        if (!ReferenceEquals(_frameResize, resize))
            return;
        ...
        if (resize.LastWritten == null && point == resize.Down)
            return;
        var scale = resize.Scale;
        var minWidth = Math.Max(160, MinWidth) * scale;
        var minHeight = Math.Max(100, MinHeight) * scale;
        var bounds = ChromeResize.Apply(resize.Bounds, resize.Hit, point.X - resize.Down.X, point.Y - resize.Down.Y, minWidth, minHeight, Math.Max(minWidth, MaxWidth * scale), Math.Max(minHeight, MaxHeight * scale));
        bounds = CanonicalResizeBounds(bounds);
        if (resize.LastWritten == bounds)
            return;
        ...
        if (!resize.NativeBounds.Request(bounds))
        {
            EndFrameResize(resize, false);
            return;
        }
        ...
        var moving = _movingFromCaption;
        _movingFromCaption = true;
        try
        {
            resize.Chrome.WriteBounds(bounds);
        }
        finally
        {
            _movingFromCaption = moving;
        }
    }
    ...
    private void EndFrameResize(FrameResize resize, bool restore)
    {
        ...
        if (restore)
            cleanup.Attempt(() => stillOwned = CurrentResize(resize) && resize.NativeBounds.Observe(resize.Chrome.ReadBounds()));
        _frameResize = null;
        _resizeGeneration++;
        var clock = _resizeClock;
        _resizeClock = null;
        cleanup.Attempt(resize.Dispose);
        cleanup.Attempt(() => clock?.Dispose());
        ...
        if (resize.Grip is { } grip && !ReferenceEquals(_frameResize?.Grip, grip))
            cleanup.Attempt(grip.ReleasePointerCaptures);
        cleanup.ThrowIfFailed();
    }

    private void CancelFrameResize(bool restore)
    {
        if (_frameResize is { } resize)
            EndFrameResize(resize, restore);
    }
    ...
    private void FailFrameResize(Exception error)
    {
        ...
        {
            error = new AggregateException("Floating resize and cleanup failed.", error, cleanup);
        }
    ...
    {
        if (sender is not ResizeGrip grip || !e.GetCurrentPoint(grip).Properties.IsLeftButtonPressed)
            return;
        try
        {
            ...
            if (e.Pointer.PointerDeviceType == PointerDeviceType.Mouse)
                _resizeClock = new NativeDragClock(TickFrameResize, FailFrameResize);
        }
        catch (Exception error)
        {
            FailFrameResize(error);
        }
    }

    private void ResizeMoved(object sender, PointerRoutedEventArgs e)
    {
        if (sender is not ResizeGrip grip || _frameResize is not { } resize || !OwnsResizePointer(resize, grip, e.Pointer.PointerId))
            return;
        try
        {
            MoveFrameResize(resize, ResizePoint(grip, e));
            e.Handled = true;
        }
        catch (Exception error)
        {
    ...
    {
        if (_frameResize is { } resize && OwnsResizePointer(resize, sender, e.Pointer.PointerId))
            EndFrameResize(resize, true);
    }

    private static bool OwnsResizePointer(FrameResize resize, object sender, uint pointer) => resize.Pointer == pointer && ReferenceEquals(resize.Grip, sender);
    private bool IsResizeEdgeEnabled(ChromeHit hit)
    {
        ...
        return hit switch
        {
            ChromeHit.Left => Enabled(border.Left),
            ChromeHit.Right => Enabled(border.Right),
            ChromeHit.Top => Enabled(border.Top),
            ChromeHit.Bottom => Enabled(border.Bottom),
    ...
    private static DockRect CanonicalResizeBounds(DockRect bounds)
    {
        if (OperatingSystem.IsMacOS())
            return bounds; // AppKit uses fractional points.
        var x = Math.Round(bounds.X);
        var y = Math.Round(bounds.Y);
        return OperatingSystem.IsWindows() ? new(x, y, Math.Max(1, Math.Round(bounds.Right) - x), Math.Max(1, Math.Round(bounds.Bottom) - y)) : new(x, y, Math.Round(bounds.Width), Math.Round(bounds.Height));
    }

    private void TickFrameResize()
    {
        if (_frameResize is not { } resize)
            return;
        if (!CurrentResize(resize))
        {
            EndFrameResize(resize, false);
        ...
        if (!_dragCoordinates.TryGetPointer(resize.Window, out var state))
            throw new PlatformNotSupportedException("Native resize pointer state is unavailable.");
        if (state.EscapeDown)
        {
            EndFrameResize(resize, true);
            return;
