...
public sealed class DockDropPlan
{
    ...
    private DockDropPlan(LayoutContent content, ILayoutGroup target, DropTargetType type, Rect bounds, int insertionIndex)
    {
        ...
        _root = (LayoutRoot)content.Root!;
        _manager = _root.Manager;
        _atRoot = (int)type <= 3;
    ...
    public static DockDropPlan? Create(LayoutContent content, ILayoutGroup target, DropTargetType type, Rect bounds, int insertionIndex = -1)
    {
        ...
        if (!double.IsFinite(bounds.X + bounds.Y + bounds.Width + bounds.Height) || bounds.Width <= 0 || bounds.Height <= 0)
            return null;
        if (content.Root is not LayoutRoot root || !ReferenceEquals(target.Root, root))
            return null;
    ...
    public bool CanExecute
    {
        get
        {
            if (!ReferenceEquals(_root.Manager, _manager) || _manager != null && !ReferenceEquals(_manager.Layout, _root))
                return false;
    ...
    public bool Execute()
    {
        ...
        if (_atRoot)
            DockOperations.DockToRoot(Content, Position);
            ...
            DockOperations.Dock(Content, Target, Position, InsertionIndex, _asDocument);
        if (!ReferenceEquals(_root.Manager, _manager) || !ReferenceEquals(Content.Root, _root))
            return false;
