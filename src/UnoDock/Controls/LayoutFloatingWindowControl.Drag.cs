...
{
    private readonly Border _dragHandle = new() { Name = "PART_FloatingDragHandle", Background = DockChrome.Transparent };
    ...
    private sealed class CaptionDrag(DockSurface surface, long generation, LayoutRoot root, Window? window,
        Point down, Point origin, DockRect bounds, uint? pointer, bool native)
    {
    ...
    private void InitializeCaptionDrag()
    {
        ...
        IsEnabledChanged += (_, _) => { if (!IsEnabled) CancelCaptionDrag(); };
    }

    private bool Current(CaptionDrag drag) => ReferenceEquals(_captionDrag, drag) && !_hostDisposed &&
        !IsMaximized && !_minimized && ReferenceEquals(_window, drag.Window) && ReferenceEquals(Model.Root, drag.Root) &&
        ReferenceEquals(drag.Root.Manager?.Surface, drag.Surface) && drag.Surface.OwnsFloatingDrag(this, drag.Generation);

    private CaptionDrag? BeginCaptionDrag(Point point, uint? pointer, bool native)
    {
        if (_captionDrag != null || _hostDisposed || IsMaximized || _minimized || !IsEnabled || IsContentImmutable ||
        ...
        if (native)
        {
            drag.Started = true; SetIsDragging(true);
            if (!Current(drag)) { CancelCaptionDrag(); return null; }
        }
        return drag;
    }
    ...
    {
        if (!e.GetCurrentPoint(_dragHandle).Properties.IsLeftButtonPressed) return;
        try
        {
    ...
    private Point CaptionPoint(PointerRoutedEventArgs e)
    {
        ...
        return _window == null
            ...
            : _dragCoordinates.ToScreen(_dragHandle, point);
    }
    private void CaptionMoved(object sender, PointerRoutedEventArgs e)
    {
        if (_captionDrag is not { Pointer: { } pointer } drag || pointer != e.Pointer.PointerId) return;
        try { MoveCaption(drag, CaptionPoint(e), InputState.ControlDown); e.Handled = true; }
        catch (Exception error) when (DockCoordinates.IsUnavailable(error)) { FailCaptionDrag(error); }
    }
    private void MoveCaption(CaptionDrag drag, Point point, bool suppress)
    {
        if (!Current(drag)) { CancelCaptionDrag(); return; }
        var dx = point.X - drag.Down.X; var dy = point.Y - drag.Down.Y;
        var scale = drag.Window != null && !OperatingSystem.IsMacOS() ? DesktopWindowCoordinates.Scale(this) : 1;
        if (!drag.Started)
        {
            if (Math.Abs(dx) < 4 * scale && Math.Abs(dy) < 4 * scale) return;
            drag.Started = true; SetIsDragging(true);
    ...
    {
        if (_captionDrag?.Pointer == e.Pointer.PointerId) CancelCaptionDrag();
    }
    private void CompleteCaption(CaptionDrag drag, Point point, bool suppress)
    {
        if (!Current(drag)) return;
        if (!drag.Started) { drag.Surface.CancelDrag(); return; }
        var surfacePoint = drag.Window == null ? point : _dragCoordinates.FromScreen(point, drag.Surface);
    ...
    internal void EndFloatingDragCapture(DockSurface surface, long generation, bool restore)
    {
        ...
        try
        {
            if (restore && !_hostDisposed && ReferenceEquals(_window, drag.Window) && ReferenceEquals(Model.Root, drag.Root) &&
                ReferenceEquals(drag.Root.Manager?.Surface, surface) && !IsMaximized && !_minimized)
            {
                _movingFromCaption = true;
                try
                {
                    if (drag.Window != null) DesktopWindowCoordinates.MoveNative(drag.Window, drag.Origin);
    ...
    {
        if (_captionDrag is not { } drag) return;
        if (drag.Surface.OwnsFloatingDrag(this, drag.Generation)) drag.Surface.CancelDrag();
        else EndFloatingDragCapture(drag.Surface, drag.Generation, false);
    }
    private void FailCaptionDrag(Exception error)
    {
        try { CancelCaptionDrag(); }
        catch (Exception cleanup) { error = new AggregateException("Native drag and cleanup failed.", error, cleanup); }
        ReportFilterFailure(error);
    }
    private void StartDragClock()
    {
        if (_dragClock != null || _captionDrag == null) return;
        _dragClock = new NativeDragClock(TickCaptionDrag, FailCaptionDrag);
    }
    private void TickCaptionDrag()
    {
        if (_captionDrag is not { } drag || drag.CompletionQueued) return;
        if (!Current(drag)) { CancelCaptionDrag(); return; }
    ...
    {
        if (!Current(drag) || drag.CompletionQueued) return;
        ...
        if (!DispatcherQueue.TryEnqueue(() =>
        {
            ...
            try
            {
                ...
                else if (state.LeftDown) { drag.CompletionQueued = false; }
                else CompleteCaption(drag, state.Position, state.ControlDown);
            }
    ...
    private void ObserveNativeCaption(AppWindowChangedEventArgs e)
    {
        ...
        if (_captionDrag == null) _lastNativeOrigin = origin;
    }
