...
public partial class NavigatorWindow
{
    ...
    private void PreviewItem(LayoutItem? item)
    {
        if (!DispatcherQueue.HasThreadAccess)
            throw new InvalidOperationException("Navigator selection requires its owning UI thread.");
    ...
    private void DirectSelectionChanged(DependencyPropertyChangedEventArgs e, bool document)
    {
        ...
        if (_publishing && ReferenceEquals(e.NewValue, published))
            return;
    ...
    {
        if (_selecting || _handlingDirectSelection || !_hasDirectSelection)
            return;
        ...
        try
        {
            ...
            {
                if (--remaining < 0)
                    throw new InvalidOperationException("Navigator direct-selection callbacks did not converge.");
                ...
                if (IsWindowClosed || _sessionRoot == null)
                    continue;
                if (!ReferenceEquals(item, GetValue(document ? SelectedDocumentProperty : SelectedAnchorableProperty)))
                    continue;
                ...
                try
                {
                    ...
                    if (session != _sessionVersion || request != _directSelectionVersion)
                        continue;
        ...
        {
            _handlingDirectSelection = false;
            _hasDirectSelection = false;
