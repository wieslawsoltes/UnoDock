...
public class LayoutAnchorGroupControl : ContentControl, ILayoutControl
{
    ...
    public LayoutAnchorGroupControl(LayoutAnchorGroup model)
    {
        _model = model;
        Content = _panel;
    }
    ...
    internal void Update(DockingManager manager)
    {
        ...
        for (var i = 0; i < models.Length; i++)
        {
            ...
            if (child == null)
                Children.Insert(Math.Min(i, Children.Count), child = new(models[i]));
            else if (Children.IndexOf(child) != i)
                Children.Move(Children.IndexOf(child), i);
