...
public partial class LayoutAutoHideWindowControl : ContentControl, ILayoutControl
{
    ...
    public static readonly DependencyProperty AnchorableStyleProperty = DependencyProperty.Register(nameof(AnchorableStyle), typeof(Style), typeof(LayoutAutoHideWindowControl),
        new PropertyMetadata(null, (d, _) => ((LayoutAutoHideWindowControl)d).ApplyAnchorableStyle()));
    ...
    public LayoutAutoHideWindowControl()
    {
        ...
        Padding = new(0); BorderThickness = new(0); MinWidth = MinHeight = 0; IsTabStop = false;
        for (var i = 0; i < 3; i++) { _root.ColumnDefinitions.Add(new()); _root.RowDefinitions.Add(new()); }
        ...
        _resizer.ResizeBy += (_, delta) => { _resizer.BeginResize(); _resizer.UpdateResize(delta); _resizer.EndResize(false); };
        PointerEntered += (_, _) => { _pointerInside = true; _manager?.Surface?.StopAutoHideTimer(); };
        PointerExited += (_, _) => { _pointerInside = false; _manager?.Surface?.StartAutoHideTimer(); };
        GotFocus += (_, _) =>
        {
            _manager?.Surface?.StopAutoHideTimer();
            if (_model is { IsEnabled: true, IsAutoHidden: true } model && ReferenceEquals(model.Root, _manager?.Layout)) model.IsActive = true;
        };
        LostFocus += (_, _) =>
        {
            var version = _openVersion;
            DispatcherQueue.TryEnqueue(() => { if (version == _openVersion && _model != null && !RetainOpen) _manager?.Surface?.StartAutoHideTimer(); });
        };
        Unloaded += (_, _) => CloseView();
        IsEnabledChanged += (_, _) => { if (!IsEnabled) CancelResize(); };
    ...
    internal void Open(LayoutAnchorable model, bool activate = true)
    {
        ...
        if (!model.IsAutoHidden || !model.IsEnabled || !ReferenceEquals(model.Root, manager.Layout)) return;
        if (!ReferenceEquals(model, _model))
        {
            ...
            if (_openVersion != closingVersion + 1 || !model.IsAutoHidden || !ReferenceEquals(model.Root, manager.Layout)) return;
            _model = model; _manager = manager; _openVersion++;
            MenuContext.SetTarget(this, model);
            model.PropertyChanged += ModelChanged;
            var openingVersion = _openVersion;
            var item = manager.GetLayoutItemFromModel(model); item.UpdateView();
            if (!IsCurrent(model, manager, openingVersion)) return;
            VisualParenting.Detach(item.View);
            if (!IsCurrent(model, manager, openingVersion)) return;
            item.View.Visibility = Visibility.Visible; _presenter.Content = item.View;
        ...
        if (activate) model.IsActive = true;
    ...
    private void Pin()
    {
        ...
        var manager = _manager; manager?.CloseAutoHide();
        if (model.IsAutoHidden && ReferenceEquals(model.Root, manager?.Layout)) model.ToggleAutoHide();
    }
    ...
    private bool IsCurrent(LayoutAnchorable model, DockingManager manager, long version) =>
        version == _openVersion && ReferenceEquals(model, _model) && ReferenceEquals(manager, _manager) &&
        model.IsEnabled && model.IsAutoHidden && ReferenceEquals(model.Root, manager.Layout);
    internal void SetViewport(Rect viewport, Size surfaceSize)
    {
        if (_model is not { } model || _manager is not { } manager) return;
        var version = _openVersion;
        if (_viewport != viewport) { CancelResize(); if (!IsCurrent(model, manager, version)) return; _viewport = viewport; }
        UpdateChrome();
        if (!IsCurrent(model, manager, version)) return;
        ...
        Width = horizontal ? extent : double.NaN; Height = horizontal ? double.NaN : extent;
    ...
    internal void UpdateChrome()
    {
        if (_model is not { } model || _manager is not { } manager) return;
        var version = _openVersion;
        var p = DockChrome.Palette(manager); var side = model.GetSide();
        ...
        if (!IsCurrent(model, manager, version)) return;
        _resizer.Horizontal = horizontal; _resizer.Background = p.Border;
        for (var i = 0; i < 3; i++)
        {
        ...
        Grid.SetRow(_layout, 1); Grid.SetColumn(_layout, 1);
        ...
        if (_contextMenu == null || !ReferenceEquals(_contextSource, manager.AnchorableContextMenu))
        {
            ...
            _contextMenu.Opening += MenuOpened; _contextMenu.Closed += MenuClosed; ContextFlyout = _contextMenu;
        }
    }
    private void MenuOpened(object? sender, object e) { _menuOpen = true; _manager?.Surface?.StopAutoHideTimer(); }
    private void MenuClosed(object? sender, object e) { _menuOpen = false; if (!RetainOpen) _manager?.Surface?.StartAutoHideTimer(); }
    private void DetachMenu()
    {
        var menu = _contextMenu; var wasOpen = _menuOpen;
        _menuOpen = false; _contextMenu = _contextSource = null; ContextFlyout = null;
        if (menu != null)
        {
            menu.Opening -= MenuOpened; menu.Closed -= MenuClosed;
    ...
    {
        if (_model == null) return base.MeasureOverride(constraint);
    ...
    protected override Size ArrangeOverride(Size finalSize)
    {
        ...
        _root.Clip = new RectangleGeometry { Rect = new(0, 0, Math.Max(0, bounded.Width), Math.Max(0, bounded.Height)) };
    ...
    {
        var manager = _manager; var model = _model; var version = ++_openVersion;
        _model = null; _manager = null; _pointerInside = false;
        ...
        DetachMenu();
        if (version != _openVersion) return;
        _presenter.Content = null; _titleView.ContentTemplate = null; _titleView.Content = _title; _title.Text = "";
        ContextFlyout = null; Visibility = Visibility.Collapsed;
        ...
        if (version == _openVersion && ReferenceEquals(manager?.AutoHideWindow, this)) manager.SetAutoHideHost(null);
    }
}
