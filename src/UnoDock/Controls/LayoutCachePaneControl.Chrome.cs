...
public partial class LayoutCachePaneControl
{
    ...
    private readonly ContentPresenter _titlePresenter = new() { Name = "PART_ToolCaption" };
    ...
    private void RevealSelectedHeader(LayoutContent? selected)
    {
        ...
        if (ReferenceEquals(selected, _lastHeaderSelection) && index == _lastHeaderIndex) return;
        _lastHeaderSelection = selected; _lastHeaderIndex = index;
        var generation = ++_headerGeneration;
        if (selected == null || Pane is not LayoutDocumentPane) return;
        DispatcherQueue.TryEnqueue(() =>
        {
            if (generation != _headerGeneration || XamlRoot == null || !ReferenceEquals(Selector?.SelectedContent, selected) ||
                _tabBar.Visibility != Visibility.Visible || !_tabs.TryGetValue(selected, out var tab)) return;
            UpdateLayout();
            if (_scroll.ViewportWidth <= 0 || _scroll.ScrollableWidth <= 0) return;
            ...
            if (Math.Abs(offset - desired) > .25) _scroll.ChangeView(desired, null, null, true);
        });
    }
    private void InitializeChrome()
    {
        Padding = new(0); MinHeight = 0; MinWidth = 0;
        ...
        Grid.SetColumn(_documentsButton, 1); _tabBar.Children.Add(_documentsButton);
        _layout.Children.Add(_tabBar);
        _scroll.HorizontalScrollBarVisibility = ScrollBarVisibility.Hidden;
        _scroll.Padding = new(0); _scroll.MaxHeight = double.PositiveInfinity;
        _title.Margin = new(2, 0, 0, 0); _title.TextTrimming = TextTrimming.CharacterEllipsis;
        _titlePresenter.Content = _title; _titlePresenter.VerticalContentAlignment = VerticalAlignment.Center;
        _titleRow.ColumnDefinitions.Add(new() { Width = new(1, GridUnitType.Star) });
        _titleRow.ColumnDefinitions.Add(new() { Width = GridLength.Auto }); _titleRow.Children.Add(_titlePresenter);
        var actions = new StackPanel { Orientation = Orientation.Horizontal, VerticalAlignment = VerticalAlignment.Center };
        _menuButton = DockChrome.Icon(DockGlyph.Menu, () => { if (_titleModel != null && _titleManager != null) DockVisuals.Menu(_titleManager, _titleModel).ShowAt(_menuButton); }, "Tool window options");
        _pinButton = DockChrome.Icon(DockGlyph.Pin, () => _titleModel?.ToggleAutoHide(), "Auto-hide tool");
        _hideButton = DockChrome.Icon(DockGlyph.Close, () => { if (_titleModel != null) DockVisuals.CloseOrHide(_titleModel); }, "Hide or close tool");
        actions.Children.Add(_menuButton); actions.Children.Add(_pinButton); actions.Children.Add(_hideButton);
        Grid.SetColumn(actions, 1); _titleRow.Children.Add(actions);
        _titlePresenter.PointerPressed += (_, e) => { if (_titleModel != null) _titleManager?.BeginDrag(_titleModel, _titlePresenter, e); };
        _titlePresenter.DoubleTapped += (_, _) => { if (_titleModel?.IsFloating == true) _titleModel.Dock(); else _titleModel?.Float(); };
    }
    private void UpdatePaneChrome(ILayoutGroup pane, LayoutContent[] models, DockingManager manager)
    {
        var p = DockChrome.Palette(manager); var tool = pane is LayoutAnchorablePane;
        _layout.BorderBrush = p.Border; _layout.BorderThickness = new(1); _layout.Background = p.Surface;
        _content.Background = p.Surface; _content.Margin = new(2, tool ? 0 : 1, 2, 2);
        _headers.Background = p.Header; _tabBar.Background = p.Header;
        Grid.SetRow(_tabBar, tool ? 2 : 0); Grid.SetRow(_content, 1); Grid.SetRow(_titleRow, 0);
    ...
    private void UpdateTitle(bool visible, LayoutAnchorable? selected, DockingManager manager)
    {
        ...
        var p = DockChrome.Palette(manager); _title.FontSize = p.FontSize; _title.Foreground = p.Foreground;
    ...
    {
        if (Pane?.Root?.Manager is not { } manager) return;
        var root = manager.Layout; var pane = Pane;
        var menu = new MenuFlyout();
        foreach (var model in Items.Where(c => c is not LayoutDocument { IsVisible: false }).ToArray())
        {
            var item = new ToggleMenuFlyoutItem { Text = model.Title ?? "Untitled", IsChecked = model.IsSelected, IsEnabled = model.IsEnabled };
            item.Click += (_, _) =>
            {
