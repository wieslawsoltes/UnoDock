...
public partial class NavigatorWindow
{
    ...
    {
        _revealVersion++;
        if (_waitingForLayout != null && _layoutReady != null)
            _waitingForLayout.LayoutUpdated -= _layoutReady;
        _waitingForLayout = null;
        _layoutReady = null;
    }
    ...
    {
        CancelReveal();
        if (_sessionRoot == null || _selected == null)
            return;
        ...
        {
            if (version != _revealVersion || session != _sessionVersion || _sessionRoot == null || !ReferenceEquals(selected, _selected) || !Eligible(selected) || !ReferenceEquals(list, selected is LayoutDocumentItem ? _documentsList : _anchorablesList))
                return;
            if (TryReveal(list, selected))
                return;
            ...
            handler = (_, _) =>
            {
                ...
                if (version != _revealVersion || session != _sessionVersion)
                    return;
                _waitingForLayout = null;
                _layoutReady = null;
                DispatcherQueue.TryEnqueue(() => Attempt(attempts + 1));
            };
            _layoutReady = handler;
            list.LayoutUpdated += handler;
        }
    }
    ...
    private bool TryReveal(ListBox list, LayoutItem selected)
    {
        ...
        if (scroll?.Content is not UIElement content || scroll.ViewportHeight <= 0)
            return false;
        ...
        if (!double.IsFinite(top) || !double.IsFinite(bottom) || !double.IsFinite(current))
            return true;
