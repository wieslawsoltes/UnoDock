...
public sealed class DockGuideTarget
{
    public DockGuideTarget(DockDropPlan plan, Rect detectionRect)
    {
        ...
        if (!double.IsFinite(detectionRect.X) || !double.IsFinite(detectionRect.Y) || !double.IsFinite(detectionRect.Right) || !double.IsFinite(detectionRect.Bottom) || detectionRect.Width <= 0 || detectionRect.Height <= 0)
            throw new ArgumentOutOfRangeException(nameof(detectionRect));
