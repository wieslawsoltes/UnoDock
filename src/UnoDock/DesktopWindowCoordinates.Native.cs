...
public sealed partial class DesktopWindowCoordinates
{
    ...
    internal bool TryGetPointer(Window window, out DesktopPointerState state)
    {
        ...
        {
            state = MacDesktopInterop.Pointer();
            return true;
        }
        ...
        if (OperatingSystem.IsLinux() && Uno.UI.Xaml.WindowHelper.GetNativeWindow(window) is Uno.UI.NativeElementHosting.X11NativeWindow native)
        {
            ...
            try
            {
                ...
                state = new(new(Marshal.ReadInt16(reply, 16), Marshal.ReadInt16(reply, 18)), (mask & 0x100) != 0, (mask & 4) != 0, EscapeDownX11());
                return true;
            }
            finally
            {
    ...
    internal static void MoveNative(Window window, Point origin)
    {
        ...
        if (OperatingSystem.IsMacOS())
        {
            MacDesktopInterop.Move(window, origin);
            return;
        }
    ...
    internal bool IsNativeCaption(Window window, Point screen)
    {
        ...
        if (OperatingSystem.IsWindows())
        {
            ...
            if (!W32.GetWindowRect(handle, out var frame))
                return false;
        ...
        if (OperatingSystem.IsLinux() && Uno.UI.Xaml.WindowHelper.GetNativeWindow(window) is Uno.UI.NativeElementHosting.X11NativeWindow native)
        {
            var id = Id(native.WindowId);
            var root = RootX11(id);
            var top = TopLevelX11(id, root);
            if (top == id)
                return false; // No window-manager decoration (for example Xvfb).
    ...
    internal IDisposable? ConfigureOwner(Window window, Window? owner, bool tool)
    {
        ...
        if (owner == null || ReferenceEquals(window, owner))
            return null;
        if (OperatingSystem.IsWindows())
            return W32.Own(WindowsHandle(window), WindowsHandle(owner), tool);
#if !WINDOWS
        if (OperatingSystem.IsMacOS())
    ...
    private static class W32
    {
        ...
        internal static Point ClientOrigin(nint handle)
        {
            ...
            if (!ClientToScreen(handle, ref point))
                throw new InvalidOperationException("The native client has closed.");
            return new(point.X, point.Y);
        }

        internal static IDisposable Own(nint child, nint owner, bool tool)
        {
            var oldOwner = GetLong(child, -8);
            var oldStyle = GetLong(child, -20);
            var lease = new OwnerLease(child, oldOwner, oldStyle);
            try
            {
                SetLong(child, -8, owner);
                if (tool)
                    SetLong(child, -20, (nint)((oldStyle.ToInt64() | 0x80L) & ~0x40000L));
                return lease;
            }
            catch
            {
        ...
        private sealed class OwnerLease(nint child, nint owner, nint style) : IDisposable
        {
            ...
            public void Dispose()
            {
                ...
                if (window == 0 || !IsWindow(window))
                    return;
                SetLong(window, -8, owner);
                SetLong(window, -20, style);
            }
        }
    ...
    private bool EscapeDownX11()
    {
        ...
        if (_escapeKeycode == null)
        {
            ...
            if (setup == 0)
                throw new InvalidOperationException("The X11 display setup is unavailable.");
            // xcb_setup_t: min_keycode/max_keycode at bytes 34/35.
            var first = Marshal.ReadByte(setup, 34);
            var last = Marshal.ReadByte(setup, 35);
            var count = last - first + 1;
            if (count is <= 0 or > 255)
                throw new InvalidOperationException("Invalid X11 keyboard range.");
            ...
            try
            {
                ...
                if ((long)count * per > (uint)Marshal.ReadInt32(map, 4))
                    throw new InvalidOperationException("Incomplete X11 key mapping reply.");
                _escapeKeycode = 0;
                for (var row = 0; row < count && _escapeKeycode == 0; row++)
                    for (var col = 0; col < per; col++)
                        if (Marshal.ReadInt32(map, 32 + (row * per + col) * 4) == 0xff1b)
                        {
                            _escapeKeycode = (byte)(first + row);
                            break;
                        }
            }
            finally
        ...
        try
        {
            CheckReply(keys, keyError);
            var code = _escapeKeycode!.Value;
            return (Marshal.ReadByte(keys, 8 + code / 8) & (1 << (code % 8))) != 0;
        }
        finally
        {
            Xcb.Free(keys);
    ...
    private void PropertyX11(uint window, uint property, uint type, uint[] values)
    {
        ...
        {
            if (error != 0)
                throw new InvalidOperationException("Unable to configure native X11 floating-window ownership.");
        }
        finally
        {
            Xcb.Free(error);
    ...
    private uint[] ReadAtomsX11(uint window, uint property)
    {
        ...
        try
        {
            ...
            for (var i = 0; i < result.Length; i++)
                result[i] = unchecked((uint)Marshal.ReadInt32(reply, 32 + i * 4));
            return result;
        }
        finally
        {
    ...
    {
        [DllImport("libxcb.so.1", EntryPoint = "xcb_query_pointer")]
        internal static extern uint Pointer(XcbConnection connection, uint window);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_query_pointer_reply")]
        internal static extern nint PointerReply(XcbConnection connection, uint cookie, out nint error);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_get_setup")]
        internal static extern nint Setup(XcbConnection connection);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_get_keyboard_mapping")]
        internal static extern uint KeyboardMap(XcbConnection connection, byte first, byte count);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_get_keyboard_mapping_reply")]
        internal static extern nint KeyboardMapReply(XcbConnection connection, uint cookie, out nint error);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_query_keymap")]
        internal static extern uint Keymap(XcbConnection connection);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_query_keymap_reply")]
        internal static extern nint KeymapReply(XcbConnection connection, uint cookie, out nint error);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_intern_atom")]
        internal static extern uint Atom(XcbConnection connection, byte onlyExisting, ushort length, [MarshalAs(UnmanagedType.LPUTF8Str)] string name);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_intern_atom_reply")]
        internal static extern nint AtomReply(XcbConnection connection, uint cookie, out nint error);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_change_property_checked")]
        internal static extern uint Property(XcbConnection connection, byte mode, uint window, uint property, uint type, byte format, uint length, [In] uint[] data);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_get_property")]
        internal static extern uint GetProperty(XcbConnection connection, byte delete, uint window, uint property, uint type, uint offset, uint length);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_get_property_reply")]
        internal static extern nint GetPropertyReply(XcbConnection connection, uint cookie, out nint error);
    }
#endif
}
