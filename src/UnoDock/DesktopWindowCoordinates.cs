...
public sealed partial class DesktopWindowCoordinates : IScreenWindowCoordinates, IDisposable
{
    ...
    {
        Verify();
        Validate(source, default);
        Validate(destination, default);
        ...
        if (OperatingSystem.IsMacOS())
            return point => FromScreen(ToScreen(source, point), destination);
        var from = GetNative(source);
        var to = GetNative(destination);
        Point offset;
        ...
        {
            var a = Win32Origin(w.Hwnd);
            var b = Win32Origin(v.Hwnd);
        ...
        var destinationScale = Scale(destination);
...
#else
        if (OperatingSystem.IsMacOS())
            return Internal.MacDesktopInterop.ToScreen(WindowFor(source) ?? throw Unsupported(), local);
    ...
    {
        Verify();
        Validate(destination, screenPoint);
        Point point;
#if WINDOWS
        ...
        point = Microsoft.UI.Content.ContentCoordinateConverter.CreateForWindowId(destination.XamlRoot!.ContentIslandEnvironment.AppWindowId).ConvertScreenToLocal(new Windows.Graphics.PointInt32 { X = checked((int)Math.Round(screenPoint.X)), Y = checked((int)Math.Round(screenPoint.Y)) });
#else
        if (OperatingSystem.IsMacOS())
        {
    ...
    private void Verify()
    {
        if (Environment.CurrentManagedThreadId != _thread)
            throw new InvalidOperationException("Use desktop coordinates on the creating UI thread.");
    ...
    private static object GetNative(FrameworkElement element)
    {
        ...
        if (window == null)
            throw new InvalidOperationException("The visual no longer belongs to a live desktop window.");
    ...
    private static uint Id(IntPtr handle)
    {
        ...
        if (value == 0 || value > uint.MaxValue)
            throw new InvalidOperationException("Invalid X11 window identifier.");
        return (uint)value;
    }
    ...
    private XcbConnection Connection
    {
        get
        {
            if (_xcb is { IsClosed: false, IsInvalid: false } && Xcb.Error(_xcb) == 0)
                return _xcb;
            _xcb?.Dispose();
            _xcb = null;
            try
            {
                ...
                {
                    connection.Dispose();
                    throw new InvalidOperationException("Unable to open a coordinate-query connection to the X11 display.");
                }
            ...
            {
                throw new PlatformNotSupportedException("X11 coordinate conversion requires libxcb.so.1.", e);
            }
            catch (EntryPointNotFoundException e)
            {
                throw new PlatformNotSupportedException("X11 coordinate query functions are unavailable.", e);
            }
        }
    }
    ...
    private Point TranslateX11(uint source, uint target)
    {
        ...
        try
        {
            ...
            if (Marshal.ReadByte(reply, 1) == 0)
                throw new InvalidOperationException("Windows are on different X11 screens.");
    ...
    private static void CheckReply(IntPtr reply, IntPtr error)
    {
        if (error != IntPtr.Zero)
            throw new InvalidOperationException("X11 coordinate query rejected (error " + Marshal.ReadByte(error, 1) + "). The window may have closed.");
        if (reply == IntPtr.Zero)
            throw new InvalidOperationException("The X11 display disconnected during a coordinate query.");
    }
    ...
    {
        [DllImport("libxcb.so.1", EntryPoint = "xcb_connect", CallingConvention = CallingConvention.Cdecl)]
        internal static extern IntPtr Connect(IntPtr display, out int screen);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_disconnect", CallingConvention = CallingConvention.Cdecl)]
        internal static extern void Disconnect(IntPtr connection);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_connection_has_error", CallingConvention = CallingConvention.Cdecl)]
        internal static extern int Error(XcbConnection connection);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_translate_coordinates", CallingConvention = CallingConvention.Cdecl)]
        internal static extern uint Translate(XcbConnection connection, uint source, uint target, short x, short y);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_translate_coordinates_reply", CallingConvention = CallingConvention.Cdecl)]
        internal static extern IntPtr TranslateReply(XcbConnection connection, uint cookie, out IntPtr error);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_get_geometry", CallingConvention = CallingConvention.Cdecl)]
        internal static extern uint Geometry(XcbConnection connection, uint drawable);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_get_geometry_reply", CallingConvention = CallingConvention.Cdecl)]
        internal static extern IntPtr GeometryReply(XcbConnection connection, uint cookie, out IntPtr error);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_unmap_window_checked", CallingConvention = CallingConvention.Cdecl)]
        internal static extern uint Unmap(XcbConnection connection, uint window);
        [DllImport("libxcb.so.1", EntryPoint = "xcb_request_check", CallingConvention = CallingConvention.Cdecl)]
        internal static extern IntPtr RequestCheck(XcbConnection connection, uint cookie);
