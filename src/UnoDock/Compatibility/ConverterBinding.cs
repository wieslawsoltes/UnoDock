...
public sealed class ConverterBinding : IDisposable
{
        ...
        public ConverterBinding(FrameworkElement target, DependencyProperty targetProperty, Func<object?> readSource, Func<object?, object?> convert, IEnumerable<INotifyPropertyChanged>? sources = null, string? sourceProperty = null, Action<object?>? writeSource = null, Func<object?, object?>? convertBack = null, object? fallbackValue = null, bool useFallbackValue = false)
    {
        ...
        _hasFallback = useFallbackValue;
        _dispatcher = target.DispatcherQueue;
        ...
        if (_sources.Any(source => source == null))
            throw new ArgumentException("A notification source cannot be null.", nameof(sources));
        var bindings = Targets.GetOrCreateValue(target);
        if (bindings.ContainsKey(targetProperty))
            throw new InvalidOperationException("A converter binding already owns this target property.");
        ...
        try
        {
            ...
            if (writeSource != null)
                _targetToken = target.RegisterPropertyChangedCallback(targetProperty, TargetChanged);
            UpdateTarget();
        }
        catch
        {
            _disposed = true;
            bindings.Remove(targetProperty);
            for (var i = 0; i < subscribed; i++)
                _sources[i].PropertyChanged -= SourceChanged;
            if (_targetToken is { } token)
                target.UnregisterPropertyChangedCallback(targetProperty, token);
            throw;
        }
    }
    ...
    public void UpdateTarget()
    {
        ...
        _dirty = true;
        if (_updating || _sourceWriteDepth != 0)
            return;
        _updating = true;
        try
        {
            ...
            {
                if (++iterations > 64)
                    throw new InvalidOperationException("Converter binding did not converge after 64 reentrant transfers.");
                _dirty = false;
                var value = _convert(_readSource());
                if (_disposed || ReferenceEquals(value, BindingValue.DoNothing))
                    continue;
                ...
                if (ReferenceEquals(value, BindingValue.DoNothing) || ReferenceEquals(value, DependencyProperty.UnsetValue))
                    continue;
                if (Equals(_target.GetValue(_property), value))
                    continue;
                _targetWriteDepth++;
                try
                {
    ...
    {
        if (_disposed || !string.IsNullOrEmpty(_sourceProperty) && !string.IsNullOrEmpty(args.PropertyName) && args.PropertyName != _sourceProperty)
            return;
        ...
        }))
        {
            Interlocked.Exchange(ref _queued, 0);
            if (!_disposed)
                throw new InvalidOperationException("The target dispatcher is shutting down.");
        }
    }

    private void TargetChanged(DependencyObject sender, DependencyProperty property)
    {
        if (_disposed || _targetWriteDepth != 0 || _sourceWriteDepth != 0)
            return;
        VerifyAccess();
        _sourceWriteDepth++;
        try
    ...
    public void Dispose()
    {
        ...
        if (_disposed)
            return;
        _disposed = true;
        if (Targets.TryGetValue(_target, out var bindings))
            bindings.Remove(_property);
        foreach (var source in _sources)
            source.PropertyChanged -= SourceChanged;
        if (_targetToken is { } token)
            _target.UnregisterPropertyChangedCallback(_property, token);
        _targetToken = null;
    }
