...
public sealed class RoutedCommand : ICommand
{
    ...
    { Name = name; OwnerType = ownerType; _canExecute = canExecute; _execute = execute; }
    ...
    public void Execute(object? parameter) { if (CanExecute(parameter)) _execute(parameter, null); }
    public void Execute(object? parameter, object? target) { if (CanExecute(parameter, target)) _execute(parameter, target); }
