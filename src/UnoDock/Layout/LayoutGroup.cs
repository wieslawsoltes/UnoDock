...
public abstract class LayoutGroup<T> : LayoutGroupBase, ILayoutGroup, IXmlSerializable, ILayoutElementWithVisibility where T : class, ILayoutElement
{
    ...
    {
        var index = IndexOfChild(oldElement);
        if (index < 0)
            throw new ArgumentException("The old element is not a child.", nameof(oldElement));
    ...
    {
        if (this is ILayoutPositionableElement { CanRepositionItems: false })
            return;
