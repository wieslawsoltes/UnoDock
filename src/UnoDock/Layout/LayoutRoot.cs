...
public partial class LayoutRoot : LayoutElement, ILayoutContainer, ILayoutRoot, IXmlSerializable
{
    ...
    public LayoutPanel RootPanel
    {
        ...
        set
        {
            ...
            LayoutTree.ReplaceSlot(this, ref _panel, value, nameof(RootPanel));
        }
    }
    ...
    public IEnumerable<ILayoutElement> Children
    {
        get
        {
            ...
            if (_left != null)
                yield return _left;
            foreach (var f in FloatingWindows)
                yield return f;
    ...
    internal LayoutAnchorSide GetSide(AnchorSide side) => side switch
    {
        AnchorSide.Top => TopSide,
        AnchorSide.Left => LeftSide,
        AnchorSide.Bottom => BottomSide,
        _ => RightSide
    };
    public void RemoveChild(ILayoutElement element)
    {
        switch (element)
        {
            case LayoutFloatingWindow window:
                FloatingWindows.Remove(window);
                break;
            case LayoutAnchorable anchorable:
                Hidden.Remove(anchorable);
                break;
            case LayoutPanel when ReferenceEquals(element, _panel):
                RootPanel = new(new LayoutDocumentPane());
                break;
            case LayoutAnchorSide side when ReferenceEquals(side, _top):
                TopSide = new();
                break;
            case LayoutAnchorSide side when ReferenceEquals(side, _right):
                RightSide = new();
                break;
            case LayoutAnchorSide side when ReferenceEquals(side, _bottom):
                BottomSide = new();
                break;
            case LayoutAnchorSide side when ReferenceEquals(side, _left):
                LeftSide = new();
                break;
        }
    }
    ...
    {
        if (!ReferenceEquals(oldElement.Parent, this))
            throw new ArgumentException("The element is not a direct child.", nameof(oldElement));
        switch (oldElement)
        {
            case LayoutPanel when newElement is LayoutPanel p:
                RootPanel = p;
                break;
            case LayoutFloatingWindow f when newElement is LayoutFloatingWindow n:
                FloatingWindows[FloatingWindows.IndexOf(f)] = n;
                break;
            case LayoutAnchorable a when newElement is LayoutAnchorable n:
                Hidden[Hidden.IndexOf(a)] = n;
                break;
            case LayoutAnchorSide s when newElement is LayoutAnchorSide n:
                ...
                break;
            default:
                throw new ArgumentException("Incompatible replacement type.", nameof(newElement));
        }
    }
