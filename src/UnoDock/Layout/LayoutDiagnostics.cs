...
internal static class LayoutDiagnostics
{
    internal static void Write(ILayoutElement root, TextWriter writer, int indentation)
    {
        ...
        while (pending.TryPop(out var entry))
        {
            ...
            {
                writer.Write(" Id=\"");
                writer.Write(Escape(content.ContentId));
                writer.Write("\" Title=\"");
                writer.Write(Escape(content.Title));
                writer.Write('"');
                ...
                if (content is LayoutAnchorable { IsHidden: true })
                    writer.Write(" Hidden");
                if (content is LayoutAnchorable { IsAutoHidden: true })
                    writer.Write(" AutoHidden");
            }

            if (entry.Element is ILayoutOrientableGroup oriented)
            {
                writer.Write(" Orientation=");
                writer.Write(oriented.Orientation);
            }
            ...
            if (entry.Element is ILayoutContainer container)
            {
                ...
                writer.Write(children.Length.ToString(CultureInfo.InvariantCulture));
                var depth = Math.Min(entry.Depth, 128) + 1;
                for (var index = children.Length - 1; index >= 0; index--)
                    pending.Push((children[index], depth));
            }
    ...
    private static string Escape(string? text) => text?.Replace("\\", "\\\\", StringComparison.Ordinal).Replace("\"", "\\\"", StringComparison.Ordinal).Replace("\r", "\\r", StringComparison.Ordinal).Replace("\n", "\\n", StringComparison.Ordinal).Replace("\t", "\\t", StringComparison.Ordinal) ?? "";
}
