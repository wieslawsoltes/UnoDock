...
public class LayoutAnchorableFloatingWindow : LayoutFloatingWindow, ILayoutElementWithVisibility
{
    ...
    public LayoutAnchorablePaneGroup? RootPanel
    {
        ...
        set
        {
            if (ReferenceEquals(value, _panel))
                return;
    ...
    {
        IsVisible = _panel?.IsVisible == true;
        Notify(nameof(IsSinglePane));
        Notify(nameof(SinglePane));
    }

    public void ComputeVisibility() => RefreshVisibility();
    public override void RemoveChild(ILayoutElement element)
    {
        if (ReferenceEquals(element, _panel))
            RootPanel = null;
    }

    public override void ReplaceChild(ILayoutElement oldElement, ILayoutElement newElement)
    {
        if (!ReferenceEquals(oldElement, _panel) || newElement is not LayoutAnchorablePaneGroup panel)
            throw new ArgumentException("Invalid anchorable group replacement.");
        RootPanel = panel;
    }
