...
internal sealed class PaneSelection(ILayoutGroup pane, Action<string> notify)
{
    ...
    public int Index
    {
        ...
        set
        {
            if (value < -1 || value >= pane.ChildrenCount)
    ...
    private void Select(LayoutContent? value)
    {
        if (_changing)
        {
            ...
            _requested = value;
            _pending = !ReferenceEquals(_selected, value);
            return;
        }
        ...
        try
        {
            ...
            {
                if (pass == 64)
                    throw new InvalidOperationException("Pane selection observers did not converge after 64 transitions.");
                value = _requested;
                _pending = false;
                if (value != null && pane.IndexOfChild(value) < 0)
                    value = null;
                if (ReferenceEquals(_selected, value))
                    continue;
                ...
                foreach (var c in pane.Children.OfType<LayoutContent>().ToArray())
                    if (!ReferenceEquals(c, value) && c.IsSelected && pane.IndexOfChild(c) >= 0)
                        c.IsSelected = false;
                if (value != null && pane.IndexOfChild(value) >= 0)
    ...
    public void CollectionChanged()
    {
        ...
        Select(pane.Children.OfType<LayoutContent>().FirstOrDefault(c => c.IsSelected && c.IsEnabled) ?? pane.Children.OfType<LayoutContent>().FirstOrDefault(c => c.IsEnabled));
    }
}
