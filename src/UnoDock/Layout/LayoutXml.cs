...
internal static class LayoutXml
{
    ...
    internal static void ReadInto(LayoutElement element, XmlReader reader)
    {
        ...
        if (element is LayoutRoot root) FixReferences(root);
    }
    internal static LayoutRoot ReadRoot(LayoutSnapshotNode snapshot)
    {
        if (snapshot.Name != nameof(LayoutRoot)) throw new XmlException("Expected LayoutRoot.");
        var root = new LayoutRoot();
        using (root.BeginUpdate()) { Populate(root, snapshot); FixReferences(root); }
        return root;
    }
    internal static void WriteBody(LayoutElement element, XmlWriter writer)
    {
        ...
        foreach (var (key, value) in node.Attributes) writer.WriteAttributeString(key, value);
        foreach (var child in node.Children) LayoutSnapshotXml.Write(child, writer);
    }
    internal static LayoutSnapshotNode CaptureRoot(LayoutRoot root) { AssignIds(root); return Capture(root); }
    private static void AssignIds(LayoutElement root)
    {
        ...
        {
            if (element.SerializationId.Length != 0 && ids.Add(element.SerializationId)) continue;
            do { element.SerializationId = "p" + (++sequence).ToString(Invariant); } while (!ids.Add(element.SerializationId));
        }
    }
    private static LayoutSnapshotNode Capture(LayoutElement element, string? name = null)
    {
        ...
        void Put(string key, object? value)
        {
            ...
            node.Attributes[key] = value switch { double d => d.ToString("R", Invariant), bool b => XmlConvert.ToString(b), DateTime dt => XmlConvert.ToString(dt, XmlDateTimeSerializationMode.RoundtripKind), _ => Convert.ToString(value, Invariant)! };
        }
        if (element is ILayoutContainer && element is not LayoutRoot) Put("Id", element.SerializationId);
        if (element is ILayoutOrientableGroup orientable) Put("Orientation", orientable.Orientation);
        if (element is ILayoutPositionableElement position)
        {
            Put("DockWidth", Length(position.DockWidth)); Put("DockHeight", Length(position.DockHeight));
            Put("DockMinWidth", position.DockMinWidth); Put("DockMinHeight", position.DockMinHeight);
            Put("FloatingLeft", position.FloatingLeft); Put("FloatingTop", position.FloatingTop); Put("FloatingWidth", position.FloatingWidth); Put("FloatingHeight", position.FloatingHeight);
            Put("IsMaximized", position.IsMaximized); Put("CanRepositionItems", position.CanRepositionItems); Put("AllowDuplicateContent", position.AllowDuplicateContent);
        }
        if (element is LayoutContent content)
        {
            ...
            {
                Put("CanHide", anchorable.CanHide); Put("CanAutoHide", anchorable.CanAutoHide); Put("CanDockAsTabbedDocument", anchorable.CanDockAsTabbedDocument);
                Put("AutoHideWidth", anchorable.AutoHideWidth); Put("AutoHideHeight", anchorable.AutoHideHeight); Put("AutoHideMinWidth", anchorable.AutoHideMinWidth); Put("AutoHideMinHeight", anchorable.AutoHideMinHeight);
            }
        }
        ...
        { Put("PreviousContainerId", container.SerializationId); Put("PreviousContainerIndex", previous.PreviousContainerIndex); }
        if (element is LayoutAnchorablePane ap) Put("Name", ap.Name);
        if (element is LayoutDocumentPane dp) Put("ShowHeader", dp.ShowHeader);
        if (element is ILayoutContentSelector selector) Put("SelectedContentIndex", selector.SelectedContentIndex);
        switch (element)
        {
            case LayoutRoot root:
                node.Children.Add(Capture(root.RootPanel, "RootPanel"));
                node.Children.Add(Capture(root.TopSide, "TopSide")); node.Children.Add(Capture(root.RightSide, "RightSide"));
                node.Children.Add(Capture(root.BottomSide, "BottomSide")); node.Children.Add(Capture(root.LeftSide, "LeftSide"));
    ...
    private static void Populate(LayoutElement element, LayoutSnapshotNode node)
    {
        ...
        void D(string key, Action<double> set) { if (S(key) is { } text) { if (!double.TryParse(text, NumberStyles.Float, Invariant, out var value) || !double.IsFinite(value)) throw new XmlException("Invalid finite number: " + key); set(value); } }
        void B(string key, Action<bool> set) { if (S(key) is { } text) { if (text == "1") set(true); else if (text == "0") set(false); else if (bool.TryParse(text, out var value)) set(value); else throw new XmlException("Invalid boolean: " + key); } }
        ...
        if (element is ILayoutOrientableGroup orientable && S("Orientation") is { } o)
            orientable.Orientation = o switch { "Horizontal" => Orientation.Horizontal, "Vertical" => Orientation.Vertical, _ => throw new XmlException("Invalid orientation.") };
        if (element is ILayoutPositionableElement position)
        {
            if (S("DockWidth") is { } width) position.DockWidth = ParseLength(width);
            if (S("DockHeight") is { } height) position.DockHeight = ParseLength(height);
            D("DockMinWidth", v => position.DockMinWidth = v); D("DockMinHeight", v => position.DockMinHeight = v);
            D("FloatingLeft", v => position.FloatingLeft = v); D("FloatingTop", v => position.FloatingTop = v); D("FloatingWidth", v => position.FloatingWidth = v); D("FloatingHeight", v => position.FloatingHeight = v);
            B("IsMaximized", v => position.IsMaximized = v); B("CanRepositionItems", v => position.CanRepositionItems = v); B("AllowDuplicateContent", v => position.AllowDuplicateContent = v);
        }
        if (element is LayoutContent content)
        {
            ...
            if (content is LayoutAnchorable anchorable)
            {
                B("CanHide", v => anchorable.CanHide = v); B("CanAutoHide", v => anchorable.CanAutoHide = v); B("CanDockAsTabbedDocument", v => anchorable.CanDockAsTabbedDocument = v);
                D("AutoHideWidth", v => anchorable.AutoHideWidth = v); D("AutoHideHeight", v => anchorable.AutoHideHeight = v); D("AutoHideMinWidth", v => anchorable.AutoHideMinWidth = v); D("AutoHideMinHeight", v => anchorable.AutoHideMinHeight = v);
            }
        ...
        if (element is LayoutAnchorGroup anchorGroup) { anchorGroup.PreviousContainerId = S("PreviousContainerId"); anchorGroup.PreviousContainerIndex = I("PreviousContainerIndex"); }
        if (element is LayoutAnchorablePane ap) ap.Name = S("Name");
        if (element is LayoutDocumentPane dp) B("ShowHeader", v => dp.ShowHeader = v);
        if (element is LayoutRoot root)
        {
            var seen = new HashSet<string>(StringComparer.Ordinal);
            root.FloatingWindows.Clear(); root.Hidden.Clear();
            foreach (var child in node.Children)
            {
                if (!seen.Add(child.Name)) throw new XmlException("Duplicate layout root slot: " + child.Name);
                switch (child.Name)
                {
                    case "RootPanel": root.RootPanel = Build<LayoutPanel>(child); break;
                    case "TopSide": root.TopSide = Build<LayoutAnchorSide>(child); break;
                    case "RightSide": root.RightSide = Build<LayoutAnchorSide>(child); break;
                    case "BottomSide": root.BottomSide = Build<LayoutAnchorSide>(child); break;
                    case "LeftSide": root.LeftSide = Build<LayoutAnchorSide>(child); break;
                    case "FloatingWindows": foreach (var f in child.Children) root.FloatingWindows.Add(Create(f) as LayoutFloatingWindow ?? throw new XmlException("Expected floating window.")); break;
                    case "Hidden": foreach (var h in child.Children) root.Hidden.Add(Create(h) as LayoutAnchorable ?? throw new XmlException("Expected hidden anchorable.")); break;
                    default: throw new XmlException("Unknown root slot: " + child.Name);
                }
            }
        }
        else if (element is LayoutDocumentFloatingWindow df)
        {
            if (node.Children.Count > 1) throw new XmlException("A document floating window has one document.");
        ...
        {
            if (node.Children.Count > 1) throw new XmlException("An anchorable floating window has one root group.");
    ...
    private static T Build<T>(LayoutSnapshotNode node) where T : LayoutElement, new() { var value = new T(); Populate(value, node); return value; }
    private static LayoutElement Create(LayoutSnapshotNode node) => node.Name switch
    {
        nameof(LayoutPanel) => Build<LayoutPanel>(node), nameof(LayoutDocumentPane) => Build<LayoutDocumentPane>(node),
        nameof(LayoutDocumentPaneGroup) => Build<LayoutDocumentPaneGroup>(node), nameof(LayoutAnchorablePane) => Build<LayoutAnchorablePane>(node),
        nameof(LayoutAnchorablePaneGroup) => Build<LayoutAnchorablePaneGroup>(node), nameof(LayoutAnchorGroup) => Build<LayoutAnchorGroup>(node),
        nameof(LayoutAnchorSide) => Build<LayoutAnchorSide>(node), nameof(LayoutDocument) => Build<LayoutDocument>(node),
        nameof(LayoutAnchorable) => Build<LayoutAnchorable>(node), nameof(LayoutDocumentFloatingWindow) => Build<LayoutDocumentFloatingWindow>(node),
        nameof(LayoutAnchorableFloatingWindow) => Build<LayoutAnchorableFloatingWindow>(node), _ => throw new XmlException("Unknown layout node: " + node.Name)
    };
    private static void FixReferences(LayoutRoot root)
    {
        ...
        foreach (var container in root.Descendents().OfType<ILayoutContainer>())
        {
            ...
            if (id.Length > 0 && !ids.TryAdd(id, container)) throw new XmlException("Duplicate layout container ID: " + id);
        }
    ...
    private static GridLength ParseLength(string text)
    {
        ...
        var star = text.EndsWith('*'); if (star) text = text[..^1];
        if (star && text.Length == 0) return new(1, GridUnitType.Star);
        if (!double.TryParse(text, NumberStyles.Float, Invariant, out var value) || !double.IsFinite(value) || value < 0) throw new XmlException("Invalid grid length.");
