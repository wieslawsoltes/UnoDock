...
public static class DockOperations
{
    ...
    public static void Float(LayoutContent content)
    {
        ...
        if (!content.CanFloat || !CanMove(content) || content.IsFloating || content.Root is not LayoutRoot root)
            return;
        var manager = root.Manager;
        using var transition = manager?.BeginTransition(content, true);
        ...
        else if (content is LayoutAnchorable tool)
            floating = new LayoutAnchorableFloatingWindow
            {
                RootPanel = new(new LayoutAnchorablePane(tool))
            };
        else
            throw new ArgumentException("Unknown layout content type.", nameof(content));
    ...
    {
        ArgumentNullException.ThrowIfNull(content);
        if (!CanMove(content) || content.Root is not LayoutRoot root)
            return;
        var manager = root.Manager;
    ...
    {
        if (!CanMove(content) || content.Root is not LayoutRoot root || content is LayoutAnchorable { CanDockAsTabbedDocument: false })
            return;
        var manager = root.Manager;
        using var transition = manager?.BeginTransition(content, false);
        if (manager != null && transition == null)
            return;
        using var batch = root.BeginUpdate();
        if (content.Parent is LayoutDocumentPane)
        {
            content.IsActive = true;
            return;
        }
    ...
    private static void InsertDocument(LayoutRoot root, LayoutContent content)
    {
        ...
        if (content is LayoutDocument doc)
        {
            ...
            if (manager != null && !ReferenceEquals(manager.Layout, root) || !ReferenceEquals(pane.Root, root))
                return;
            if (!handled && !ReferenceEquals(doc.Parent, pane))
                pane.Children.Add(doc);
            if (ReferenceEquals(doc.Root, root))
                manager?.LayoutUpdateStrategy?.AfterInsertDocument(root, doc);
        }
        else if (AllowsDrop(pane, content))
    ...
    public static void AddAnchorable(LayoutRoot root, LayoutAnchorable content, AnchorableShowStrategy strategy)
    {
        ...
        if (manager != null && !ReferenceEquals(manager.Layout, root) || !ReferenceEquals(existing.Root, root))
            return;
        if (!handled && !ReferenceEquals(content.Parent, existing))
            existing.Children.Add(content);
        if (ReferenceEquals(content.Root, root))
            manager?.LayoutUpdateStrategy?.AfterInsertAnchorable(root, content);
        if (ReferenceEquals(content.Root, root))
    ...
    {
        if (!content.CanAutoHide || content.Root is not LayoutRoot root)
            return;
        using var batch = root.BeginUpdate();
        if (content.Parent is LayoutAnchorGroup group)
        {
            ...
            {
                destination = new();
                AddAtRoot(root, destination, group.GetSide());
            }
        ...
        else if (content.Parent is LayoutAnchorablePane pane)
        {
            ...
            {
                PreviousContainer = pane,
                PreviousContainerIndex = (pane.Parent as ILayoutGroup)?.IndexOfChild(pane) ?? 0
            };
    ...
    public static bool CanContain(ILayoutGroup target, LayoutContent content) => target is LayoutDocumentPane && (content is LayoutDocument || content is LayoutAnchorable { CanDockAsTabbedDocument: true }) || target is LayoutAnchorablePane or LayoutAnchorGroup && content is LayoutAnchorable;
    ...
    public static bool CanDock(LayoutContent content, ILayoutGroup target, DockPosition position, bool asDocument)
    {
        ...
        if (position == DockPosition.Inside)
            return CanContain(target, content) && AllowsDrop(target, content);
        if (position is not (DockPosition.Left or DockPosition.Right or DockPosition.Top or DockPosition.Bottom))
            return false;
        if (target is not ILayoutPanelElement || target.Parent is not ILayoutGroup)
            return false;
        var orientation = position is DockPosition.Left or DockPosition.Right ? Orientation.Horizontal : Orientation.Vertical;
        return !asDocument || root.Manager?.AllowMixedOrientation != false || target.Parent is not LayoutDocumentPaneGroup group || group.ChildrenCount <= 1 || group.Orientation == orientation;
    }
    ...
    public static void Dock(LayoutContent content, ILayoutGroup target, DockPosition position, int insertionIndex, bool asDocument)
    {
        ArgumentNullException.ThrowIfNull(content);
        ArgumentNullException.ThrowIfNull(target);
        if (!CanDock(content, target, position, asDocument) || target.Root is not LayoutRoot root)
            return;
        if (position == DockPosition.Inside)
        {
            if (!CanContain(target, content))
                return;
            var initiatingManager = root.Manager;
            using var transition = initiatingManager?.BeginTransition(content, false);
            if (initiatingManager != null && transition == null)
                return;
            if (!CanDock(content, target, position, asDocument))
                return;
            using var batch = root.BeginUpdate();
            if (ReferenceEquals(content.Parent, target))
            {
                ...
                if (insertionIndex >= 0 && target is ILayoutPane pane)
                {
                    ...
                    if (destination != from)
                        pane.MoveChild(from, destination);
                }
            }
            else
                target.InsertChildAt(insertionIndex < 0 ? target.ChildrenCount : Math.Clamp(insertionIndex, 0, target.ChildrenCount), content);
        ...
        if (target is not ILayoutPanelElement targetElement || target.Parent is not ILayoutGroup parent)
            return;
        ...
        if (manager != null && edgeTransition == null)
            return;
        if (!CanDock(content, target, position, asDocument) || !ReferenceEquals(targetElement.Parent, parent))
            return;
        using (root.BeginUpdate())
        {
            ...
            else
            {
                ...
                {
                    LayoutDocumentPaneGroup when newPane is ILayoutDocumentPane && targetElement is ILayoutDocumentPane => new LayoutDocumentPaneGroup
                    {
                        Orientation = orientation
                    },
                    LayoutAnchorablePaneGroup when newPane is ILayoutAnchorablePane && targetElement is ILayoutAnchorablePane => new LayoutAnchorablePaneGroup
                    {
                        Orientation = orientation
                    },
    ...
    private static bool AllowsDrop(ILayoutGroup target, LayoutContent content) => ReferenceEquals(content.Parent, target) || target is not ILayoutPositionableElement { AllowDuplicateContent: false } || !target.Children.OfType<LayoutContent>().Any(other => !ReferenceEquals(other, content) && string.Equals(other.Title, content.Title, StringComparison.Ordinal) && string.Equals(other.ContentId, content.ContentId, StringComparison.Ordinal));
    public static void DockIntoEmptyDocumentGroup(LayoutContent content, LayoutDocumentPaneGroup group)
    {
        ArgumentNullException.ThrowIfNull(content);
        ArgumentNullException.ThrowIfNull(group);
        if (group.ChildrenCount != 0 || !CanMove(content) || content.Root is not LayoutRoot root || !ReferenceEquals(group.Root, root) || content is LayoutAnchorable { CanDockAsTabbedDocument: false })
            return;
        var manager = root.Manager;
        using var transition = manager?.BeginTransition(content, false);
        if (manager != null && transition == null || group.ChildrenCount != 0 || !ReferenceEquals(group.Root, root))
            return;
        using var batch = root.BeginUpdate();
        var pane = new LayoutDocumentPane();
        group.Children.Add(pane);
        pane.Children.Add(content);
        content.SetPrevious(null, -1);
        content.IsActive = true;
        root.CollectGarbage();
    }
    ...
    public static void DockToRoot(LayoutContent content, DockPosition position)
    {
        ...
        AddAtRoot(root, pane, position switch
        {
    ...
    internal static void AddAtRoot(LayoutRoot root, ILayoutPanelElement pane, AnchorSide side)
    {
        ...
        panel.Children.Insert(before ? 0 : panel.ChildrenCount, pane);
    }
}
