...
public sealed partial class GalleryPage
{
    private void ShowNavigatorLab()
    {
        ...
        Add("20 pt", "large", () =>
        {
            manager.Resources["UnoDock.FontSize"] = 20d;
            manager.Refresh();
        });
        Add("12 pt", "normal", () =>
        {
        ...
        {
            Title = "Navigator quality",
            ContentId = "navigator-lab:" + Guid.NewGuid().ToString("N"),
            Content = panel
        };
        ...
        void Populate(int count)
        {
            ...
            for (var i = 0; i < count; i++)
                documents.Children.Add(new LayoutDocument { ContentId = "navigator-editor:" + i, Title = $"Document {i:D3}.cs", Description = $"Project / Source / Document {i:D3}.cs", Content = new TextBox { AcceptsReturn = true, Text = WorkspaceTextProjection.ForEditor($"// Document {i:D3}\n// Edit, switch to another document, then return here."), TextWrapping = TextWrapping.Wrap, Padding = new(14) } });
            ...
            {
                var command = new NavigatorLabCommand(() =>
                {
                    activations++;
                    model.IsActive = true;
                    UpdateStatus();
                }, () => !blocked && model.IsEnabled && ReferenceEquals(model.Root, manager.Layout));
                activationCommands.Add(command);
                manager.GetLayoutItemFromModel(model).ActivateCommand = command;
            }
        ...
        void AssignSelection(bool tool)
        {
            ...
            if (navigator == null)
            {
                ...
                navigator = manager.FindVisualChildren<NavigatorWindow>().FirstOrDefault();
            }

            if (navigator == null)
                return;
            if (tool)
                navigator.SelectedAnchorable = navigator.Anchorables.LastOrDefault();
            else
                navigator.SelectedDocument = navigator.Documents.FirstOrDefault(item => !ReferenceEquals(item, navigator.SelectedDocument));
            UpdateStatus();
        }

        SampleButton Add(string title, string id, Action action)
        {
            var button = SampleChrome.Button(title, action);
            button.Padding = new(7, 1, 7, 1);
            button.Height = 25;
