...
public sealed partial class GalleryPage
{
    ...
    {
        var workspace = _mvvmWorkspace; _mvvmWorkspace = null;
        workspace?.Dispose();
    }
    private async Task SaveWorkspaceLayoutAsync()
    {
        ...
        try
        {
            if (_pageDisposed || epoch != _workspaceEpoch) return;
            ...
            temporary = null;
            if (!_pageDisposed && epoch == _workspaceEpoch) Log("Layout saved to application storage; document buffers are not serialized.");
        }
        finally
        {
            try { if (temporary != null) await temporary.DeleteAsync(); }
            finally { _layoutSaveGate.Release(); }
        }
    }
    private async Task RestoreWorkspaceLayoutAsync()
    {
        var epoch = _workspaceEpoch; var request = ++_restoreRequest; var root = Dock.Layout; var workspace = _mvvmWorkspace;
        var file = await ApplicationData.Current.LocalFolder.GetFileAsync("workspace.xml");
        var xml = await FileIO.ReadTextAsync(file);
        if (_pageDisposed || epoch != _workspaceEpoch || request != _restoreRequest || !ReferenceEquals(root, Dock.Layout)) return;
        if (workspace?.IsCurrent == true) workspace.RestoreLayout(xml);
        else
        {
            var serializer = new XmlLayoutSerializer(Dock);
            serializer.LayoutSerializationCallback += (_, e) =>
            {
                if (_pageDisposed || epoch != _workspaceEpoch) throw new InvalidOperationException("Sample changed during layout restoration.");
                if (e.Model.ContentId != null && _content.TryGetValue(e.Model.ContentId, out var content)) e.Content = content;
                else if (e.Content == null) e.Cancel = true;
            };
            serializer.Deserialize(new StringReader(xml));
        }
        if (!_pageDisposed && epoch == _workspaceEpoch) Log("Layout restored using current application-owned content identities.");
    }
}
