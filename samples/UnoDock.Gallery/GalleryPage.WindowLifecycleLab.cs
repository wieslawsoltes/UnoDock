...
public sealed partial class GalleryPage
{
    private void ShowWindowLifecycleLab()
    {
        ...
        panel.Children.Add(new TextBlock { Text = "Maximize and serialize: saved coordinates must remain the normal rectangle. Cancel a close, dock the editor back, and switch between documents/tools with Ctrl+Tab. Escape cancels the navigator; releasing Control commits and restores editor focus.", TextWrapping = TextWrapping.Wrap, MaxWidth = 850 });
        var rows = new ObservableCollection<string>();
        var status = new TextBlock
        {
            TextWrapping = TextWrapping.Wrap
        };
        var saved = new TextBox
        {
            AcceptsReturn = true,
            IsReadOnly = true,
            MinHeight = 130,
        ...
        {
            Content = "Cancel specimen window closing",
            IsChecked = false
        };
        ...
        {
            var model = specimen;
            status.Text = model == null ? "Create a specimen to start." : $"Persisted normal DIPs: ({model.FloatingLeft:F1}, {model.FloatingTop:F1}), {model.FloatingWidth:F1} × {model.FloatingHeight:F1}\n" + $"Model maximized: {model.IsMaximized}; floating: {model.IsFloating}; native host: {observed?.NativeWindow != null}";
        }

        void Initialized(object? sender, EventArgs e)
        {
            Record("Initialized: derived model is available");
            Describe();
        }

        void Closing(object? sender, CancelEventArgs e)
        {
            e.Cancel |= protect.IsChecked == true;
            Record("Closing; cancelled=" + e.Cancel);
        }

        void Closed(object? sender, EventArgs e)
        {
            Record("Closed: permanent control lifetime ended");
            Describe();
        }

        void StateChanged(object? sender, EventArgs e)
        {
            Record("StateChanged; maximized=" + observed?.IsMaximized);
            Describe();
        }
        ...
        void Detach()
        {
            if (observed == null)
                return;
            observed.Initialized -= Initialized;
            observed.Closing -= Closing;
            observed.Closed -= Closed;
            observed.StateChanged -= StateChanged;
            observed = null;
        }
        ...
        void Create(bool native)
        {
            ...
            specimen.FloatingLeft = 90;
            specimen.FloatingTop = 100;
            specimen.FloatingWidth = 540;
            specimen.FloatingHeight = 320;
            var pane = Dock.Layout.Descendents().OfType<LayoutDocumentPane>().First();
            pane.Children.Add(specimen);
        ...
        Add("Create managed", () => Create(false));
        Add("Create native", () => Create(true));
        Add("Maximize", () =>
        {
            if (observed != null)
                SystemCommands.MaximizeWindow(observed);
        });
        Add("Minimize", () =>
        {
            if (observed != null)
                SystemCommands.MinimizeWindow(observed);
        });
        Add("Restore", () =>
        {
            if (observed != null)
                SystemCommands.RestoreWindow(observed);
        });
        Add("Close", () => observed?.Close());
        Add("Dock", () => specimen?.Dock());
        Add("Serialize", () =>
        {
            using var writer = new StringWriter(System.Globalization.CultureInfo.InvariantCulture);
            new UnoDock.Layout.Serialization.XmlLayoutSerializer(Dock).Serialize(writer);
            saved.Text = writer.ToString();
            Record("Serialized current model; inspect FloatingWidth/Height and IsMaximized");
        });
        ...
        panel.Unloaded += (_, _) =>
        {
            if (!ReferenceEquals(lab.Root, Dock.Layout))
                Detach();
        };
        Dock.Layout.Descendents().OfType<LayoutDocumentPane>().First().Children.Add(lab);
        lab.IsActive = true;
        Describe();
