...
public sealed class MvvmWorkspace : INotifyPropertyChanged, IDisposable
{
    ...
    {
        _dock = dock; _log = log; _storage = storage ?? new LocalWorkspaceStorage();
        Files = new(_files);
        ...
        Documents.Add(first); first.IsOpen = true; Documents.Add(second); second.IsOpen = true;
        ...
        _tools.Add(explorer); _tools.Add(properties); _tools.Add(output);
        using (var batch = _dock.BeginLayoutUpdate())
        {
            ...
            var right = new LayoutAnchorablePane(Tool(properties)) { DockWidth = new(220), DockMinWidth = 150 };
            var bottom = new LayoutAnchorablePane(Tool(output)) { DockHeight = new(125), DockMinHeight = 70 };
            var center = new LayoutPanel(documents) { Orientation = Orientation.Vertical }; center.Children.Add(bottom);
            var panel = new LayoutPanel(left) { Orientation = Orientation.Horizontal }; panel.Children.Add(center); panel.Children.Add(right);
            _dock.Layout = new() { RootPanel = panel };
            _dock.DocumentsSource = Documents; _dock.AnchorablesSource = _tools;
        }
    ...
    public bool ProtectDirtyDocuments { get => _protectDirty; set { if (_protectDirty == value) return; _protectDirty = value; Changed(); } }
    ...
    {
        EnsureCurrent(); var document = CreateDocument("Untitled " + _nextId + ".txt", "");
        OpenDocument(document); return document;
    }
    private WorkspaceDocument CreateDocument(string name, string text)
    {
        WorkspaceDocument document = null!;
        document = new("mvvm-" + _nextId++, name, text,
            () => Observe(SaveAsync(document)), () => RevertDocument(document), () => CloseDocument(document));
        document.PropertyChanged += DocumentChanged; _files.Add(document); return document;
    }
    public void OpenDocument(WorkspaceDocument document)
    {
        ...
        if (!IsCurrent || !Documents.Contains(document)) return;
        document.IsOpen = true; SynchronizeBindings();
        var model = Model(document);
        if (model != null) model.IsActive = true;
        SetActive(document); Status = "Opened " + document.Name;
    }
    public void CloseDocument(WorkspaceDocument document)
    {
        EnsureOwned(document);
        if (document.IsSaving) { Status = "A save is in progress; the document stays open."; return; }
        Model(document)?.Close();
    }
    public void RevertDocument(WorkspaceDocument document)
    {
        EnsureOwned(document); if (!document.IsOpen || document.IsSaving) return;
        document.Revert(); if (IsCurrent) Status = "Reverted " + document.Name + " to its last saved buffer.";
    }
    internal async Task<bool> SaveAsync(WorkspaceDocument document)
    {
        EnsureOwned(document);
        if (!document.IsOpen || document.IsSaving) return false;
        var snapshot = document.Text;
        try
        {
            document.IsSaving = true;
            if (!IsCurrent || !Documents.Contains(document)) return false;
            ...
            if (IsCurrent) { Status = "Saved " + document.Name + (document.IsDirty ? "; newer edits remain unsaved." : "."); _log(Status); }
            return true;
        }
        finally { if (IsCurrent) document.IsSaving = false; }
    }
    internal async Task SaveAllAsync()
    {
        ...
        {
            if (!IsCurrent) break;
            if (Documents.Contains(document) && !document.IsSaving) await SaveAsync(document);
        }
    }
    private async void Observe(Task operation)
    {
        try { await operation; }
        catch (Exception error) { if (IsCurrent) { Status = "Operation failed: " + error.Message; _log(Status); } }
    }
    public string CaptureLayout()
    {
        EnsureCurrent(); using var text = new StringWriter(); new XmlLayoutSerializer(_dock).Serialize(text); return text.ToString();
    }
    public void RestoreLayout(string xml)
    {
        EnsureCurrent(); ArgumentNullException.ThrowIfNull(xml);
        if (_restoring) throw new InvalidOperationException("This workspace already has a layout restore in progress.");
        _restoring = true;
        try
        {
            using (var batch = _dock.BeginLayoutUpdate())
            {
                ...
                foreach (var document in Documents.Where(d => !wanted.Contains(d)).ToArray()) Documents.Remove(document);
                for (var i = 0; i < wanted.Length; i++)
                {
                    var index = Documents.IndexOf(wanted[i]);
                    if (index < 0) Documents.Insert(i, wanted[i]); else if (index != i) Documents.Move(index, i);
                }
                foreach (var document in _files) document.IsOpen = Documents.Contains(document);
            }
            if (IsCurrent) { ConnectRoot(); ActiveChanged(this, EventArgs.Empty); Status = "Layout restored by stable identity; unsaved buffers were preserved."; }
        }
    ...
    {
        if (!IsCurrent || e.Document.Content is not WorkspaceDocument document || !Documents.Contains(document)) return;
        if (document.IsSaving || (ProtectDirtyDocuments && document.IsDirty))
        { e.Cancel = true; Status = document.IsSaving ? "Save in progress; close cancelled." : "Unsaved changes: save or revert " + document.Name + " before closing."; }
    }
    private void DocumentClosed(object? sender, DocumentClosedEventArgs e)
    {
        if (!IsCurrent || e.Document.Content is not WorkspaceDocument document || !Documents.Contains(document)) return;
        Documents.Remove(document); document.IsOpen = false;
        if (!IsCurrent) return;
        ReleaseBinding(e.Document);
        if (ReferenceEquals(ActiveDocument, document)) SetActive(_dock.Layout.LastFocusedDocument?.Content as WorkspaceDocument ?? Documents.FirstOrDefault());
        Status = "Closed " + document.Name + "; its buffer remains in Workspace.";
    }
    private void LayoutChanged(object? sender, EventArgs e) { if (IsCurrent) ConnectRoot(); }
    private void ConnectRoot()
    {
        if (!IsCurrent) return;
        if (!ReferenceEquals(_root, _dock.Layout))
        {
            if (_root != null) _root.Updated -= RootUpdated;
            foreach (var model in _bindings.Keys.ToArray()) ReleaseBinding(model);
            _root = _dock.Layout; _root.Updated += RootUpdated;
        }
        SynchronizeBindings();
    }
    private void RootUpdated(object? sender, EventArgs e) { if (IsCurrent) SynchronizeBindings(); }
    private void SynchronizeBindings()
    {
        if (_synchronizing || !IsCurrent) return;
        _synchronizing = true;
        try
        {
            foreach (var stale in _bindings.Keys.Where(m => !ReferenceEquals(m.Root, _dock.Layout)).ToArray()) ReleaseBinding(stale);
            foreach (var model in _dock.Layout.Descendents().OfType<LayoutDocument>().ToArray())
            {
                if (!IsCurrent) break;
                if (_bindings.ContainsKey(model) || model.Content is not WorkspaceDocument document || !_files.Contains(document)) continue;
                var item = _dock.GetLayoutItemFromModel(model); var registrations = new List<BoundProperty>(); _bindings.Add(model, registrations);
                ...
                {
                    if (!IsCurrent || !ReferenceEquals(model.Root, _dock.Layout)) return;
                    var binding = new Binding { Source = document, Path = new(path), Mode = BindingMode.OneWay };
                    registrations.Add(new(item, property, binding)); item.SetBinding(property, binding);
                }
            }
        }
    ...
    {
        if (!_bindings.Remove(model, out var bindings)) return;
    ...
    {
        if (!IsCurrent) return;
        if (_dock.Layout.LastFocusedDocument?.Content is WorkspaceDocument document && Documents.Contains(document)) SetActive(document);
        else if (ActiveDocument != null && !Documents.Contains(ActiveDocument)) SetActive(Documents.FirstOrDefault());
    }
    ...
    private void EnsureOwned(WorkspaceDocument document) { EnsureCurrent(); ArgumentNullException.ThrowIfNull(document); if (!_files.Contains(document)) throw new ArgumentException("Document belongs to another workspace.", nameof(document)); }
    private void Changed([CallerMemberName] string? name = null) => PropertyChanged?.Invoke(this, new(name));

    private FrameworkElement BuildExplorer()
    {
        var grid = new Grid { Padding = new(6) }; grid.RowDefinitions.Add(new() { Height = GridLength.Auto }); grid.RowDefinitions.Add(new() { Height = new(1, GridUnitType.Star) });
        var tools = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 4 };
        tools.Children.Add(CommandButton("New", NewCommand, "MvvmNewDocument")); tools.Children.Add(CommandButton("Save all", SaveAllCommand, "MvvmSaveAll"));
        grid.Children.Add(tools);
        var list = new ListView { ItemsSource = Files, ItemTemplate = (DataTemplate)Application.Current.Resources["WorkspaceFileTemplate"], SelectionMode = ListViewSelectionMode.Single };
        list.ItemContainerStyle = new Style(typeof(ListViewItem)) { Setters = { new Setter(Control.MinHeightProperty, 26d), new Setter(Control.PaddingProperty, new Thickness(4, 1, 4, 1)) } };
        AutomationProperties.SetAutomationId(list, "MvvmFiles");
        list.DoubleTapped += (_, _) => { if (IsCurrent && list.SelectedItem is WorkspaceDocument document) OpenDocument(document); };
        list.KeyDown += (_, e) => { if (!e.Handled && e.Key == Windows.System.VirtualKey.Enter && IsCurrent && list.SelectedItem is WorkspaceDocument document) { OpenDocument(document); e.Handled = true; } };
        Grid.SetRow(list, 1); grid.Children.Add(list); return grid;
    }
    private FrameworkElement BuildProperties()
    {
        var stack = new StackPanel { Padding = new(9), Spacing = 9, DataContext = this };
        Text("ActiveDocument.Name", 13); Text("ActiveDocument.ContentId", 11);
        var readOnly = new CheckBox { Content = "Read-only editor", FontSize = 12, MinHeight = 26 };
        readOnly.SetBinding(CheckBox.IsCheckedProperty, new Binding { Path = new("ActiveDocument.IsReadOnly"), Mode = BindingMode.TwoWay }); stack.Children.Add(readOnly);
        var protect = new CheckBox { Content = "Protect unsaved buffers", FontSize = 12, MinHeight = 26 };
        protect.SetBinding(CheckBox.IsCheckedProperty, new Binding { Source = this, Path = new(nameof(ProtectDirtyDocuments)), Mode = BindingMode.TwoWay }); stack.Children.Add(protect);
        stack.Children.Add(new TextBlock { Text = "Characters / Lines", FontSize = 11 }); Text("ActiveDocument.CharacterCount", 12); Text("ActiveDocument.LineCount", 12);
        ...
        return new ScrollViewer { Content = stack, HorizontalScrollBarVisibility = ScrollBarVisibility.Disabled };
        void Text(string path, double size) { var text = new TextBlock { FontSize = size, TextWrapping = TextWrapping.Wrap }; text.SetBinding(TextBlock.TextProperty, new Binding { Path = new(path) }); stack.Children.Add(text); }
    }
    private FrameworkElement BuildOutput()
    {
        var text = new TextBlock { Margin = new(10), FontSize = 12, TextWrapping = TextWrapping.Wrap };
        text.SetBinding(TextBlock.TextProperty, new Binding { Source = this, Path = new(nameof(Status)) }); return text;
    }
    private static Button CommandButton(string text, System.Windows.Input.ICommand command, string id)
    {
        var button = new Button { Content = text, Command = command, FontSize = 12, MinHeight = 24, Padding = new(6, 2, 6, 2), HorizontalAlignment = HorizontalAlignment.Stretch };
        AutomationProperties.SetAutomationId(button, id); return button;
    }
    private static LayoutAnchorable Tool(WorkspaceTool tool) => new() { ContentId = tool.ContentId, Title = tool.Title, Content = tool, CanClose = false };
    public void Dispose()
    {
