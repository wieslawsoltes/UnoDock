...
public sealed partial class GalleryPage
{
    ...
    {
        var manager = new GalleryDockingManager { MinHeight = 420, DockingGuideMode = DockingGuideMode.GuidesOnly,
        ...
        var status = new TextBlock { Margin = new(10, 0, 10, 6), TextWrapping = TextWrapping.Wrap,
        ...
        var modes = new ComboBox { ItemsSource = Enum.GetValues<DockingGuideMode>(), SelectedItem = manager.DockingGuideMode, MinWidth = 160 };
        Microsoft.UI.Xaml.Automation.AutomationProperties.SetName(modes, "Docking guide input policy");
        modes.SelectionChanged += (_, _) => { if (modes.SelectedItem is DockingGuideMode mode) manager.DockingGuideMode = mode; };
        controls.Children.Add(modes);
        Add("Reset", Populate);
        Add("Large guides", () => { manager.Resources["UnoDock.GuideSize"] = 44d; manager.Refresh(); });
        Add("Stock size", () => { manager.Resources.Remove("UnoDock.GuideSize"); manager.Refresh(); });
        ...
        {
            foreach (var tool in manager.Layout.Descendents().OfType<LayoutAnchorable>()) tool.CanDockAsTabbedDocument = !tool.CanDockAsTabbedDocument;
        ...
        var veto = new CheckBox { Content = "Veto drops" }; controls.Children.Add(veto);
        ...
        panel.Children.Add(bar); Grid.SetRow(status, 1); panel.Children.Add(status); Grid.SetRow(manager, 2); panel.Children.Add(manager);
        Populate();
        var document = new LayoutDocument { Title = "Docking guides", ContentId = "docking-guides-lab:" + Guid.NewGuid().ToString("N"), Content = panel };
        ...
        void Populate()
        {
            ...
            foreach (var title in new[] { "Workspace.cs", "Readme.md", "Notes.txt" })
                documents.Children.Add(new LayoutDocument { Title = title, ContentId = "guide-doc:" + title,
            ...
            var tools = new LayoutAnchorablePane { DockWidth = new(210) };
            foreach (var title in new[] { "Solution Explorer", "Toolbox" })
                tools.Children.Add(new() { Title = title, ContentId = "guide-tool:" + title, Content = new TextBox { Text = title, AcceptsReturn = true } });
            var properties = new LayoutAnchorablePane(new LayoutAnchorable { Title = "Properties", ContentId = "guide-properties", Content = new TextBox { Text = "CanDockAsTabbedDocument = true" } }) { DockWidth = new(200) };
            var docGroup = new LayoutDocumentPaneGroup(documents);
            var workspace = new LayoutPanel(tools); workspace.Children.Add(docGroup); workspace.Children.Add(properties);
            manager.Layout = new() { RootPanel = workspace }; documents.Children[0].IsActive = true;
            manager.Refresh();
        }
        void Add(string text, Action action)
        { var button = new Button { Content = text, FontSize = 12, Padding = new(8, 4, 8, 4) }; button.Click += (_, _) => action(); controls.Children.Add(button); }
    }
}
