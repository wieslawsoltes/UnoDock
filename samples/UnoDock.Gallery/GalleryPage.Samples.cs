...
public sealed partial class GalleryPage : IDisposable
{
    ...
    private void BuildSampleShell()
    {
        ...
        AddMenu("Layout", ("Float / Dock", "float", ToggleFloating), ("Auto-hide / Pin", "pin", TogglePin),
            ("New vertical tab group", "split-right", () => Split(UnoDock.Core.DockPosition.Right)),
            ("New horizontal tab group", "split-bottom", () => Split(UnoDock.Core.DockPosition.Bottom)),
            ("Show hidden tools", "show-tools", ShowHiddenTools), ("Reset current sample", "reset", () => SwitchSample(CurrentSample)));
        AddMenu("Samples", ("Docking", "classic", () => SwitchSample(SampleKind.Classic)),
            ("IDE workspace", "workspace", () => SwitchSample(SampleKind.Workspace)),
            ("MVVM binding", "binding", () => SwitchSample(SampleKind.Binding)), ("Add 1,000 tabs", "stress", Stress));
        AddMenu("Diagnostics", ("Public contracts", "parity", ShowParityLab), ("Converters", "converters", ShowConverterLab),
            ("Native windows", "windows", ShowNativeWindowLab), ("Window shell", "shell", ShowShellLab),
            ("Window lifecycle", "lifecycle", ShowWindowLifecycleLab), ("Input extensions", "input", ShowInputExtensionsLab),
            ("Visual observations", "visual", ShowVisualParityLab), ("Navigator", "navigator", ShowNavigatorLab),
            ("Docking guides", "guides", ShowDockingGuidesLab), ("Splitters", "splitters", ShowSplitterLab),
            ("Auto-hide", "autohide", ShowAutoHideLab), ("Menus", "menus", ShowMenuLab));
        AddMenu("View", ("Left-to-right", "ltr", () => Dock.FlowDirection = FlowDirection.LeftToRight),
            ("Right-to-left", "rtl", () => Dock.FlowDirection = FlowDirection.RightToLeft),
            ("Normal density", "normal-density", () => SetDensity(12)), ("Large text", "large-text", () => SetDensity(18)));
        _sampleShell.Children.Add(menu);

        var toolbar = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 7, Margin = new(6, 3, 6, 3) };
        toolbar.Children.Add(Label("Sample:"));
        _samplePicker = Picker("SampleSelector", new[] { "Docking", "IDE workspace", "MVVM binding" }, 158);
        _samplePicker.SelectionChanged += (_, _) => { if (!_selectingSample && _samplePicker.SelectedIndex >= 0) SwitchSample((SampleKind)_samplePicker.SelectedIndex); };
        ...
        reset.Padding = new(8, 1, 8, 1); reset.MinHeight = 24; toolbar.Children.Add(reset);
        var tools = new ScrollViewer { Content = toolbar, HorizontalScrollMode = ScrollMode.Enabled,
            HorizontalScrollBarVisibility = ScrollBarVisibility.Auto, VerticalScrollBarVisibility = ScrollBarVisibility.Disabled };
        Grid.SetRow(tools, 1); _sampleShell.Children.Add(tools);
        Dock.Margin = new(2, 0, 2, 0); Dock.FontSize = 12;
        Grid.SetRow(Dock, 2); _sampleShell.Children.Add(Dock);
        _status.FontSize = 11; _status.Margin = new(7, 3, 7, 0); _status.TextTrimming = TextTrimming.CharacterEllipsis;
        Grid.SetRow(_status, 3); _sampleShell.Children.Add(_status);
        ...
        void AddMenu(string title, params (string Label, string Id, Action Invoke)[] commands)
        {
            ...
            foreach (var command in commands)
            {
                ...
                var item = new MenuFlyoutItem { Text = command.Label };
        ...
        void Tool(string title, string command, string data)
        {
            ...
            button.Content = icon; button.Width = 26; button.Height = 24; button.Configure(SampleChrome.Default(false));
            AutomationProperties.SetAutomationId(button, "SampleToolbar-" + command); toolbar.Children.Add(button);
        }
    ...
    internal void ExecuteSampleCommand(string id)
    {
        ...
        if (!_sampleCommands.TryGetValue(id, out var action)) throw new ArgumentException("Unknown sample command.", nameof(id));
        try { action(); } catch (Exception error) { Log(error.Message); _status.Text = error.Message; }
    }
    internal void SetSampleTheme(SampleTheme theme)
    {
        ...
        if (_sampleShell != null)
        {
            ...
            foreach (var button in _sampleShell.FindVisualChildren<SampleButton>()) button.Configure(SampleChrome.Default(dark));
        }
    ...
    {
        if (Dock.Layout.ActiveContent is not { } active) return;
        if (active.IsFloating) active.Dock(); else active.Float();
    }
    private void TogglePin()
    {
        if (Dock.Layout.ActiveContent is LayoutAnchorable tool) tool.ToggleAutoHide();
        else _status.Text = "Select a tool window before changing auto-hide.";
    }
    private void ShowHiddenTools()
    {
        foreach (var tool in Dock.Layout.Hidden.ToArray()) tool.Show();
    }
    ...
    internal void SwitchSample(SampleKind sample)
    {
        ...
        try
        {
            ...
            Dock.DocumentsSource = null; Dock.AnchorablesSource = null;
            Dock.LayoutItemTemplate = null; Dock.LayoutItemContainerStyle = null;
            _content.Clear(); CurrentSample = sample;
            switch (sample)
            {
                case SampleKind.Classic: BuildClassicSample(); break;
                case SampleKind.Workspace: ResetWorkbench(); break;
    ...
    private void BuildClassicSample()
    {
        ...
        properties.CanHide = false; properties.CanClose = false; properties.AutoHideWidth = 240;
        var contentButton = new Button { Content = "Document 1 Content", HorizontalAlignment = HorizontalAlignment.Center,
            VerticalAlignment = VerticalAlignment.Center, MinHeight = 0, Padding = new(8, 3, 8, 3), FontSize = 12 };
        ...
        var documents = new LayoutDocumentPane(first); documents.Children.Add(second);
        var alarms = new ListBox { ItemsSource = new[] { "Alarm 1", "Alarm 2", "Alarm 3" }, FontSize = 12, BorderThickness = new(0) };
        ...
        var right = new LayoutAnchorablePane(alarmModel); right.Children.Add(journal);
        var rightGroup = new LayoutAnchorablePaneGroup { DockWidth = new(125), DockMinWidth = 100 }; rightGroup.Children.Add(right);
        var documentGroup = new LayoutDocumentPaneGroup(); documentGroup.Children.Add(documents);
        var panel = new LayoutPanel(new LayoutAnchorablePane(properties) { DockWidth = new(200), DockMinWidth = 150 });
        panel.Orientation = Orientation.Horizontal; panel.Children.Add(documentGroup); panel.Children.Add(rightGroup);
        ...
        Dock.Layout = root; first.IsActive = true;
    }
    private LayoutAnchorable ToolModel(string id, string title, object content)
    {
        _content[id] = content;
        return new() { ContentId = id, Title = title, Content = content };
    }
    internal string SerializeSample()
    {
        using var writer = new StringWriter(); new XmlLayoutSerializer(Dock).Serialize(writer); return writer.ToString();
    }
    internal void RestoreSample(string xml)
    {
        ...
        {
            if (e.Model.ContentId is { } id && _content.TryGetValue(id, out var content)) e.Content = content;
        };
