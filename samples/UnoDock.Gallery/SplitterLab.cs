...
public sealed partial class GalleryPage
{
    private void ShowSplitterLab()
    {
        ...
        container.Children.Add(bar); Grid.SetRow(status, 1); container.Children.Add(status); Grid.SetRow(manager, 3); container.Children.Add(manager);
        var numeric = new TextBox { Text = "220", Width = 90, MinHeight = 0, Height = 26, Padding = new(4, 1, 4, 1), FontSize = 12 };
        AutomationProperties.SetName(numeric, "Leading pane size in DIPs");
        var accessible = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 6, Margin = new(6, 0, 6, 4) };
        ...
        ActionButton("Apply size", () =>
        {
            ...
            Range().SetValue(value); manager.Refresh(); manager.UpdateLayout(); ReportRange();
        });
        ActionButton("Minimum", () => { var value = Range(); value.SetValue(value.Minimum); manager.Refresh(); manager.UpdateLayout(); ReportRange(); });
        ActionButton("Maximum", () => { var value = Range(); value.SetValue(value.Maximum); manager.Refresh(); manager.UpdateLayout(); ReportRange(); });
        ActionButton("Focus divider", () => { Peer().SetFocus(); status.Text = "Keyboard: arrows move 10 DIPs; Page Up/Down move 50; Home/End use limits. Escape cancels pointer resizing."; });
        var accessibilityBar = new ScrollViewer { Content = accessible, HorizontalScrollBarVisibility = ScrollBarVisibility.Auto, VerticalScrollBarVisibility = ScrollBarVisibility.Disabled };
        Grid.SetRow(accessibilityBar, 2); container.Children.Add(accessibilityBar);
        LayoutGridResizerAutomationPeer Peer()
        {
            ...
            return FrameworkElementAutomationPeer.CreatePeerForElement(splitter) as LayoutGridResizerAutomationPeer
                ?? throw new InvalidOperationException("The divider automation peer is unavailable.");
        }
        IRangeValueProvider Range() => Peer().GetPattern(PatternInterface.RangeValue) as IRangeValueProvider
            ?? throw new InvalidOperationException("The divider has no range provider.");
        void ReportRange()
        {
            var value = Range(); numeric.Text = value.Value.ToString("R", CultureInfo.InvariantCulture);
        ...
        {
            var button = SampleChrome.Button(text, () => { try { action(); } catch (Exception error) { status.Text = error.Message; } });
            button.Padding = new(6, 2, 6, 2); button.Height = 26; accessible.Children.Add(button);
        }
        Populate(false);
        ...
        { var button = SampleChrome.Button(text, action); button.Padding = new(6, 2, 6, 2); button.Height = 26; controls.Children.Add(button); }
    }
}
