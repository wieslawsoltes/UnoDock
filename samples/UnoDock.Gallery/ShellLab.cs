...
public sealed partial class GalleryPage
{
    ...
    {
        var stack = new StackPanel { Spacing = 12, Padding = new(24) };
        ...
        var info = new TextBlock { TextWrapping = TextWrapping.Wrap, FontFamily = new FontFamily("Consolas") };
        var metrics = new TextBlock { TextWrapping = TextWrapping.Wrap };
        var chrome = new WindowChrome { CaptionHeight = 38, ResizeBorderThickness = new(8), CornerRadius = new(8) };
        ...
        void Apply()
        {
            var host = Host(); if (host == null) { Describe(); return; }
            if (host.NativeWindow is { } native) WindowChrome.SetWindowChrome(native, chrome);
            else WindowChrome.SetWindowChrome(host, chrome);
            DispatcherQueue.TryEnqueue(Describe);
        }
        void Create(bool native)
        {
            if (model?.Root != null) { model.CanClose = true; model.Close(); }
            ...
            model.FloatingWidth = 620; model.FloatingHeight = 400; model.FloatingLeft = 80; model.FloatingTop = 90;
            Dock.Layout.Descendents().OfType<LayoutDocumentPane>().First().Children.Add(model);
            model.Float(); Dock.Refresh(); DispatcherQueue.TryEnqueue(Apply);
        }
        var actions = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 6 };
        Add("Create in-surface", () => Create(false)); Add("Create native", () => Create(true));
        Add("Apply chrome", Apply);
        Add("Detach chrome", () =>
        {
            if (Host() is { } host) { WindowChrome.SetWindowChrome(host, null); if (host.NativeWindow is { } native) WindowChrome.SetWindowChrome(native, null); }
            Describe();
        });
        ...
        foreach (var (label, command) in new[] { ("Maximize", SystemCommands.MaximizeWindowCommand), ("Minimize", SystemCommands.MinimizeWindowCommand), ("Restore", SystemCommands.RestoreWindowCommand), ("Close", SystemCommands.CloseWindowCommand) })
        {
            var button = new Button { Content = label };
            button.Click += (_, _) => { if (Host() is { } host) command.Execute(null, host); Describe(); };
            commands.Children.Add(button);
        }
        var menu = new Button { Content = "System menu" }; commands.Children.Add(menu);
        menu.Click += (_, _) => { if (Host() is { } host) SystemCommands.CreateSystemMenu(host).ShowAt(menu); };
        ...
        Toggle("Protect the experiment from closing", false, v => { protectClosing = v; if (model != null) model.CanClose = !v; Describe(); });
        stack.Children.Add(info); stack.Children.Add(metrics);
        var timer = new DispatcherTimer { Interval = TimeSpan.FromMilliseconds(350) };
        timer.Tick += (_, _) => Describe();
        stack.Loaded += (_, _) => { Describe(); timer.Start(); }; stack.Unloaded += (_, _) => timer.Stop();
        var document = Document("shell-lab-" + _nextDocument++, "Window shell", new ScrollViewer { Content = stack });
        Dock.Layout.Descendents().OfType<LayoutDocumentPane>().First().Children.Add(document); document.IsActive = true;
        void Add(string label, Action execute)
        {
            var button = new Button { Content = label };
            button.Click += (_, _) => { try { execute(); } catch (Exception e) { Log(e.Message); } }; actions.Children.Add(button);
        }
        void Slider(string label, double min, double max, double initial, Action<double> change)
        {
            var control = new Slider { Header = label, Minimum = min, Maximum = max, Value = initial, Width = 460, HorizontalAlignment = HorizontalAlignment.Left };
            control.ValueChanged += (_, e) => { change(e.NewValue); DispatcherQueue.TryEnqueue(Describe); }; stack.Children.Add(control);
        }
        void Toggle(string label, bool initial, Action<bool> change)
        {
            var control = new CheckBox { Content = label, IsChecked = initial };
            control.Checked += (_, _) => change(true); control.Unchecked += (_, _) => change(false); stack.Children.Add(control);
        }
    }
}
