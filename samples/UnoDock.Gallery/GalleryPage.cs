...
public sealed partial class GalleryPage : Page
{
    ...
    public GalleryPage()
    {
        ...
        Dock.DocumentClosing += (_, e) =>
        {
            if (_protectDraft && e.Document.ContentId == "draft")
            {
                e.Cancel = true;
                Log("Draft close cancelled. Disable protection in Properties to close it.");
            }
        };
    ...
    private async Task RunCore(Func<Task> operation)
    {
        try
        {
            await operation();
    ...
    {
        _events.Insert(0, $"{DateTime.Now:HH:mm:ss}  {value}");
        while (_events.Count > 250)
    ...
    private void ResetWorkbench()
    {
        ...
        var inspector = new LayoutAnchorable
        {
        ...
        var left = new LayoutAnchorablePane(explorer)
        {
            DockWidth = new(200),
            DockMinWidth = 140
        };
        var right = new LayoutAnchorablePane(properties)
        {
            DockWidth = new(230),
            DockMinWidth = 180
        };
        ...
        bottom.Children.Add(inspector);
        var center = new LayoutPanel(docs)
        {
    ...
    private UIElement Welcome()
    {
        ...
        {
            "01  Drag a tab to a pane edge to split, or into its center to join a tab group.",
            "02  Float a document, resize the window, and dock it back without losing the editor instance.",
            "03  Pin or auto-hide Explorer, Properties and Output. Hover the side buttons to reveal them.",
            "04  Save your layout, rearrange the workspace, then restore it by content ID.",
            "05  Open MVVM mode for observable collections, property bindings and data templates.",
            "06  Use Ctrl+Tab for the MRU navigator, Ctrl+F4 to close, and arrow keys on focused splitters.",
            "07  Use the Properties tool to test close cancellation, immutable content and capability flags."
        }

        )
            stack.Children.Add(new TextBlock { Text = text, TextWrapping = TextWrapping.Wrap, FontSize = 14 });
        var note = new InfoBar
        {
            ...
            Message = "This is an independently implemented preview, not a WPF binary replacement. See the repository's API report and platform validation matrix."
        };
    ...
    private UIElement Explorer()
    {
        ...
        list.DoubleTapped += (_, _) =>
        {
            ...
            {
                var doc = Dock.Layout.Descendents().OfType<LayoutDocument>().FirstOrDefault(d => text.Trim().Equals(d.Title, StringComparison.Ordinal));
                if (doc != null)
                    doc.IsActive = true;
            }
        };
    ...
    private UIElement Properties()
    {
        ...
        Check("Use in-surface floating windows", OperatingSystem.IsBrowser(), v =>
        {
            Dock.FloatingWindowMode = v ? FloatingWindowMode.InSurface : FloatingWindowMode.Auto;
            Log("Floating host mode applies to newly created windows.");
        });
        Check("Active content may float", true, v =>
        {
            if (Dock.Layout.ActiveContent is { } c)
                c.CanFloat = v;
        });
        Check("Active content may close", true, v =>
        {
            if (Dock.Layout.ActiveContent is { } c)
                c.CanClose = v;
        });
        var delay = new Slider
        {
            Minimum = 100,
            Maximum = 2000,
            Value = Dock.AutoHideWindowClosingTimer,
            Header = "Auto-hide close delay (ms)"
        };
        delay.ValueChanged += (_, e) => Dock.AutoHideWindowClosingTimer = (int)e.NewValue;
        stack.Children.Add(delay);
        var thickness = new Slider
        {
            Minimum = 2,
            Maximum = 14,
            Value = 6,
            Header = "Splitter size"
        };
        thickness.ValueChanged += (_, e) =>
        {
            Dock.GridSplitterWidth = e.NewValue;
            Dock.GridSplitterHeight = e.NewValue;
        };
        stack.Children.Add(thickness);
        var run = new Button
        {
            Content = "Run runtime checks",
            HorizontalAlignment = HorizontalAlignment.Stretch
        };
        run.Click += async (_, _) =>
        {
            ...
            try
            {
                var result = await Testing.RuntimeTests.Run(Dock, Path.Combine(ApplicationData.Current.LocalFolder.Path, "test-results"));
                Log("Runtime tests finished with exit code " + result);
            }
            catch (Exception e)
            {
                Log(e.Message);
        ...
        void Check(string text, bool initial, Action<bool> changed)
        {
    ...
    private void AddDocument()
    {
        ...
        {
            _mvvmWorkspace.NewDocument();
            return;
        }

        if (Dock.DocumentsSource != null)
        {
            _notes.Add(new("note-" + _nextDocument, "Note " + _nextDocument++, "An observable source insertion."));
            return;
        }

        var number = _nextDocument++;
        var document = Document("document-" + number, $"Document {number}.txt", Editor("Start writing here…"));
        var pane = Dock.Layout.LastFocusedDocument?.Parent as LayoutDocumentPane ?? Dock.Layout.Descendents().OfType<LayoutDocumentPane>().First();
    ...
    {
        if (Dock.Layout.ActiveContent is { Parent: ILayoutGroup group } content)
            DockOperations.Dock(content, group, position);
    }
    ...
    private void ShowXml()
    {
        if (_mvvmWorkspace?.IsCurrent == true)
        {
            ...
            var document = _mvvmWorkspace.NewDocument();
            document.Name = "Layout.xml";
            document.Text = xml;
            return;
        }
        ...
        Dock.Layout.Descendents().OfType<LayoutDocumentPane>().First().Children.Add(doc);
        doc.IsActive = true;
    }
    ...
    private void Stress()
    {
        if (_mvvmWorkspace?.IsCurrent == true)
        {
            _mvvmWorkspace.AddDocuments(1000);
            return;
        }

        using var batch = Dock.BeginLayoutUpdate();
        using var tree = Dock.Layout.BeginUpdate();
        var pane = Dock.Layout.Descendents().OfType<LayoutDocumentPane>().First();
        for (var i = 0; i < 1000; i++)
            pane.Children.Add(Document("stress-" + _nextDocument, "Tab " + _nextDocument++, "Lazy document content " + i));
