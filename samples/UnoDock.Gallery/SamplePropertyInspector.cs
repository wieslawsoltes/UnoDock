...
{
    private enum EditorKind { Text, Boolean, Choice, Color }
    private sealed record Field(string Category, string Name, string Description, Func<string> Read,
        Action<string>? Write = null, EditorKind Kind = EditorKind.Text, string[]? Choices = null);
    ...
    private readonly TextBox _search = new() { PlaceholderText = "Search properties", MinHeight = 0, Height = 25, FontSize = 12, Padding = new(4, 1, 4, 1) };
    private readonly StackPanel _rows = new();
    private readonly Grid _columns = new();
    private readonly TextBlock _descriptionTitle = new() { FontSize = 12, FontWeight = Microsoft.UI.Text.FontWeights.SemiBold };
    private readonly TextBlock _description = new() { FontSize = 11, TextWrapping = TextWrapping.Wrap, MaxLines = 3 };
    ...
    internal double NameColumnWidth
    {
        ...
        set
        {
            if (!double.IsFinite(value) || value < 64 || value > 400) throw new ArgumentOutOfRangeException(nameof(value));
            _nameWidth = value; ApplyColumnWidth();
        }
    }
    ...
    internal SamplePropertyInspector(DockingManager manager)
    {
        ...
        foreach (var height in new[] { GridLength.Auto, GridLength.Auto, GridLength.Auto, GridLength.Auto,
            new GridLength(1, GridUnitType.Star), GridLength.Auto }) host.RowDefinitions.Add(new() { Height = height });
        ...
        var search = new Grid { Margin = new(4, 2, 4, 4) };
        search.ColumnDefinitions.Add(new() { Width = new(1, GridUnitType.Star) }); search.ColumnDefinitions.Add(new() { Width = new(23) });
        search.Children.Add(_search);
        var clear = SampleChrome.Button("×", () => Filter(""), "Clear property search"); Grid.SetColumn(clear, 1); search.Children.Add(clear);
        Grid.SetRow(search, 2); host.Children.Add(search);
        _columns.ColumnDefinitions.Add(new() { Width = new(_nameWidth) }); _columns.ColumnDefinitions.Add(new() { Width = new(1, GridUnitType.Star) });
        _columns.Children.Add(new TextBlock { Text = "Property", FontSize = 11, Margin = new(4, 3, 3, 3) });
        var valueLabel = new TextBlock { Text = "Value", FontSize = 11, Margin = new(4, 3, 3, 3) }; Grid.SetColumn(valueLabel, 1); _columns.Children.Add(valueLabel);
        var divider = new Thumb { Width = 5, MinHeight = 0, HorizontalAlignment = HorizontalAlignment.Right, IsTabStop = true,
        ...
        divider.KeyDown += (_, e) =>
        {
            ...
            { ResizeColumn((e.Key == VirtualKey.Right ? 8 : -8) * (FlowDirection == FlowDirection.RightToLeft ? -1 : 1)); e.Handled = true; }
            else if (e.Key == VirtualKey.Home) { NameColumnWidth = 96; e.Handled = true; }
        };
        divider.DoubleTapped += (_, _) => NameColumnWidth = 96;
        _columns.Children.Add(divider); Grid.SetRow(_columns, 3); host.Children.Add(_columns);
        var scroll = new ScrollViewer { Content = _rows, HorizontalScrollBarVisibility = ScrollBarVisibility.Disabled };
        Grid.SetRow(scroll, 4); host.Children.Add(scroll);
        var help = new StackPanel { Padding = new(5), Spacing = 3, MinHeight = 66 };
        help.Children.Add(_descriptionTitle); help.Children.Add(_description);
        var helpBorder = new Border { Child = help, BorderThickness = new(0, 1, 0, 0), BorderBrush = SampleChrome.Color(0xa0a0a0) };
        Grid.SetRow(helpBorder, 5); host.Children.Add(helpBorder); Content = host;
        AutomationProperties.SetAutomationId(_search, "PropertySearch"); AutomationProperties.SetName(this, "Document properties");
        _search.TextChanged += (_, _) => ApplyFilter();
        Loaded += OnLoaded; Unloaded += OnUnloaded; ActualThemeChanged += OnThemeChanged;
    ...
    private void OnLoaded(object sender, RoutedEventArgs args)
    {
        ...
        AttachRoot(); SelectDocument(); Paint();
    }
    ...
    {
        if (!_attached || ReferenceEquals(_root, _manager.Layout)) return;
    ...
    {
        if (ReferenceEquals(sender, _root) && args.PropertyName is nameof(LayoutRoot.LastFocusedDocument) or nameof(LayoutRoot.ActiveContent)) SelectDocument();
    }
    private void OnDocumentChanged(object? sender, PropertyChangedEventArgs args)
    {
        if (!ReferenceEquals(sender, _document)) return;
        if (args.PropertyName == nameof(LayoutContent.Content)) SelectDocument();
        else if (args.PropertyName == nameof(LayoutElement.Parent))
        {
            ...
            var epoch = _epoch;
            DispatcherQueue.TryEnqueue(() => { if (epoch == _epoch) SelectDocument(); });
        }
        else RefreshValues();
    }
    private void SelectDocument()
    {
        ...
        if (ReferenceEquals(document, _document) && ReferenceEquals(document?.Content, _content)) { RefreshValues(); return; }
        ClearSelection(); _document = document; _content = document?.Content;
        if (document == null) { _heading.Text = "No document selected"; return; }
        document.PropertyChanged += OnDocumentChanged;
        foreach (var field in CreateFields(document)) AddRow(field);
        Reorder(); RefreshValues(); Paint();
    }
    private IEnumerable<Field> CreateFields(LayoutContent document)
    {
        ...
        if (document is LayoutDocument movable) yield return Bool("CanMove", "Allow the document to move between panes.", () => movable.CanMove, value => movable.CanMove = value);
        ...
        if (_content is not FrameworkElement element) yield break;
        ...
        Observe(element, FrameworkElement.HorizontalAlignmentProperty); Observe(element, FrameworkElement.VerticalAlignmentProperty); Observe(element, FrameworkElement.MarginProperty);
        if (element is Control control)
        {
            ...
            Observe(control, Control.FontSizeProperty); Observe(control, Control.BackgroundProperty); Observe(control, Control.ForegroundProperty); Observe(control, Control.PaddingProperty);
        }
        if (element is TextBox editor)
        {
            ...
            Observe(editor, TextBox.IsReadOnlyProperty); Observe(editor, TextBox.AcceptsReturnProperty); Observe(editor, TextBox.TextWrappingProperty);
        }
    }
    private static Field Bool(string name, string description, Func<bool> read, Action<bool> write) =>
        new("Behavior", name, description, () => read().ToString(), value => write(ParseBoolean(value)), EditorKind.Boolean);
    private static Field Choice<T>(string category, string name, string description, Func<T> read, Action<T> write) where T : struct, Enum =>
        new(category, name, description, () => read().ToString(), value =>
        {
    ...
    {
        var epoch = _epoch;
        var token = value.RegisterPropertyChangedCallback(property, (_, _) => { if (epoch == _epoch) RefreshValues(); });
        _tokens.Add((value, property, token));
    }
    private void AddRow(Field field)
    {
        var grid = new Grid { MinHeight = 24, BorderThickness = new(0, 0, 0, 1) };
        grid.ColumnDefinitions.Add(new() { Width = new(_nameWidth) }); grid.ColumnDefinitions.Add(new() { Width = new(1, GridUnitType.Star) });
        grid.RowDefinitions.Add(new() { Height = GridLength.Auto }); grid.RowDefinitions.Add(new() { Height = GridLength.Auto });
        var label = new TextBlock { Text = field.Name, FontSize = 12, Margin = new(4, 4, 3, 2), TextTrimming = TextTrimming.CharacterEllipsis };
        ToolTipService.SetToolTip(label, field.Description); grid.Children.Add(label);
        var error = new TextBlock { FontSize = 11, TextWrapping = TextWrapping.Wrap, Margin = new(4, 0, 4, 3), Visibility = Visibility.Collapsed };
        var row = new Row(field, _epoch, grid, label, error); _entries.Add(row);
        FrameworkElement input;
        if (field.Write == null) input = row.ReadOnly = new TextBlock { FontSize = 12, Margin = new(3, 4, 2, 2), TextTrimming = TextTrimming.CharacterEllipsis };
        else if (field.Kind == EditorKind.Boolean)
        {
            var check = row.Boolean = new CheckBox { MinHeight = 0, MinWidth = 0, Height = 24, Padding = new(2, 0, 0, 0), HorizontalAlignment = HorizontalAlignment.Left };
            check.Checked += (_, _) => Commit(row, "True"); check.Unchecked += (_, _) => Commit(row, "False"); input = check;
        }
        else if (field.Kind == EditorKind.Choice)
        {
            var combo = row.Choice = new ComboBox { ItemsSource = field.Choices, MinHeight = 0, MinWidth = 0, Height = 24, FontSize = 12, Padding = new(3, 0, 0, 0), HorizontalAlignment = HorizontalAlignment.Stretch };
            combo.SelectionChanged += (_, _) => { if (combo.SelectedItem is string choice) Commit(row, choice); }; input = combo;
        }
        else
        {
            var text = row.Text = new TextBox { MinHeight = 0, MinWidth = 0, FontSize = 12, Padding = new(3, 1, 3, 1), BorderThickness = new(0), CornerRadius = new(0) };
            text.LostFocus += (_, _) => { if (HasDraft(row) && _synchronizing == 0) Commit(row, text.Text, true); };
            text.KeyDown += (_, args) =>
            {
                if (args.Key == VirtualKey.Enter) { if (HasDraft(row)) Commit(row, text.Text, true); args.Handled = true; }
                else if (args.Key == VirtualKey.Escape) { Cancel(row); args.Handled = true; }
            };
            if (field.Kind == EditorKind.Color)
            {
                var holder = new Grid(); holder.ColumnDefinitions.Add(new() { Width = new(17) }); holder.ColumnDefinitions.Add(new() { Width = new(1, GridUnitType.Star) });
                row.Swatch = new Border { Width = 12, Height = 12, BorderThickness = new(1), BorderBrush = SampleChrome.Color(0x808080), VerticalAlignment = VerticalAlignment.Center };
                holder.Children.Add(row.Swatch); Grid.SetColumn(text, 1); holder.Children.Add(text); input = holder;
            }
            else input = text;
        }
        var focusTarget = (FrameworkElement?)row.Text ?? row.Boolean ?? (FrameworkElement?)row.Choice ?? input;
        AutomationProperties.SetName(focusTarget, field.Name); AutomationProperties.SetHelpText(focusTarget, field.Description);
        AutomationProperties.SetAutomationId(focusTarget, "Property-" + field.Name);
        focusTarget.GotFocus += (_, _) => { if (IsCurrent(row)) ShowDescription(row); };
        label.PointerPressed += (_, _) => { if (IsCurrent(row)) ShowDescription(row); };
    ...
    private bool IsCurrent(Row row) => !_disposed && _attached && row.Epoch == _epoch && _entries.Contains(row) &&
        ReferenceEquals(_root, _manager.Layout) && ReferenceEquals(_document, CurrentDocument()) &&
        ReferenceEquals(_document?.Root, _root) && ReferenceEquals(_document?.Content, _content);
    ...
    {
        if (_synchronizing != 0 || _writing != 0 || row.Field.Write == null || !IsCurrent(row)) return false;
        ShowDescription(row);
        try
        {
            ...
            {
                _writing++;
                try { row.Field.Write(value); }
                finally { _writing--; }
            }
            if (!IsCurrent(row)) return false; // Application callbacks own a replacement selection.
            LastError = null; row.Error.Visibility = Visibility.Collapsed;
            Synchronize(row, discardDraft: true); return true;
        }
        ...
        finally { if (_refreshPending) { _refreshPending = false; RefreshValues(); } }
    }
    private void Cancel(Row row)
    {
        if (!IsCurrent(row)) return;
        row.Error.Visibility = Visibility.Collapsed; LastError = null; Synchronize(row, discardDraft: true);
    }
    ...
    private void Synchronize(Row row, bool discardDraft = false)
    {
        ...
        if (!IsCurrent(row) || (!discardDraft && HasDraft(row))) return;
        _synchronizing++;
        try
        {
            var value = row.Field.Read(); row.Displayed = value;
            if (row.Text != null && row.Text.Text != value) row.Text.Text = value;
            if (row.Boolean != null) row.Boolean.IsChecked = ParseBoolean(value);
            if (row.Choice != null) row.Choice.SelectedItem = value;
            if (row.ReadOnly != null) row.ReadOnly.Text = value;
            if (row.Swatch != null) row.Swatch.Background = value.StartsWith('#') ? ParseColor(value) : null;
        }
    ...
    {
        if (_disposed || !_attached) return;
        if (_writing != 0 || _synchronizing != 0) { _refreshPending = true; return; }
        _heading.Text = _content is { } content ? $"{content.GetType().Name}  {_document?.Title}" : "No document selected";
        foreach (var row in _entries.ToArray()) Synchronize(row);
    }
    internal void Filter(string text) { _search.Text = text; ApplyFilter(); }
    internal void SetOrder(bool alphabetical) { _alphabetical = alphabetical; Reorder(); }
        ...
        try
        {
            ...
            foreach (var row in ordered)
            {
                if (!_alphabetical && !_groups.ContainsKey(row.Field.Category))
                {
                    ...
                    button.HorizontalContentAlignment = HorizontalAlignment.Left; button.Padding = new(4, 1, 3, 1);
                    var header = new Border { Child = button, MinHeight = 23 };
                    _groups.Add(name, new(header, button, name)); _rows.Children.Add(header);
                }
                _rows.Children.Add(row.View);
            }
            ApplyFilter(); ApplyColumnWidth(); Paint();
        }
        finally { _synchronizing--; }
    }
    ...
    {
        if (double.IsFinite(delta)) NameColumnWidth = Math.Clamp(_nameWidth + delta, 64, Math.Clamp(ActualWidth - 56, 64, 400));
    }
    private void ApplyColumnWidth()
    {
        ...
        foreach (var row in _entries) row.View.ColumnDefinitions[0].Width = new(width);
    }
    ...
    private void Paint()
    {
        ...
        foreach (var row in _entries)
        {
            row.View.BorderBrush = line; row.Label.Foreground = foreground;
            if (row.ReadOnly != null) row.ReadOnly.Foreground = foreground;
            row.Error.Foreground = SampleChrome.Color(dark ? 0xff9999u : 0xb42318u);
        }
        foreach (var group in _groups.Values) group.View.Background = category;
        foreach (var button in this.FindVisualChildren<SampleButton>()) button.Configure(SampleChrome.Default(dark));
    }
    private void ClearSelection()
    {
        _epoch++; _synchronizing++;
        try
        {
            if (_document != null) _document.PropertyChanged -= OnDocumentChanged;
            _document = null; _content = null;
            foreach (var (value, property, token) in _tokens) value.UnregisterPropertyChangedCallback(property, token);
            _tokens.Clear(); _entries.Clear(); _rows.Children.Clear(); _groups.Clear(); LastError = null; ShowDescription(null);
        }
        finally { _synchronizing--; }
    }
    private void Detach()
    {
        if (!_attached) return;
        _attached = false; _manager.LayoutChanged -= OnLayoutChanged; _manager.ActiveContentChanged -= OnActiveChanged;
        if (_root != null) _root.PropertyChanged -= OnRootChanged;
        _root = null; _refreshPending = false; ClearSelection();
    }
    ...
    private static Thickness ParseThickness(string value, bool nonnegative)
    {
        ...
        if (parts.Length is not (1 or 2 or 4)) throw new ArgumentException("Enter one, two or four comma-separated numbers.");
        var values = parts.Select(part => ParseNumber(part, nonnegative ? 0 : -100000, 100000)).ToArray();
        return values.Length switch { 1 => new(values[0]), 2 => new(values[0], values[1], values[0], values[1]), _ => new(values[0], values[1], values[2], values[3]) };
    }
