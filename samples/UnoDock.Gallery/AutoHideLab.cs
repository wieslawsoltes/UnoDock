...
public sealed partial class GalleryPage
{
    private void ShowAutoHideLab()
    {
        ...
        Add("Minimum defaults", () =>
        {
            foreach (var tool in tools)
                tool.AutoHideWidth = tool.AutoHideHeight = 0;
        });
        ...
        Add("Cancel resize", () =>
        {
            foreach (var resizer in manager.FindVisualChildren<LayoutGridResizerControl>())
                resizer.CancelDrag();
        });
        Add("XML", () =>
        {
            using var xml = new StringWriter();
            new XmlLayoutSerializer(manager).Serialize(xml);
            status.Text = xml.ToString();
        });
        ...
        var statusView = new ScrollViewer
        {
            Content = status,
            MaxHeight = 140,
            VerticalScrollBarVisibility = ScrollBarVisibility.Auto
        };
        Grid.SetRow(statusView, 1);
        ...
        {
            Title = "Auto-hide quality",
            ContentId = "auto-hide:" + Guid.NewGuid().ToString("N"),
            Content = panel
        };
        ...
        void Populate()
        {
            ...
            }.Select(side =>
            {
                ...
                {
                    if (e.PropertyName is "AutoHideWidth" or "AutoHideHeight")
                        UpdateStatus();
                };
                return tool;
            }).ToArray();
