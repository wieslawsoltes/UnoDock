...
internal sealed class ConverterLab : UserControl
{
    ...
    {
        Content = "Retained initial value"
    };
    private readonly TextBox _editor = new()
    {
        Header = "Two-way editor (source normalizes whitespace)"
    };
    ...
    public ConverterLab()
    {
        ...
        content.Children.Add(new TextBlock { Text = "TARGET VALUE", Opacity = .65 });
        content.Children.Add(_result);
        content.Children.Add(_editor);
        content.Children.Add(_source);
        var explanation = new TextBlock
        {
            Text = "Two-way transfers use independent write guards, drain reentrant source notifications, marshal background notifications to the UI thread, and enforce one binding owner per target property. Bindings are disposed on unload and reattached on load.",
        ...
        {
            Content = "Send a source change from a worker thread"
        };
        background.Click += async (_, _) =>
        {
            ...
            try
            {
                await Task.Run(() => _model.Edit = "Worker update " + DateTime.Now.ToString("HH:mm:ss", CultureInfo.InvariantCulture));
            }
            finally
            {
    ...
    private void Attach()
    {
        ...
        try
        {
            _bindings.Add(new(_result, ContentControl.ContentProperty, () => _model.Text, value => _model.Mode switch
            {
                1 => BindingValue.DoNothing,
    ...
    {
        private string _text = "First value", _edit = "Edit me";
        private int _mode;
        public string Text
        {
            get => _text;
