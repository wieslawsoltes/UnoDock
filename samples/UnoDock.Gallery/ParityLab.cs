...
public sealed partial class GalleryPage
{
    private void ShowParityLab()
    {
        ...
        panel.Children.Add(new TextBlock { Text = "Inspect arranged drop areas, validate all 19 target kinds, execute a selected plan, or exercise shared menu bindings. Plans revalidate ownership and capabilities when executed.", TextWrapping = TextWrapping.Wrap });
        var summary = new TextBlock
        {
            TextWrapping = TextWrapping.Wrap
        };
        var contents = new ComboBox
        {
            Header = "Content to move",
            HorizontalAlignment = HorizontalAlignment.Stretch,
            DisplayMemberPath = "Title"
        };
        var plans = new ComboBox
        {
            Header = "Validated drop plans",
            HorizontalAlignment = HorizontalAlignment.Stretch
        };
        var areas = new TextBox
        {
            AcceptsReturn = true,
            IsReadOnly = true,
            MaxHeight = 220,
        ...
        {
            Content = "Refresh measured areas"
        };
        var execute = new Button
        {
            Content = "Execute selected plan"
        };
        ...
        native.Unchecked += (_, _) =>
        {
            Dock.FloatingWindowMode = FloatingWindowMode.InSurface;
            Dock.Refresh();
        };
        var floatSelected = new Button
        {
            Content = "Float selected content"
        };
        floatSelected.Click += (_, _) =>
        {
            if (contents.SelectedItem is LayoutContent selected && selected.CanFloat)
            {
                selected.Float();
                Refresh();
            }
        ...
        {
            Content = "Open 40 tabs for drag scrolling"
        };
        overflow.Click += (_, _) =>
        {
        ...
        var duplicate = new CheckBox
        {
            Content = "Selected pane allows duplicate Title + ContentId",
            IsChecked = true
        };
        ...
        {
            Content = "Use a shared, data-bound document context menu"
        };
        ...
        foreach (var command in new[]
        {
            ...
            ("Float", "FloatCommand"),
            ("Dock as document", "DockAsDocumentCommand"),
            ("Close", "CloseCommand"),
            ("Close others", "CloseAllButThisCommand")
        }
        ...
        {
            if (contents.SelectedItem is LayoutContent { Parent: LayoutDocumentPane p })
                p.AllowDuplicateContent = allow;
            else if (contents.SelectedItem is LayoutContent { Parent: LayoutAnchorablePane a })
                a.AllowDuplicateContent = allow;
            UpdatePlans();
        }
        ...
        void UpdatePlans()
        {
            if (contents.SelectedItem is not LayoutContent content)
            {
                plans.ItemsSource = Array.Empty<ComboBoxItem>();
                return;
            }
