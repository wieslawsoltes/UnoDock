...
public sealed partial class GalleryPage
{
    private void ShowDropDownLab()
    {
        ...
        var first = new UnoDock.Controls.DropDownButton
        {
            Content = "Primary  ▾",
            Padding = new(8, 2, 8, 2),
            MinHeight = 22,
        ...
        var second = new UnoDock.Controls.DropDownButton
        {
            Content = "Secondary  ▾",
            Padding = new(8, 2, 8, 2),
            MinHeight = 22,
        ...
        var triggers = new StackPanel
        {
            Orientation = Orientation.Horizontal,
            Spacing = 8
        };
        triggers.Children.Add(first);
        triggers.Children.Add(second);
        var area = new GalleryDropDownArea(Record)
        {
            ...
            Content = new Border
            {
                ...
                Child = new TextBlock
                {
                    Text = "Right-click here, or focus this area and press the context-menu key / Shift+F10.\nThe protected right-button hooks run before the opening.",
                    TextWrapping = TextWrapping.Wrap
                }
            }
        };
        ...
        {
            Title = "Dropdown contracts",
            ContentId = "dropdown-lab:" + Guid.NewGuid().ToString("N"),
        ...
        AddMenu("Change opening context", () =>
        {
            first.DropDownContextMenuDataContext = "Updated primary";
            Record("Changed primary context while keeping the shared menu.");
        });
        AddMenu("Close document", () => document.Close());
        var policies = new StackPanel
        {
            Orientation = Orientation.Horizontal,
            Spacing = 12
        };
        ...
        {
            Content = "Veto right-button opening"
        };
        veto.Checked += (_, _) => area.Veto = true;
        veto.Unchecked += (_, _) => area.Veto = false;
        var disabled = new CheckBox
        {
            Content = "Disable primary trigger"
        };
        disabled.Checked += (_, _) => first.IsEnabled = false;
        disabled.Unchecked += (_, _) => first.IsEnabled = true;
        var rtl = new CheckBox
        {
        ...
        policies.Children.Add(veto);
        policies.Children.Add(disabled);
        policies.Children.Add(rtl);
        ...
        panel.Children.Add(triggers);
        panel.Children.Add(policies);
        ...
        void AddMenu(string label, Action action)
        {
            ...
            item.Click += (_, _) =>
            {
                Record(label + " via " + (item.DataContext?.ToString() ?? "no context"));
                action();
            };
            menu.Items.Add(item);
        }
    ...
    private sealed class GalleryDropDownArea(Action<string> record) : DropDownControlArea
    {
        ...
        {
            record("Right press: pointer " + e.PointerId);
