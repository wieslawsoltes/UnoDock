...
internal sealed class InputLabPane : LayoutDocumentPaneControl
{
    ...
    internal InputLabPane(LayoutDocumentPane model, InputLabState state, Action<string> record, bool bindSelection) : base(model)
    {
        ...
        _record = record;
        if (bindSelection)
            SetBinding(SelectedIndexProperty, new Binding { Source = state, Path = new(nameof(state.Index)), Mode = BindingMode.TwoWay });
    }
    ...
    protected override void OnSelectionChanged(SelectionChangedEventArgs e)
    {
        ...
        if (_state.Redirect && SelectedIndex == 1 && Items.Count() > 2)
            SelectedIndex = 2;
    }
}
