...
public sealed class TestRunner
{
    ...
    public void Test(string name, Action body) => _cases.Add((name, () =>
    {
        body();
    ...
    public async Task<int> Run(string directory, string suite)
    {
        ...
        new XDocument(new XElement("testsuite", new XAttribute("name", suite), new XAttribute("tests", results.Count), new XAttribute("failures", results.Count(r => r.Error != null)), results.Select(r => new XElement("testcase", new XAttribute("name", r.Name), new XAttribute("time", r.Seconds.ToString("R", System.Globalization.CultureInfo.InvariantCulture)), r.Error == null ? null : new XElement("failure", r.Error))))).Save(Path.Combine(directory, suite + ".xml"));
