...
public static class VisualScene
{
    ...
    private static LayoutDocument Doc(string id, string title, string text) => new()
    {
        ContentId = id,
        Title = title,
        Content = Text(text)
    };
    private static LayoutAnchorable Tool(string id, string title, object content) => new()
    {
        ContentId = id,
        Title = title,
        Content = content,
    ...
    public static async Task Capture(DockingManager templateSource, string output)
    {
        ...
        try
        {
            ...
            )
            {
                ...
                await Task.Delay(50);
                Check.Near(SceneContent.Width, manager.ActualWidth);
                Check.Near(SceneContent.Height, manager.ActualHeight);
    ...
    internal static XElement Measure(DockingManager manager, string scenario)
    {
        ...
        {
            if (element is not ILayoutControl && element is not TextBox)
                continue;
