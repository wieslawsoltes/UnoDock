...
public static class SourceIdentityTests
{
    public static Task<int> Run(string output)
    {
        ...
        foreach (var tools in new[] { false, true })
        {
            ...
            {
                using var manager = new DockingManager(); var keep = new EqualityTrap(); var remove = new EqualityTrap();
                Set(manager, new[] { keep, remove }, tools); var model = Models(manager, tools).Single(m => ReferenceEquals(m.Content, keep));
                Set(manager, new[] { keep }, tools);
                Check.Equal(1, Models(manager, tools).Length); Check.Same(model, Models(manager, tools).Single());
                Set(manager, null, tools); Check.Equal(0, Models(manager, tools).Length);
            });
            foreach (var keepDirect in new[] { false, true })
                tests.Test(prefix + "removing one model/payload alias preserves the remaining owner: " + keepDirect, () =>
                {
                    using var manager = new DockingManager(); var payload = new EqualityTrap();
                    LayoutContent model = tools ? new LayoutAnchorable { Content = payload } : new LayoutDocument { Content = payload };
                    if (model is LayoutDocument document) manager.Layout.RootPanel.Children.Add(new LayoutDocumentPane(document));
                    else manager.Layout.RootPanel.Children.Add(new LayoutAnchorablePane((LayoutAnchorable)model));
                    var parent = model.Parent;
                    Set(manager, new object[] { payload, model }, tools); Check.Equal(1, Models(manager, tools).Length);
                    Set(manager, new[] { keepDirect ? (object)model : payload }, tools);
                    Check.Same(parent, model.Parent); Check.Same(model, Models(manager, tools).Single());
                    Set(manager, null, tools); Check.True(model.Parent == null);
                });
            tests.Test(prefix + "strategy replacement cannot receive the previous strategy's AfterInsert", () =>
            {
                using var manager = new DockingManager(); var oldAfter = 0; var newAfter = 0;
                var next = new Strategy { After = () => newAfter++ };
                manager.LayoutUpdateStrategy = new Strategy
                {
                    Before = _ => { manager.LayoutUpdateStrategy = next; return false; }, After = () => oldAfter++
                };
                Set(manager, new[] { new object() }, tools);
                Check.Equal(1, Models(manager, tools).Length); Check.Equal(0, oldAfter); Check.Equal(1, newAfter);
            });
            tests.Test(prefix + "removed default destination is retried without losing source content", () =>
            {
                using var manager = new DockingManager(); var before = 0; var after = 0;
                manager.LayoutUpdateStrategy = new Strategy
                {
                    Before = target =>
                    {
                        if (before++ == 0 && target is LayoutElement element) element.Parent?.RemoveChild(element);
                        return false;
                    },
                    After = () => after++
                };
                var value = new object(); Set(manager, new[] { value }, tools);
                Check.Equal(2, before); Check.Equal(1, after); Check.Same(value, Models(manager, tools).Single().Content);
            });
    ...
    private static LayoutContent[] Models(DockingManager manager, bool tools) => manager.Layout.Descendents().OfType<LayoutContent>()
        .Where(m => tools ? m is LayoutAnchorable : m is LayoutDocument).ToArray();
    ...
    {
        public IEnumerator GetEnumerator() { replace(); return new NeverRead(); }
