...
internal static class NavigatorCommitTests
{
    internal static async Task<int> Run(string output)
    {
        ...
        var mutations = new (string Name, Action<Fixture> Mutate)[]
        {
            ...
            ("disabled target ABA", f =>
            {
                f.Target.IsEnabled = false;
                f.Target.IsEnabled = true;
            }),
            ("closed document or hidden tool", f =>
            {
                if (f.Target is LayoutDocument doc)
                    doc.Close();
                else
                    ((LayoutAnchorable)f.Target).Hide();
            }),
            ("removed target", f => f.Remove()),
            ("reinserted target ABA", f =>
            {
                f.Remove();
                f.Insert();
            }),
            ("replaced workspace", f => f.Host.Layout = Replacement()),
            ("replaced workspace ABA", f =>
            {
                f.Host.Layout = Replacement();
                f.Host.Layout = f.Root;
            }),
            ("replaced command", f => f.Item.ActivateCommand = new ProbeCommand(() =>
            {
            }, () => true)),
            ("replaced command ABA", f =>
            {
            ...
            ("changed preview ABA", f =>
            {
                f.Nav.PreviewDocument((LayoutDocumentItem)f.Host.GetLayoutItemFromModel(f.A));
                f.SelectTarget();
            }),
            ("disabled manager", f => f.Host.IsEnabled = false),
            ("disabled manager ABA", f =>
            {
                f.Host.IsEnabled = false;
                f.Host.IsEnabled = true;
            }),
            ("disabled navigator ABA", f =>
            {
        ...
        foreach (var tool in new[]
        {
            ...
            )
            {
                ...
                Add(prefix + ": harmless title changes do not veto activation", tool, async f =>
                {
                    ...
                    f.Item.ActivateCommand = new ProbeCommand(() => executed++, () =>
                    {
                        f.Target.Title = "Renamed during query";
                        return true;
                    });
                    f.Commit(close);
                    Check.Equal(1, executed);
                ...
                foreach (var fromQuery in new[]
                {
                    ...
                    Add(prefix + ": throwing " + (fromQuery ? "CanExecute" : "Execute") + " leaves a usable next session", tool, async f =>
                    {
                        ...
                        f.SelectTarget();
                        var executed = 0;
                        f.Item.ActivateCommand = new ProbeCommand(() => executed++, () => true);
                        f.Commit(true);
                        Check.Equal(1, executed);
                        Check.True(f.Current == null);
                    });
        ...
        Add("stale input cannot close a replacement navigator", false, async f =>
        {
            ...
            f.Show();
            await Ready(f.Nav);
            Call(old, "CloseNavigatorForInput", true);
            Check.Same(f.Nav, f.Current);
        });
        Add("CanExecute opening another navigator invalidates the closing activation", false, async f =>
        {
            ...
            {
                next = new(f.Host);
                Surface(f.Host, "ShowNavigator", next);
                return true;
            });
            f.Commit(true);
            Check.Equal(0, executed);
            Check.Same(next, f.Current);
            await Ready(next!);
        });
        ...
        {
            Surface(f.Host, "CloseNavigator", false);
            var old = new NavigatorWindow(f.Host);
            NavigatorWindow? next = null;
            ...
            try
            {
                Surface(f.Host, "ShowNavigator", old);
                Check.Same(next, f.Current);
                await Ready(next!);
                Check.False(old.IsLoaded);
                Check.True(Within(next!, FocusManager.GetFocusedElement(f.Host.XamlRoot!) as DependencyObject));
            }
            finally
            {
                old.UnregisterPropertyChangedCallback(NavigatorWindow.DocumentsProperty, token);
            }
        });
        Add("throwing initialization publication removes its reservation and allows retry", false, async f =>
        {
            Surface(f.Host, "CloseNavigator", false);
            var broken = new NavigatorWindow(f.Host);
            ...
            try
            {
                ...
                try
                {
                    Surface(f.Host, "ShowNavigator", broken);
                }
                catch (Exception error)
                {
                    observed = error;
                ...
                Check.Same(failure, observed);
                Check.True(f.Current == null);
                Check.False(broken.IsLoaded);
            }
            finally
            {
                broken.UnregisterPropertyChangedCallback(NavigatorWindow.DocumentsProperty, token);
            }
        ...
        Add("foreign navigator rejected without disturbing current session", false, async f =>
        {
            ...
            var foreign = new NavigatorWindow(other);
        ...
        if (OperatingSystem.IsLinux() && Environment.GetEnvironmentVariable("UNODOCK_NATIVE_INPUT_TESTS") == "1")
        {
            Add("XTEST: Enter commits the selected document through the guarded close", false, async f =>
            {
                ...
                await FocusForNativeKeys(f.Nav, input);
                input.KeyPress(0xff0d);
                await Wait(() => f.Current == null);
                Check.Same(f.Target, f.Host.Layout.ActiveContent);
            });
    ...
    private static LayoutRoot Replacement() => new()
    {
        RootPanel = new(new LayoutDocumentPane(new LayoutDocument { ContentId = "replacement", Title = "Replacement" }))
    };
    ...
    {
        await Wait(() => navigator.IsLoaded && navigator.ActualHeight > 0);
        await Task.Delay(25);
    }
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        internal readonly LayoutDocument B = new()
        {
            ContentId = "B",
            Title = "Second.cs",
            Content = new TextBox
        ...
        {
            if (Item is LayoutDocumentItem doc)
        ...
        {
            if (Target is LayoutDocument doc)
                _documents.Children.Remove(doc);
            else
                _tools.Children.Remove((LayoutAnchorable)Target);
        }

        internal void Insert()
        {
            if (Target is LayoutDocument doc)
                _documents.Children.Add(doc);
            else
                _tools.Children.Add((LayoutAnchorable)Target);
        }

        internal void Commit(bool close)
        {
            if (close)
                Surface(Host, "CloseNavigator", true);
            else
