...
internal static class NavigatorRevocationTests
{
    internal static async Task<int> Run(string output)
    {
        ...
        var window = new Window { Title = "UnoDock navigator revocation acceptance" };
        ...
        var mutations = new (string Name, bool Revokes, Action<Fixture> Apply)[]
        {
            ...
            ("preview changed and restored", true, f => { f.Nav.PreviewDocument(f.Document(f.A)); f.PreviewTarget(); }),
            ("preview cleared", true, f => f.Nav.PreviewDocument(null)),
            ("property changed", true, f => f.AssignOther()),
            ("property changed and restored", true, f => { f.AssignOther(); f.AssignTarget(); }),
            ("property cleared", true, f => f.ClearTarget()),
            ("command changed and restored", true, f => { var command = f.Item.ActivateCommand; f.Item.ActivateCommand = null; f.Item.ActivateCommand = command; }),
            ("model disabled and restored", true, f => { f.Target.IsEnabled = false; f.Target.IsEnabled = true; }),
            ("manager disabled and restored", true, f => { f.Host.IsEnabled = false; f.Host.IsEnabled = true; }),
            ("navigator disabled and restored", true, f => { f.Nav.IsEnabled = false; f.Nav.IsEnabled = true; }),
            ("parent removed and restored", true, f => f.Reinsert()),
            ("root replaced and restored", true, f => { var root = f.Host.Layout; f.Host.Layout = new(); f.Host.Layout = root; }),
        ...
        foreach (var duringUnload in new[] { false, true })
        foreach (var mutation in mutations)
        {
            ...
            tests.Test("close revocation: " + label, async () =>
            {
                ...
                await f.Show(); f.PreviewTarget();
                var mutationsObserved = 0; var queries = 0; var executions = 0;
                void Mutate() { mutationsObserved++; mutation.Apply(f); }
                f.Item.ActivateCommand = new Command(() => { queries++; if (!duringUnload) Mutate(); return true; },
                    () => { executions++; f.Target.IsActive = true; });
                if (duringUnload) f.Nav.Unloaded += (_, _) => Mutate();
        ...
        foreach (var mutation in mutations.Take(10).Where(m => !m.Name.StartsWith("property", StringComparison.Ordinal)))
            tests.Test("direct tool revocation: " + (closed ? "Closed/" : "Closing/") + mutation.Name, async () =>
            {
                ...
                await f.Show(); f.Nav.PreviewDocument(f.Document(f.A));
                var executions = 0; var observed = 0;
                ...
                void Mutate() { observed++; mutation.Apply(f); }
                f.Item.ActivateCommand = new Command(() => true, () => executions++);
                if (closed) f.Nav.Closed += (_, _) => Mutate();
                else f.Nav.Closing += (_, _) => Mutate();
                f.AssignTarget();
                Check.True(observed >= 1);
                Check.Equal(0, executions);
                Check.Same(f.A, f.Host.Layout.ActiveContent);
            });
        try { return await tests.Run(output, "navigator-revocation"); }
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        internal readonly LayoutDocument A = new() { Title = "First", ContentId = "A" };
        private readonly LayoutDocument _b = new() { Title = "Second", ContentId = "B" };
        private readonly LayoutAnchorable _tool = new() { Title = "Properties", ContentId = "tool" };
        private readonly LayoutAnchorable _other = new() { Title = "Output", ContentId = "other" };
        ...
        internal Fixture(Window window, bool tool)
        {
            ...
            _documents = new(A); _documents.Children.Add(_b);
            _tools = new(_tool) { DockWidth = new(200) }; _tools.Children.Add(_other);
            var panel = new LayoutPanel(_tools); panel.Children.Add(_documents);
            Host.Layout = new() { RootPanel = panel }; A.IsActive = true;
            Nav = new(Host); _window = window; window.Content = Host; window.Activate();
        }
        ...
        {
            if (Item is LayoutDocumentItem document) Nav.PreviewDocument(document);
            else Nav.PreviewAnchorable((LayoutAnchorableItem)Item);
        }
        internal void AssignTarget()
        {
            if (Item is LayoutDocumentItem document) Nav.SelectedDocument = document;
            else Nav.SelectedAnchorable = (LayoutAnchorableItem)Item;
        }
        internal void AssignOther()
        {
            if (Target is LayoutDocument) Nav.SelectedDocument = Document(A);
            else Nav.SelectedAnchorable = (LayoutAnchorableItem)Host.GetLayoutItemFromModel(_other);
        }
        internal void ClearTarget()
        {
            if (Target is LayoutDocument) Nav.SelectedDocument = null;
            else Nav.SelectedAnchorable = null;
        }
        internal void Reinsert()
        {
            if (Target is LayoutDocument document) { _documents.Children.Remove(document); _documents.Children.Add(document); }
            else { _tools.Children.Remove(_tool); _tools.Children.Add(_tool); }
        }
        public void Dispose()
        {
            try { Host.Dispose(); }
    ...
    private static object? Surface(DockingManager host, string name, params object?[] args) =>
        Call(typeof(DockingManager).GetProperty("Surface", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(host)!, name, args);
}
