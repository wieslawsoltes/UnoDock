...
internal static class NavigatorSelectionTests
{
    ...
    {
        using var resource = typeof(NavigatorSelectionTests).Assembly.GetManifestResourceStream("VisualFixtures.navigator-selection-observations.xml") ?? throw new InvalidOperationException("Missing original navigator setter observations.");
        ...
        foreach (var observation in cases)
        {
            ...
            if (name.EndsWith("/disabled", StringComparison.Ordinal) || name is "document/query-reselect" or "document/execute-reselect")
                continue;
            ...
            tests.Test("original direct-selection state and event replay: " + name, async () =>
            {
                ...
                if (scenario is "veto" or "noop" or "query-reselect" or "execute-reselect" or "query-throws" or "execute-throws")
                {
                    f.Target(tool).ActivateCommand = new Command(() =>
                    {
                        ...
                        if (scenario == "query-reselect")
                        {
                            f.Trace.Add("query-reselect");
                            f.Nav.SelectedDocument = f.Document(1);
                        }
                    ...
                    }, () =>
                    {
                        ...
                        if (scenario == "execute-reselect")
                        {
                            f.Trace.Add("execute-reselect");
                            f.Nav.SelectedDocument = f.Document(1);
                        }
                ...
                EventHandler<CancelEventArgs> closing = (_, e) =>
                {
                    ...
                    {
                        e.Cancel = true;
                        f.Trace.Add("cancel-close");
                    }

                    if (scenario == "closing-reselect")
                    {
                        f.Trace.Add("closing-reselect");
                        f.Nav.SelectedDocument = f.Document(1);
                    }
                };
                ...
                try
                {
                    ...
                    foreach (var step in observation.Elements("Step"))
                    {
                        ...
                        try
                        {
                            if (step.Attribute("name")!.Value == "same-again")
                                f.Assign(tool);
                            else if (scenario == "null")
                            {
                                if (tool)
                                    f.Nav.SelectedAnchorable = null;
                                else
                                    f.Nav.SelectedDocument = null;
                            }
                            else if (scenario == "same")
                            {
                                if (tool)
                                    f.Nav.SelectedAnchorable = f.Nav.SelectedAnchorable;
                                else
                                    f.Nav.SelectedDocument = f.Nav.SelectedDocument;
                            }
                            else if (scenario == "setvalue")
                                f.Nav.SetValue(tool ? NavigatorWindow.SelectedAnchorableProperty : NavigatorWindow.SelectedDocumentProperty, f.Target(tool));
                            else
                                f.Assign(tool);
                        }
        ...
        )
        {
            ...
            tests.Test("direct selection: explicit preview does not query or execute: " + category, async () =>
            {
                ...
                if (tool)
                    f.Nav.PreviewAnchorable((LayoutAnchorableItem)f.Target(tool));
                else
                    f.Nav.PreviewDocument((LayoutDocumentItem)f.Target(tool));
                Check.Equal(0, calls);
                Check.True(f.Visible);
                Check.Same(f.Docs[0], host.Layout.ActiveContent);
            ...
            tests.Test("direct selection: disabled target stays inert instead of reproducing unsafe original activation: " + category, async () =>
            {
                ...
                f.Target(tool).LayoutElement.IsEnabled = false;
                f.Assign(tool);
                Check.Equal(0, calls);
                Check.True(f.Visible);
                Check.Same(f.Docs[0], host.Layout.ActiveContent);
            ...
            foreach (var mutation in new[]
            {
                ...
                tests.Test("direct selection: rejects CanExecute ownership change: " + category + "/" + mutation, async () =>
                {
                    ...
                    await f.Show();
                    var calls = 0;
                    NavigatorWindow? replacement = null;
                    ...
                    command = new Command(() =>
                    {
                        ...
                        {
                            case "command-ABA":
                                target.ActivateCommand = new Command(() => true, () =>
                                {
                                });
                                target.ActivateCommand = command;
                                break;
                            case "enabled-ABA":
                                target.LayoutElement.IsEnabled = false;
                                target.LayoutElement.IsEnabled = true;
                                break;
                            case "parent-ABA":
                                if (tool)
                                {
                                    f.Tools.Children.Remove(f.ToolModels[2]);
                                    f.Tools.Children.Add(f.ToolModels[2]);
                                }
                                else
                                {
                                    f.Pane.Children.Remove(f.Docs[2]);
                                    f.Pane.Children.Add(f.Docs[2]);
                                }
                            ...
                            case "root-replaced":
                                ...
                                break;
                            case "root-ABA":
                                var root = host.Layout;
                                host.Layout = new();
                                host.Layout = root;
                                break;
                            case "cancel":
                                Surface(host, "CloseNavigator", false);
                                break;
                            case "replacement-navigator":
                                Surface(host, "CloseNavigator", false);
                                replacement = new(host);
                                Surface(host, "ShowNavigator", replacement);
                                break;
                        }

                        return true;
                    }, () => calls++);
                    f.Target(tool).ActivateCommand = command;
                    f.Assign(tool);
                    Check.Equal(0, calls);
                    if (replacement != null)
            ...
            tests.Test("direct selection: a title update during query does not invalidate activation: " + category, async () =>
            {
                ...
                f.Target(tool).ActivateCommand = new Command(() =>
                {
                    f.Target(tool).LayoutElement.Title = "renamed";
                    return true;
                }, () => count++);
                f.Assign(tool);
                Check.Equal(1, count);
            ...
            tests.Test("direct selection: repeated identical assignment does not requery a veto: " + category, async () =>
            {
                ...
                f.Target(tool).ActivateCommand = new Command(() =>
                {
                    count++;
                    return false;
                }, () => throw new InvalidOperationException("Veto was ignored."));
                f.Assign(tool);
                f.Assign(tool);
                Check.Equal(1, count);
                Check.True(f.Visible);
            ...
            tests.Test("direct selection: query exception releases guards for a different subsequent selection: " + category, async () =>
            {
                using var f = new Fixture(host);
                await f.Show();
                var failure = new ApplicationException("one query");
                ...
                var executed = 0;
                f.Target(tool).ActivateCommand = new Command(() => true, () => executed++);
                f.Assign(tool);
                Check.Equal(1, executed);
                Check.False(f.Visible);
            });
        }
        ...
        tests.Test("direct selection: tool Closed callback cannot activate into a replacement navigator", async () =>
        {
            ...
            f.Assign(true);
            Check.Equal(0, executed);
            Check.True(replacement != null);
            Check.True((bool)Surface(host, "OwnsNavigator", replacement!)!);
        });
        tests.Test("direct selection: document hide does not close lifecycle and the same view can reopen", async () =>
        {
            ...
            f.Assign(false);
            Check.False(f.Visible);
            Check.False(f.Closed);
            Surface(host, "ShowNavigator", f.Nav);
            await Ready(f.Nav);
            Check.True(f.Visible);
            Check.False(f.Closed);
        });
        ...
        tests.Test("direct selection: closing handler exception leaves a usable live session", async () =>
        {
            using var f = new Fixture(host);
            await f.Show();
            var failure = new ApplicationException("closing");
            EventHandler<CancelEventArgs> handler = (_, _) => throw failure;
            f.Nav.Closing += handler;
            Exception? observed = null;
            try
            {
                f.Assign(true);
        ...
        tests.Test("direct selection: queued query reselect chooses the new document without executing stale command", async () =>
        {
            ...
            await f.Show();
            var executed = 0;
            f.Target(false).ActivateCommand = new Command(() =>
            {
                f.Nav.SelectedDocument = f.Document(1);
                return true;
            }, () => executed++);
            f.Assign(false);
            Check.Equal(0, executed);
            Check.Same(f.Docs[1], host.Layout.ActiveContent);
            Check.False(f.Visible);
        });
        ...
        tests.Test("direct selection: a foreign adapter never executes in the local manager", async () =>
        {
            ...
            other.Layout.RootPanel.Children.Add(new LayoutDocumentPane(model));
            var calls = 0;
            var item = (LayoutDocumentItem)other.GetLayoutItemFromModel(model);
            item.ActivateCommand = new Command(() => true, () => calls++);
            f.Nav.SelectedDocument = item;
            Check.Equal(0, calls);
            Check.True(f.Visible);
            Check.Same(f.Docs[0], host.Layout.ActiveContent);
        });
        foreach (var rtl in new[]
        {
            ...
            {
                using var f = new Fixture(host);
                f.Nav.FlowDirection = rtl ? FlowDirection.RightToLeft : FlowDirection.LeftToRight;
                await f.Show();
                f.Target(false).ActivateCommand = new Command(() => false, () =>
                {
                });
                f.Assign(false);
                host.UpdateLayout();
                ...
                Check.True(row is { ActualWidth: > 0, ActualHeight: > 0 });
                Check.Same(f.Target(false), list.SelectedItem);
                Check.True(f.Visible);
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        internal async Task Show()
        {
            ...
            Nav.PreviewDocument(Document(1));
            Host.UpdateLayout();
            await Task.Delay(30);
        }
        ...
        {
            if (tool)
                Nav.SelectedAnchorable = (LayoutAnchorableItem)Target(true);
            else
                Nav.SelectedDocument = Document(2);
        }
    ...
    {
        for (var i = 0; i < 150 && (!nav.IsLoaded || nav.ActualHeight <= 0); i++)
            await Task.Delay(20);
