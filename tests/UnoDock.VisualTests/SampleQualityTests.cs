...
internal static class SampleQualityTests
{
    private sealed class InitializedManager : DockingManager
    {
        ...
        protected override void OnInitialized(EventArgs e) { Calls++; Initializing?.Invoke(); base.OnInitialized(e); }
    }
    private sealed class InitializedPanel(LayoutPanel model) : LayoutGridControl<ILayoutPanelElement>(model)
    {
        internal int Calls;
        protected override void OnInitialized(EventArgs e) { Calls++; base.OnInitialized(e); }
        protected override void OnFixChildrenDockLengths() { }
    }
    private sealed class LeavePanel : DocumentPaneTabPanel
    {
        internal int Leaves;
        protected override void OnMouseLeave(DockMouseEventArgs e) { Leaves++; e.Handled = true; base.OnMouseLeave(e); }
    }
    internal static async Task<int> Run(GalleryPage source, string output)
    {
        using var page = new GalleryPage { Width = 1000, Height = 720 };
        var window = new Window { Content = page, Title = "UnoDock sample acceptance" };
        ...
        try
        {
            ...
            Add("classic sample has the public Properties/Documents/Alarms arrangement", () =>
            {
                ...
                var properties = Tool("properties"); Check.False(properties.CanHide); Check.False(properties.CanClose);
                ...
                Check.True(Tool("agenda").IsAutoHidden); Check.True(Tool("contacts").IsAutoHidden);
                return Task.CompletedTask;
            });
            Add("compact menu replaces the previous oversized brand and scrolling command strip", () =>
            {
                var menu = page.FindVisualChildren<MenuBar>().Single(); Check.Equal(5, menu.Items.Count);
                Check.True(menu.ActualHeight <= 28.1); Check.True(page.Dock.ActualHeight >= page.ActualHeight - 90);
            ...
            {
                var second = Docs().Single(d => d.ContentId == "document2"); second.IsActive = true;
                await Wait(() => page.PropertyInspector?.SelectedContentId == "document2");
                Tool("alarms").IsActive = true; await Task.Delay(30);
            ...
            Add("inspector commits live font, title and color edits", async () =>
            {
                var second = Docs().Single(d => d.ContentId == "document2"); second.IsActive = true;
                await Wait(() => page.PropertyInspector?.SelectedContentId == "document2");
                var inspector = page.PropertyInspector!; var editor = (TextBox)second.Content!;
                Check.True(inspector.TryEdit("FontSize", "24")); Check.Near(24, editor.FontSize);
                Check.True(inspector.TryEdit("Title", "Edited document")); Check.Equal("Edited document", second.Title);
                Check.True(inspector.TryEdit("Background", "#FFEEDDCC"));
                Check.Equal((byte)0xee, ((SolidColorBrush)editor.Background).Color.R);
                Check.True(inspector.TryEdit("Width", "Auto")); Check.True(double.IsNaN(editor.Width));
            });
            foreach (var value in new[] { "NaN", "Infinity", "-1", "0", "10000", "not a number" })
                Add("inspector rejects invalid font size: " + value, async () =>
                {
                    var first = Docs().First(); await Wait(() => page.PropertyInspector?.SelectedContentId == first.ContentId);
                    var content = (Control)first.Content!; var before = content.FontSize;
                    Check.False(page.PropertyInspector!.TryEdit("FontSize", value)); Check.Near(before, content.FontSize);
            ...
            {
                var inspector = page.PropertyInspector!; await Wait(() => inspector.VisibleFieldCount > 0);
                Check.False(inspector.TryEdit("ContentId", "broken"));
                inspector.Filter("FontSize"); Check.Equal(1, inspector.VisibleFieldCount);
                inspector.Filter("absent-field"); Check.Equal(0, inspector.VisibleFieldCount);
                inspector.Filter(""); Check.True(inspector.VisibleFieldCount >= 10);
            });
            Add("layout XML restore retains edited document control identity", async () =>
            {
                var second = Docs().Single(d => d.ContentId == "document2"); var editor = (TextBox)second.Content!;
                editor.Text = "Unsaved editor buffer"; second.IsActive = true;
                var xml = page.SerializeSample();
                Check.True(xml.Contains("document2", StringComparison.Ordinal));
                page.RestoreSample(xml); await Task.Delay(80);
                var restored = Docs().Single(d => d.ContentId == "document2");
                Check.Same(editor, restored.Content); Check.Equal("Unsaved editor buffer", editor.Text);
                Check.Same(page.Dock.Layout, restored.Root);
            });
            Add("theme switches retain the root and both document content instances", async () =>
            {
                var root = page.Dock.Layout; var content = Docs().Select(d => d.Content).ToArray();
                foreach (var theme in Enum.GetValues<SampleTheme>())
                {
                    page.SetSampleTheme(theme); await Task.Delay(50); page.Dock.Refresh();
            ...
            Add("large text expands actual tool captions and document tabs", async () =>
            {
                ...
                page.ExecuteSampleCommand("large-text"); await Task.Delay(80); page.UpdateLayout();
            ...
            {
                page.SwitchSample(SampleKind.Binding); await Task.Delay(60);
                Check.Equal(2, Docs().Count()); page.ExecuteSampleCommand("new"); await Task.Delay(60); Check.Equal(3, Docs().Count());
                page.SwitchSample(SampleKind.Workspace); await Task.Delay(60); Check.True(Docs().Any(d => d.ContentId == "code"));
                page.SwitchSample(SampleKind.Classic); await Task.Delay(60); Check.Equal(2, Docs().Count());
            });
            Add("narrow sample preserves an accessible document viewport", async () =>
            {
                page.Width = 640; page.Height = 480; await Task.Delay(80); page.UpdateLayout();
                var pane = page.Dock.FindVisualChildren<LayoutDocumentPaneControl>().Single();
                Check.True(pane.ActualWidth >= 200); Check.True(pane.ActualHeight >= 300);
                await VisualCapture.Save(page, Path.Combine(output, "visuals", "sample-narrow.png"));
                page.Width = 1000; page.Height = 720;
            });
            foreach (var scenario in new[] { "classic", "workspace", "dark", "rtl", "large-text" })
                Add("capture independently rendered sample: " + scenario, async () =>
                {
                    ...
                    if (scenario == "large-text") page.ExecuteSampleCommand("large-text");
                    await Task.Delay(150); page.Dock.Refresh(); page.UpdateLayout(); await Task.Delay(30);
            ...
            {
                page.Width = 1004; page.Height = 727;
                foreach (var scenario in new[] { "classic", "selected-editor", "rtl" })
                {
                    page.SwitchSample(SampleKind.Classic);
                    page.Dock.FlowDirection = scenario == "rtl" ? FlowDirection.RightToLeft : FlowDirection.LeftToRight;
                    if (scenario == "selected-editor") Docs().Single(d => d.ContentId == "document2").IsActive = true;
                    await Task.Delay(150); page.Dock.Refresh(); page.UpdateLayout();
                    Check.Near(1000, page.Dock.ActualWidth); Check.Near(640, page.Dock.ActualHeight);
            ...
            {
                using var manager = new InitializedManager { Template = source.Dock.Template, Width = 300, Height = 180 };
                Check.Equal(0, manager.Calls);
                var original = page.Content; var host = new Grid(); page.Content = host;
                try
                {
                    host.Children.Add(manager); await Wait(() => manager.IsLoaded); Check.Equal(1, manager.Calls);
                    host.Children.Remove(manager); await Task.Delay(30); host.Children.Add(manager); await Wait(() => manager.IsLoaded); Check.Equal(1, manager.Calls);
                }
                finally { host.Children.Clear(); page.Content = original; }
            });
            tests.Test("initialization: callback root replacement is the one rendered", async () =>
            {
                using var manager = new InitializedManager { Template = source.Dock.Template, Width = 300, Height = 180 };
                var replacement = new LayoutRoot { RootPanel = new LayoutPanel(new LayoutDocumentPane(new LayoutDocument { Title = "Replacement", ContentId = "replacement" })) };
                manager.Initializing = () => manager.Layout = replacement;
                var original = page.Content; page.Content = manager;
                try { await Wait(() => manager.IsLoaded && manager.FindVisualChildren<LayoutDocumentTabItem>().Any()); Check.Same(replacement, manager.Layout); }
                finally { page.Content = original; }
            });
            tests.Test("initialization: grid hook runs once after a derived constructor", async () =>
            {
                var panel = new InitializedPanel(new LayoutPanel()) { Width = 200, Height = 100 }; Check.Equal(0, panel.Calls);
                var original = page.Content; page.Content = panel;
                try { await Wait(() => panel.IsLoaded); Check.Equal(1, panel.Calls); }
                finally { page.Content = original; }
            });
            tests.Test("logical content enumeration is a stable snapshot across layout replacement", async () =>
            {
                page.SwitchSample(SampleKind.Classic); await Task.Delay(60); page.Dock.Refresh();
                var iterator = page.Dock.LogicalChildrenPublic;
                page.SwitchSample(SampleKind.Workspace); await Task.Delay(60);
                var count = 0; while (iterator.MoveNext()) count++; Check.True(count > 0);
            });
            if (OperatingSystem.IsLinux() && Environment.GetEnvironmentVariable("UNODOCK_NATIVE_INPUT_TESTS") == "1")
                tests.Test("XTEST: document header leave dispatches its real protected hook", async () =>
                {
                    ...
                    var root = new Grid(); var panel = new LeavePanel { Width = 200, Height = 40, Background = new SolidColorBrush(Microsoft.UI.Colors.White), HorizontalAlignment = HorizontalAlignment.Left, VerticalAlignment = VerticalAlignment.Top };
                    root.Children.Add(panel); page.Content = root;
                    try
                    {
                        await Wait(() => panel.IsLoaded); using var input = new X11TestInput();
                        input.MoveTo(panel, new(20, 20)); await Task.Delay(80); var before = panel.Leaves;
                        input.MoveTo(root, new(500, 300)); await Wait(() => panel.Leaves > before);
            ...
            {
                page.SwitchSample(SampleKind.Classic); page.SetSampleTheme(SampleTheme.Generic);
                page.Dock.FlowDirection = FlowDirection.LeftToRight; page.Width = 1000; page.Height = 720;
                page.ExecuteSampleCommand("normal-density"); await Task.Delay(60); page.Dock.Refresh(); page.UpdateLayout();
                await body();
            });
        }
