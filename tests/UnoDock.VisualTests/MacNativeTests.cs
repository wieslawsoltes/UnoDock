...
internal static class MacNativeTests
{
    ...
    internal static async Task<int> Run(string output)
    {
        ...
        tests.Test("AppKit: top-left UI points have downward Y and independent screen basis", async () =>
        {
            ...
            try
            {
                ...
                Check.Near(0, below.X - top.X, .01);
                Check.Near(0, right.Y - top.Y, .01);
                var returned = coordinates.FromScreen(new(top.X + 17.5, top.Y - 29.25), content);
                Check.Near(47.5, returned.X, .01);
                Check.Near(59.25, returned.Y, .01);
            }
            finally
            {
                window.Content = null;
        ...
        tests.Test("AppKit: callback failure stops clock and never crosses native ABI", () =>
        {
            var ticks = 0;
            var failures = 0;
            var failure = new InvalidOperationException("AppKit clock acceptance failure");
        ...
        tests.Test("AppKit: callback executes in the concrete tracking mode", () =>
        {
            ...
            Track(.12);
            Check.True(callbacks > 0);
            Check.Equal(callbacks, matched);
            return Task.CompletedTask;
        });
    ...
    private static void Track(double duration)
    {
        ...
        try
        {
            ...
            for (var i = 0; i < 1024; i++)
            {
                ...
                if (result == 1)
                    return; // Finished: no sources/timers in this mode.
    ...
    private static async Task Wait(Func<bool> predicate)
    {
        for (var i = 0; i < 100 && !predicate(); i++)
            await Task.Delay(20);
        Check.True(predicate(), "AppKit host was not ready.");
    }
    ...
    [DllImport(CF, EntryPoint = "CFRunLoopGetCurrent")]
    private static extern nint CurrentLoop();
    [DllImport(CF, EntryPoint = "CFRunLoopCopyCurrentMode")]
    private static extern nint CopyCurrentMode(nint loop);
    [DllImport(CF, EntryPoint = "CFEqual")]
    [return: MarshalAs(UnmanagedType.I1)]
    private static extern bool Equal(nint a, nint b);
    [DllImport(CF, EntryPoint = "CFRelease")]
    private static extern void Release(nint value);
    [DllImport(CF, EntryPoint = "CFRunLoopRunInMode")]
    private static extern int RunMode(nint mode, double seconds, [MarshalAs(UnmanagedType.I1)] bool returnAfterSourceHandled);
}
