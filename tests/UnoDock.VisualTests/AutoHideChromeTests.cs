...
{
    internal static void Register(TestRunner tests, DockingManager dock, FrameworkElement scene,
        Func<LayoutAutoHideWindowControl> window, Func<LayoutAnchorable> model, Func<Task> reset, Func<Task> settle)
    {
        tests.Test("auto-hide caption dropdown uses current shared-menu context and retains expiry", async () =>
        {
            await reset();
            var entry = new MenuFlyoutItem { Text = "Application command" };
            var menu = new MenuFlyout(); menu.Items.Add(entry); dock.AnchorableContextMenu = menu;
            try
            {
                await settle(); var button = window().FindVisualChildren<Button>().Single(b => b.Name == "PART_AutoHideMenuButton");
                Check.Equal("Auto-hidden tool menu", AutomationProperties.GetName(button));
                var peer = FrameworkElementAutomationPeer.CreatePeerForElement(button)!;
                ((IInvokeProvider)peer.GetPattern(PatternInterface.Invoke)!).Invoke();
                await Task.Delay(50); Check.Same(button, menu.Target);
                Check.Same(dock.GetLayoutItemFromModel(model()), entry.DataContext);
                await Task.Delay(180); Check.True(dock.AutoHideWindow != null);
            }
            finally { menu.Hide(); dock.AnchorableContextMenu = null; await Task.Delay(100); }
        });
        tests.Test("auto-hide caption keeps observed compact height and explicit palette overrides", async () =>
        {
            ...
            try
            {
                ...
                await settle(); Check.Same(custom, title.Background); Check.Near(30, title.ActualHeight, .2);
            }
            finally { dock.Resources.Remove("UnoDock.AutoHideTitleBrush"); dock.Resources.Remove("UnoDock.AutoHideTitleHeight"); }
        });
        tests.Test("auto-hide capture paints the complete rail and all six editor text rows", async () =>
        {
            ...
            var bitmap = new RenderTargetBitmap(); await bitmap.RenderAsync(scene, 1000, 640);
            var pixels = (await bitmap.GetPixelsAsync()).ToArray();
            Check.Equal((byte)255, pixels[(300 * 1000 + 10) * 4 + 3]);
            Check.Equal((byte)255, pixels[(300 * 1000 + 10) * 4]);
            var client = new RenderTargetBitmap(); await client.RenderAsync(window(), 326, 640);
            pixels = (await client.GetPixelsAsync()).ToArray();
            var rows = 0; var inkBefore = false;
            ...
            for (var y = 24; y < 145; y++)
            {
                ...
                for (var x = 8; x < 250; x++)
                {
                    ...
                    if (pixels[offset + 3] > 200 && pixels[offset] < 128 && pixels[offset + 1] < 128 && pixels[offset + 2] < 128) { ink = true; break; }
                }
                if (ink && !inkBefore) rows++; inkBefore = ink;
            }
