...
internal static class MvvmChromeTests
{
    internal static async Task<int> Run(string output)
    {
        ...
        var window = new Window { Content = dock, Title = "UnoDock compact MVVM chrome" };
        window.AppWindow.Resize(new() { Width = 1100, Height = 800 }); window.Activate();
        try
        {
            await Wait(() => dock.IsLoaded && dock.ActualWidth > 0);
            foreach (var scene in new[] { "generic", "light", "dark", "rtl" })
                tests.Test("MVVM chrome: compact geometry, bound commands and capture: " + scene, async () =>
                {
                    ...
                    using var workspace = new MvvmWorkspace(dock, _ => { }, new MemoryStorage());
                    await Settle();
                    ...
                    Check.Near(31, commandBar.ActualHeight); Check.Near(23, statusBar.ActualHeight);
                    var buttons = editor.FindVisualChildren<SampleButton>().ToArray(); Check.Equal(3, buttons.Length);
                    ...
                    Check.Same(document.SaveCommand, save.Command); Check.False(save.IsEnabled);
            ...
            {
                using var workspace = new MvvmWorkspace(dock, _ => { }, new MemoryStorage()); await Settle();
                var document = workspace.Documents[0]; var editor = Editor(document); var buttons = editor.FindVisualChildren<SampleButton>().ToArray();
                editor.Editor.Text = "draft kept through theme changes"; await Wait(() => document.IsDirty);
                foreach (var theme in new[] { ElementTheme.Light, ElementTheme.Dark, ElementTheme.Light })
                {
                    dock.RequestedTheme = theme; dock.Theme = new FluentTheme(theme); await Settle();
                    Check.Same(editor, Editor(document)); Check.Equal("draft kept through theme changes", document.Text);
