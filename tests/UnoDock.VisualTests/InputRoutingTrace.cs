...
public static class InputRoutingTrace
{
    public static async Task<int> Run(DockingManager host, string output)
    {
        ...
        var result = 2;
        try { result = await InputExtensionTests.Run(host, output); return result; }
        finally
        {
            ...
            await File.WriteAllTextAsync(Path.Combine(output, "input-routing-trace.json"),
                JsonSerializer.Serialize(new { schema = 1, result, records }, new JsonSerializerOptions { WriteIndented = true }));
        }
        void Capture(string phase, PointerRoutedEventArgs e)
        {
            var point = e.GetCurrentPoint(root); var path = new List<string>();
            ...
            if (records.Count == 256) records.Dequeue();
            records.Enqueue(new(watch.Elapsed.TotalMilliseconds, phase, e.Pointer.PointerId,
                point.Properties.PointerUpdateKind.ToString(), point.Properties.IsLeftButtonPressed,
                point.Position.X, point.Position.Y, e.Handled, string.Join("/", path)));
        }
    }
    public sealed record Record(double Milliseconds, string Phase, uint Pointer, string Update, bool LeftPressed,
        double X, double Y, bool Handled, string Route);
}
