...
public static class DockGuideTests
{
    public static async Task<int> Run(DockingManager templateSource, string output)
    {
        ...
        using var dock = new DockingManager { Width = 1000, Height = 640, Template = templateSource.Template, RequestedTheme = ElementTheme.Light,
            FloatingWindowMode = FloatingWindowMode.InSurface, CrossWindowCoordinates = null };
        var overlay = new OverlayWindow { Width = 1000, Height = 640 };
        var scene = new Grid { Width = 1000, Height = 640 }; scene.Children.Add(dock); scene.Children.Add(overlay);
        var window = new Window { Content = scene, Title = "UnoDock guide regression laboratory" };
        ...
        try
        {
            ...
            { using var manager = new DockingManager(); Check.Equal(DockingGuideMode.GuidesOnly, manager.DockingGuideMode); Check.False(manager.ShowDocumentPaneToolGuides); });
            tests.Test("document compass exposes only document pane targets", async () =>
            {
                await Reset(); var guides = Guides(Document());
                Check.Equal(5, guides.Count); Check.True(guides.All(g => g.Type >= DropTargetType.DocumentPaneDockLeft && g.Type <= DropTargetType.DocumentPaneDockInside));
            });
            tests.Test("extended tool compass exposes root, document and separate tool targets", async () =>
            {
                await Reset(); var guides = Guides(Tool()); Check.Equal(13, guides.Count);
            ...
            {
                await Reset(); var content = Tool();
                foreach (var guide in Guides(content))
                {
                    var plan = dock.GetDropPlan(content, Center(guide.DetectionRect)); Check.True(plan != null);
                    Check.Equal(guide.Type, plan!.Type); Check.Same(guide.Plan.Target, plan.Target);
                }
            });
            tests.Test("root-edge guide wins over the underlying tool pane", async () =>
            {
                await Reset(); var c = Tool(); var guide = Guides(c).Single(g => g.Type == DropTargetType.DockingManagerDockLeft);
                var p = dock.GetDropPlan(c, Center(guide.DetectionRect))!; Check.Same(dock.Layout.RootPanel, p.Target); Check.Equal(DropTargetType.DockingManagerDockLeft, p.Type);
            });
            tests.Test("guide-only mode leaves neutral pane content undocked", async () =>
            {
                await Reset(); dock.DockingGuideMode = DockingGuideMode.GuidesOnly;
            ...
            {
                await Reset(); dock.DockingGuideMode = DockingGuideMode.EdgesOnly;
                Check.Equal(0, Guides(Tool()).Count); Check.True(dock.GetDropPlan(Tool(), new(310, 170)) != null);
            });
            tests.Test("CanMove removes document guide intents", async () =>
            { await Reset(); var c = Document(); c.CanMove = false; Check.Equal(0, Guides(c).Count); });
            tests.Test("CanRepositionItems removes source tool guide intents", async () =>
            { await Reset(); var c = Tool(); ((LayoutAnchorablePane)c.Parent!).CanRepositionItems = false; Check.Equal(0, Guides(c).Count); });
            tests.Test("tool docking permission suppresses document glyphs but keeps outer tool glyphs", async () =>
            {
                await Reset(); var c = Tool(); c.CanDockAsTabbedDocument = false;
                var guides = Guides(c); Check.Equal(8, guides.Count);
            ...
            {
                await Reset(); var c = Document(); var pane = (LayoutDocumentPane)c.Parent!;
                var parent = (ILayoutGroup)pane.Parent!;
                var group = new LayoutDocumentPaneGroup { Orientation = Orientation.Horizontal };
                parent.ReplaceChild(pane, group); group.Children.Add(pane);
                var second = new LayoutDocumentPane(c); group.Children.Add(second);
                await Settle(); dock.AllowMixedOrientation = false;
            ...
            {
                await Reset(); var g = Guides(Tool()).First(); dock.Layout = new();
                Check.False(g.Plan.CanExecute); Check.False(g.HitTest(Center(g.DetectionRect))); Check.False(g.Plan.Execute());
            });
            tests.Test("capability changes invalidate previously displayed guide", async () =>
            {
                await Reset(); var c = Document(); var g = Guides(c).First(); c.CanMove = false;
                Check.False(g.HitTest(Center(g.DetectionRect))); Check.False(g.Plan.Execute());
            });
            tests.Test("foreign manager models have no displayed intents", async () =>
            {
                await Reset(); using var other = new DockingManager(); var c = new LayoutDocument();
                other.Layout = new() { RootPanel = new(new LayoutDocumentPane(c)) }; Check.Equal(0, Guides(c).Count);
            });
            tests.Test("outer tool glyph executes as a tool pane beside documents", async () =>
            {
                await Reset(); var c = Tool(); var g = Guides(c).Single(g => g.Type == DropTargetType.DocumentPaneDockAsAnchorableLeft);
                Check.True(g.Plan.Execute()); Check.True(c.Parent is LayoutAnchorablePane); Check.False(ReferenceEquals(c.Parent, g.Plan.Target));
            });
            tests.Test("inner tool glyph executes as a document tab", async () =>
            {
                await Reset(); var c = Tool(); var g = Guides(c).Single(g => g.Type == DropTargetType.DocumentPaneDockInside);
                Check.True(g.Plan.Execute()); Check.Same(g.Plan.Target, c.Parent); Check.True(c.Parent is LayoutDocumentPane);
            });
            tests.Test("preview cancellation preserves layout through guide execution", async () =>
            {
                await Reset(); var c = Tool(); var parent = c.Parent; var g = Guides(c).Last();
                RoutedEventHandler veto = (_, e) => ((DockEventArgs)e).Cancel = true; dock.PreviewDock += veto;
                try { Check.False(g.Plan.Execute()); Check.Same(parent, c.Parent); } finally { dock.PreviewDock -= veto; }
            });
            tests.Test("overlay remains open over neutral space without guessing a preview", async () =>
            {
                await Reset(); var guides = Guides(Tool()); overlay.ShowGuides(guides, null, dock); await Settle();
                Check.True(overlay.IsOpen); Check.True(overlay.CurrentPlan == null); Check.Equal(13, overlay.DisplayedGuides.Count);
            });
            tests.Test("overlay renders all guide regions at their hit-test rectangles", async () =>
            {
                await Reset(); var guides = Guides(Tool()); overlay.ShowGuides(guides, guides[4].Plan, dock); await Settle();
                foreach (var g in guides)
                {
                    var v = Glyph(g.Type); var r = v.TransformToVisual(overlay).TransformBounds(new(0, 0, v.ActualWidth, v.ActualHeight));
                    Check.Near(g.DetectionRect.X, r.X, 1); Check.Near(g.DetectionRect.Y, r.Y, 1);
                    Check.Near(g.DetectionRect.Width, r.Width, 1); Check.Near(g.DetectionRect.Height, r.Height, 1);
                }
            });
            tests.Test("guide containers survive hover and palette changes", async () =>
            {
                await Reset(); var c = Tool(); var guides = Guides(c); overlay.ShowGuides(guides, null, dock); await Settle(); var v = Glyph(guides[4].Type);
                overlay.ShowGuides(Guides(c), guides[4].Plan, dock); Check.Same(v, Glyph(guides[4].Type));
                dock.Theme = new FluentTheme(ElementTheme.Dark); dock.Refresh(); overlay.ShowGuides(Guides(c), null, dock); Check.Same(v, Glyph(guides[4].Type));
            });
            tests.Test("overlay never captures pointer or focus", async () =>
            { await Reset(); overlay.ShowGuides(Guides(Tool()), null, dock); Check.False(overlay.IsHitTestVisible); Check.False(overlay.IsTabStop); });
            tests.Test("hide releases plan and target references", async () =>
            { await Reset(); overlay.ShowGuides(Guides(Tool()), null, dock); overlay.Hide(); Check.False(overlay.IsOpen); Check.Equal(0, overlay.DisplayedGuides.Count); Check.False(overlay.FindVisualChildren<FrameworkElement>().Any(e => e.Name.StartsWith("Guide_", StringComparison.Ordinal))); });
            tests.Test("duplicate identity rejected before replacing current overlay", async () =>
            {
                await Reset(); var g = Guides(Tool()).First(); overlay.ShowGuides([g], g.Plan, dock);
                Check.Throws<ArgumentException>(() => overlay.ShowGuides([g, g], null, dock)); Check.Same(g.Plan, overlay.CurrentPlan);
            });
            tests.Test("overlay rejects stale intent rather than drawing a forbidden target", async () =>
            {
                await Reset(); var c = Tool(); var g = Guides(c); ((LayoutAnchorablePane)c.Parent!).CanRepositionItems = false;
                overlay.ShowGuides(g, g[0].Plan, dock); Check.False(overlay.IsOpen); Check.Equal(0, overlay.DisplayedGuides.Count);
            });
            tests.Test("guide size override changes hit regions and rendering together", async () =>
            {
                await Reset(); dock.Resources["UnoDock.GuideSize"] = 44d; var guides = Guides(Tool());
                overlay.ShowGuides(guides, null, dock); await Settle();
                foreach(var g in guides) { Check.Near(g.Type <= DropTargetType.DockingManagerDockBottom ? g.Type is DropTargetType.DockingManagerDockLeft or DropTargetType.DockingManagerDockRight ? 48 : 43.5 : 44, g.DetectionRect.Width); Check.Near(g.DetectionRect.Width, Glyph(g.Type).ActualWidth, 1); }
            });
            tests.Test("invalid density falls back without propagating NaN", async () =>
            { await Reset(); dock.Resources["UnoDock.GuideSize"] = double.NaN; Check.True(Guides(Document()).All(g => Math.Abs(g.DetectionRect.Width - 88d / 3) < 1e-6)); });
            ...
            {
                await Reset(); scene.FlowDirection = FlowDirection.RightToLeft; await Settle();
                var guides = Guides(Tool()); overlay.ShowGuides(guides, null, dock); await Settle();
                foreach (var g in guides)
                {
                    var v = Glyph(g.Type); var r = v.TransformToVisual(overlay).TransformBounds(new(0, 0, v.ActualWidth, v.ActualHeight));
                    Check.Near(g.DetectionRect.X, r.X, 1); Check.Near(g.DetectionRect.Y, r.Y, 1);
            ...
            {
                await Reset(); Check.Throws<ArgumentOutOfRangeException>(() => dock.GetDockingGuides(Tool(), new(double.NaN, 0)));
            ...
            {
                await Reset(); var guides = Guides(Tool()); overlay.ShowGuides(guides, null, dock); await Settle(); var v = Glyph(guides[0].Type);
                ...
                overlay.ApplyTemplate(); await Settle(); overlay.ShowGuides(guides, null, dock);
                Check.Same(v, Glyph(guides[0].Type)); overlay.ClearValue(Control.TemplateProperty); overlay.ApplyTemplate();
            });
            foreach (var toolMode in new[] { false, true })
            {
                var useTool = toolMode;
                tests.Test("original stock guide geometry replay: " + (useTool ? "tool" : "document"), async () =>
                {
                    await Reset(); dock.ShowDocumentPaneToolGuides = false; dock.DockingGuideMode = DockingGuideMode.GuidesOnly;
                    ...
                    var source = useTool ? (LayoutContent)new LayoutAnchorable { Title = "Dragged tool" } : new LayoutDocument { Title = "Dragged document" };
                    if (useTool) tools.Children.Add((LayoutAnchorable)source); else target.Children.Add(source);
                    tools.Children.Add(new LayoutAnchorable { Title = "Other tool" });
                    var panel = new LayoutPanel(tools); panel.Children.Add(target); dock.Layout = new() { RootPanel = panel };
                    source.FloatingLeft = 40; source.FloatingTop = 60; source.FloatingWidth = 280; source.FloatingHeight = 180;
                    source.Float(); await Settle();
                    ...
                    foreach (var item in named)
                    {
                        ...
                        var g = guides.Single(g => g.Type == type); var visual = Glyph(type);
                        ...
                        Check.Near(expected.X, g.DetectionRect.X, 1); Check.Near(expected.Y, g.DetectionRect.Y, 1);
                        Check.Near(expected.Width, g.DetectionRect.Width, 1); Check.Near(expected.Height, g.DetectionRect.Height, 1);
                        Check.Near(expected.X, actual.X, 1); Check.Near(expected.Y, actual.Y, 1);
                        Check.Near(expected.Width, actual.Width, 1); Check.Near(expected.Height, actual.Height, 1);
                        differences.Add(new XElement("Guide", new XAttribute("type", type), new XAttribute("dx", actual.X - expected.X), new XAttribute("dy", actual.Y - expected.Y),
                            new XAttribute("dWidth", actual.Width - expected.Width), new XAttribute("dHeight", actual.Height - expected.Height)));
                    }
                    var directory = Path.Combine(output, "visuals"); Directory.CreateDirectory(directory);
            ...
            foreach (var name in new[] { "guides-document", "guides-tool", "guides-tool-active", "guides-dark", "guides-rtl" })
            {
                var scenario = name;
                tests.Test("capture actual docking compass: " + name, async () =>
                {
                    await Reset(); if (scenario == "guides-dark") dock.Theme = new FluentTheme(ElementTheme.Dark);
                    if (scenario == "guides-rtl") scene.FlowDirection = FlowDirection.RightToLeft;
                    var content = scenario == "guides-document" ? (LayoutContent)Document() : Tool();
                    await Settle(); var guides = Guides(content); Check.True(guides.Count > 0);
                    overlay.ShowGuides(guides, scenario == "guides-tool-active" ? guides.Last().Plan : null, dock); await Settle();
            ...
            if (OperatingSystem.IsLinux() || OperatingSystem.IsWindows())
            {
                tests.Test("native guide geometry projects outside the primary client and executes in the target root", async () =>
                {
                    await WithNativeTarget(async (source, target, native, coordinates) =>
                    {
                        ...
                        await Settle();
                        Check.Same(parent, source.Parent); Check.Same(view, dock.GetLayoutItemFromModel(source).View);
                    });
                });
            ...
            if (OperatingSystem.IsLinux() && Environment.GetEnvironmentVariable("UNODOCK_NATIVE_INPUT_TESTS") == "1")
            {
                ...
                {
                    await Reset(); dock.DockingGuideMode = DockingGuideMode.GuidesOnly; var c = Tool(); var target = Guides(c).Single(g => g.Type == DropTargetType.DockingManagerDockRight);
                    var source = dock.FindVisualChildren<LayoutAnchorableTabItem>().First(t => ReferenceEquals(t.Model, c));
                    using var input = new X11TestInput(); await input.Begin(source, new(12, 10));
                    await input.Drop(dock, Center(target.DetectionRect)); await Settle();
                    Check.True(c.Parent is LayoutAnchorablePane); Check.Equal(AnchorSide.Right, c.GetSide());
                });
                tests.Test("native cross-window blank-body release does not create an unintended floating window", async () =>
                {
                    await WithNativeTarget(async (source, target, native, coordinates) =>
                    {
                        ...
                        var parent = source.Parent; var count = dock.FloatingWindows.Count();
                        using var input = new X11TestInput();
                        window.Activate(); await Task.Delay(120);
                        await input.Begin(tab, new(12, 10));
                        input.MoveTo(native, new(55, native.ActualHeight - 65)); await Task.Delay(120);
                        ...
                        foreach (var guide in adorners.DisplayedGuides)
                        {
                            ...
                            Check.Near(guide.DetectionRect.X, actual.X, 1); Check.Near(guide.DetectionRect.Y, actual.Y, 1);
                        }
                        input.Release(); await Settle();
                        Check.Same(parent, source.Parent); Check.False(source.IsFloating); Check.Equal(count, dock.FloatingWindows.Count());
                        Check.False(adorners.IsOpen);
                    });
                });
                tests.Test("native capture cancellation clears all guide adorners", async () =>
                {
                    await Reset(); dock.DockingGuideMode = DockingGuideMode.GuidesOnly; var c = Document(); var source = dock.FindVisualChildren<LayoutDocumentTabItem>().First(t => ReferenceEquals(t.Model, c));
                    using var input = new X11TestInput(); await input.Begin(source, new(12, 10));
                    input.MoveTo(dock, new(600, 320)); await Task.Delay(150);
                    Check.True(dock.FindVisualChildren<OverlayWindow>().Any(o => o.DisplayedGuides.Count > 0));
                    input.Escape(); input.Release(); await Settle();
                    Check.False(dock.FindVisualChildren<OverlayWindow>().Any(o => o.IsOpen));
                });
            }
            return await tests.Run(output, "docking-guides");
        }
        finally { registration.Dispose(); overlay.Hide(); window.Content = null; window.Close(); }
        ...
        async Task WithNativeTarget(Func<LayoutAnchorable, LayoutAnchorable, LayoutFloatingWindowControl, DesktopWindowCoordinates, Task> action)
        {
            ...
            dock.CrossWindowCoordinates = coordinates; dock.FloatingWindowMode = FloatingWindowMode.Native;
            ...
            target.FloatingLeft = 1250; target.FloatingTop = 100; target.FloatingWidth = 420; target.FloatingHeight = 320;
            try
            {
                target.Float(); dock.Refresh();
                var deadline = DateTime.UtcNow.AddSeconds(4);
                while (!dock.FloatingWindows.Any(w => w.NativeWindow != null && w.IsLoaded && w.ActualWidth > 0))
                { if (DateTime.UtcNow >= deadline) throw new TimeoutException("Native guide target did not load."); await Task.Delay(25); }
                var native = dock.FloatingWindows.Single(w => w.NativeWindow != null);
                await Task.Delay(100); native.UpdateLayout();
                await action(source, target, native, coordinates);
            }
            finally
            {
                dock.Layout = new(); dock.Refresh(); await Task.Delay(40);
                dock.FloatingWindowMode = FloatingWindowMode.InSurface; dock.CrossWindowCoordinates = null;
                window.Activate();
            }
        }
        ...
        async Task Settle() { dock.Refresh(); scene.UpdateLayout(); await Task.Delay(65); scene.UpdateLayout(); }
        async Task Reset()
        {
            overlay.Hide(); overlay.ClearValue(Control.TemplateProperty); scene.FlowDirection = FlowDirection.LeftToRight;
            dock.Resources.Remove("UnoDock.GuideSize"); dock.Theme = null; dock.RequestedTheme = ElementTheme.Light;
            dock.DockingGuideMode = DockingGuideMode.GuidesAndEdges; dock.ShowDocumentPaneToolGuides = true; dock.AllowMixedOrientation = false;
            VisualScene.Populate(dock); window.Activate(); await Settle();
        }
    }
}
