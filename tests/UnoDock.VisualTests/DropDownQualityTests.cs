...
public static class DropDownQualityTests
{
    private sealed class Area : DropDownControlArea
    {
        ...
        {
            Downs++;
            Check.Equal(DockMouseButton.Right, e.ChangedButton);
    ...
    private sealed class Trigger
    {
        ...
        internal MenuFlyout Menu
        {
            ...
            {
                if (Button != null)
                    Button.DropDownContextMenu = value;
                else
                    Area!.DropDownContextMenu = value;
            }
        }
        ...
        internal object? Context
        {
            ...
            {
                if (Button != null)
                    Button.DropDownContextMenuDataContext = value;
                else
                    Area!.DropDownContextMenuDataContext = value;
            }
        }
        ...
        {
            if (Button != null)
                Check.Equal(false, Button.IsChecked);
        }
    }
    ...
    public static async Task<int> Run(string output)
    {
        ...
        )
        {
            ...
            Add("replacement from DataContextChanged cannot open old menu", async t =>
            {
                ...
                {
                    if (ReferenceEquals(row.DataContext, context))
                        t.Menu = replacement;
                };
                t.Menu = old;
                t.Context = context;
                ...
                Check.False(replacement.IsOpen);
                Check.Unset(row);
                t.CheckClosed();
                t.Open();
                await Wait(() => replacement.IsOpen);
            });
            ...
            Add("inherited trigger context is live only while open", async t =>
            {
                ...
                t.Open();
                await Wait(() => menu.IsOpen);
                Check.Same(first, row.DataContext);
                t.View.DataContext = latest;
                Check.Same(latest, row.DataContext);
                t.Close();
                await Wait(() => !menu.IsOpen);
                t.View.DataContext = new object();
            ...
            Add("explicit row context survives opening and cleanup", async t =>
            {
                ...
                t.Close();
                await Wait(() => !menu.IsOpen);
                Check.Same(local, row.ReadLocalValue(FrameworkElement.DataContextProperty));
            });
            ...
            Add("failed context assignment cleans up and can be retried", async t =>
            {
                ...
                row.DataContextChanged += (_, _) =>
                {
                    if (fail && ReferenceEquals(row.DataContext, context))
                    {
                        fail = false;
                ...
                t.Menu = menu;
                t.Context = context;
                Check.Throws<InvalidOperationException>(t.Open);
                Check.False(menu.IsOpen);
                Check.Unset(row);
                t.CheckClosed();
                t.Open();
            ...
            Add("shared menu transfers ownership between two triggers", async t =>
            {
                ...
                try
                {
                    ...
                    await Wait(() => menu.IsOpen);
                    next.Open();
                    await Wait(() => menu.IsOpen && ReferenceEquals(row.DataContext, last));
                    t.Close();
                    root.Children.Remove(t.View);
                    await Task.Delay(80);
                    Check.True(menu.IsOpen);
                    Check.Same(last, row.DataContext);
                    t.CheckClosed();
                }
                finally
                {
            ...
            Add("shared takeover during context cleanup does not clear the new owner", async t =>
            {
                ...
                try
                {
                    ...
                    row.DataContextChanged += (_, _) =>
                    {
                        if (armed && !ReferenceEquals(row.DataContext, first))
                        {
                            armed = false;
                            next.Open();
                        }
            ...
            Add("five open/close cycles preserve row identity and release contexts", async t =>
            {
                ...
                for (var i = 0; i < 5; i++)
                {
                    ...
                    t.Open();
                    await Wait(() => menu.IsOpen);
                    Check.Same(value, row.DataContext);
                    t.Close();
                    await Wait(() => !menu.IsOpen);
                    Check.Unset(row);
                    Check.Same(row, menu.Items[0]);
        ...
        if (OperatingSystem.IsLinux() && Environment.GetEnvironmentVariable("UNODOCK_NATIVE_INPUT_TESTS") == "1")
        {
            foreach (var veto in new[]
            {
                ...
                tests.Test("XTEST right press/release visits hooks once; veto=" + veto, async () =>
                {
                    var t = new Trigger(true);
                    root.Children.Add(t.View);
                    try
                    {
                        await Wait(() => t.View.IsLoaded);
                        ...
                        t.Area!.Veto = veto;
                        var opens = 0;
                        t.Menu.Opened += (_, _) => opens++;
                        using var input = new X11TestInput();
                        input.MoveTo(t.View, new(30, 25));
                        await Task.Delay(50);
                        input.Press(3);
                        await Task.Delay(50);
                        input.Release(3);
                        await Task.Delay(180);
                        Check.Equal(1, t.Area.Downs);
                        Check.Equal(1, t.Area.Ups);
                        Check.Equal(veto ? 0 : 1, opens);
                        Check.Equal(!veto, t.Menu.IsOpen);
                    }
                    finally
                    {
                        t.Close();
            ...
            tests.Test("XTEST keyboard context menu key and Escape", async () =>
            {
                ...
                try
                {
                    ...
                    {
                        throw new InvalidOperationException("Application key did not open menu. Last native key: " + t.Area!.LastKey, error);
                    }

                    await Task.Delay(80);
                    input.Escape();
                    await Wait(() => !t.Menu.IsOpen);
                    Check.Equal(0, t.Area!.Downs);
                    Check.Equal(0, t.Area.Ups);
                }
                finally
                {
                    t.Close();
        ...
        try
        {
            return await tests.Run(output, "dropdown-quality");
        }
        finally
        {
            window.Close();
    ...
    private static class Check
    {
        ...
        internal static T Throws<T>(Action body)
            where T : Exception => UnoDock.Testing.Check.Throws<T>(body);
