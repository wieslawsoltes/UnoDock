...
internal static partial class XamlWorkspaceTests
{
    internal static async Task<int> Run(string output)
    {
        ...
        foreach (var density in Enum.GetValues<DockDensity>())
            tests.Test("theme: density update preserves realized pane and content: " + density, async () =>
            {
                ...
                var content = document.Content;
                ((FluentTheme)manager.Theme!).Density = density;
                var expected = density == DockDensity.Compact ? 19d : density == DockDensity.Comfortable ? 31d : 43d;
        ...
        tests.Test("theme: invalid CLR and native DP settings cannot poison subsequent changes", () =>
        {
            ...
            Check.Throws<ArgumentOutOfRangeException>(() => theme.Density = (DockDensity)999);
            ...
            theme.Density = DockDensity.Touch;
            Check.Equal(DockDensity.Touch, theme.Density);
        });
