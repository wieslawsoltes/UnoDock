...
internal static class UnoThemeTests
{
    internal static async Task<int> Run(string output)
    {
        ...
        {
            using var f = new Fixture(); await f.Show(); f.Manager.Theme = new FluentTheme(ElementTheme.Dark);
            var custom = new SolidColorBrush(Microsoft.UI.Colors.Crimson);
            f.Manager.Resources["UnoDock.PaneBrush"] = custom; f.Manager.Refresh();
            Check.Same(custom, Property<Brush>(Palette(f.Manager), "Surface"));
            Check.Equal(Microsoft.UI.Colors.Crimson, custom.Color);
            f.Manager.Resources.Remove("UnoDock.PaneBrush"); f.Manager.Refresh();
        ...
        {
            using var f = new Fixture(); await f.Show(); f.Manager.Theme = new FluentTheme(ElementTheme.Light);
            ...
            f.Manager.Refresh(); Check.Same(replacement, Property<Brush>(Palette(f.Manager), "Surface"));
        });
        tests.Test("Uno resources: Generic remains an explicit legacy palette", async () =>
        {
            using var f = new Fixture(); await f.Show(); f.Manager.Theme = new GenericTheme(); f.Manager.Refresh();
        ...
        {
            using var f = new Fixture(); await f.Show(); f.Manager.Theme = new GenericTheme(); var legacy = Palette(f.Manager);
            f.Manager.Theme = new FluentTheme(); var fluent = Palette(f.Manager);
            foreach (var name in new[] { "FontSize", "TitleHeight", "TabHeight", "ToolTabHeight", "RailThickness" })
    ...
    private static object Palette(DockingManager manager) => typeof(DockingManager).Assembly.GetType("UnoDock.Internal.DockChrome", true)!
        .GetMethod("Palette", BindingFlags.Static | BindingFlags.NonPublic)!.Invoke(null, [manager])!;
    private static T Property<T>(object value, string name) => (T)value.GetType().GetProperty(name)!.GetValue(value)!;
    private sealed class Fixture : IDisposable
    {
        internal readonly DockingManager Manager = new() { Width = 800, Height = 480, FloatingWindowMode = FloatingWindowMode.InSurface };
        internal readonly Grid Scope = new();
        internal readonly LayoutDocument Document = new() { Title = "Retained editor", ContentId = "theme-editor", Content = new TextBox { Text = "Unsaved text" } };
        ...
        internal Fixture()
        {
            Scope.Resources.ThemeDictionaries["Light"] = new ResourceDictionary { ["LayerFillColorDefaultBrush"] = LightSurface, ["TextFillColorPrimaryBrush"] = LightText };
            Scope.Resources.ThemeDictionaries["Dark"] = new ResourceDictionary { ["LayerFillColorDefaultBrush"] = DarkSurface, ["TextFillColorPrimaryBrush"] = DarkText };
            Manager.Layout = new() { RootPanel = new(new LayoutDocumentPane(Document)) }; Document.IsActive = true;
            Scope.Children.Add(Manager); _window = new() { Content = Scope, Title = "UnoDock theme acceptance" };
            _window.AppWindow.Resize(new() { Width = 920, Height = 650 }); _window.Activate();
        }
        internal async Task Show() { await Wait(() => Manager.IsLoaded && Manager.ActualWidth > 0); Manager.Refresh(); await Task.Delay(30); }
        public void Dispose() { Manager.Dispose(); _window.Content = null; _window.Close(); }
    }
    private static async Task Wait(Func<bool> predicate)
    { for (var i = 0; i < 100 && !predicate(); i++) await Task.Delay(20); Check.True(predicate(), "Theme host did not settle."); }
}
