...
public static class VisualParityTests
{
    public static async Task<int> Run(DockingManager host, string output)
    {
        ...
        foreach (var scenario in new[]
        {
            ...
            tests.Test("original per-item auto-hide sequence: " + scenario, () =>
            {
                ...
                var items = new[]
                {
                    ...
                    "second",
                    "third"
                }.Select(id => new LayoutAnchorable { ContentId = id, Title = id }).ToArray();
                ...
                if (scenario == "ineligible-sibling")
                    items[1].CanAutoHide = false;
                items[0].ToggleAutoHide();
                if (scenario is "two" or "pin-one")
                    items[1].ToggleAutoHide();
                if (scenario == "pin-one")
                    items[0].ToggleAutoHide();
        ...
        tests.Test("ineligible auto-hide target is unchanged", () =>
        {
            ...
            a.ToggleAutoHide();
            Check.Same(pane, a.Parent);
            Check.False(a.IsAutoHidden);
        });
        ...
        try
        {
            tests.Test("tool tab strip is below content; document strip is above", async () =>
            {
                ...
                var docTab = Tab(doc);
                var toolTab = Tab(tool);
                var editorBounds = Bounds((FrameworkElement)doc.Content!);
                var toolBounds = Bounds((FrameworkElement)tool.Content!);
            ...
            {
                await Reset();
                var first = Document("editor");
                var next = Document("readme");
                Check.True(Visible(NamedButton(Tab(first), "Close tab")));
                Check.False(Visible(NamedButton(Tab(next), "Close tab")));
                next.IsActive = true;
                await Settle();
                Check.False(Visible(NamedButton(Tab(first), "Close tab")));
                Check.True(Visible(NamedButton(Tab(next), "Close tab")));
                next.CanClose = false;
                await Settle();
                Check.False(Visible(NamedButton(Tab(next), "Close tab")));
            });
            ...
            {
                await Reset();
                var tool = Tool("explorer");
                var view = Pane(tool);
                var pin = NamedButton(view, "Auto-hide tool");
                var close = NamedButton(view, "Hide or close tool");
                Check.True(Visible(pin));
                Check.True(Visible(close));
                tool.CanAutoHide = false;
                tool.CanHide = false;
                tool.CanClose = false;
                await Settle();
                Check.False(Visible(pin));
                Check.False(Visible(close));
                Check.Same(view, Pane(tool));
            });
            tests.Test("caption close follows model cancellation rather than discarding view", async () =>
            {
                await Reset();
                var tool = Tool("explorer");
                var pane = tool.Parent;
                tool.Hiding += (_, e) => e.Cancel = true;
                Invoke(NamedButton(Pane(tool), "Hide or close tool"));
                await Settle();
                Check.Same(pane, tool.Parent);
                Check.False(tool.IsHidden);
            });
            tests.Test("caption pin moves requested item and retains its sibling", async () =>
            {
                ...
                var a = Tool("explorer");
                var b = Tool("toolbox");
                var original = a.Parent;
                Invoke(NamedButton(Pane(a), "Auto-hide tool"));
                await Settle();
                Check.True(a.IsAutoHidden);
                Check.Same(original, b.Parent);
            ...
            tests.Test("rail label reserves vertical layout size", async () =>
            {
                ...
                Check.True(anchor.ActualHeight > anchor.ActualWidth * 2);
                Check.Near(26, anchor.ActualWidth + anchor.Margin.Left + anchor.Margin.Right, 1);
                var side = dock.FindVisualChildren<LayoutAnchorSideControl>().Single(c => c.IsLeftSide);
                Check.Near(26, side.ActualWidth, 1);
            });
            tests.Test("auto-hide popup close honors cancellation and then hides the model", async () =>
            {
                ...
                await Settle();
                var anchor = dock.FindVisualChildren<LayoutAnchorControl>().Single(c => ReferenceEquals(c.Model, a));
                Invoke(anchor.FindVisualChildren<Button>().Single());
                await Settle();
                ...
                a.Hiding += cancel;
                Invoke(NamedButton(popup, "Hide or close auto-hidden tool"));
                await Settle();
                Check.True(a.IsAutoHidden);
                Check.True(Visible(popup));
                a.Hiding -= cancel;
                Invoke(NamedButton(popup, "Hide or close auto-hidden tool"));
                await Settle();
                Check.True(a.IsHidden);
                Check.False(Visible(popup));
            });
            tests.Test("auto-hide popup pin restores the original pane and retained presenter", async () =>
            {
                ...
                var a = Tool("explorer");
                var pane = a.Parent;
                var presenter = dock.GetLayoutItemFromModel(a).View;
                a.ToggleAutoHide();
                await Settle();
                var anchor = dock.FindVisualChildren<LayoutAnchorControl>().Single(c => ReferenceEquals(c.Model, a));
                Invoke(anchor.FindVisualChildren<Button>().Single());
                await Settle();
                var popup = dock.FindVisualChildren<LayoutAutoHideWindowControl>().Single(c => ReferenceEquals(c.Model, a));
                Invoke(NamedButton(popup, "Pin tool"));
                await Settle();
                Check.Same(pane, a.Parent);
                Check.False(a.IsAutoHidden);
                Check.Same(presenter, dock.GetLayoutItemFromModel(a).View);
            });
            tests.Test("live theme switching keeps presenter and changes compact caption contrast", async () =>
            {
                ...
                var view = Pane(a);
                var presenter = dock.GetLayoutItemFromModel(a).View;
                var button = NamedButton(view, "Auto-hide tool");
                var light = ((SolidColorBrush)button.Foreground).Color;
                dock.RequestedTheme = ElementTheme.Dark;
                await Settle();
                var dark = ((SolidColorBrush)button.Foreground).Color;
                Check.True(light != dark);
                Check.Same(presenter, dock.GetLayoutItemFromModel(a).View);
                Check.Same(view, Pane(a));
                dock.RequestedTheme = ElementTheme.Light;
                await Settle();
                Check.Equal(light, ((SolidColorBrush)button.Foreground).Color);
            });
            tests.Test("invalid density override falls back and valid override changes measured strip", async () =>
            {
                await Reset();
                var tab = Tab(Document("editor"));
                dock.Resources["UnoDock.TabHeight"] = double.NaN;
                await Settle();
                Check.Near(19, tab.Height);
                dock.Resources["UnoDock.TabHeight"] = 32d;
                await Settle();
                Check.Near(31, tab.Height);
                dock.Resources.Remove("UnoDock.TabHeight");
                await Settle();
                Check.Near(19, tab.Height);
            });
            tests.Test("original title templates are rendered by compact captions", async () =>
            {
                ...
                await Settle();
                Check.True(Pane(Tool("explorer")).FindVisualChildren<TextBlock>().Any(t => t.Text == "custom caption marker"));
                dock.AnchorableTitleTemplate = null;
                await Settle();
            ...
            tests.Test("default document icon is visible without replacing model content", async () =>
            {
                ...
                var image = new Microsoft.UI.Xaml.Media.Imaging.WriteableBitmap(1, 1);
                doc.IconSource = image;
                await Settle();
                Check.True(Tab(doc).FindVisualChildren<Image>().Any(i => ReferenceEquals(i.Source, image) && Visible(i)));
                Check.Same(content, doc.Content);
                doc.IconSource = null;
                await Settle();
                Check.False(Tab(doc).FindVisualChildren<Image>().Any(Visible));
            });
            tests.Test("overflow selection exposes selected header and returning first restores offset", async () =>
            {
                await Reset();
                var first = Document("editor");
                var pane = (LayoutDocumentPane)first.Parent!;
                for (var i = 0; i < 35; i++)
                    pane.Children.Add(new LayoutDocument { Title = $"Overflow document {i:D2}", ContentId = "overflow-" + i });
                await Settle();
                var selected = pane.Children[^1];
                ...
                Check.True(scroll.HorizontalOffset > 100);
                Check.True(Bounds(Tab(selected)).Right <= Bounds(scroll).Right + 1);
                first.IsActive = true;
                await Settle();
                await Task.Delay(80);
                Check.Near(0, scroll.HorizontalOffset, 1);
            });
            tests.Test("queued overflow reveal cannot affect a replacement layout", async () =>
            {
                await Reset();
                var pane = (LayoutDocumentPane)Document("editor").Parent!;
                pane.Children.Add(new LayoutDocument { Title = "old", ContentId = "old" });
                pane.Children[^1].IsActive = true;
                VisualScene.Populate(dock);
                await Settle();
                Check.Equal("editor", dock.Layout.ActiveContent!.ContentId);
            ...
            tests.Test("RTL native client projection contains root mirroring", async () =>
            {
                ...
                var ltr = coordinates.ToScreen(dock, default);
                ...
                var roundtrip = coordinates.FromScreen(coordinates.ToScreen(dock, point), dock);
                Check.Near(point.X, roundtrip.X, 1);
                Check.Near(point.Y, roundtrip.Y, 1);
            ...
            tests.Test("RTL and LTR native windows translate through physical axes", async () =>
            {
                ...
                try
                {
                    ...
                    grid.UpdateLayout();
                    var origin = coordinates.ToScreen(dock, default);
