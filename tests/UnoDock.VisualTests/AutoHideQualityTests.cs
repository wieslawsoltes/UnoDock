...
public static class AutoHideQualityTests
{
    ...
    private sealed class Manager : DockingManager
    {
        ...
        {
            base.OnAutoHideWindowChanged(e);
            HostChanged?.Invoke(e);
        }
    }
    ...
    public static async Task<int> Run(DockingManager templateSource, string output)
    {
        ...
        FocusFlyout flyout = null!;
        LayoutGridResizerControl splitter = null!;
        AnchorableShowStrategy side = AnchorableShowStrategy.Left;
        object surface = null!;
        ...
        try
        {
            using (var stream = typeof(AutoHideQualityTests).Assembly.GetManifestResourceStream("VisualFixtures.auto-hide-window-observations.xml")!)
            {
                ...
                foreach (var row in observations.Root!.Elements("Window"))
                    ...
                    {
                        await Reset(Enum.Parse<AnchorableShowStrategy>(row.Attribute("side")!.Value), requested: (bool)row.Attribute("explicitSize")! ? (double)row.Attribute("autoHideWidth")! : 0, requestedHeight: (double)row.Attribute("autoHideHeight")!);
            ...
            foreach (var direction in new[]
            {
                ...
                foreach (var rtl in new[]
                {
                    ...
                    tests.Test($"dedicated gutter, deferred preview and committed extent: {direction}/RTL={rtl}", async () =>
                    {
                        ...
                        Move(40 * Sign());
                        await Settle();
                        Check.Near(initial, Pixels());
                        Check.Near(stored, Requested());
                        Check.Equal(1, Ghosts().Length);
                        End();
                        await Settle();
                        Check.Near(initial + 40, Pixels(), .2);
                        Check.Near(stored + 40, Requested(), .2);
                        Check.Equal(0, Ghosts().Length);
                        Call(dock, "CloseAutoHide");
                        Open();
                        await Settle();
                        Check.Near(stored + 40, Requested());
                        Check.Near(initial + 40, Pixels(), .2);
                    });
            tests.Test("zero-displacement resize preserves unspecified model size", async () =>
            {
                await Reset(requested: 0, requestedHeight: 0);
                Begin();
                Move(20);
                Move(0);
                End();
                await Settle();
                Check.Near(0, tool.AutoHideWidth);
                Check.Near(106, flyout.ActualWidth, .1);
            });
            ...
            tests.Test("tiny viewport clips presentation without rewriting saved sizes", async () =>
            {
                ...
                dock.Width = 70;
                await Settle();
                Check.True(flyout.ActualWidth <= 44.1);
                Check.Near(saved, Requested());
                Begin();
                Move(0);
                End();
                Check.Near(saved, Requested());
            ...
            tests.Test("NaN displacement cancels and permits a fresh gesture", async () =>
            {
                await Reset();
                Begin();
                Check.Throws<ArgumentOutOfRangeException>(() => Move(double.NaN));
                Check.False(splitter.IsDragging);
                Check.Equal(0, Ghosts().Length);
                Begin();
                Move(10);
                End();
                Check.Near(330, Requested());
            });
            tests.Test("external dimension edit wins over pending resize", async () =>
            {
                ...
                Begin();
                Move(50);
                tool.AutoHideWidth = 270;
                await Settle();
                Check.False(splitter.IsDragging);
                End();
                Check.Near(270, Requested());
                Check.Equal(0, Ghosts().Length);
            });
            tests.Test("minimum edit invalidates pending transaction", async () =>
            {
                ...
                Begin();
                Move(50);
                tool.AutoHideMinWidth = 180;
                await Settle();
                End();
                Check.Near(320, Requested());
                Check.False(splitter.IsDragging);
            });
            tests.Test("root replacement invalidates old preview and releases content", async () =>
            {
                ...
                await Settle();
                End();
                Check.Near(320, old.AutoHideWidth);
                Check.Equal(0, Ghosts().Length);
                Check.True(dock.AutoHideWindow == null);
                Check.True(flyout.Model == null);
            });
            tests.Test("switching flyouts cannot apply old displacement to new model", async () =>
            {
                ...
                next.AddToLayout(dock, AnchorableShowStrategy.Right);
                next.ToggleAutoHide();
                Call(dock, "OpenAutoHide", next, false);
                await Settle();
                End();
                Check.Near(320, old.AutoHideWidth);
                Check.Near(260, next.AutoHideWidth);
            ...
            tests.Test("disabled tool closes flyout and discards resize", async () =>
            {
                ...
                await Settle();
                End();
                Check.Near(320, tool.AutoHideWidth);
                Check.True(dock.AutoHideWindow == null);
                Check.Equal(0, Ghosts().Length);
            });
            ...
            tests.Test("throwing model observer rolls back only operation-owned size", async () =>
            {
                ...
                {
                    if (e.PropertyName == nameof(tool.AutoHideWidth) && !once)
                    {
                        once = true;
                ...
                Begin();
                Move(40);
                Check.Throws<InvalidOperationException>(End);
                Check.Near(320, Requested());
                Check.Equal(0, Ghosts().Length);
                Begin();
                Move(20);
                End();
                Check.Near(340, Requested());
            });
            tests.Test("observer replacement value is not overwritten", async () =>
            {
                ...
                Move(40);
                Check.Throws<InvalidOperationException>(End);
                Check.Near(280, Requested());
                Check.Equal(0, Ghosts().Length);
            });
            tests.Test("retained flyout content and editor focus survive model refresh", async () =>
            {
                await Reset();
                var view = dock.GetLayoutItemFromModel(tool).View;
                var editor = (TextBox)tool.Content!;
                Check.True(editor.Focus(FocusState.Programmatic));
                tool.Title = "Changed";
                await Settle();
                Check.Same(view, dock.GetLayoutItemFromModel(tool).View);
                Check.Same(editor, FocusManager.GetFocusedElement(editor.XamlRoot!));
                Check.True(tool.IsActive);
            });
            tests.Test("hover path preserves active document while activation path selects tool", async () =>
            {
                ...
                await Settle();
                Check.Same(tool, dock.Layout.ActiveContent);
            });
            tests.Test("activation callback replacing layout cannot publish a stale flyout", async () =>
            {
                ...
                tool.PropertyChanged += (_, e) =>
                {
                    if (e.PropertyName == nameof(tool.IsActive) && tool.IsActive)
                        dock.Layout = new();
                };
                Call(dock, "OpenAutoHide", tool, true);
                await Settle();
            ...
            {
                await Reset();
                Check.True(((TextBox)tool.Content!).Focus(FocusState.Programmatic));
                await Task.Delay(30);
                Call(surface, "ExpireAutoHide");
                Check.Same(flyout, dock.AutoHideWindow);
            ...
            {
                await Reset();
                var menu = (MenuFlyout)flyout.ContextFlyout;
            ...
            {
                await Reset();
                var command = new MenuFlyoutItem
                {
                    Text = "Owned action"
                };
            ...
            tests.Test("host close callback can reopen retained flyout without old cleanup removing it", async () =>
            {
                ...
                try
                {
                    ...
                    await Settle();
                    Check.Same(next, dock.AutoHideWindow!.Model);
                    Check.True(dock.AutoHideWindow.IsLoaded);
                    Check.Near(216, dock.AutoHideWindow.ActualWidth, .2);
                    Check.True(old.IsAutoHidden);
                }
                finally
                {
                    dock.HostChanged = null;
            ...
            {
                await Reset();
                var requested = new LayoutAnchorable
                {
                    Title = "Requested",
                    ContentId = "requested",
                    Content = new TextBox()
                };
                var replacement = new LayoutAnchorable
                {
                    Title = "Replacement",
                    ContentId = "replacement",
                    Content = new TextBox()
                };
                requested.AddToLayout(dock, AnchorableShowStrategy.Right);
                requested.ToggleAutoHide();
                replacement.AddToLayout(dock, AnchorableShowStrategy.Bottom);
                replacement.ToggleAutoHide();
                dock.HostChanged = e =>
                {
                    if (e.NewValue == null)
                    {
                        dock.HostChanged = null;
                        Call(dock, "OpenAutoHide", replacement, false);
                    }
                };
                try
                {
                    Call(dock, "OpenAutoHide", requested, false);
                    await Settle();
                    Check.Same(replacement, dock.AutoHideWindow!.Model);
                    Check.True(dock.AutoHideWindow.IsLoaded);
                }
                finally
                {
            ...
            tests.Test("throwing host-close observer leaves no flyout model or capture", async () =>
            {
                ...
                {
                    if (e.NewValue == null)
                        throw new InvalidOperationException("Owned callback failure");
                };
                try
                {
                    Check.Throws<InvalidOperationException>(() => Call(dock, "CloseAutoHide"));
                }
                finally
                {
                    dock.HostChanged = null;
            ...
            tests.Test("title selector replacing the root cannot leave a stale host", async () =>
            {
                await Reset();
                Call(dock, "CloseAutoHide");
                var selector = new CallbackTemplateSelector
                {
                    Callback = () => dock.Layout = new()
                };
                ...
                {
                    dock.AnchorableTitleTemplateSelector = null;
                }
            });
            tests.Test("title selector redirecting to another flyout owns the final chrome", async () =>
            {
                ...
                var selector = new CallbackTemplateSelector
                {
                    Callback = () => Call(dock, "OpenAutoHide", next, false)
                };
                dock.AnchorableTitleTemplateSelector = selector;
                try
                {
            ...
            tests.Test("font density expands caption without replacing retained content", async () =>
            {
                ...
                Check.Near(24, caption.FontSize);
                Check.True(caption.ActualHeight >= 24);
                Check.Same(view, dock.GetLayoutItemFromModel(tool).View);
            ...
            foreach (var direction in new[]
            {
                ...
                {
                    await Reset(direction);
                    Check.False((bool)Call(splitter, "ResizeFromKey", horizontal ? VirtualKey.Down : VirtualKey.Right)!);
            ...
            foreach (var variant in new[]
            {
                ...
                tests.Test("capture auto-hide flyout and independent preview: " + variant, async () =>
                {
                    ...
                    await Settle();
                    Check.Equal(1, Ghosts().Length);
                    await VisualCapture.Save(scene, Path.Combine(output, "visuals", "auto-hide-" + variant + ".png"));
                    splitter.CancelDrag();
                });
            if (OperatingSystem.IsLinux() && Environment.GetEnvironmentVariable("UNODOCK_NATIVE_INPUT_TESTS") == "1")
            {
                tests.Test("XTEST hover does not activate tool and click does", async () =>
                {
                    ...
                    input.MoveTo(anchor, new(anchor.ActualWidth / 2, anchor.ActualHeight / 2));
                    await Task.Delay(100);
                    Check.Same(doc, dock.Layout.ActiveContent);
                    Check.True(dock.AutoHideWindow != null);
                    input.Press();
                ...
                foreach (var rtl in new[]
                {
                    ...
                    tests.Test("XTEST deferred resize primary release: RTL=" + rtl, async () =>
                    {
                        ...
                        input.MoveTo(scene, new(start.X + 45, start.Y));
                        await Task.Delay(60);
                        Check.Near(320, tool.AutoHideWidth);
                        Check.True(splitter.IsDragging);
                        input.Release();
                        await Settle();
                        Check.Near(365, tool.AutoHideWidth, 1.1);
                    });
        ...
        async Task Reset(AnchorableShowStrategy direction = AnchorableShowStrategy.Left, bool rtl = false, double requested = 320, double requestedHeight = 320)
        {
            ...
            doc = new LayoutDocument
            {
                ...
                Content = new TextBox
                {
                    Text = "The document retains its editor and geometry while an auto-hidden tool is previewed or resized.",
                    AcceptsReturn = true,
            ...
            tool = new()
            {
                ...
                ContentId = "tool",
                AutoHideWidth = requested,
                AutoHideHeight = requestedHeight,
                Content = new TextBox
                {
                    AcceptsReturn = true,
                    FontSize = 12,
            ...
            tool.AddToLayout(dock, direction | AnchorableShowStrategy.Most);
            tool.ToggleAutoHide();
            doc.IsActive = true;
            window.Activate();
            await Settle();
            Open();
            await Settle();
            flyout = (FocusFlyout)dock.AutoHideWindow!;
            flyout.Keep = false;
    ...
    private static object? Call(object target, string method, params object[] args)
    {
        ...
        while (type != null && (info = type.GetMethod(method, BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.DeclaredOnly)) == null)
            type = type.BaseType;
        try
        {
