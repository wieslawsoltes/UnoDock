...
public static class MenuContextLifetimeTests
{
    public static Task<int> Run(string output)
    {
        ...
        Register(tests);
        return tests.Run(output, "menu-context-lifetime");
    }
    ...
    private static void Register(TestRunner tests)
    {
        tests.Test("menu context replacement inside assignment wins for the complete item tree", () =>
        {
            ...
            {
                if (ReferenceEquals(first.DataContext, original))
                    Apply(menu, replacement);
            };
            Apply(menu, original);
            Check.Same(replacement, first.DataContext);
            Check.Same(replacement, second.DataContext);
            Clear(menu);
            Unassigned(first);
            Unassigned(second);
        });
        tests.Test("menu clear callback can reopen the same context without losing its new assignment", () =>
        {
            ...
            first.DataContextChanged += (_, _) =>
            {
                if (reentered || first.DataContext != null)
                    return;
                reentered = true;
                Apply(menu, context);
            };
            Clear(menu);
            Check.True(reentered);
            Check.Same(context, first.DataContext);
            Check.Same(context, second.DataContext);
            Clear(menu);
            Unassigned(first);
            Unassigned(second);
        });
        tests.Test("menu clear callback can replace the context without invalidating enumeration", () =>
        {
            ...
            first.DataContextChanged += (_, _) =>
            {
                if (changed || first.DataContext != null)
                    return;
                changed = true;
                Apply(menu, next);
            };
            ...
            Check.Same(next, second.DataContext);
            Clear(menu);
            Unassigned(first);
            Unassigned(second);
        });
        tests.Test("menu clear during assignment prevents stale assignments to later rows", () =>
        {
            ...
            first.DataContextChanged += (_, _) =>
            {
                if (ReferenceEquals(first.DataContext, context))
                    Clear(menu);
            };
            Apply(menu, context);
            Unassigned(first);
        ...
        tests.Test("menu context cleanup preserves explicit application edits made during assignment", () =>
        {
            ...
            {
                if (ReferenceEquals(first.DataContext, context))
                    first.DataContext = owned;
            };
            Apply(menu, context);
            Clear(menu);
            Check.Same(owned, first.DataContext);
            Unassigned(second);
        });
        ...
        tests.Test("menu recursive submenu assignment redirects all descendants", () =>
        {
            ...
            var original = new object();
            var replacement = new object();
            child.DataContextChanged += (_, _) =>
            {
                if (ReferenceEquals(child.DataContext, original))
                    Apply(menu, replacement);
            };
            Apply(menu, original);
            Check.Same(replacement, first.DataContext);
            Check.Same(replacement, second.DataContext);
            Check.Same(replacement, sub.DataContext);
            Check.Same(replacement, child.DataContext);
            Clear(menu);
            Unassigned(first);
            Unassigned(second);
            Unassigned(sub);
        ...
        tests.Test("menu callback can retain one application context while clearing the temporary scope", () =>
        {
            ...
            Apply(menu, context);
            second.DataContext = application;
            Clear(menu);
            Unassigned(first);
        ...
        tests.Test("menu context operation releases partial assignments after a callback exception", () =>
        {
            ...
            second.DataContextChanged += (_, _) =>
            {
                if (ReferenceEquals(second.DataContext, context))
                    throw error;
            };
            var actual = Check.Throws<InvalidOperationException>(() => Apply(menu, context));
            Check.Same(error, actual);
            Unassigned(first);
            Unassigned(second);
            var next = new object();
            Apply(menu, next);
        ...
        tests.Test("ContextMenuEx context replacement during old-scope cleanup agrees with its final property", () =>
        {
            ...
            var redirected = new object();
            menu.MenuDataContext = initial;
            var changed = false;
            first.DataContextChanged += (_, _) =>
            {
                if (changed || first.DataContext != null)
                    return;
                changed = true;
                menu.MenuDataContext = redirected;
            };
            menu.MenuDataContext = requested;
            Check.Same(redirected, menu.MenuDataContext);
            Check.Same(redirected, first.DataContext);
            Check.Same(redirected, second.DataContext);
        ...
        tests.Test("nonconvergent menu context callbacks are bounded and do not poison the next operation", () =>
        {
            ...
            first.DataContextChanged += (_, _) =>
            {
                if (!loop || first.DataContext == null)
                    return;
                calls++;
                Apply(menu, ReferenceEquals(first.DataContext, a) ? b : a);
            };
            var error = Check.Throws<InvalidOperationException>(() => Apply(menu, a));
            Check.True(error.Message.Contains("converge", StringComparison.Ordinal));
            Check.True(calls > 1 && calls < 100);
            Unassigned(first);
            Unassigned(second);
            loop = false;
            Apply(menu, b);
            Check.Same(b, first.DataContext);
        ...
        tests.Test("menu assignment and cleanup exceptions retain both causes while releasing other rows", () =>
        {
            ...
            var context = new object();
            var assigned = false;
            var original = new InvalidOperationException("assignment callback");
            var cleanup = new InvalidOperationException("cleanup callback");
            first.DataContextChanged += (_, _) =>
            {
                if (ReferenceEquals(first.DataContext, context))
                    assigned = true;
                else if (assigned && first.DataContext == null)
                    throw cleanup;
            };
            second.DataContextChanged += (_, _) =>
            {
            ...
            Check.Equal(2, error.InnerExceptions.Count);
            Check.Same(original, error.InnerExceptions[0]);
            Check.Same(cleanup, error.InnerExceptions[1]);
            Unassigned(first);
            Unassigned(second);
            assigned = false;
            Clear(menu);
