...
internal static class FloatingDragCleanupTests
{
    internal static async Task<int> Run(string output)
    {
        ...
        foreach (var fault in new[] { "guides", "caption", "both" })
            tests.Test($"cleanup/{(native ? "native" : "surface")}/{(tools ? "tools" : "document")}/{(commit ? "release" : "cancel")}/{fault}", async () =>
            {
                ...
                var leaves = error is AggregateException aggregate ? aggregate.Flatten().InnerExceptions.ToArray() : new[] { error! };
                Check.Equal(expected.Count, observed.Count);
                Check.Equal(expected.Count, leaves.Length);
                for (var i = 0; i < expected.Count; i++) { Check.Same(expected[i], observed[i]); Check.Same(expected[i], leaves[i]); }
                if (expected.Count == 1) Check.Same(expected[0], error);
                f.AssertIdle();
                ...
                var next = f.BeginAndPaint(); Check.True(next > generation);
                f.End(next, false); f.AssertIdle();
                ...
                void Watch(DependencyObject owner, DependencyProperty property, Func<bool> terminal, string label)
                {
                    ...
                    expected.Add(failure); var delivered = false;
                    callbacks.Add(owner, property, () =>
                    {
                        if (delivered || !terminal()) return;
                        delivered = true; observed.Add(failure); throw failure;
                    });
                }
            });
        tests.Test("overlay cleanup: a Visibility observer failure still retires guide visuals and preview", async () =>
        {
            using var f = new Fixture(false, false); await f.Show(); f.BeginAndPaint();
            var marker = new InvalidOperationException("Visibility callback failure");
            using var callbacks = new Callbacks(); var delivered = false;
            callbacks.Add(f.Overlay, UIElement.VisibilityProperty, () =>
            {
                if (delivered || f.Overlay.Visibility != Visibility.Collapsed) return;
                delivered = true; throw marker;
            });
        ...
        foreach (var show in new[] { false, true })
            tests.Test("overlay cleanup: a callback's replacement survives " + (show ? "ShowPreview" : "Hide"), async () =>
            {
                ...
                using var callbacks = new Callbacks(); var reopened = false;
                callbacks.Add(f.Overlay, UIElement.VisibilityProperty, () =>
                {
                    if (reopened || f.Overlay.Visibility != Visibility.Collapsed) return;
                    reopened = true; f.Overlay.ShowPreview(replacement, brush);
                });
                if (show) f.Overlay.ShowPreview(original, brush); else f.Overlay.Hide();
                Check.True(reopened); Check.True(f.Overlay.IsOpen); Check.Same(replacement, f.Overlay.CurrentPlan);
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        {
            _native = native;
            Manager = new() { Width = 1000, Height = 640, FloatingWindowMode = native ? FloatingWindowMode.Native : FloatingWindowMode.InSurface, Theme = new FluentTheme(ElementTheme.Light) };
            ...
            if (tools)
            {
                Source = Enumerable.Range(0, 2).Select(i => (LayoutContent)new LayoutAnchorable { Title = "Tool " + i, ContentId = "tool" + i,
                    CanDockAsTabbedDocument = true, Content = new TextBox { Text = "Retained tool draft " + i },
                    FloatingLeft = 150, FloatingTop = 140, FloatingWidth = 420, FloatingHeight = 300 }).ToArray();
            ...
            {
                var document = new LayoutDocument { Title = "Floating document", ContentId = "floating", Content = new TextBox { Text = "Retained document draft" },
                    FloatingLeft = 150, FloatingTop = 140, FloatingWidth = 420, FloatingHeight = 300 };
            ...
            _window = new() { Content = Manager, Title = "UnoDock drag cleanup acceptance" };
            _registration = Microsoft.Windows.Shell.SystemCommands.RegisterWindow(_window);
            _window.AppWindow.Resize(new() { Width = 1100, Height = 780 }); _window.Activate();
        }
        internal async Task Show()
        {
            ...
            Manager.UpdateLayout(); Control.UpdateLayout(); await Task.Delay(40);
        }
        ...
        internal long BeginAndPaint()
        {
            ...
            if (_native) Call(Control, "StartDragClock");
            ...
            var brush = new SolidColorBrush(Microsoft.UI.Colors.CornflowerBlue);
            if (!Overlay.IsOpen) Overlay.ShowPreview(plan, brush);
            if (!SourceOverlay.IsOpen) SourceOverlay.ShowPreview(plan, brush);
            Check.True(Control.IsDragging); Check.True(Overlay.IsOpen); Check.True(SourceOverlay.IsOpen);
            return generation;
        }
        internal void End(long generation, bool commit)
        {
            if (commit) Call(Surface, "CompleteFloatingDrag", Control, generation, Center(), false);
            else Call(Surface, "CancelDrag");
        }
        internal void AssertIdle()
        {
            ...
            AssertHidden(Overlay); AssertHidden(SourceOverlay);
        }
        public void Dispose()
        {
            try { Manager.Dispose(); }
    ...
    {
        Check.False(overlay.IsOpen); Check.True(overlay.CurrentPlan == null); Check.Equal(0, overlay.DisplayedGuides.Count);
    ...
    private sealed class Callbacks : IDisposable
    {
        ...
        internal void Add(DependencyObject owner, DependencyProperty property, Action callback) =>
            _tokens.Add((owner, property, owner.RegisterPropertyChangedCallback(property, (_, _) => callback())));
        public void Dispose()
        { foreach (var entry in _tokens) entry.Owner.UnregisterPropertyChangedCallback(entry.Property, entry.Token); _tokens.Clear(); }
    }
    ...
    private static T Get<T>(object owner, string name)
    {
        for (var type = owner.GetType(); type != null; type = type.BaseType)
            if (type.GetField(name, BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly) is { } field) return (T)field.GetValue(owner)!;
    ...
    private static object? Call(object owner, string name, params object?[] args)
    {
        for (var type = owner.GetType(); type != null; type = type.BaseType)
        {
            var method = type.GetMethods(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.DeclaredOnly)
    ...
    { for (var i = 0; i < 100 && !ready(); i++) await Task.Delay(20); Check.True(ready(), "Drag cleanup fixture did not become ready."); }
}
