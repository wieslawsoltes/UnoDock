...
internal static class DesktopFloatingTests
{
    internal static async Task<int> Run(string output)
    {
        ...
        )
        {
            foreach (var edge in Enumerable.Range(0, 4))
                Add($"tool subtree survives root edge {edge}", native, true, async f =>
                {
                    ...
                    var docked = new List<LayoutContent>();
                    var preview = new List<LayoutContent>();
                    ...
                    Check.Equal(3, preview.Count);
                    Check.Equal(3, docked.Count);
                    Check.True(preview.SequenceEqual(f.Source));
                    Check.True(docked.SequenceEqual(f.Source));
                    Check.Same(f.Manager.Layout, group.Root);
                    for (var i = 0; i < parents.Length; i++)
                        Check.Same(parents[i], f.Source[i].Parent);
            ...
            Add("document docks into the existing document pane", native, false, async f =>
            {
                ...
                Check.True(session.Execute(plan));
                Check.Same(f.Documents, f.Source[0].Parent);
                Check.Same(editor, f.Source[0].Content);
                await Task.CompletedTask;
            });
            Add("second-tab veto precedes every library mutation", native, true, async f =>
            {
                ...
                var calls = 0;
                var docked = 0;
                f.Manager.PreviewDock += (_, e) =>
                {
                    calls++;
                    if (ReferenceEquals(((DockEventArgs)e).Content, f.Source[1]))
                        ((DockEventArgs)e).Cancel = true;
                };
                f.Manager.Docked += (_, _) => docked++;
                using var session = f.Session();
                Check.False(session.Execute(f.InsidePlan()));
                Check.Equal(2, calls);
                Check.Equal(0, docked);
                for (var i = 0; i < parents.Length; i++)
                    Check.Same(parents[i], f.Source[i].Parent);
            ...
            foreach (var mutation in new[]
            {
                ...
                Add("preflight revocation: " + mutation, native, true, async f =>
                {
                    ...
                    RoutedEventHandler callback = (_, _) =>
                    {
                        ...
                        {
                            case "enabled ABA":
                                f.Source[1].IsEnabled = false;
                                f.Source[1].IsEnabled = true;
                                break;
                            case "parent ABA":
                                var pane = (LayoutAnchorablePane)f.Source[1].Parent!;
                                pane.Children.Remove((LayoutAnchorable)f.Source[1]);
                                pane.Children.Add((LayoutAnchorable)f.Source[1]);
                                break;
                            case "target children ABA":
                                var tool = f.Tools.Children[0];
                                f.Tools.Children.Remove(tool);
                                f.Tools.Children.Add(tool);
                                break;
                            case "root ABA":
                                var root = f.Manager.Layout;
                                f.Manager.Layout = new();
                                f.Manager.Layout = root;
                                break;
                            default:
                                throw new InvalidOperationException("application preview failure");
                        }
                    };
                    ...
                    if (mutation == "throwing preview")
                    {
                        ...
                        f.Manager.PreviewDock -= callback;
                        using var retry = f.Session();
                        Check.True(retry.Execute(plan));
                    }
                    else
                    {
                        Check.False(session.Execute(plan));
            ...
            {
                var host = f.Control.NativeWindow;
                var editors = f.Source.Select(c => c.Content).ToArray();
                ...
                {
                    var theme = new FluentTheme(mode);
                    f.Manager.Theme = theme;
                    f.Manager.Refresh();
                    await Task.Delay(50);
                    Check.Same(host, f.Control.NativeWindow);
                    var frame = Field<Grid>(f.Control, "_frame");
                    var caption = Field<TextBlock>(f.Control, "_caption");
                ...
                await VisualCapture.Save(f.Control, path);
            });
        }
        ...
        )
        {
            Add("native origin observes a synchronous move without relying on cached notifications", true, tools, async f =>
            {
                ...
                Check.Near(target.X, actual.X, 1);
                Check.Near(target.Y, actual.Y, 1);
                ...
                Check.Near(origin.X, actual.X, 1);
                Check.Near(origin.Y, actual.Y, 1);
                await Task.CompletedTask;
            });
            Add("native coordinate round-trip and source-window stacking exclusion", true, tools, async f =>
            {
                ...
                )
                {
                    ...
                    Check.Near(point.X, roundTrip.X, 1.1);
                    Check.Near(point.Y, roundTrip.Y, 1.1);
                    ...
                    Check.Near(point.X, reversed.X, 1.1);
                    Check.Near(point.Y, reversed.Y, 1.1);
                }
                ...
                foreach (var cancel in new[]
                {
                    ...
                    Add("XTEST: native caption " + (cancel ? "Escape restores geometry" : "moves and docks on release"), true, tools, async f =>
                    {
                        ...
                        if (cancel)
                        {
                            ...
                            await Wait(() => input.ScreenPoint(f.Control, new(0, 0)) == origin && f.Source.All(c => c.FloatingLeft == modelOrigin.X && c.FloatingTop == modelOrigin.Y));
                            await Task.Delay(80);
        ...
        void Add(string name, bool native, bool tools, Func<Fixture, Task> body) => tests.Test($"{(native ? "native" : "surface")}/{(tools ? "tools" : "document")}: {name}", async () =>
        {
            using var fixture = new Fixture(native, tools);
            await fixture.Show();
            await body(fixture);
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        internal readonly LayoutAnchorablePane Tools = new(new LayoutAnchorable { Title = "Existing tool", ContentId = "existing", Content = new TextBox { Text = "Existing tool editor" } })
        {
            DockWidth = new(220)
        };
        ...
        internal Fixture(bool native, bool tools)
        {
            ...
            if (tools)
            {
                Source = Enumerable.Range(0, 3).Select(i => (LayoutContent)new LayoutAnchorable { Title = "Tool " + i, ContentId = "tool" + i, Content = new TextBox { Text = "Unsaved buffer " + i, AcceptsReturn = true }, FloatingLeft = 180, FloatingTop = 160, FloatingWidth = 480, FloatingHeight = 320 }).ToArray();
                var first = new LayoutAnchorablePane((LayoutAnchorable)Source[0]);
                first.Children.Add((LayoutAnchorable)Source[1]);
                Group = new()
                {
                    Orientation = Orientation.Vertical
                };
                Group.Children.Add(first);
                Group.Children.Add(new LayoutAnchorablePane((LayoutAnchorable)Source[2]));
        ...
        internal async Task Show()
        {
            ...
            Control.MessageFilterFailed += (_, error) => NativeFailures.Add(error);
