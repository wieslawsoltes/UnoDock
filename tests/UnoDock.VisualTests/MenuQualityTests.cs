    ...
    public static async Task<int> Run(DockingManager templateSource, string output)
    {
        ...
        try
        {
            ...
            foreach (var scenario in observations.Root!.Elements("Scenario"))
            {
                ...
                Add("original public menu order, labels, states and row geometry: " + name, async () =>
                {
                    ...
                    var rows = menu.Items.OfType<MenuFlyoutItem>().ToArray();
                    var expected = scenario.Elements("Item").ToArray();
                    Check.Equal(expected.Length, rows.Length);
                    Check.Equal(rows.Length, menu.Items.Count);
                    for (var i = 0; i < rows.Length; i++)
                    {
                        ...
                        if (row.Visibility == Visibility.Visible)
                            Check.Near((double)data.Attribute("height")!, row.ActualHeight, .15);
                    }
            ...
            Add("one item retains menu and all row identities through 100 refreshes", async () =>
            {
                ...
                await Settle();
                Check.Same(menu, target.ContextFlyout);
                for (var i = 0; i < rows.Length; i++)
                    Check.Same(rows[i], menu.Items[i]);
            });
            Add("document and tool own distinct action sets with no separators", () =>
            {
                var documents = Menu(doc);
                var tools = Menu(tool);
                Check.False(ReferenceEquals(documents, tools));
                Check.Equal(9, documents.Items.Count);
                Check.Equal(6, tools.Items.Count);
            ...
            {
                var menu = await Open(doc);
                var row = Row(menu, "FloatCommand");
                var presenter = Presenter();
                doc.CanFloat = false;
                await Settle();
                Check.False(row.IsEnabled);
                Check.Same(presenter, Presenter());
                doc.CanFloat = true;
                await Settle();
                Check.True(row.IsEnabled);
                Check.Same(row, Row(Menu(doc), "FloatCommand"));
            });
            ...
            Add("command subscriptions exist only during the opening session", async () =>
            {
                ...
                dock.GetLayoutItemFromModel(doc).FloatCommand = command;
                var menu = Menu(doc);
                Check.Equal(0, command.Subscriptions);
                for (var i = 0; i < 3; i++)
                {
                    await Open(doc);
                    Check.Equal(1, command.Subscriptions);
                    menu.Hide();
                    await Settle();
                    Check.Equal(0, command.Subscriptions);
                }
            ...
            {
                var old = new TestCommand();
                var next = new TestCommand();
                var item = dock.GetLayoutItemFromModel(doc);
                item.FloatCommand = old;
                var menu = await Open(doc);
                var row = Row(menu, "FloatCommand");
                item.FloatCommand = next;
                await Settle();
                Check.Equal(0, old.Subscriptions);
                Check.Equal(1, next.Subscriptions);
                row.Command.Execute(null);
                Check.Equal(0, old.Executions);
                Check.Equal(1, next.Executions);
                menu.Hide();
                await Settle();
                Check.Equal(0, next.Subscriptions);
            });
            Add("worker command notifications marshal to UI and coalesce a burst", async () =>
            {
                ...
                await Settle();
                Check.False(Row(menu, "FloatCommand").IsEnabled);
                Check.True(command.Queries - before < 40, "Command burst caused an unbounded UI refresh storm.");
                menu.Hide();
                await Settle();
                var closed = command.Queries;
                await Task.Run(command.Raise);
                await Task.Delay(90);
                Check.Equal(closed, command.Queries);
            });
            Add("queued notifications cannot act after root replacement", async () =>
            {
                ...
                var menu = await Open(doc);
                var row = Row(menu, "FloatCommand");
                var wrapper = row.Command;
                command.Raise();
                dock.Layout = new();
                await Settle();
                Check.Equal(0, command.Subscriptions);
                Check.False(wrapper.CanExecute(null));
                wrapper.Execute(null);
                Check.Equal(0, command.Executions);
            });
            Add("retained menu cannot execute on a document moved to another root", () =>
            {
                var menu = Menu(doc);
                var row = Row(menu, "CloseCommand");
                var replacement = new LayoutDocumentPane();
                var foreign = new LayoutRoot
                {
                    RootPanel = new(replacement)
                };
                pane.Children.Remove(doc);
                replacement.Children.Add(doc);
                row.Command.Execute(null);
                Check.Same(foreign, doc.Root);
                Check.False(row.Command.CanExecute(null));
                return Task.CompletedTask;
            });
            Add("CanExecute replacement cannot execute a superseded command", () =>
            {
                ...
                Row(menu, "FloatCommand").Command.Execute(null);
                Check.Equal(0, old.Executions);
                Check.Equal(0, next.Executions);
                Row(menu, "FloatCommand").Command.Execute(null);
                Check.Equal(1, next.Executions);
                return Task.CompletedTask;
            });
            ...
            Add("application command exception is propagated without disabling future invocations", () =>
            {
                ...
                var row = Row(menu, "FloatCommand");
                Check.Equal("application-command", Check.Throws<InvalidOperationException>(() => row.Command.Execute(null)).Message);
                command.Action = null;
                row.Command.Execute(null);
                Check.Equal(2, command.Executions);
                return Task.CompletedTask;
            });
            Add("custom menu takeover ends subscriptions and never changes its template or command", async () =>
            {
                var command = new TestCommand();
                dock.GetLayoutItemFromModel(doc).FloatCommand = command;
                var old = await Open(doc);
                var customCommand = new TestCommand();
                var custom = new MenuFlyout();
                var customRow = new MenuFlyoutItem
                {
                    Text = "Application",
                    Command = customCommand
                };
                custom.Items.Add(customRow);
                var template = customRow.Template;
                dock.DocumentContextMenu = custom;
                await Settle();
                Check.Same(custom, Menu(doc));
                Check.Same(template, customRow.Template);
                Check.Same(customCommand, customRow.Command);
                Check.Equal(0, command.Subscriptions);
                Check.False(Row(old, "FloatCommand").Command.CanExecute(null));
                dock.DocumentContextMenu = null;
                await Settle();
                Check.Same(old, Menu(doc));
            });
            Add("shared custom menu context follows the current header and restores local overrides", async () =>
            {
                ...
                menu.Items.Add(overridden);
                dock.DocumentContextMenu = menu;
                await Settle();
                await Open(doc);
                Check.Same(dock.GetLayoutItemFromModel(doc), automatic.DataContext);
                Check.Same(own, overridden.DataContext);
                menu.Hide();
                await Settle();
                Check.Same(DependencyProperty.UnsetValue, automatic.ReadLocalValue(FrameworkElement.DataContextProperty));
                await Open(other);
                Check.Same(dock.GetLayoutItemFromModel(other), automatic.DataContext);
                Check.Same(own, overridden.DataContext);
            });
            Add("auto-hide rail custom menu receives tool adapter context", async () =>
            {
                ...
                await Settle();
                await Open(tool);
                Check.Same(dock.GetLayoutItemFromModel(tool), row.DataContext);
                menu.Hide();
                await Settle();
            ...
            Add("localization updates retained row labels at reopening", async () =>
            {
                ...
                try
                {
                    Strings.Translations["zz"] = new Dictionary<string, string>
                    {
                        ["Document_Close"] = "Independent translated close"
                    };
                    Strings.Culture = CultureInfo.GetCultureInfo("zz");
                    await Open(doc);
                    Check.Same(row, Row(menu, "CloseCommand"));
                    Check.Equal("Independent translated close", row.Text);
                }
                finally
                {
                    Strings.Culture = culture;
                    Strings.Translations.Remove("zz");
                }
            });
            Add("bulk close respects protected and cancelled documents", () =>
            {
                var cancelled = new LayoutDocument();
                pane.Children.Add(cancelled);
                cancelled.Closing += (_, e) => e.Cancel = true;
                other.CanClose = false;
                dock.GetLayoutItemFromModel(doc).CloseAllCommand!.Execute(null);
                Check.True(doc.Parent == null);
                Check.Same(pane, other.Parent);
                Check.Same(pane, cancelled.Parent);
                return Task.CompletedTask;
            });
            Add("bulk close from another adapter cannot recursively enter the same root", () =>
            {
                var initiator = dock.GetLayoutItemFromModel(doc);
                var second = dock.GetLayoutItemFromModel(other);
                var calls = 0;
                doc.Closing += (_, _) =>
                {
                    calls++;
                    second.CloseAllCommand!.Execute(null);
                };
                initiator.CloseAllCommand!.Execute(null);
                Check.Equal(1, calls);
                Check.True(doc.Parent == null);
                Check.True(other.Parent == null);
                return Task.CompletedTask;
            });
            Add("bulk close stops on root replacement without closing old or new remainder", () =>
            {
                var replacement = new LayoutDocument();
                var next = new LayoutRoot
                {
                    RootPanel = new(new LayoutDocumentPane(replacement))
                };
                doc.Closing += (_, _) => dock.Layout = next;
                dock.GetLayoutItemFromModel(doc).CloseAllCommand!.Execute(null);
                Check.Same(next, dock.Layout);
                Check.True(other.Parent != null);
                Check.True(replacement.Parent != null);
                return Task.CompletedTask;
            });
            Add("bulk close does not close a remainder moved to another workspace", () =>
            {
                ...
                var foreign = new LayoutRoot
                {
                    RootPanel = new(destination)
                };
                doc.Closing += (_, _) =>
                {
                    pane.Children.Remove(other);
                    destination.Children.Add(other);
                };
                dock.GetLayoutItemFromModel(doc).CloseAllCommand!.Execute(null);
                Check.Same(foreign, other.Root);
                return Task.CompletedTask;
            });
            Add("bulk close exception releases its reentrancy guard", () =>
            {
                EventHandler<CancelEventArgs> callback = (_, _) => throw new InvalidOperationException("close-observer");
                doc.Closing += callback;
                var command = dock.GetLayoutItemFromModel(doc).CloseAllCommand!;
                Check.Throws<InvalidOperationException>(() => command.Execute(null));
                doc.Closing -= callback;
                Check.True(command.CanExecute(null));
            ...
            {
                var protectedDoc = new LayoutDocument
                {
                    CanClose = false
                };
                pane.Children.Add(protectedDoc);
                dock.GetLayoutItemFromModel(doc).CloseAllButThisCommand!.Execute(null);
                Check.Same(pane, doc.Parent);
                Check.Same(pane, protectedDoc.Parent);
                Check.True(other.Parent == null);
                return Task.CompletedTask;
            });
            Add("group navigation follows document-pane-group siblings not unrelated root panes", async () =>
            {
                var next = new LayoutDocumentPane(new LayoutDocument());
                var group = new LayoutDocumentPaneGroup(pane);
                group.Children.Add(next);
                dock.Layout.RootPanel.Children.Add(group);
                await Settle();
                var menu = Menu(doc);
                Check.True(Row(menu, "MoveToNextTabGroupCommand").IsEnabled);
                Row(menu, "MoveToNextTabGroupCommand").Command.Execute(null);
                await Settle();
                Check.Same(next, doc.Parent);
            ...
            foreach (var dark in new[]
            {
                ...
                foreach (var explicitTheme in new[]
                {
                    ...
                    Add($"coherent menu foreground/background: dark={dark}, explicit={explicitTheme}", async () =>
                    {
                        ...
                        if (explicitTheme)
                            dock.Theme = new FluentTheme(!dark ? ElementTheme.Dark : ElementTheme.Light);
                        await Settle();
                        var menu = await Open(doc);
            ...
            Add("menu palette and density overrides apply without replacing rows", async () =>
            {
                var menu = Menu(doc);
                var row = Row(menu, "CloseCommand");
                var brush = new SolidColorBrush(Microsoft.UI.Colors.DarkBlue);
                dock.Resources["UnoDock.MenuBrush"] = brush;
                dock.Resources["UnoDock.MenuRowHeight"] = 34d;
                dock.Resources["UnoDock.MenuMinWidth"] = 300d;
                await Settle();
                await Open(doc);
                Check.Same(brush, Presenter().Background);
                Check.Same(row, Row(menu, "CloseCommand"));
                Check.Near(34, row.ActualHeight, .15);
                Check.True(Presenter().ActualWidth >= 300);
            });
            Add("large fonts expand row geometry instead of clipping", async () =>
            {
                dock.Resources["UnoDock.FontSize"] = 24d;
                await Settle();
                var menu = await Open(doc);
                foreach (var row in menu.Items.OfType<MenuFlyoutItem>().Where(r => r.Visibility == Visibility.Visible))
                {
                    Check.True(row.ActualHeight >= 38);
                    Check.Near(24, row.FontSize);
                    Check.True(row.FindVisualChildren<TextBlock>().Single(t => t.Text == row.Text).ActualHeight > 20);
                }
            });
            Add("real menu items retain native Invoke automation and matching names", async () =>
            {
                ...
                var menu = await Open(doc);
                var row = Row(menu, "FloatCommand");
                Check.Equal(row.Text, AutomationProperties.GetName(row));
            ...
            foreach (var name in new[]
            {
                ...
                Add("capture compact native menu: " + name, async () =>
                {
                    ...
                    if (name == "large-font")
                        dock.Resources["UnoDock.FontSize"] = 22d;
                    await Settle();
                    var menu = await Open(name.StartsWith("tool", StringComparison.Ordinal) ? tool : doc);
                    var presenter = Presenter();
                    await VisualCapture.Save(presenter, System.IO.Path.Combine(output, "visuals", "menu-" + name + ".png"));
                    new XDocument(new XElement("Menu", new XAttribute("name", name), new XAttribute("width", presenter.ActualWidth), new XAttribute("height", presenter.ActualHeight), menu.Items.OfType<MenuFlyoutItem>().Select(r => new XElement("Row", new XAttribute("command", r.Tag!), new XAttribute("text", r.Text), new XAttribute("enabled", r.IsEnabled), new XAttribute("visibility", r.Visibility), new XAttribute("height", r.ActualHeight))))).Save(System.IO.Path.Combine(output, "visuals", "menu-" + name + ".xml"));
                }, reset: false);
            if (OperatingSystem.IsLinux() && Environment.GetEnvironmentVariable("UNODOCK_NATIVE_INPUT_TESTS") == "1")
            {
                ...
                Add("XTEST Escape dismisses menu without executing and releases observers", async () =>
                {
                    ...
                    dock.GetLayoutItemFromModel(doc).FloatCommand = command;
                    var menu = await Open(doc);
                    Row(menu, "FloatCommand").Focus(FocusState.Keyboard);
                    using var input = new X11TestInput();
                    input.Escape();
                    await Settle();
                    Check.Equal(0, command.Executions);
                ...
                Add("XTEST Enter invokes focused native menu item", async () =>
                {
                    ...
                    dock.GetLayoutItemFromModel(doc).FloatCommand = command;
                    var menu = await Open(doc);
                    Check.True(Row(menu, "FloatCommand").Focus(FocusState.Keyboard));
                    using var input = new X11TestInput();
                    input.KeyPress(0xff0d);
                    await Settle();
                    Check.Equal(1, command.Executions);
                });
            }
        ...
        async Task Reset(string name = "document")
        {
            ...
            foreach (var key in new[]
            {
                ...
                dock.Resources.Remove("UnoDock." + key);
            ...
            pane = new(doc);
            if (name != "document-single")
                pane.Children.Add(other);
            tool = new()
            {
                ContentId = "tool",
                ...
                {
                    Text = "Owned tool content"
                }
            };
            var panel = new LayoutPanel(new LayoutAnchorablePane(tool) { DockWidth = new(200) });
            panel.Children.Add(pane);
            if (name == "document-multiple-groups")
                panel.Children.Add(new LayoutDocumentPane(new LayoutDocument { ContentId = "third" }));
            dock.Layout = new()
            {
                RootPanel = panel
            };
            if (name == "document-no-close")
                doc.CanClose = false;
            if (name == "document-no-float")
                doc.CanFloat = false;
            if (name == "tool-no-hide")
                tool.CanHide = false;
            if (name == "tool-no-autohide")
                tool.CanAutoHide = false;
            if (name == "tool-no-document")
                tool.CanDockAsTabbedDocument = false;
            if (name == "tool-autohidden")
                tool.ToggleAutoHide();
            await Settle();
        }
        ...
        FrameworkElement Target(LayoutContent model) => model is LayoutAnchorable { IsAutoHidden: true } ? dock.FindVisualChildren<LayoutAnchorControl>().Single(c => ReferenceEquals(c.Model, model)) : model is LayoutAnchorable ? dock.FindVisualChildren<LayoutAnchorablePaneControl>().First(c => ReferenceEquals(c.Model, model.Parent)) : dock.FindVisualChildren<LayoutTabItemBase>().First(c => ReferenceEquals(c.Model, model));
        MenuFlyout Menu(LayoutContent model) => (MenuFlyout)Target(model).ContextFlyout;
        async Task<MenuFlyout> Open(LayoutContent model)
        {
            var menu = Menu(model);
            opened = menu;
            menu.ShowAt(Target(model));
            await Settle();
            return menu;
        }
