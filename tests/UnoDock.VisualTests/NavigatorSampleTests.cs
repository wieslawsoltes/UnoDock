...
internal static class NavigatorSampleTests
{
    internal static async Task<int> Run(string output)
    {
        ...
        Add("navigator sample: compact real controls and command status are realized", async (page, panel, manager) =>
        {
            ...
            await VisualCapture.Save(panel, path);
        });
        Add("navigator sample: command rows wrap without clipping or replacing native controls", async (_, panel, manager) =>
        {
            var buttons = panel.FindVisualChildren<SampleButton>().ToArray();
            var originalCommands = manager.Layout.Descendents().OfType<LayoutContent>().Select(model => manager.GetLayoutItemFromModel(model).ActivateCommand).ToArray();
            ...
            )
            {
                ...
                var rectangles = buttons.Select(button => button.TransformToVisual(panel).TransformBounds(new Windows.Foundation.Rect(0, 0, button.ActualWidth, button.ActualHeight))).ToArray();
                foreach (var rect in rectangles)
                {
                    ...
                    Check.True(rect.Top >= 0 && rect.Bottom <= manager.TransformToVisual(panel).TransformPoint(new()).Y, "A command overlaps the drawing workspace.");
                }
                ...
                for (var i = 0; i < rectangles.Length; i++)
                    for (var j = i + 1; j < rectangles.Length; j++)
                    {
                        var a = rectangles[i];
                        var b = rectangles[j];
                        Check.True(a.Right <= b.Left + .25 || b.Right <= a.Left + .25 || a.Bottom <= b.Top + .25 || b.Bottom <= a.Top + .25, "Wrapped command bounds overlap.");
                    }
                ...
                Check.True(originalCommands.SequenceEqual(manager.Layout.Descendents().OfType<LayoutContent>().Select(model => manager.GetLayoutItemFromModel(model).ActivateCommand)));
        ...
        Add("navigator sample: property buttons retain a vetoed selection then hide documents and close tools", async (_, panel, manager) =>
        {
            ...
            Check.Equal(0, closing);
            Check.Equal(0, closed);
            Check.Same(target, manager.Layout.ActiveContent);
            Check.True(Status(panel).Contains("Committed: 1", StringComparison.Ordinal));
            await ReadyEditor(panel, target);
            await Invoke("open");
            ...
            async Task Invoke(string id)
            {
                ...
                var peer = FrameworkElementAutomationPeer.CreatePeerForElement(button) ?? throw new InvalidOperationException("Sample button has no native automation peer.");
                var provider = peer.GetPattern(PatternInterface.Invoke) as IInvokeProvider ?? throw new InvalidOperationException("Sample button does not expose Invoke.");
                provider.Invoke();
                await Task.Delay(80);
                panel.UpdateLayout();
            }
        ...
        Add("navigator sample: command activation retains the realized editor and unsaved buffer", async (_, panel, manager) =>
        {
            ...
            await ReadyEditor(panel, first);
            Check.Same(editor, first.Content);
            Check.Equal(draft, editor.Text);
            Check.True(Status(panel).Contains("Committed: 2", StringComparison.Ordinal));
        });
        ...
        if (OperatingSystem.IsLinux() && Environment.GetEnvironmentVariable("UNODOCK_NATIVE_INPUT_TESTS") == "1")
            Add("XTEST: navigator sample policy vetoes and then allows an actual Enter commit", async (_, panel, manager) =>
            {
                ...
                await Wait(() => Current(manager) == null);
                Check.Same(original, manager.Layout.ActiveContent);
                Check.True(Status(panel).Contains("Committed: 0", StringComparison.Ordinal));
                await Click("policy");
                await Click("open");
                await Wait(() => Current(manager)is { IsLoaded: true, ActualHeight: > 0 });
    ...
    private static async Task ReadyEditor(Grid panel, LayoutContent model)
    {
        ...
        panel.UpdateLayout();
        await Task.Delay(50);
        Check.True(panel.FindVisualChildren<TextBox>().Any(view => ReferenceEquals(view, editor)), "Active model editor is not attached to the actual sample tree.");
