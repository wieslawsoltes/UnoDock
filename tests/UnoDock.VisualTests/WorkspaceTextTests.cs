...
internal static class WorkspaceTextTests
{
    internal static void Register(TestRunner tests)
    {
        ...
        foreach (var fixture in fixtures)
        {
            ...
            {
                var document = Document(fixture.Text); var notifications = 0;
                ...
                Check.Equal(0, notifications); Check.False(document.IsDirty);
                Check.Same(fixture.Text, document.Text); Check.Same(projection, document.EditorText);
                Check.Equal(fixture.Text.Length, document.CharacterCount); Check.Equal(fixture.Lines, document.LineCount);
            });
        }
        foreach (var delimiter in new[] { "\n", "\r\n", "\r" })
        {
            ...
            tests.Test("text projection: inserted lines use first original delimiter: " + label, () =>
            {
                ...
                Check.Equal("first" + delimiter + "new" + delimiter + "last",
                    WorkspaceTextProjection.ApplyEditorEdit(original, "first\rnew\rlast"));
            });
        ...
        tests.Test("text projection: UTF16 edits preserve surrounding surrogate pairs and accents", () =>
        {
            ...
            Check.Equal("\U0001F642\r\n\u03A9 e\u0301!\n\U0001F680",
                WorkspaceTextProjection.ApplyEditorEdit(original, "\U0001F642\r\u03A9 e\u0301!\r\U0001F680"));
        });
        tests.Test("text projection: save baseline and revert retain exact mixed text", () =>
        {
            var document = Document("a\r\nb\nc\r");
            document.EditorText = "a\rb+\rc\r"; var saved = document.Text;
            Check.Equal("a\r\nb+\nc\r", saved); Check.True(document.IsDirty);
            document.AcceptSaved(saved); Check.False(document.IsDirty);
            document.EditorText += "later"; Check.True(document.IsDirty);
            document.Revert(); Check.Same(saved, document.Text); Check.False(document.IsDirty);
        });
        tests.Test("text projection: cache invalidates before model observers run", () =>
        {
            var document = Document("old\ntext"); _ = document.EditorText;
            string? observed = null;
            document.PropertyChanged += (_, e) => { if (e.PropertyName == nameof(WorkspaceDocument.Text)) observed = document.EditorText; };
            document.Text = "new\r\ntext";
            Check.Equal("new\rtext", observed); Check.Equal("new\rtext", document.EditorText);
        });
        tests.Test("text projection: deterministic 10000 edit sequences preserve visible content and no-op identity", () =>
        {
            ...
            var alphabet = new[] { "a", "b", " ", "\t", "\u03A9", "\U0001F642", "\r", "\n", "\r\n" };
            for (var iteration = 0; iteration < 10000; iteration++)
            {
                var builder = new StringBuilder();
                for (var i = 0; i < random.Next(1, 70); i++) builder.Append(alphabet[random.Next(alphabet.Length)]);
                var original = builder.ToString(); var before = Canonical(original);
                var start = random.Next(before.Length + 1); var end = random.Next(start, before.Length + 1);
                var edited = before[..start] + alphabet[random.Next(alphabet.Length)] + before[end..];
                var expected = Canonical(edited); var result = WorkspaceTextProjection.ApplyEditorEdit(original, edited);
                ...
                if (before == expected) Check.Same(original, result);
            }
        });
    ...
    private static string Canonical(string input)
    {
        ...
        {
            var c = input[i];
            if (c == '\r') { result.Append('\r'); if (i + 1 < input.Length && input[i + 1] == '\n') i++; }
