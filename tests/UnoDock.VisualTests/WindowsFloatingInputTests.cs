...
internal static class WindowsFloatingInputTests
{
    ...
    {
        if (!OperatingSystem.IsWindows() || Environment.GetEnvironmentVariable("UNODOCK_NATIVE_INPUT_TESTS") != "1")
            return 0;
        ...
        )
        {
            foreach (var outcome in new[]
            {
                ...
                tests.Test($"SendInput: {(tools ? "tool group" : "document")} client caption {outcome}", async () =>
                {
                    ...
                    await f.Show();
                    using var input = new NativeInput();
                    ...
                    if (outcome == "cancel")
                    {
                        ...
                        input.Release();
                        await Task.Delay(80);
                        Check.Equal(origin.X, window.AppWindow.Position.X);
                        Check.Equal(origin.Y, window.AppWindow.Position.Y);
                    }
                    else
                    {
                        input.Release();
            ...
            tests.Test($"SendInput: {(tools ? "tool group" : "document")} actual OS title bar docks after WM_EXITSIZEMOVE", async () =>
            {
                ...
                using var f = new Fixture(tools, systemTitleBar: true);
                await f.Show();
                using var input = new NativeInput();
                var native = f.Control.NativeWindow!;
                input.Focus(native);
                ...
                var started = 0;
                var token = f.Control.RegisterPropertyChangedCallback(LayoutFloatingWindowControl.IsDraggingProperty, (_, _) =>
                {
                    if (f.Control.IsDragging)
                        started++;
                });
                try
                {
                ...
                {
                    f.Control.UnregisterPropertyChangedCallback(LayoutFloatingWindowControl.IsDraggingProperty, token);
                }
            });
        }
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        internal Fixture(bool tools, bool systemTitleBar = false)
        {
            ...
            if (tools)
            {
                Source = Enumerable.Range(0, 3).Select(i => (LayoutContent)new LayoutAnchorable { Title = "Floating tool " + i, ContentId = "tool" + i, CanDockAsTabbedDocument = true, Content = new TextBox { Text = "Retained draft " + i }, FloatingLeft = 150, FloatingTop = 140, FloatingWidth = 480, FloatingHeight = 320 }).ToArray();
        ...
        internal async Task Show()
        {
            ...
            Control.MessageFilterFailed += (_, e) => Failures.Add(e);
            ...
            await Wait(() => Target.ActualWidth > 0 && Target.ActualHeight > 0);
            await Task.Delay(100);
        }
    ...
    internal sealed class NativeInput : IDisposable
    {
        ...
        {
            if (!OperatingSystem.IsWindows() || Environment.GetEnvironmentVariable("UNODOCK_NATIVE_INPUT_TESTS") != "1")
                throw new InvalidOperationException("Dedicated Windows input must be explicitly enabled.");
            if (!GetCursorPos(out _initial))
        ...
        {
            if (!SetCursorPos(checked((int)Math.Round(point.X)), checked((int)Math.Round(point.Y))))
                throw new Win32Exception(Marshal.GetLastPInvokeError());
        }
        ...
        internal void KeyUp(ushort key)
        {
            if (!_keys.Remove(key))
                return;
            Send(new()
        ...
        public void Dispose()
        {
            ...
            finally
            {
                foreach (var key in _keys.ToArray())
                    KeyUp(key);
                SetCursorPos(_initial.X, _initial.Y);
            }
        }
    ...
    private static async Task Wait(Func<bool> ready)
    {
        for (var i = 0; i < 120 && !ready(); i++)
            await Task.Delay(25);
        Check.True(ready(), "The native input transition did not settle.");
    }
}
