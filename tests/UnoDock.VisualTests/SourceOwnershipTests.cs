...
public static class SourceOwnershipTests
{
    public static Task<int> Run(string output)
    {
        ...
        )
        {
            ...
            foreach (var property in new[]
            {
                ...
                Add("descriptor replacement stops stale publication: " + property, () =>
                {
                    ...
                    Check.Same(current, Models(manager, tools).Single().Content);
                    Check.Equal(1, descriptor.Callbacks);
                });
            Add("BeforeInsert replacement skips default placement and stale AfterInsert", () =>
            {
                ...
                manager.LayoutUpdateStrategy = new Strategy
                {
                    Before = (_, model, _) =>
                    {
                        if (ReferenceEquals(model.Content, stale))
                            Set(manager, new[] { current }, tools);
                        return false;
                    },
                    After = (_, model) => after.Add(model)
                };
                Set(manager, new[] { stale }, tools);
                Check.Equal(1, after.Count);
                Check.Same(current, after.Single().Content);
                Check.Same(current, Models(manager, tools).Single().Content);
            });
            Add("BeforeInsert same-source mutation retries retained value exactly once", () =>
            {
                ...
                Check.Equal(2, Models(manager, tools).Length);
                Check.Equal(2, after);
            ...
            {
                using var manager = new DockingManager();
                ILayoutContainer? expected = null;
                manager.LayoutUpdateStrategy = new Strategy
                {
                    Before = (root, model, _) =>
                    {
                        expected = Place(root, model);
                        return false;
                    }
                };
            ...
            Add("throwing AfterInsert keeps tracking so later removal cannot orphan a view", () =>
            {
                ...
                Set(manager, source, tools);
                Check.Throws<ApplicationException>(() => source.Add(value));
            ...
            Add("removal cannot detach a model transferred to another manager", () =>
            {
                ...
                Set(manager, source, tools);
                var model = Models(manager, tools).Single();
                var parent = Place(other.Layout, model);
                source.Remove(value);
                Check.Same(parent, model.Parent);
                Check.Same(other.Layout, model.Root);
            });
            ...
            Add("foreign direct model fails preflight without removing either source", () =>
            {
                ...
                Check.Same(original, Models(manager, tools).Single().Content);
                Check.Same(other.Layout, foreign.Root);
                Set(manager, null, tools);
                Check.Equal(0, Models(manager, tools).Length);
                Check.Same(other.Layout, foreign.Root);
            });
            ...
            {
                using var manager = new DockingManager();
                LayoutContent model = tools ? new LayoutAnchorable() : new LayoutDocument();
                var parent = Place(manager.Layout, model);
                var source = new ObservableCollection<object>
                {
                    model
                };
            ...
            Add("root replacement during descriptors never fills the detached old root", () =>
            {
                ...
                Check.Equal(1, Models(manager, tools).Length);
                Check.Same(descriptor, Models(manager, tools).Single().Content);
            });
            ...
            Add("non-converging callbacks are bounded and the manager remains recoverable", () =>
            {
                ...
                Check.Equal(1, Models(manager, tools).Length);
                Check.Same(source[0], Models(manager, tools).Single().Content);
            });
        ...
        tests.Test("source ownership: document enumeration replacing tool source never enumerates stale tools", () =>
        {
            ...
            Check.Same(doc, Models(manager, false).Single().Content);
            Check.Same(tool, Models(manager, true).Single().Content);
        });
        tests.Test("source ownership: both enumerations complete before either removal", () =>
        {
            ...
            manager.AnchorablesSource = new[]
            {
                tool
            };
            var batch = manager.BeginLayoutUpdate();
            manager.DocumentsSource = Array.Empty<object>();
    ...
    private static LayoutContent[] Models(DockingManager manager, bool tools) => manager.Layout.Descendents().OfType<LayoutContent>().Where(model => tools ? model is LayoutAnchorable : model is LayoutDocument).ToArray();
    private static ILayoutContainer Place(LayoutRoot root, LayoutContent model)
    {
        ...
        var tools = new LayoutAnchorablePane((LayoutAnchorable)model);
        root.RootPanel.Children.Add(tools);
        return tools;
    }
    ...
    private sealed class PoisonValue
    {
        ...
        {
            Calls++;
            throw new ApplicationException("stale value evaluated");
        }
    }
    ...
    private sealed class CallbackDescriptor(string property, Action callback) : IDockContent
    {
        ...
        public string Title
        {
            ...
            {
                Invoke("title");
                return property == "string" ? null! : "Title";
            }
        }
    ...
    private sealed class CallbackSequence(object[] values, Action? move = null, Action? current = null, Action? dispose = null) : IEnumerable
    {
        ...
        private bool Next(ref int index)
        {
            if (!_moved)
            {
                _moved = true;
