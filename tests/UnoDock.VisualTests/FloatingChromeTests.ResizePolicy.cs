...
internal static partial class FloatingChromeTests
{
    internal static async Task<int> RunResizePolicy(string output, bool tools)
    {
        ...
        tests.Test($"resize-policy/{kind}: RTL grips remain on physical native edges", async () =>
        {
            ...
            await f.Show();
            f.Control.FlowDirection = FlowDirection.RightToLeft;
            await Task.Delay(80);
            ...
            Check.Equal(FlowDirection.RightToLeft, f.Caption.FlowDirection);
            f.AssertEditors();
        });
        tests.Test($"resize-policy/{kind}: zero border sides cannot resize through their corners", async () =>
        {
            using var f = new Fixture(tools);
            await f.Show();
            f.Control.ResizeBorderThickness = new(0, 5, 5, 0);
            await Task.Delay(60);
            ...
            )
            {
                ...
                Check.True(Call(f.Control, "BeginFrameResize", edge, new Point(0, 0), null, null) == null, "An absent border still accepts a resize: " + edge);
            }
            ...
            {
                Check.Equal(Visibility.Visible, f.Grip(edge).Visibility);
                Check.True(BeginResize(f, edge) != null);
        ...
        tests.Test($"resize-policy/{kind}: a stationary resize does not clamp an existing oversized frame", async () =>
        {
            using var f = new Fixture(tools);
            await f.Show();
            f.Control.MaxWidth = 300;
            f.Control.MaxHeight = 220;
            await Task.Delay(60);
            var before = FloatingChromeProbe.Bounds(f.Native);
            var resize = BeginResize(f, ChromeHit.BottomRight);
            Call(f.Control, "MoveFrameResize", resize, new Point(0, 0));
            await Task.Delay(80);
            ...
            tests.Test($"resize-policy/{kind}: policy mutation revokes the capture permanently: {policy}", async () =>
            {
                ...
                switch (policy)
                {
                    case "MinWidth":
                        ...
                        f.Control.MinWidth = 160;
                        break;
                    case "MinHeight":
                        f.Control.MinHeight = 120;
                        f.Control.MinHeight = 100;
                        break;
                    case "MaxWidth":
                        f.Control.MaxWidth = 900;
                        f.Control.MaxWidth = double.PositiveInfinity;
                        break;
                    case "MaxHeight":
                        f.Control.MaxHeight = 900;
                        f.Control.MaxHeight = double.PositiveInfinity;
                        break;
                    case "FlowDirection":
                        f.Control.FlowDirection = FlowDirection.RightToLeft;
                        f.Control.FlowDirection = FlowDirection.LeftToRight;
                        break;
                    case "ResizeBorderThickness":
                        f.Control.ResizeBorderThickness = new(8);
                        f.Control.ResizeBorderThickness = new(5);
                        break;
                    case "IsEnabled":
                        f.Control.IsEnabled = false;
                        f.Control.IsEnabled = true;
                        break;
                    case "Manager.IsEnabled":
                ...
                await Task.Delay(80);
                Near(afterPolicy, FloatingChromeProbe.Bounds(f.Native));
                f.AssertEditors();
        ...
        foreach (var cancel in new[]
        {
            ...
            tests.Test($"resize-policy/{kind}: an application native move wins over resize, cancel={cancel}", async () =>
            {
                ...
                if (cancel)
                    ...
                    Call(f.Control, "MoveFrameResize", resize, new Point(120, 110));
                await Task.Delay(100);
                Check.False(f.Control.IsResizing);
                Near(applicationBounds, FloatingChromeProbe.Bounds(f.Native));
                f.AssertEditors();
            });
        tests.Test($"resize-policy/{kind}: source parent ABA cannot revive the old resize", async () =>
        {
            ...
            using (f.Manager.Layout.BeginUpdate())
            {
                ...
                {
                    var item = (LayoutAnchorable)f.Source[0];
                    pane.Children.Remove(item);
                    pane.Children.Insert(0, item);
                }
                else
                {
                    var model = (LayoutDocumentFloatingWindow)f.Control.Model;
                    var item = model.RootDocument;
                    model.RootDocument = null;
                    model.RootDocument = item;
                }
            }

            Check.False(f.Control.IsResizing);
            var before = FloatingChromeProbe.Bounds(f.Native);
            Call(f.Control, "MoveFrameResize", resize, new Point(200, 100));
            await Task.Delay(80);
            Near(before, FloatingChromeProbe.Bounds(f.Native));
            f.AssertEditors();
        });
        tests.Test($"resize-policy/{kind}: presenter replacement withdraws resize ownership", async () =>
        {
            ...
            await f.Show();
            var resize = BeginResize(f, ChromeHit.BottomRight);
            f.Native.AppWindow.SetPresenter(OverlappedPresenter.Create());
            await Task.Delay(80);
            var before = FloatingChromeProbe.Bounds(f.Native);
            Call(f.Control, "MoveFrameResize", resize, new Point(100, 90));
            await Task.Delay(100);
            Check.False(f.Control.IsResizing);
            Near(before, FloatingChromeProbe.Bounds(f.Native));
        });
        tests.Test($"resize-policy/{kind}: invalid begin and released sessions do not retain policy observers", async () =>
        {
            ...
            for (var i = 0; i < 8; i++)
            {
                var resize = BeginResize(f, ChromeHit.Right);
                Call(f.Control, "EndFrameResize", resize, false);
            }
            ...
            var next = BeginResize(f, ChromeHit.Bottom);
            var before = FloatingChromeProbe.Bounds(f.Native);
            Call(f.Control, "MoveFrameResize", next, new Point(0, 25));
            await Task.Delay(80);
            Near(before.Height + 25, FloatingChromeProbe.Bounds(f.Native).Height);
            Call(f.Control, "EndFrameResize", next, true);
            await Task.Delay(80);
            Near(before, FloatingChromeProbe.Bounds(f.Native));
            f.AssertEditors();
        });
        if ((OperatingSystem.IsLinux() || OperatingSystem.IsWindows()) && Environment.GetEnvironmentVariable("UNODOCK_NATIVE_INPUT_TESTS") == "1")
        {
            foreach (var edge in Edges)
                tests.Test($"resize-policy/{kind}: physical RTL pointer resizes {edge}", async () =>
                {
                    ...
                    input.MoveTo(f.Manager, new(ownerPoint.X + 22, ownerPoint.Y + 16));
                    await Task.Delay(100);
                    input.Release();
            ...
            tests.Test($"resize-policy/{kind}: retained input from another grip cannot end a successor capture", async () =>
            {
                ...
                await f.Show();
                using var input = new PointerInput();
                var first = f.Grip(ChromeHit.Right);
                var second = f.Grip(ChromeHit.Bottom);
                PointerRoutedEventArgs? firstPress = null;
                PointerRoutedEventArgs? secondPress = null;
                ...
                try
                {
                    input.MoveTo(first, new(first.ActualWidth / 2, first.ActualHeight / 2));
                    await Task.Delay(60);
                    input.Press();
                    await Wait(() => firstPress != null && f.Control.IsResizing);
                    input.Release();
                    await Wait(() => !f.Control.IsResizing);
                    input.MoveTo(second, new(second.ActualWidth / 2, second.ActualHeight / 2));
                    await Task.Delay(60);
                    input.Press();
                    await Wait(() => secondPress != null && f.Control.IsResizing);
            ...
            tests.Test($"resize-policy/{kind}: policy revocation releases a physical capture and clock", async () =>
            {
                ...
                Check.False(f.Control.IsResizing);
                var before = FloatingChromeProbe.Bounds(f.Native);
                input.MoveTo(f.Manager, new(ownerPoint.X + 100, ownerPoint.Y + 80));
                await Task.Delay(100);
                input.Release();
                await Task.Delay(100);
                Near(before, FloatingChromeProbe.Bounds(f.Native));
