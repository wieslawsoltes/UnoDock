...
public static class InputExtensionTests
{
    public static async Task<int> Run(DockingManager host, string output)
    {
        ...
        tests.Test("clearing pane emits empty selection", () =>
        {
            ...
            pane.Children.Clear();
            Check.Equal(-1, view.SelectedIndex);
            Check.True(view.SelectedItem == null);
            Release(view);
        });
        tests.Test("selection callback can request another selection without losing it", () =>
        {
            ...
            {
                if (ReferenceEquals(view.SelectedItem, b))
                    view.SelectedItem = c;
            };
            view.SelectedItem = b;
            Check.Same(c, pane.SelectedContent);
            Check.Same(c, view.SelectedItem);
            Check.Equal(2, view.Calls);
            Check.False(a.IsSelected);
            Check.False(b.IsSelected);
            Check.True(c.IsSelected);
            Release(view);
        });
        tests.Test("model observer reentrant selection is drained in order", () =>
        {
            ...
            {
                if (e.PropertyName == nameof(pane.SelectedContent) && ReferenceEquals(pane.SelectedContent, b))
                    pane.SelectedContentIndex = 2;
            };
        ...
        tests.Test("selection callback removal does not enumerate a mutating collection", () =>
        {
            ...
            a.IsSelectedChanged += (_, _) =>
            {
                if (!a.IsSelected && pane.Children.Contains(b))
                    pane.Children.Remove(b);
            };
            pane.SelectedContentIndex = 1;
        ...
        tests.Test("throwing selection subscriber does not poison future transitions", () =>
        {
            ...
            Check.Throws<InvalidOperationException>(() => view.SelectedIndex = 1);
            view.SelectionChanged -= bad;
            view.SelectedItem = a;
            Check.Same(a, pane.SelectedContent);
            Check.False(b.IsSelected);
        ...
        tests.Test("nonconvergent model selection observers are bounded and recoverable", () =>
        {
            ...
            PropertyChangedEventHandler loop = (_, e) =>
            {
                if (e.PropertyName == nameof(pane.SelectedContent))
                    pane.SelectedContentIndex = 1 - pane.SelectedContentIndex;
            };
            pane.PropertyChanged += loop;
            Check.Throws<InvalidOperationException>(() => pane.SelectedContentIndex = 1);
            pane.PropertyChanged -= loop;
            pane.SelectedContentIndex = -1;
            Check.True(pane.SelectedContent == null);
        });
        ...
        tests.Test("focus hooks observe native transition and activate only after focus", async () =>
        {
            ...
            await InWindow(new StackPanel { Children = { outside, probe } }, async () =>
            {
                ...
                await Task.Delay(30);
                probe.Previews = probe.Completed = 0;
                probe.DuringPreview = () => Check.Same(a, root.ActiveContent);
                Check.True(inside.Focus(FocusState.Programmatic));
                await Task.Delay(40);
                Check.True(probe.Previews > 0);
                Check.True(probe.Completed > 0);
                Check.Same(b, root.ActiveContent);
                Check.Same(inside, probe.NewFocus);
                Check.Same(outside, probe.OldFocus);
            });
        });
        tests.Test("preview focus cancellation prevents activation and post-focus callback", async () =>
        {
            ...
            await InWindow(new StackPanel { Children = { outside, probe } }, async () =>
            {
                ...
                probe.Cancel = true;
                probe.Previews = probe.Completed = 0;
                inside.Focus(FocusState.Programmatic);
                await Task.Delay(30);
                Check.True(probe.Previews > 0);
                Check.Equal(0, probe.Completed);
        ...
        tests.Test("custom pane and tab factories participate in actual rendering", async () =>
        {
            ...
            await InWindow(manager, async () =>
            {
                ...
                await Task.Delay(40);
                Check.Equal(1, manager.DocumentPanes);
                Check.Equal(2, manager.Tabs);
                var view = manager.FindVisualChildren<FactoryPane>().Single();
                Check.Same(pane, view.Model);
        ...
        if (OperatingSystem.IsLinux() && Environment.GetEnvironmentVariable("UNODOCK_NATIVE_INPUT_TESTS") == "1")
        {
            foreach (var mode in new[]
            {
                ...
                tests.Test("XTEST protected tab press path: " + mode, async () =>
                {
                    ...
                    a.IsActive = true;
                    var tab = new InputProbe
                    {
                        Model = b,
                        PreviewVeto = mode == "preview-veto",
                        DownVeto = mode == "down-veto"
                    };
                    await InWindow(tab, async () =>
                    {
                        using var input = new X11TestInput();
                        input.MoveTo(tab, new(30, 12));
                        await Task.Delay(60);
                        input.Press();
                        await Task.Delay(60);
                        input.Release();
                        await Task.Delay(80);
                        Check.Equal(1, tab.Previews);
                        Check.Equal(mode == "preview-veto" ? 0 : 1, tab.Downs);
            ...
            tests.Test("XTEST middle-button hook closes exactly once", async () =>
            {
                ...
                await InWindow(tab, async () =>
                {
                    ...
                    input.Release(2);
                    await Task.Delay(80);
                    Check.Equal(1, tab.MiddleDowns);
                    Check.False(pane.Children.Contains(b));
                });
            });
            tests.Test("XTEST ordinary right-button release reaches override once", async () =>
            {
                ...
                await InWindow(tab, async () =>
                {
                    ...
                    input.Release(3);
                    await Task.Delay(60);
                    Check.Equal(1, tab.RightUps);
                    Check.Equal(0, tab.Ups);
                });
            });
            ...
            tests.Test("XTEST release override vetoes a real edge-compatibility docking operation", async () =>
            {
                ...
                try
                {
                    ...
                    Check.Equal(1, probe.Ups);
                    Check.True(probe.Moves > 0);
                    Check.Same(pane, b.Parent);
                    probe.ReleaseVeto = false;
                    await input.Begin(probe, new(30, 12));
                    await input.Drop(control, new(control.ActualWidth / 2, control.ActualHeight - 10));
                ...
                {
                    host.DockingGuideMode = guideMode;
                    host.Layout = old;
                    host.Refresh();
                    await Task.Delay(80);
                }
    ...
    private static WeakReference AbandonView(LayoutDocumentPane pane) => new(new SelectionProbe(pane));
    ...
    {
        public int Previews, Completed;
        public bool Cancel, Suppress;
    ...
    private sealed class FactoryProbe : DockingManager
    {
        ...
        {
            DocumentPanes++;
            return new FactoryPane(model, this);
        }
    }

    private sealed class FactoryPane(LayoutDocumentPane model, FactoryProbe owner) : LayoutDocumentPaneControl(model)
    {
        protected override LayoutTabItemBase CreateTabItem(LayoutContent model)
        {
            owner.Tabs++;
            return new InputProbe();
        }
    }
    ...
    {
        public int Calls;
        public bool Suppress, Throw;
        public bool AllOnUi = true;
        protected override bool OnReceiveWeakEvent(Type managerType, object sender, EventArgs e)
        {
            Calls++;
            AllOnUi &= DispatcherQueue.HasThreadAccess;
            if (Throw)
                throw new InvalidOperationException("source hook");
            return !Suppress && base.OnReceiveWeakEvent(managerType, sender, e);
        }
    }
    ...
    private sealed class InputProbe : LayoutDocumentTabItem
    {
        ...
        protected override void OnMouseRightButtonUp(DockMouseButtonEventArgs e)
        {
            RightUps++;
        ...
        {
            if (e.ChangedButton == DockMouseButton.Middle)
                MiddleDowns++;
            base.OnMouseDown(e);
        }

        protected override void OnMouseMove(DockMouseEventArgs e)
        {
            Moves++;
            base.OnMouseMove(e);
        }
