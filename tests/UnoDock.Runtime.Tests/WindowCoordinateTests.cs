...
public static partial class WindowCoordinateTests
{
    public static async Task<int> Run(string output, DockingManager? host = null)
    {
        ...
        {
            var adapter = new DesktopWindowCoordinates();
            adapter.Dispose();
            adapter.Dispose();
        ...
        tests.Test("manager owns only its default coordinate adapter", () =>
        {
            ...
            var owned = (DesktopWindowCoordinates)manager.CrossWindowCoordinates!;
            var external = new ExternalCoordinates();
            manager.CrossWindowCoordinates = external;
            manager.Dispose();
            Check.False(external.Disposed);
        ...
        tests.Test("floating single-content property retains public setter", () =>
        {
            ...
            control.SingleContentLayoutItem = null;
        ...
        if (OperatingSystem.IsLinux() || OperatingSystem.IsWindows())
        {
            var a = new Window
            {
                Title = "UnoDock coordinate test A"
            };
            var b = new Window
            {
                Title = "UnoDock coordinate test B"
            };
            var source = new Grid
            {
                Width = 100,
            ...
            var destination = new Grid
            {
                ...
                HorizontalAlignment = HorizontalAlignment.Left,
                VerticalAlignment = VerticalAlignment.Top,
                Margin = new(29, 31, 0, 0)
            };
            var ar = new Grid();
            ar.Children.Add(source);
            a.Content = ar;
            var br = new Grid();
            br.Children.Add(destination);
            b.Content = br;
            using var adapter = new DesktopWindowCoordinates();
            var closed = false;
            try
            {
                ...
                {
                    source.RenderTransform = new ScaleTransform
                    {
                        ScaleX = 1.5,
                        ScaleY = .75
                    };
                    destination.RenderTransform = new ScaleTransform
                    {
                        ScaleX = .5,
                        ScaleY = 1.25
                    };
                    ...
                    finally
                    {
                        ...
                        destination.RenderTransform = null;
                    }
                });
                tests.Test("moving a native window invalidates no cached origin", async () =>
                {
                    ...
                    Check.Near(17, current.Y - previous.Y, 1);
                });
                ...
                if (host != null)
                {
                    ...
                    tests.Test("native tool-window drop plan transfers between XamlRoots", async () =>
                    {
                        ...
                        try
                        {
                            ...
                            toolA.AddToLayout(host, AnchorableShowStrategy.Left);
                            toolB.AddToLayout(host, AnchorableShowStrategy.Right);
                            toolA.Float();
                            toolB.Float();
                            host.Refresh();
                            ...
                            var area = host.GetDropAreas().OfType<DropArea<FrameworkElement>>().Single(a => a.AreaElement is ILayoutControl c && ReferenceEquals(c.Model, target));
                            ...
                            host.Refresh();
                            Check.Same(target, toolA.Parent);
                            Check.Same(view, host.GetLayoutItemFromModel(toolA).View);
                    ...
                    tests.Test("closed native target invalidates a previously accepted plan", async () =>
                    {
                        ...
                        try
                        {
                            host.FloatingWindowMode = FloatingWindowMode.Native;
                            host.Layout = new();
                            source.AddToLayout(host, AnchorableShowStrategy.Left);
                            target.AddToLayout(host, AnchorableShowStrategy.Right);
                            target.Float();
                            host.Refresh();
    ...
    private static void CloseTestWindow(Window window)
    {
        ...
        typeof(DesktopWindowCoordinates).GetMethod("HideNativeClientBeforeClose", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static)!.Invoke(null, [window]);
        window.Content = null;
        window.Close();
    }
    ...
    private static async Task Until(Func<bool> condition)
    {
        ...
        while (!condition())
        {
            if (DateTime.UtcNow >= deadline)
                throw new TimeoutException("Desktop window did not reach the requested state.");
            await Task.Delay(20);
        }
    }
    ...
    {
        Check.Near(a.X, b.X, tolerance);
        Check.Near(a.Y, b.Y, tolerance);
    }
