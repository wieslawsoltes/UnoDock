...
public static class RuntimeTests
{
    public static async Task<int> Run(DockingManager host, string output)
    {
        ...
        tests.Test("auto-hide roundtrip preserves pane", () => { var a = new LayoutAnchorable(); var b = new LayoutAnchorable(); var p = new LayoutAnchorablePane(a); p.Children.Add(b); var r = Root(p); a.ToggleAutoHide(); Check.True(a.IsAutoHidden); Check.False(b.IsAutoHidden); a.ToggleAutoHide(); Check.Same(p, a.Parent); Check.Same(p, b.Parent); AssertTree(r); });
        tests.Test("auto-hide eligibility is per item, not per sibling", () => { var a = new LayoutAnchorable(); var b = new LayoutAnchorable { CanAutoHide = false }; var p = new LayoutAnchorablePane(a); p.Children.Add(b); Root(p); a.ToggleAutoHide(); Check.True(a.IsAutoHidden); Check.Same(p, b.Parent); });
