...
public static partial class ShellTests
{
    public static async Task<int> Run(string output, DockingManager host)
    {
        ...
        tests.Test("chrome attached input has independent local values", () =>
        {
            ...
            Check.True(WindowChrome.GetIsHitTestVisibleInChrome(a)); Check.False(WindowChrome.GetIsHitTestVisibleInChrome(b));
            a.ClearValue(WindowChrome.IsHitTestVisibleInChromeProperty); Check.False(WindowChrome.GetIsHitTestVisibleInChrome(a));
        });
        tests.Test("clone retains values without sharing configuration", () =>
        {
            var chrome = new WindowChrome { CaptionHeight = 47, ResizeBorderThickness = new(1,2,3,4), GlassFrameThickness = new(5,6,7,8), CornerRadius = new(9), ShowSystemMenu = false };
            var clone = chrome.Clone(); Check.False(ReferenceEquals(chrome, clone));
            Check.Near(47, clone.CaptionHeight); Check.Equal(chrome.ResizeBorderThickness, clone.ResizeBorderThickness);
            Check.Equal(chrome.GlassFrameThickness, clone.GlassFrameThickness); Check.Equal(chrome.CornerRadius, clone.CornerRadius); Check.False(clone.ShowSystemMenu);
            clone.CaptionHeight = 24; Check.Near(47, chrome.CaptionHeight);
        });
        ...
        {
            var chrome = new WindowChrome { GlassFrameThickness = new(0, -7, 0, 0) };
        ...
        {
            var chrome = new WindowChrome { CaptionHeight = 24 }; var calls = 0;
            ...
            Check.Near(24, chrome.CaptionHeight); Check.Equal(0, calls);
        });
        tests.Test("chrome change notifications publish committed values", () =>
        {
            var chrome = new WindowChrome(); var count = 0;
            chrome.PropertyChanged += (_, e) => { Check.Equal("CaptionHeight", e.PropertyName); Check.Near(51, chrome.CaptionHeight); count++; };
            chrome.CaptionHeight = 51; chrome.CaptionHeight = 51; Check.Equal(1, count);
        });
        tests.Test("same-thread shell metrics are observable and stable", () =>
        {
            var value = SystemParameters2.Current; Check.Same(value, SystemParameters2.Current);
            Check.Equal(value.WindowGlassColor, value.WindowGlassBrush.Color);
            var changes = 0; System.ComponentModel.PropertyChangedEventHandler handler = (_, _) => changes++;
            value.PropertyChanged += handler;
            try { value.Refresh(); Check.Equal(0, changes); } finally { value.PropertyChanged -= handler; }
            Check.True(double.IsFinite(value.WindowCaptionHeight)); Check.True(value.WindowCaptionHeight >= 0);
        });
        ...
        if (!OperatingSystem.IsWindows()) tests.Test("unsupported native metrics are not invented", () =>
        {
            var value = SystemParameters2.Current; Check.False(value.IsGlassEnabled); Check.Near(0, value.WindowCaptionHeight);
        ...
        {
            var a = new LayoutDocumentFloatingWindowControl(new()); var b = new LayoutDocumentFloatingWindowControl(new());
            var original = a.ResizeBorderThickness;
            var chrome = new WindowChrome { ResizeBorderThickness = new(12), CornerRadius = new(7) };
            try
            {
                WindowChrome.SetWindowChrome(a, chrome); WindowChrome.SetWindowChrome(b, chrome); await Tick();
                Check.Equal(new Thickness(12), a.ResizeBorderThickness); Check.Equal(new Thickness(12), b.ResizeBorderThickness);
                Check.True(WindowChrome.GetAppliedCapabilities(a).HasFlag(WindowChromeCapabilities.ManagedFrame));
                WindowChrome.SetWindowChrome(a, null); chrome.ResizeBorderThickness = new(9); await Tick();
                Check.Equal(original, a.ResizeBorderThickness); Check.Equal(new Thickness(9), b.ResizeBorderThickness);
                Check.True(WindowChrome.GetWindowChrome(a) == null); Check.Same(chrome, WindowChrome.GetWindowChrome(b));
            }
            finally { WindowChrome.SetWindowChrome(a, null); WindowChrome.SetWindowChrome(b, null); }
        });
        tests.Test("queued chrome update cannot mutate a detached host", async () =>
        {
            var control = new LayoutDocumentFloatingWindowControl(new()); var original = control.ResizeBorderThickness;
            WindowChrome.SetWindowChrome(control, new() { ResizeBorderThickness = new(19) });
            WindowChrome.SetWindowChrome(control, null); await Tick(); Check.Equal(original, control.ResizeBorderThickness);
        });
        tests.Test("detach restores last applied value while a new value is queued", async () =>
        {
            var control = new LayoutDocumentFloatingWindowControl(new()); var original = control.ResizeBorderThickness;
            var chrome = new WindowChrome { ResizeBorderThickness = new(17) };
            WindowChrome.SetWindowChrome(control, chrome); await Tick();
            chrome.ResizeBorderThickness = new(23); WindowChrome.SetWindowChrome(control, null);
            await Tick(); Check.Equal(original, control.ResizeBorderThickness);
        });
        tests.Test("chrome detach preserves independently changed host value", async () =>
        {
            ...
            WindowChrome.SetWindowChrome(control, new() { ResizeBorderThickness = new(13) }); await Tick();
            control.ResizeBorderThickness = new(22); WindowChrome.SetWindowChrome(control, null); Check.Equal(new Thickness(22), control.ResizeBorderThickness);
        });
        tests.Test("document docking captions remain client-area chrome exclusions", async () =>
        {
            var previous = host.Layout; var mode = host.FloatingWindowMode;
            try
            {
        ...
        {
            var previous = host.Layout; var mode = host.FloatingWindowMode;
            var doc = new LayoutDocument { Title = "Shell test", ContentId = "shell", Content = new TextBox { Text = "retained" }, FloatingWidth = 350, FloatingHeight = 230 };
            try
            {
                host.FloatingWindowMode = FloatingWindowMode.InSurface;
                ...
                SystemCommands.MaximizeWindowCommand.Execute(null, window); Check.True(window.IsMaximized); Check.True(doc.IsMaximized);
                SystemCommands.RestoreWindowCommand.Execute(null, window); Check.False(window.IsMaximized); Check.Near(350, doc.FloatingWidth); Check.Near(230, doc.FloatingHeight);
                SystemCommands.MinimizeWindow(window); Check.Equal(Visibility.Collapsed, window.Visibility);
                Check.True(SystemCommands.RestoreWindowCommand.CanExecute(window));
                window.Activate(); Check.Equal(Visibility.Visible, window.Visibility); Check.Same(content, doc.Content);
                Check.False(SystemCommands.RestoreWindowCommand.CanExecute(window));
                doc.CanClose = false; Check.False(SystemCommands.CloseWindowCommand.CanExecute(window));
                doc.CanClose = true;
                EventHandler<System.ComponentModel.CancelEventArgs> cancel = (_, e) => e.Cancel = true;
                doc.Closing += cancel; SystemCommands.CloseWindow(window); Check.True(doc.Root != null);
                doc.Closing -= cancel; SystemCommands.CloseWindow(window); Check.True(doc.Root == null);
        ...
        if (OperatingSystem.IsLinux() || OperatingSystem.IsWindows() || OperatingSystem.IsMacOS())
        {
            ...
            {
                var content = new ContentControl { Content = new TextBlock { Text = "Shell lifecycle" } };
                var window = new Window { Title = "Shell lifecycle", Content = content };
                using var lease = SystemCommands.RegisterWindow(window);
                var closed = false; window.Closed += (_, _) => closed = true;
                window.Activate(); await Tick();
                try
                {
                    ...
                    var chrome = new WindowChrome(); WindowChrome.SetWindowChrome(window, chrome); await Tick();
                    Check.Same(chrome, WindowChrome.GetWindowChrome(window));
                    WindowChrome.SetWindowChrome(window, null); Check.True(WindowChrome.GetWindowChrome(window) == null);
                    SystemCommands.CloseWindow(content); await Tick(); Check.True(closed);
                    Check.False(SystemCommands.CloseWindowCommand.CanExecute(window));
                }
            ...
            {
                var window = new Window { Content = new Grid() }; var closed = false;
                window.Closed += (_, _) => closed = true;
                window.Activate(); await Tick();
                try
                {
                    Check.True(SystemCommands.CloseWindowCommand.CanExecute(window));
                    SystemCommands.CloseWindow(window); await Tick(); Check.True(closed);
                    Check.False(SystemCommands.CloseWindowCommand.CanExecute(window));
                }
                finally { if (!closed) window.Close(); }
            });
            tests.Test("released registration retains closed-target guard", async () =>
            {
                ...
                var lease = SystemCommands.RegisterWindow(window); lease.Dispose(); lease.Dispose();
                window.Close(); await Tick();
                Check.False(SystemCommands.CloseWindowCommand.CanExecute(window));
            });
            tests.Test("native window command rejects worker-thread calls", async () =>
            {
                var window = new Window(); using var lease = SystemCommands.RegisterWindow(window);
                try { await Task.Run(() => Check.Throws<InvalidOperationException>(() => SystemCommands.CloseWindow(window))); }
