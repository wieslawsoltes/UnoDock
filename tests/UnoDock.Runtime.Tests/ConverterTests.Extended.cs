...
public static partial class ConverterTests
{
    ...
    {
        using var stream = typeof(ConverterTests).Assembly.GetManifestResourceStream("ConverterFixtures.converter-details.xml")
            ?? throw new InvalidOperationException("Missing URI and multi-value reference observations.");
        ...
        foreach (var record in cases)
        {
            ...
            tests.Test($"extended reference {record.Name}: {key} -> {target}, reverse={reverse}", () =>
            {
                ...
                {
                    "null" => null, "image" => new BitmapImage(),
                    ...
                    "string" => "not-an-image", "empty-string" => "", "integer" => 17, "boolean" => true,
                    _ => throw new InvalidOperationException("Unknown input " + key)
                };
                var type = target switch { "image-source" => typeof(ImageSource), "bitmap-image" => typeof(BitmapImage),
                    "uri" => typeof(Uri), "string" => typeof(string), _ => Target(target) };
                Verify(record.Elements().Single(), record.Name == "MultiCase"
                    ? () => reverse ? multi.ConvertBack(Visibility.Visible, [typeof(bool), typeof(bool)], null!, Invariant)
                        : multi.Convert((object[])input!, type, null!, Invariant)
                    : () => reverse ? converter.ConvertBack(input!, type, null!, Invariant) : converter.Convert(input!, type, null!, Invariant),
                    input, manager, document, tool, nameof(UriSourceToBitmapImageConverter));
            });
        }
    }
    private static object[]? ExtendedValues(string key) => key switch
    {
        ...
        {
            "visible" => (object)Visibility.Visible, "collapsed" => Visibility.Collapsed,
            "true" => true, "false" => false, "null" => null!, "one" => 1, "string" => "true",
            "unset" => DependencyProperty.UnsetValue, "nothing" => BindingValue.DoNothing,
