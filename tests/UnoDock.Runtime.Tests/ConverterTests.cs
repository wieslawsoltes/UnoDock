...
public static partial class ConverterTests
{
    ...
    public static async Task<int> Run(string output)
    {
        ...
        using var fixture = typeof(ConverterTests).Assembly.GetManifestResourceStream("ConverterFixtures.converters.xml")
            ?? throw new InvalidOperationException("Missing original converter observations.");
        ...
        foreach (var record in cases)
        {
            ...
            tests.Test($"reference {name}: {key} -> {targetKey}, reverse={reverse}", () =>
            {
                ...
                Func<object?> invoke = record.Name == "MultiCase"
                    ? () => reverse ? ((IMultiValueConverter)converter).ConvertBack(Visibility.Visible, [typeof(bool), typeof(bool)], null!, Invariant)
                        : ((IMultiValueConverter)converter).Convert((object[])input!, targetType, null!, Invariant)
                    : () => Invoke(converter, methodName, input, targetType);
        ...
        tests.Test("native language overload preserves valid conversion", () =>
            Check.Equal(Visibility.Collapsed, new InverseBoolToVisibilityConverter().Convert(true, typeof(Visibility), null!, "pl-PL")));
        tests.Test("native reverse direction retains original exception", () =>
            Check.Throws<NotImplementedException>(() => new AnchorSideToAngleConverter().ConvertBack(90d, typeof(AnchorSide), null!, "")));
        ...
        tests.Test("item converter does not reinterpret an item as a model", () =>
            Check.Equal<object?>(null, new LayoutItemFromLayoutModelConverter().Convert(manager.GetLayoutItemFromModel(document), typeof(object), null!, Invariant)));
        tests.Test("detached former document does not retain a manager command", () =>
        {
            var model = new LayoutDocument(); var pane = new LayoutDocumentPane(model); manager.Layout.RootPanel.Children.Add(pane);
            ...
            pane.Children.Remove(model); Check.Equal<object?>(null, converter.Convert(model, typeof(object), null!, Invariant));
        });
        tests.Test("converter declaration attributes", () =>
        {
            ...
            Check.Equal(typeof(AnchorSide), attribute.SourceType); Check.Equal(typeof(double), attribute.TargetType);
    ...
    {
        try { return converter.GetType().GetMethod(method, [typeof(object), typeof(Type), typeof(object), typeof(CultureInfo)])!
                .Invoke(converter, [input, target, null, Invariant]); }
        catch (TargetInvocationException error) when (error.InnerException != null)
        { ExceptionDispatchInfo.Capture(error.InnerException).Throw(); throw; }
    }
    private static Type Target(string key) => key switch
    { "bool" => typeof(bool), "nullable-bool" => typeof(bool?), "visibility" => typeof(Visibility), _ => typeof(object) };
    private static object? Input(string key, LayoutDocument document, LayoutAnchorable tool) => key switch
    {
        "null" => null, "true" => true, "false" => false, "zero" => 0, "one" => 1, "string" => "text",
        "visible" => Visibility.Visible, "collapsed" => Visibility.Collapsed,
        "left" => AnchorSide.Left, "right" => AnchorSide.Right, "top" => AnchorSide.Top, "bottom" => AnchorSide.Bottom,
        "invalid-side" => (AnchorSide)999, "document" => new LayoutDocument(), "tool" => new LayoutAnchorable(),
        "attached-document" => document, "attached-tool" => tool, _ => throw new ArgumentOutOfRangeException(nameof(key))
    };
    private static object[] Values(string key) => key == "empty" ? [] : key.Split(',').Select(value => value == "null" ? null! : (object)bool.Parse(value)).ToArray();
    private static void Verify(XElement expected, Func<object?> invoke, object? input,
        DockingManager manager, LayoutDocument document, LayoutAnchorable tool, string converter)
    {
        if (expected.Name == "Exception")
        {
            Exception? actualException = null;
            try { invoke(); } catch (Exception error) { actualException = error; }
            Check.Equal((string?)expected.Attribute("Type"), actualException?.GetType().FullName); return;
        }
        var result = invoke();
        switch (expected.Name.LocalName)
        {
            case "Null": Check.Equal<object?>(null, result); break;
            case "DoNothing": Check.Same(BindingValue.DoNothing, result); break;
            case "UnsetValue": Check.Same(DependencyProperty.UnsetValue, result); break;
            case "InputIdentity": Check.Same(input, result); break;
            case "DocumentItem": Check.Same(manager.GetLayoutItemFromModel(document), result); break;
            case "ToolItem": Check.Same(manager.GetLayoutItemFromModel(tool), result); break;
            case "ActivateCommand": Check.Same(manager.GetLayoutItemFromModel((LayoutContent)input!).ActivateCommand, result); break;
            case "Command":
                var item = (LayoutAnchorableItem)manager.GetLayoutItemFromModel(tool);
                Check.Same(converter.StartsWith("AutoHide", StringComparison.Ordinal) ? item.AutoHideCommand : item.HideCommand, result); break;
            case "Scalar":
            ...
            case "Object" when (string?)expected.Attribute("Type") == "System.Windows.Controls.Image":
                ...
                Check.Equal(input, ((BitmapImage)((Image)result!).Source).UriSource); break;
            case "Image":
                Check.True(result is Image { Source: BitmapImage });
                var image = (Image)result!;
                Check.Equal((bool)expected.Attribute("SameUri")!, Equals(input, ((BitmapImage)image.Source).UriSource));
                Check.Equal((string?)expected.Attribute("Stretch"), image.Stretch.ToString()); break;
            default: throw new InvalidOperationException("Unrecognized reference result: " + expected);
        }
    }
    ...
    private static void BindingTests(TestRunner tests)
    {
        ...
        {
            var target = new ContentControl { Content = "retained" }; var source = new Source();
            var converter = new NullToDoNothingConverter();
            using var binding = new ConverterBinding(target, ContentControl.ContentProperty, () => source.Value,
                value => converter.Convert(value!, typeof(object), null!, Invariant), [source], fallbackValue: "fallback", useFallbackValue: true);
            Check.Equal("retained", target.Content); source.Value = "next"; Check.Equal("next", target.Content);
            source.Value = null; Check.Equal("next", target.Content);
        });
        tests.Test("UnsetValue explicitly uses fallback", () =>
        {
            var target = new ContentControl { Content = "old" };
            using var binding = new ConverterBinding(target, ContentControl.ContentProperty, () => null, _ => DependencyProperty.UnsetValue,
                fallbackValue: "fallback", useFallbackValue: true);
        ...
        {
            var target = new ContentControl(); var source = new Source { Value = "first" };
            var binding = new ConverterBinding(target, ContentControl.ContentProperty, () => source.Value, value => value, [source, source]);
            Check.Equal(1, source.Subscribers); binding.Dispose(); binding.Dispose(); Check.Equal(0, source.Subscribers);
            source.Value = "second"; Check.Equal("first", target.Content);
        ...
        tests.Test("binding constructor failure removes subscriptions", () =>
        {
            ...
            Check.Throws<FormatException>(() => new ConverterBinding(new ContentControl(), ContentControl.ContentProperty,
                () => null, _ => throw new FormatException(), [source]));
            Check.Equal(0, source.Subscribers);
        });
        tests.Test("two-way DoNothing suppresses source writes", () =>
        {
            var target = new ContentControl(); var source = new Source { Value = "source" }; var writes = 0;
            using var binding = new ConverterBinding(target, ContentControl.ContentProperty, () => source.Value, value => value,
                [source], writeSource: value => { writes++; source.Value = value; }, convertBack: value => Equals(value, "skip") ? BindingValue.DoNothing : value);
            Check.Equal(0, writes); target.Content = "skip"; Check.Equal(0, writes); Check.Equal("source", source.Value);
            target.Content = "edited"; Check.Equal(1, writes); Check.Equal("edited", source.Value);
        });
        tests.Test("two-way UnsetValue suppresses source writes", () =>
        {
            var target = new ContentControl(); var writes = 0;
            using var binding = new ConverterBinding(target, ContentControl.ContentProperty, () => "source", value => value,
                writeSource: _ => writes++, convertBack: _ => DependencyProperty.UnsetValue, fallbackValue: "fallback", useFallbackValue: true);
            target.Content = "edited"; Check.Equal(0, writes);
        });
        tests.Test("source notification reentrancy converges to newest value", () =>
        {
            var target = new ContentControl(); var source = new Source { Value = 0 };
            using var binding = new ConverterBinding(target, ContentControl.ContentProperty, () => source.Value,
                value => { if ((int)value! < 3) source.Value = (int)value + 1; return value; }, [source]);
            Check.Equal(3, target.Content);
        });
        tests.Test("non-convergent source notification fails without leaked observer", () =>
        {
            var source = new Source { Value = 0 };
            Check.Throws<InvalidOperationException>(() => new ConverterBinding(new ContentControl(), ContentControl.ContentProperty,
                () => source.Value, value => { source.Value = (int)value! + 1; return value; }, [source]));
            Check.Equal(0, source.Subscribers);
        });
        tests.Test("property-name filter and all-properties notification", () =>
        {
            var source = new Source { Value = "first" }; var target = new ContentControl(); var reads = 0;
            using var binding = new ConverterBinding(target, ContentControl.ContentProperty, () => { reads++; return source.Value; }, v => v,
                [source], sourceProperty: "Value");
            source.Notify("Other"); Check.Equal(1, reads); source.Notify(null); Check.Equal(2, reads);
        });
        tests.Test("multiple notification sources update one projection", () =>
        {
            var left = new Source { Value = "a" }; var right = new Source { Value = "b" }; var target = new ContentControl();
            using var binding = new ConverterBinding(target, ContentControl.ContentProperty, () => $"{left.Value}:{right.Value}", v => v, [left, right]);
            Check.Equal("a:b", target.Content); right.Value = "c"; Check.Equal("a:c", target.Content);
        });
        tests.Test("background source notifications marshal to UI thread", async () =>
        {
            var source = new Source { Value = "old" }; var target = new ContentControl(); var uiThread = Environment.CurrentManagedThreadId;
            using var binding = new ConverterBinding(target, ContentControl.ContentProperty,
                () => { Check.Equal(uiThread, Environment.CurrentManagedThreadId); return source.Value; }, v => v, [source]);
            await Task.Run(() => source.Value = "background");
            for (var i = 0; i < 50 && !Equals(target.Content, "background"); i++) await Task.Delay(10);
            Check.Equal("background", target.Content);
        });
        tests.Test("native binding cannot be replaced accidentally", () =>
        {
            var target = new ContentControl(); target.SetBinding(ContentControl.ContentProperty, new Binding { Source = "native" });
        ...
        tests.Test("two-way attachment requires both delegates", () =>
            Check.Throws<ArgumentException>(() => new ConverterBinding(new ContentControl(), ContentControl.ContentProperty, () => null, v => v, writeSource: _ => { })));
    }
    private sealed class Source : INotifyPropertyChanged
    {
        private object? _value; private PropertyChangedEventHandler? _changed;
        public int Subscribers { get; private set; }
        public object? Value { get => _value; set { _value = value; Notify(nameof(Value)); } }
        public event PropertyChangedEventHandler? PropertyChanged
        { add { _changed += value; Subscribers++; } remove { _changed -= value; Subscribers--; } }
