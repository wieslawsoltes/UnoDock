...
public static class RestoreOwnershipTests
{
    public static Task<int> Run(string output)
    {
        ...
        {
            using var manager = Workspace();
            var xml = Capture(manager);
            ...
            Check.Equal(2, calls);
            Check.Equal(2, Documents(manager).Length);
        });
        ...
        {
            using var first = Workspace();
            using var second = Workspace();
            var secondRoot = second.Layout;
            var xml = Capture(first);
            var serializer = new XmlLayoutSerializer(first);
            var once = false;
            serializer.LayoutSerializationCallback += (_, _) =>
            {
                if (!once)
                {
                    once = true;
                    new XmlLayoutSerializer(second).Deserialize(new StringReader(xml));
                }
            };
            serializer.Deserialize(new StringReader(xml));
            Check.False(ReferenceEquals(secondRoot, second.Layout));
            Check.Same(first, first.Layout.Manager);
            Check.Same(second, second.Layout.Manager);
        });
        tests.Test("restore: replacing the workspace during a resolver aborts all stale callbacks", () =>
        {
            ...
            {
                count++;
                manager.Layout = replacement;
            };
            Check.Throws<InvalidOperationException>(() => serializer.Deserialize(new StringReader(xml)));
            Check.Same(replacement, manager.Layout);
            Check.Same(manager, replacement.Manager);
            Check.Equal(1, count);
        });
        ...
        tests.Test("restore: throwing content resolver preserves root and permits later retry", () =>
        {
            ...
            Check.Throws<ApplicationException>(() => serializer.Deserialize(new StringReader(xml)));
            Check.Same(original, manager.Layout);
            serializer.LayoutSerializationCallback -= fail;
            serializer.Deserialize(new StringReader(xml));
            Check.Equal(2, Documents(manager).Length);
        ...
        tests.Test("restore: caller reader replacing layout cannot cause stale publication", () =>
        {
            ...
            Check.Throws<InvalidOperationException>(() => new XmlLayoutSerializer(manager).Deserialize(reader));
            Check.Same(replacement, manager.Layout);
        });
        ...
        tests.Test("restore: disposal during resolver cannot resurrect the manager", () =>
        {
            ...
            Check.Throws<InvalidOperationException>(() => serializer.Deserialize(new StringReader(xml)));
            Check.Same(original, manager.Layout);
            Check.True(original.Manager == null);
        });
        tests.Test("restore: override replacing root is validated even without calling base fixup", () =>
        {
            using var manager = Workspace();
            var replacement = new LayoutRoot();
            var serializer = new ReplacingSerializer(manager, replacement);
            Check.Throws<InvalidOperationException>(() => serializer.Deserialize(new StringReader(Capture(manager))));
            Check.Same(replacement, manager.Layout);
        });
        ...
        {
            using var manager = Workspace();
            var originals = Documents(manager);
            var expected = originals[1].Content;
            var xml = Capture(manager);
            var serializer = new XmlLayoutSerializer(manager);
        ...
        tests.Test("restore: callbacks may omit a node without invalidating unrelated nodes", () =>
        {
            ...
            serializer.Deserialize(new StringReader(xml));
            Check.Equal("second", Documents(manager).Single().ContentId);
        });
        ...
        tests.Test("restore: content setter may transfer ownership without later icon or tooltip writes", () =>
        {
            ...
            serializer.LayoutSerializationCallback += (_, e) =>
            {
                ...
                {
                    if (moved || change.PropertyName != nameof(LayoutContent.Content))
                        return;
                    moved = true;
                    e.Model.IconSource = icon;
                    e.Model.ToolTip = tooltip;
                    pane.Children.Add((LayoutDocument)e.Model);
                };
            };
            serializer.Deserialize(new StringReader(Capture(manager)));
            Check.Same(icon, pane.Children.Single().IconSource);
            Check.Same(tooltip, pane.Children.Single().ToolTip);
            Check.Equal(1, Documents(manager).Length);
        });
        tests.Test("restore: observable source adopts restored models without duplicate content", () =>
        {
            ...
            var models = Documents(manager);
            models[0].ContentId = "a";
            models[1].ContentId = "b";
            var xml = Capture(manager);
            new XmlLayoutSerializer(manager).Deserialize(new StringReader(xml));
            Check.Equal(2, Documents(manager).Length);
            Check.Equal(1, Documents(manager).Count(d => ReferenceEquals(d.Content, a)));
            source.Remove(a);
            Check.Equal(1, Documents(manager).Length);
            Check.Same(b, Documents(manager)[0].Content);
        });
        tests.Test("restore: lease is held through source reconciliation callbacks", () =>
        {
            ...
            serializer.LayoutSerializationCallback += (_, _) =>
            {
                if (once)
                    return;
                once = true;
                manager.DocumentsSource = new CallbackSequence(() =>
                {
                    Check.Throws<InvalidOperationException>(() => new XmlLayoutSerializer(manager).Deserialize(new StringReader(xml)));
        ...
        tests.Test("restore: primary and cleanup exceptions are both observable and lease released", () =>
        {
            ...
            var xml = Capture(manager);
            var serializer = new XmlLayoutSerializer(manager);
            EventHandler<LayoutSerializationCallbackEventArgs> fail = (_, _) =>
            {
                manager.DocumentsSource = new CallbackSequence(() => throw new ApplicationException("enumeration failure"));
            ...
            Check.True(aggregate != null);
            Check.Equal(2, aggregate!.InnerExceptions.Count);
            Check.Equal("resolver failure", aggregate.InnerExceptions[0].Message);
            Check.Equal("enumeration failure", aggregate.InnerExceptions[1].Message);
            manager.DocumentsSource = null;
            serializer.LayoutSerializationCallback -= fail;
            serializer.Deserialize(new StringReader(xml));
        });
        tests.Test("restore: caller-owned stream and writer stay open", () =>
        {
            ...
            writer.Write(" ");
            Check.True(writer.ToString().Length > 0);
        });
        tests.Test("restore: repeated successful restores never retain previous resolver state", () =>
        {
            ...
            for (var i = 0; i < 24; i++)
            {
                ...
                Check.Same(manager, manager.Layout.Manager);
            }
        });
    ...
    private static string Capture(DockingManager manager)
    {
        using var text = new StringWriter();
        new XmlLayoutSerializer(manager).Serialize(text);
