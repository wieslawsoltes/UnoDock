...
public static partial class WindowCoordinateTests
{
    private static void RegisterNativeInput(TestRunner tests, DockingManager host)
    {
        ...
        tests.Test("X11 pointer drag docks between native windows with visible preview", async () =>
        {
            ...
            try
            {
                ...
                host.Layout = new();
                content.AddToLayout(host, AnchorableShowStrategy.Left);
                ...
                var item = host.GetLayoutItemFromModel(content);
                var retained = item.View;
                Motion(display, start);
                await Task.Delay(40);
                Check.True(XTestFakeButtonEvent(display, 1, true, UIntPtr.Zero) != 0);
                XFlush(display);
                await Task.Delay(40);
                Motion(display, new(start.X + 20, start.Y + 8));
                await Task.Delay(40);
                Motion(display, end);
                ...
                Check.True(XTestFakeButtonEvent(display, 1, false, UIntPtr.Zero) != 0);
                XFlush(display);
            ...
            finally
            {
                ...
                {
                    XTestFakeButtonEvent(display, 1, false, UIntPtr.Zero);
                    XFlush(display);
    ...
    private static extern int XTestFakeMotionEvent(IntPtr display, int screen, int x, int y, UIntPtr delay);
    [DllImport("libXtst.so.6")]
    private static extern int XTestFakeButtonEvent(IntPtr display, uint button, [MarshalAs(UnmanagedType.Bool)] bool pressed, UIntPtr delay);
}
