...
public static class InteropTests
{
    public static async Task<int> Run(string output)
    {
        ...
        var fixtures = assembly.GetManifestResourceNames().Where(n => n.StartsWith("ReferenceFixtures.", StringComparison.Ordinal))
        ...
        foreach (var name in fixtures.Where(n => !n.EndsWith("public-defaults.xml", StringComparison.Ordinal)))
        {
            tests.Test("original layout import and roundtrip: " + name, () =>
            {
                ...
                using (var reader = xml.CreateReader()) serializer.Deserialize(reader);
                ...
                foreach (var group in manager.Layout.Descendents().OfType<ILayoutContentSelector>())
                {
                    ...
                    if (group.SelectedContent != null) Check.True(group.SelectedContent.IsSelected);
                }
                ...
                if (name.EndsWith("auto-hide.xml", StringComparison.Ordinal))
                {
                    ...
                    var tool = group.Children.Single(); tool.ToggleAutoHide();
                ...
                if (name.EndsWith("floating-document.xml", StringComparison.Ordinal))
                {
                    ...
                    Check.Near(-150, doc.FloatingLeft); Check.Near(80, doc.FloatingTop);
                    Check.Near(760, doc.FloatingWidth); Check.Near(510, doc.FloatingHeight);
                }
            });
        }
                ...
                tests.Test("observed public defaults: " + typeName, () =>
                {
                    ...
                    try
                    {
                        foreach (var property in record.Elements("Property"))
                        {
                            ...
                            if (property.Attribute("Null") != null) { Check.True(actual == null, name + " must default to null."); continue; }
        ...
        {
            foreach (var outermost in new[] { false, true })
            {
                tests.Test($"public AddToLayout topology: {side}, Most={outermost}", () =>
                {
                    using var manager = new DockingManager();
                    Load(manager, "basic.xml");
                    var added = new LayoutAnchorable { ContentId = "added", Title = "Added" };
        ...
        foreach (var boundary in new[] { 0, 1, 2, 3 })
            tests.Test("same-pane insertion boundary " + boundary, () =>
            {
                ...
                var pane = new LayoutDocumentPane(); foreach (var doc in docs) pane.Children.Add(doc);
        ...
        tests.Test("XML restores explicit non-first selection", () =>
        {
            ...
            Check.Equal("b", pane.SelectedContent!.ContentId); Check.Equal(1, pane.Children.Count(c => c.IsSelected));
        });
        ...
        {
            using var manager = new DockingManager(); var a = new LayoutDocument(); var b = new LayoutDocument();
            var first = new LayoutDocumentPane(a); first.Children.Add(b);
            var group = new LayoutDocumentPaneGroup { Orientation = Orientation.Horizontal };
            group.Children.Add(first); group.Children.Add(new LayoutDocumentPane(new LayoutDocument()));
            manager.Layout = new() { RootPanel = new(group) };
            Check.False(DockOperations.CanDock(b, first, DockPosition.Bottom));
            DockOperations.Dock(b, first, DockPosition.Bottom); Check.Same(first, b.Parent);
            ...
            DockOperations.Dock(b, first, DockPosition.Bottom); Check.False(ReferenceEquals(first, b.Parent));
        });
        tests.Test("CanMove prevents restoring floating document", () =>
        {
            using var manager = new DockingManager(); var doc = new LayoutDocument();
            manager.Layout = new() { RootPanel = new(new LayoutDocumentPane(doc)) };
            doc.Float(); Check.True(doc.IsFloating); doc.CanMove = false; doc.Dock(); Check.True(doc.IsFloating);
        });
        tests.Test("CanRepositionItems prevents dock-as-document", () =>
        {
            ...
            var pane = new LayoutAnchorablePane(tool) { CanRepositionItems = false };
            manager.Layout = new() { RootPanel = new(pane) }; tool.DockAsDocument(); Check.Same(pane, tool.Parent);
        });
        return await tests.Run(output, "interop");
    }
    private static XDocument ReadFixture(string file)
    { using var stream = typeof(InteropTests).Assembly.GetManifestResourceStream("ReferenceFixtures." + file)!; return XDocument.Load(stream); }
    private static void Load(DockingManager manager, string file)
    { using var stream = typeof(InteropTests).Assembly.GetManifestResourceStream("ReferenceFixtures." + file)!; new XmlLayoutSerializer(manager).Deserialize(stream); }
    private static string Topology(XElement node) => node.Name.LocalName + "[" + (string?)node.Attribute("Orientation") + ":" + (string?)node.Attribute("ContentId") + "](" +
        string.Join(',', node.Elements().Where(e => e.Name.LocalName is not ("TopSide" or "LeftSide" or "BottomSide" or "RightSide" or "FloatingWindows" or "Hidden")).Select(Topology)) + ")";
    private static void CheckContents(XDocument source, LayoutRoot layout)
    {
        ...
        {
            var id = node.Attribute("ContentId")!.Value; var content = actual[id];
            ...
            foreach (var property in new[] { "CanClose", "CanHide", "IsLastFocusedDocument" })
            {
                if (node.Attribute(property) is not { } attribute) continue;
        ...
        if (focused != null) Check.Equal(focused.Attribute("ContentId")!.Value, layout.LastFocusedDocument?.ContentId);
    }
}
