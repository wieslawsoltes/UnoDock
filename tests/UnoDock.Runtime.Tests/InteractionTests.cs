...
public static class InteractionTests
{
    public static async Task<int> Run(DockingManager host, string output)
    {
        ...
        var originalMode = host.FloatingWindowMode;
        var originalCoordinates = host.CrossWindowCoordinates;
        tests.Test("native coordinate provider is installed by default", () =>
        {
            using var manager = new DockingManager();
            Check.True(manager.CrossWindowCoordinates is DesktopWindowCoordinates);
        });
        ...
        {
            var docs = await Setup(host, 8);
            var pane = Pane(host);
            var scroll = Header(pane);
            var point = At(scroll, Surface(host), .5, .5);
            var plan = host.GetDropPlan(docs[0], point);
            Check.True(plan != null);
            Check.Equal(DropTargetType.DocumentPaneDockInside, plan!.Type);
            Check.True(plan.InsertionIndex >= 0);
        });
        tests.Test("edge compatibility mode retains body docking without header index", async () =>
        {
            ...
            {
                host.DockingGuideMode = previous;
            }
        });
        tests.Test("hidden models do not shift visible insertion indices", async () =>
        {
            var docs = await Setup(host, 4);
            typeof(LayoutDocument).GetProperty(nameof(LayoutDocument.IsVisible))!.SetValue(docs[0], false);
            host.Refresh();
            host.UpdateLayout();
            var tab = Pane(host).FindVisualChildren<LayoutDocumentTabItem>().First(t => ReferenceEquals(t.Model, docs[1]));
            var point = At(tab, Surface(host), .05, .5);
            var plan = host.GetDropPlan(docs[3], point);
            Check.True(plan != null);
            Check.Equal(1, plan!.InsertionIndex);
            Check.True(plan.Execute());
            Check.Same(docs[3], ((LayoutDocumentPane)docs[3].Parent!).Children[1]);
        });
        tests.Test("drag auto-scroll advances overflowing headers", async () =>
        {
            ...
            var pane = Pane(host);
            var scroll = Header(pane);
            Check.True(scroll.ScrollableWidth > 0);
            scroll.ChangeView(0, null, null, true);
            host.UpdateLayout();
            var point = At(scroll, Surface(host), .99, .4);
            var before = scroll.HorizontalOffset;
            Call(pane, "ScrollHeaderAt", point, Surface(host), .05);
            await Task.Delay(50);
            host.UpdateLayout();
        ...
        {
            await Setup(host, 3);
            var source = Header(Pane(host));
            var target = Surface(host);
            var p = new Point(13.25, 4.75);
            var expected = source.TransformToVisual(target).TransformPoint(p);
            var actual = new DesktopWindowCoordinates().Translate(source, p, target);
        ...
        tests.Test("transformed drop bounds enclose all four corners", async () =>
        {
            ...
            var target = Surface(host);
            source.RenderTransform = new RotateTransform
            {
                Angle = 25
            };
            try
        ...
        tests.Test("in-surface activation updates both hit-test and visual stacking order", async () =>
        {
            ...
            docs[0].FloatingLeft = 100;
            docs[0].FloatingTop = 100;
            docs[0].Float();
            docs[1].FloatingLeft = 100;
            docs[1].FloatingTop = 100;
            docs[1].Float();
            host.Refresh();
            host.UpdateLayout();
            await Task.Delay(50);
        ...
        tests.Test("floating single-content item supports the public dependency-property setter", () =>
        {
            ...
            manager.Layout = new()
            {
                RootPanel = new(new LayoutAnchorablePane(content))
            };
            ...
            control.SingleContentLayoutItem = item;
            Check.Same(item, control.SingleContentLayoutItem);
        ...
        if (OperatingSystem.IsLinux() && !string.IsNullOrEmpty(Environment.GetEnvironmentVariable("DISPLAY")))
        {
            ...
            tests.Test("X11 two-window translation tracks native moves and roundtrips", async () =>
            {
                ...
                try
                {
                    ...
                    var from = ((Grid)a.Content).Children[0] as FrameworkElement;
                    var to = ((Grid)b.Content).Children[0] as FrameworkElement;
                    var coordinates = new DesktopWindowCoordinates();
                    var p = new Point(10.25, 21.75);
                    var q = coordinates.Translate(from!, p, to!);
                    var back = coordinates.Translate(to!, q, from!);
                    ...
                    Check.Near(q.Y - 40 / to!.XamlRoot!.RasterizationScale, moved.Y, 1);
                }
                finally
                {
                    CloseTestWindow(a);
            ...
            tests.Test("X11 floating tool receives cross-window insertion and returns to main", async () =>
            {
                ...
                var plan = host.GetDropPlan(source, point);
                Check.True(plan != null, "No native-window insertion plan.");
                Check.Equal(DropTargetType.AnchorablePaneDockInside, plan!.Type);
                Check.True(plan.Execute());
                ...
                var backPoint = At(ToolHeader(mainPane), Surface(host), .5, .5);
                var back = host.GetDropPlan(source, backPoint);
                Check.True(back != null, "No insertion plan in the main tool header after native docking.");
                Check.True(back!.Execute());
                Check.Same(keep.Parent, source.Parent);
            });
            tests.Test("X11 native preview is painted in destination window", async () =>
            {
                ...
                var pane = floating.FindVisualChildren<LayoutAnchorablePaneControl>().Single();
                var coordinates = new DesktopWindowCoordinates();
                var point = coordinates.Translate(ToolHeader(pane), new(40, 15), Surface(host));
                var plan = host.GetDropPlan(source, point)!;
                Check.True(plan != null);
                Call(floating, "ShowDropPreview", plan, Surface(host), new SolidColorBrush(Microsoft.UI.Colors.Blue));
                var overlay = floating.FindVisualChildren<OverlayWindow>().Single();
                Check.True(overlay.IsOpen);
                Check.Same(plan, overlay.CurrentPlan);
                Call(floating, "HideDropPreview");
            ...
            tests.Test("unsupported cross-window provider omits areas and preserves layout", async () =>
            {
                ...
                Check.Same(before, source.Parent);
                host.CrossWindowCoordinates = new DesktopWindowCoordinates();
            });
        }
        ...
        if (OperatingSystem.IsLinux() && Environment.GetEnvironmentVariable("UNODOCK_NATIVE_INPUT_TESTS") == "1")
        {
            ...
            tests.Test("X11 document caption pointer capture docks back to main", async () =>
            {
                ...
                Check.True(ReferenceEquals(docs[1].Parent, docs[0].Parent), $"Document was not inserted into main pane; drag={DragState(host)}, floating={docs[0].IsFloating}, " + $"sourceParent={docs[0].Parent?.GetType().Name}, destinationParent={docs[1].Parent?.GetType().Name}, " + $"lastPoint={Surface(host).GetType().GetField("_lastDragPoint", BindingFlags.Instance | BindingFlags.NonPublic)!.GetValue(Surface(host))}.");
    ...
    private static void CloseTestWindow(Window window)
    {
        ...
        typeof(DesktopWindowCoordinates).GetMethod("HideNativeClientBeforeClose", BindingFlags.NonPublic | BindingFlags.Static)!.Invoke(null, [window]);
        window.Content = null;
        window.Close();
    }
    ...
    private static async Task<LayoutDocument[]> Setup(DockingManager host, int count)
    {
        ...
        var docs = Enumerable.Range(0, count).Select(i => new LayoutDocument { Title = $"Document {i:00} long header", ContentId = $"d{i}", Content = new TextBox { Text = $"Editor {i}" } }).ToArray();
    ...
    private static async Task<(LayoutAnchorable Source, LayoutAnchorable Keep, LayoutAnchorable Target)> NativeWorkspace(DockingManager host)
    {
        host.CrossWindowCoordinates = new DesktopWindowCoordinates();
        host.FloatingWindowMode = FloatingWindowMode.Native;
        var source = new LayoutAnchorable
        {
            Title = "Source",
            ContentId = "source"
        };
        var keep = new LayoutAnchorable
        {
            Title = "Keep",
            ContentId = "keep"
        };
        var target = new LayoutAnchorable
        {
            Title = "Target",
            ContentId = "target"
        };
        var pane = new LayoutAnchorablePane(source);
        pane.Children.Add(keep);
        pane.Children.Add(target);
        host.Layout = new()
        {
            RootPanel = new(pane)
            {
                Children =
                {
                    new LayoutDocumentPane(new LayoutDocument { Title = "Editor" })
                }
            }
        };
        host.Refresh();
        host.UpdateLayout();
        target.FloatingLeft = 1600;
        target.FloatingTop = 100;
        target.FloatingWidth = 550;
        target.FloatingHeight = 400;
        target.Float();
        host.Refresh();
        await Task.Delay(200);
        host.UpdateLayout();
    ...
    private sealed class UnavailableCoordinates : ICrossWindowCoordinates
    {
        public Point Translate(FrameworkElement source, Point p, FrameworkElement destination) => throw new PlatformNotSupportedException("Test unsupported host");
    }
