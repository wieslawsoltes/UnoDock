...
public static class WindowLifecycleTests
{
    public static async Task<int> Run(DockingManager host, string output)
    {
        ...
        {
            using var manager = Floating(out var model);
            var window = new Probe(model);
            Check.Equal(0, window.Initializations);
            Check.True(window.Model != null);
            var events = 0;
            ...
            Check.Equal(1, events);
            Call(window, "CloseHost");
            window.Initialize();
            Check.Equal(1, events);
        });
        tests.Test("closing callback can veto without losing window or content", () =>
        {
            ...
            var document = model.RootDocument;
            window.Close();
            Check.Same(document, model.RootDocument);
            Check.True(model.Root != null);
        ...
        tests.Test("recursive close dispatches one closing and closed pair", () =>
        {
            ...
            window.Close();
            Check.Equal(1, closing);
            Check.Equal(1, closed);
            Check.True(window.UserCloseObserved);
            Check.False(window.UserCloseNow);
        });
        tests.Test("model close veto does not emit host Closed", () =>
        {
            ...
            window.Close();
            Check.Equal(0, window.Closures);
            Check.True(model.RootDocument != null);
            Call(window, "CloseHost");
        });
        tests.Test("closing exception resets reentrancy and user-close flag", () =>
        {
            ...
            Check.Throws<InvalidOperationException>(window.Close);
            Check.False(window.UserCloseNow);
            window.Closing -= handler;
            window.Close();
            Check.Equal(1, window.Closures);
        ...
        tests.Test("closing callback root replacement does not close replacement content", () =>
        {
            ...
            var next = Root(new LayoutDocumentPane(replacement));
            window.Closing += (_, _) => manager.Layout = next;
            window.Close();
            Check.Same(next, replacement.Root);
            Check.True(model.RootDocument != null);
            Call(window, "CloseHost");
        });
        tests.Test("permanently closed floating host cannot execute system commands", () =>
        {
            ...
            Check.Equal(1, window.Closures);
            Check.True(window.NativeWindow == null);
            Check.False(SystemCommands.MaximizeWindowCommand.CanExecute(window));
        ...
        tests.Test("message filter defaults to unhandled without requiring Windows", () =>
        {
            ...
            var window = new Probe(model);
            var handled = false;
            Check.Equal((nint)0, window.InvokeBaseFilter(ref handled));
            Check.False(handled);
            Call(window, "CloseHost");
        });
        tests.Test("tool hide stops after a reentrant root replacement", () =>
        {
            ...
            var b = new LayoutAnchorable();
            var pane = new LayoutAnchorablePane(a);
            pane.Children.Add(b);
            var model = new LayoutAnchorableFloatingWindow
            {
                RootPanel = new LayoutAnchorablePaneGroup(pane)
            };
            manager.Layout.FloatingWindows.Add(model);
            var window = new LayoutAnchorableFloatingWindowControl(model);
            a.Hiding += (_, _) => manager.Layout = new();
            window.Hide();
            Check.Same(pane, b.Parent);
            Call(window, "CloseHost");
        });
        tests.Test("tool host clears retained single-item adapter on closure", () =>
        {
            ...
            var model = new LayoutAnchorableFloatingWindow
            {
                RootPanel = new LayoutAnchorablePaneGroup(new LayoutAnchorablePane(tool))
            };
            manager.Layout.FloatingWindows.Add(model);
            var window = new LayoutAnchorableFloatingWindowControl(model);
            window.SingleContentLayoutItem = manager.GetLayoutItemFromModel(tool);
            window.Close();
            Check.True(window.SingleContentLayoutItem == null);
            Check.False(window.CloseWindowCommand.CanExecute(null));
            Check.False(window.HideWindowCommand.CanExecute(null));
        });
        tests.Test("overlay public closing cancellation keeps preview open", () =>
        {
            ...
            overlay.Closing += veto;
            overlay.Close();
            Check.Equal(Visibility.Visible, overlay.Visibility);
            overlay.Closing -= veto;
            overlay.Close();
            Check.Equal(Visibility.Collapsed, overlay.Visibility);
        });
        void Live(string name, Func<Fixture, Task> body) => tests.Test(name, async () =>
        {
            ...
            {
                CallSurface(host, "CloseNavigator", false);
            }
        });
        Live("maximized presentation leaves all normal model bounds intact", async f =>
        {
            var window = await f.Float();
            var normal = Bounds(f.A);
            SystemCommands.MaximizeWindow(window);
            host.Refresh();
            await Tick();
            Check.True(window.IsMaximized);
            Check.Equal(normal, Bounds(f.A));
            Check.True(window.Width > normal.Width);
            SystemCommands.RestoreWindow(window);
            Check.Equal(normal, Bounds(f.A));
            Check.Near(normal.Width, window.Width);
        });
        ...
        Live("imported maximized host restores original normal rectangle", async f =>
        {
            ...
            serializer.Deserialize(new StringReader(text.ToString()));
            host.Refresh();
            await Tick();
            window = host.FloatingWindows.Single();
            Check.True(window.IsMaximized);
            var model = host.Layout.Descendents().OfType<LayoutDocument>().Single(c => c.ContentId == "a");
            Check.Equal(normal, Bounds(model));
            SystemCommands.RestoreWindow(window);
            Check.Near(normal.Width, window.Width);
            Check.Equal(normal, Bounds(model));
        });
        ...
        Live("maximize minimize restore retains original geometry", async f =>
        {
            ...
            Check.True(window.IsMaximized);
            Check.Equal(Visibility.Collapsed, window.Visibility);
            Check.Equal(original, Bounds(f.A));
            SystemCommands.RestoreWindow(window);
        ...
        Live("state hook sees committed model state exactly once per change", async f =>
        {
            ...
            {
                calls++;
                Check.Equal(window.IsMaximized, f.A.IsMaximized);
            };
            SystemCommands.MaximizeWindow(window);
            Check.Equal(1, calls);
            SystemCommands.MaximizeWindow(window);
        ...
        Live("presenter max and min snapshots cannot corrupt saved normal bounds", async f =>
        {
            ...
            Check.Equal(restored, Bounds(f.A));
            Check.False(window.IsMaximized);
        });
        foreach (var bounds in new[]
        {
            new DockRect(double.NaN, 1, 100, 100),
            new DockRect(1, double.PositiveInfinity, 100, 100),
            new DockRect(1, 1, 0, 100),
            new DockRect(1, 1, 100, -1)
        }
        ...
        Live("maximized resize follows surface without changing persisted dimensions", async f =>
        {
            ...
            {
                SystemCommands.MaximizeWindow(window);
                var previous = window.Width;
                host.Margin = new(60);
                host.UpdateLayout();
                await Tick();
                Check.True(window.Width < previous);
                Check.Equal(normal, Bounds(f.A));
            }
            finally
            {
        ...
        Live("navigator has distinct document and tool lists", async f =>
        {
            var nav = f.Navigator();
            await Tick();
            Check.Equal(2, nav.Documents.Length);
            Check.Equal(1, nav.Anchorables.Count());
            var lists = nav.FindVisualChildren<ListBox>().ToArray();
            Check.Equal(2, lists.Length);
        ...
        {
            host.GetLayoutItemFromModel(f.A).Visibility = Visibility.Collapsed;
            f.Tool.Hide();
            var nav = f.Navigator();
            await Tick();
            Check.Equal(1, nav.Documents.Length);
            Check.Same(f.B, nav.Documents[0].LayoutElement);
            Check.Equal(0, nav.Anchorables.Count());
        });
        ...
        {
            f.B.IsActive = true;
            f.A.LastActivationTimeStamp = new(2020, 3, 3);
            f.B.LastActivationTimeStamp = new(2020, 2, 2);
            f.Tool.LastActivationTimeStamp = new(2020, 1, 1);
            var nav = f.Navigator();
            await Tick();
            Check.Same(f.Tool, nav.SelectedAnchorable!.LayoutElement);
        });
        Live("navigator preview operations publish mutually exclusive dependency properties", async f =>
        {
            var nav = f.Navigator();
            await Tick();
            nav.PreviewAnchorable(nav.Anchorables.Single());
            Check.True(nav.SelectedDocument == null);
            nav.PreviewDocument(nav.Documents[0]);
            Check.True(nav.SelectedAnchorable == null);
            nav.PreviewDocument(null);
            Check.True(nav.SelectedDocument == null);
            Check.True(nav.SelectedAnchorable == null);
        });
        Live("navigator rejects foreign-manager selected items", async f =>
        {
            ...
            other.Layout = Root(new LayoutDocumentPane(document));
            nav.PreviewDocument((LayoutDocumentItem)other.GetLayoutItemFromModel(document));
        ...
        Live("removing a selected navigator item never activates detached content", async f =>
        {
            var nav = f.Navigator();
            await Tick();
            nav.PreviewDocument(nav.Documents.Single(i => ReferenceEquals(i.LayoutElement, f.A)));
            f.A.Close();
            Call(nav, "CommitSelection");
            Check.True(f.A.Root == null);
            Check.False(ReferenceEquals(f.A, host.Layout.ActiveContent));
            await Tick();
            Check.Equal(1, nav.Documents.Length);
        });
        ...
        {
            f.A.LastActivationTimeStamp = new(2020, 1, 1);
            f.B.LastActivationTimeStamp = new(2020, 2, 2);
            var nav = f.Navigator();
            await Tick();
            var first = nav.Documents[0];
            f.A.LastActivationTimeStamp = new(2030, 1, 1);
            await Tick();
        ...
        Live("new documents append to an existing navigator session", async f =>
        {
            ...
            await Tick();
            var first = nav.Documents[0];
            ((LayoutDocumentPane)f.B.Parent!).Children.Add(new LayoutDocument { ContentId = "new", Title = "New" });
            await Tick();
            Check.Equal(3, nav.Documents.Length);
            Check.Same(first, nav.Documents[0]);
            Check.Equal("new", nav.Documents[2].ContentId);
        });
        ...
        Live("navigator cancelled session unsubscribes queued layout refresh", async f =>
        {
            ...
            f.A.Title = "queued";
            CallSurface(host, "CloseNavigator", false);
            ((LayoutDocumentPane)f.B.Parent!).Children.Add(new LayoutDocument());
            await Tick();
            Check.Equal(2, nav.Documents.Length);
        });
        Live("root replacement closes navigator and invalidates stale commands", async f =>
        {
            ...
            await Tick();
            Call(nav, "CommitSelection");
            Check.False(nav.IsLoaded);
            Check.True(host.Layout.ActiveContent == null);
        });
        Live("cancelled navigator returns focus to the same editor control", async f =>
        {
            ...
            CallSurface(host, "CloseNavigator", false);
            await Tick();
            Check.Same(f.B, host.Layout.ActiveContent);
        ...
        Live("navigator respects activation command CanExecute", async f =>
        {
            var nav = f.Navigator();
            await Tick();
            var item = nav.Documents.Single(i => ReferenceEquals(i.LayoutElement, f.B));
            item.ActivateCommand = new DisabledCommand();
            nav.PreviewDocument(item);
            CallSurface(host, "CloseNavigator", true);
            Check.Same(f.A, host.Layout.ActiveContent);
        });
        Live("navigator supports original named ListBox template parts", async f =>
        {
            ...
            CallSurface(host, "ShowNavigator", nav);
            await Tick();
            nav.ApplyTemplate();
            var lists = nav.FindVisualChildren<ListBox>().ToArray();
            Check.Equal(2, lists.Length);
            Check.Equal(2, lists.Single(l => l.Name == "PART_DocumentListBox").Items.Count);
            lists.Single(l => l.Name == "PART_AnchorableListBox").SelectedIndex = 0;
            Check.True(nav.SelectedAnchorable != null);
            Check.True(nav.SelectedDocument == null);
        });
        if (OperatingSystem.IsLinux() && Environment.GetEnvironmentVariable("UNODOCK_NATIVE_INPUT_TESTS") == "1")
        {
            ...
            Live("XTEST navigator opened from native floating editor restores main editor focus", async f =>
            {
                ...
                var floating = await f.Float();
                floating.Activate();
                Check.True(f.EditorA2.Focus(FocusState.Programmatic));
                await Tick();
                input.MoveTo(f.EditorA2, new(20, 15));
                input.Press();
                input.Release();
                await Tick();
                f.B.LastActivationTimeStamp = DateTime.UtcNow.AddHours(-1);
                f.Tool.LastActivationTimeStamp = DateTime.UtcNow.AddHours(-2);
                input.KeyDown(0xffe3);
                input.KeyPress(0xff09);
                await Tick();
                var nav = host.FindVisualChildren<NavigatorWindow>().Single();
            ...
            Live("XTEST category keys select tool list and Enter commits without ListBox interception", async f =>
            {
                ...
                await Tick();
                Check.Equal(0, host.FindVisualChildren<NavigatorWindow>().Count());
                Check.Same(f.Tool, host.Layout.ActiveContent);
            });
        }
    ...
    private static void RegisterWindows(TestRunner tests)
    {
        tests.Test("Windows FilterMessage receives HWND message and supplies handled result", async () =>
        {
            ...
            try
            {
                Call(window, "ShowNative");
                await Tick();
                var hwnd = NativeHandle(window.NativeWindow!);
                Check.True(hwnd != 0);
                Check.Equal((nint)91, SendMessage(hwnd, 0x8061, 0, 0));
                Check.Equal(1, window.FilterCalls);
                Call(window, "HideHost");
                Call(window, "ShowNative");
                await Tick();
                hwnd = NativeHandle(window.NativeWindow!);
                Check.Equal((nint)91, SendMessage(hwnd, 0x8061, 0, 0));
                Check.Equal(2, window.FilterCalls);
            }
            finally
            {
                Call(window, "CloseHost");
        ...
        tests.Test("Windows native filter failure is reported without unwinding into user32", async () =>
        {
            ...
            try
            {
                Call(window, "ShowNative");
                await Tick();
                SendMessage(NativeHandle(window.NativeWindow!), 0x8061, 0, 0);
                await Tick();
                Check.Equal(1, errors);
            }
            finally
            {
                Call(window, "CloseHost");
    ...
    private sealed class Probe(LayoutDocumentFloatingWindow model) : LayoutDocumentFloatingWindowControl(model)
    {
        ...
        {
            Closures++;
            UserCloseObserved = CloseInitiatedByUser;
            base.OnClosed(e);
        }
        ...
        protected override nint FilterMessage(nint hwnd, int msg, nint wParam, nint lParam, ref bool handled)
        {
            ...
            FilterCalls++;
            if (ThrowFilter)
                throw new InvalidOperationException("filter-test");
            handled = true;
            return 91;
        }
    }
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        internal Fixture(DockingManager host)
        {
            _host = host;
            _old = host.Layout;
            _mode = host.FloatingWindowMode;
            host.FloatingWindowMode = FloatingWindowMode.InSurface;
            var editors = new StackPanel();
            editors.Children.Add(new TextBox { Text = "first editor" });
            editors.Children.Add(EditorA2);
            A = new()
            {
                ContentId = "a",
                Title = "Alpha",
            ...
            var documents = new LayoutDocumentPane(A);
            documents.Children.Add(B);
            host.Layout = Root(documents, new LayoutAnchorablePane(Tool));
            A.IsActive = true;
            host.Refresh();
            host.UpdateLayout();
        }

        internal async Task<LayoutFloatingWindowControl> Float()
        {
            A.Float();
            _host.Refresh();
            await Tick();
            return _host.FloatingWindows.Single();
        }

        internal NavigatorWindow Navigator()
        {
            var nav = new NavigatorWindow(_host);
            CallSurface(_host, "ShowNavigator", nav);
            return nav;
        }

        public void Dispose()
        {
            CallSurface(_host, "CloseNavigator", false);
            _host.FloatingWindowMode = _mode;
            _host.Layout = _old;
            _host.Refresh();
        }
    }
    ...
    {
        var panel = new UnoDock.Layout.LayoutPanel();
    ...
    private static DockingManager Floating(out LayoutDocumentFloatingWindow model)
    {
        ...
        manager.Layout = Root(new LayoutDocumentPane(document));
        document.Float();
        model = manager.Layout.FloatingWindows.OfType<LayoutDocumentFloatingWindow>().Single();
        return manager;
    }
