...
public static partial class ShellTests
{
    private static void RegisterNativeInput(TestRunner tests, DockingManager host)
    {
        ...
        foreach (var cancel in new[]
        {
            ...
            tests.Test("X11 managed left resize, detach rollback=" + cancel, async () =>
            {
                var original = host.Layout;
                var mode = host.FloatingWindowMode;
                var content = new LayoutDocument
                {
                    Title = "Managed chrome resize",
                    Content = new Grid(),
                    FloatingLeft = 50,
                    FloatingTop = 90,
                    FloatingWidth = 400,
                ...
                Check.True(display != 0, "Native input test requires an X11 display.");
                ...
                try
                {
                    ...
                    host.Layout = new()
                    {
                        RootPanel = new UnoDock.Layout.LayoutPanel(new LayoutDocumentPane(content))
                    };
                    content.Float();
                    host.Refresh();
                    ...
                    Motion(display, start);
                    await Tick();
                    Check.True(XTestFakeButtonEvent(display, 1, true, 0) != 0);
                    XFlush(display);
                    await Tick();
                    Motion(display, new(start.X + 24, start.Y));
                    await Until(() => content.FloatingLeft > before + 1);
                    Check.Near(before + 24, content.FloatingLeft, 1);
                    Check.Near(376, content.FloatingWidth, 1);
                    Check.Near(450, content.FloatingLeft + content.FloatingWidth, 1);
                    if (cancel)
                    {
                        WindowChrome.SetWindowChrome(floating, null);
                        await Tick();
                        Check.Near(before, content.FloatingLeft);
                        Check.Near(400, content.FloatingWidth);
                    }

                    Check.True(XTestFakeButtonEvent(display, 1, false, 0) != 0);
                    XFlush(display);
                    await Tick();
    ...
    private static async Task Until(Func<bool> condition)
    {
        ...
        while (!condition())
        {
            if (timer.Elapsed > TimeSpan.FromSeconds(5))
                throw new TimeoutException("Chrome test did not reach the expected state.");
            await Task.Delay(20);
        }
    }
    ...
    {
        Check.True(XTestFakeMotionEvent(display, -1, checked((int)Math.Round(point.X)), checked((int)Math.Round(point.Y)), 0) != 0);
        XFlush(display);
    }
    ...
    private static extern int XTestFakeMotionEvent(nint display, int screen, int x, int y, nuint delay);
    [DllImport("libXtst.so.6")]
    private static extern int XTestFakeButtonEvent(nint display, uint button, [MarshalAs(UnmanagedType.Bool)] bool pressed, nuint delay);
}
