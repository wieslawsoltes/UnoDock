...
internal static class FloatingResizeBoundsTests
{
    ...
    {
        var initial = new DockRect(10, 20, 400, 300);
        var target = new DockRect(30, 40, 420, 320);
        ...
        {
            var ledger = new FloatingResizeBoundsTracker(initial, true);
            var second = new DockRect(70, 80, 500, 600);
            Check.True(ledger.Request(target));
        ...
        tests.Test("resize bounds: revisiting a rectangle retires all older requests", () =>
        {
            var ledger = new FloatingResizeBoundsTracker(initial, true);
            Check.True(ledger.Request(target));
            Check.True(ledger.Request(initial));
            Check.True(ledger.Observe(initial));
            Check.False(ledger.Observe(target));
        });
        ...
        tests.Test("resize bounds: 10000 deterministic delayed frame sequences", () =>
        {
            ...
            {
                var ledger = new FloatingResizeBoundsTracker(initial, true);
                var pending = Enumerable.Range(0, random.Next(1, 20)).Select(i => new DockRect(1000 + i * 17, 2000 + i * 13, 500 + i * 11, 400 + i * 7)).ToArray();
                foreach (var next in pending)
                    Check.True(ledger.Request(next));
                ...
                Check.False(ledger.Observe(initial));
                Check.True(ledger.Observe(pending[^1]));
            }
        });
    }
}
