...
internal static class ChromeTests
{
    internal static void Register(TestRunner tests)
    {
        ...
        foreach (var (point, hit) in new[] {
            (new DockPoint(0,0), ChromeHit.TopLeft), (new DockPoint(400,0),ChromeHit.Top), (new DockPoint(799,0),ChromeHit.TopRight),
            (new DockPoint(0,300), ChromeHit.Left), (new DockPoint(799,300),ChromeHit.Right),
            (new DockPoint(0,599), ChromeHit.BottomLeft), (new DockPoint(400,599),ChromeHit.Bottom), (new DockPoint(799,599),ChromeHit.BottomRight),
            (new DockPoint(50,15), ChromeHit.Caption), (new DockPoint(50,100),ChromeHit.Client),
            (new DockPoint(-1,10), ChromeHit.Client), (new DockPoint(800,10),ChromeHit.Client) })
        ...
        tests.Test("interactive caption controls take precedence over border", () => Check.Equal(ChromeHit.Client,
            ChromeGeometry.HitTest(bounds, new(0,0), 32, 5, 5, 5, 5, true, false, new[] { new DockRect(0,0,20,20) })));
        ...
        tests.Test("overlapping interactive holes are unioned", () =>
        {
            ...
            Check.Equal(2, regions.Count); Check.Near(20, regions[0].Width); Check.Near(110, regions[1].X);
        });
        tests.Test("adjacent caption bands coalesce", () =>
        {
            ...
            Check.Equal(2, regions.Count); Check.Near(32, regions[0].Height);
        });
        ...
        foreach (var hit in Enum.GetValues<ChromeHit>()) tests.Test("frame drag constraints: " + hit, () =>
        {
            var start = new DockRect(10, 20, 400, 300);
            var result = ChromeResize.Apply(start, hit, 10000, -10000, 160, 100, 600, 500);
            if (hit is ChromeHit.Client or ChromeHit.Caption) { Check.Near(start.Width, result.Width); Check.Near(start.Height, result.Height); }
            else { Check.True(result.Width is >= 160 and <= 600); Check.True(result.Height is >= 100 and <= 500); }
            if (hit is ChromeHit.Left or ChromeHit.TopLeft or ChromeHit.BottomLeft) Check.Near(start.Right, result.Right);
            if (hit is ChromeHit.Top or ChromeHit.TopLeft or ChromeHit.TopRight) Check.Near(start.Bottom, result.Bottom);
            if (hit is ChromeHit.Right or ChromeHit.TopRight or ChromeHit.BottomRight) Check.Near(start.X, result.X);
        });
        ...
        tests.Test("random caption partition equals pointwise set difference", () =>
        {
            ...
            for (var iteration = 0; iteration < 1500; iteration++)
            {
                ...
                {
                    var x = random.NextDouble() * 100; var y = random.NextDouble() * 30;
